...
What rides in the envelope depends on the turn. A session's first send carries the **session-start briefing**: working memory (the active plan, then files changed newest first), then the **durable index** — one line per durable entry, ranked by recency (two-week half-life), use count and confidence, grouped by kind with the display caps as limits — then the curated pack and the **recent-session handoff**: the tail of the most recent other session in the scope, whichever agent ran it, read from capture (or, where capture was never enabled, from Atlas's own session transcripts) — never from an agent's own files. Later turns carry only the **delta**: what other sessions recorded since this session's sync clock. Every turn may also carry **relevant memory**: what retrieval finds for the user's words, skipped when the prompt is under three words or a continuation phrase ("continue", "ok"). The durable index lists shared-memory entries; it is not the codebase index, and not the retrieval index relevant memory is searched from.
