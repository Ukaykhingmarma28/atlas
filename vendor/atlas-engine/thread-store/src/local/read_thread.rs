...
use atlas_engine_rollout::find_thread_name_by_id;
...
async fn resolve_requested_rollout_path(
    ...
    let path = if rollout_path.is_relative() {
        store.config.atlas_agent_home.join(rollout_path)
    } else {
        rollout_path
    };
    ...
    let Some(path) = atlas_engine_rollout::existing_rollout_path(path.as_path()).await else {
        return Err(ThreadStoreError::InvalidRequest {
            message: format!(
...
async fn read_thread_from_rollout_path(
    ...
    thread.rollout_path = Some(atlas_engine_rollout::plain_rollout_path(path.as_path()));
    ...
    if thread.history_mode == ThreadHistoryMode::Legacy
        && let Ok(Some(name)) =
            find_thread_name_by_id(store.config.atlas_agent_home.as_path(), &thread.thread_id).await
        && !name.trim().is_empty()
    {
...
pub(super) async fn load_history_items(
    path: &std::path::Path,
) -> ThreadStoreResult<Vec<atlas_engine_rollout::RolloutItem>> {
...
pub(super) async fn stored_thread_from_sqlite_metadata(
    ...
    let session_meta = match read_required_session_meta_line(metadata.rollout_path.as_path()).await
    {
        ...
        Err(_)
            if atlas_engine_rollout::existing_rollout_path(metadata.rollout_path.as_path())
                .await
                .is_none() =>
        {
            None
...
pub(super) fn stored_thread_from_state_metadata(
    store: &LocalThreadStore,
    metadata: ThreadMetadata,
    parent_thread_id: Option<atlas_engine_protocol::ThreadId>,
) -> StoredThread {
    ...
    let rollout_path = atlas_engine_rollout::plain_rollout_path(metadata.rollout_path.as_path());
    ...
    match history_mode {
        ...
        ThreadHistoryMode::Legacy => {
            if let Some(title) = distinct_thread_metadata_title(metadata) {
                ...
                find_thread_name_by_id(store.config.atlas_agent_home.as_path(), &metadata.id)
                    .await
                    .ok()
                    .flatten()
...
fn stored_thread_from_meta_line(
    ...
    let rollout_path = atlas_engine_rollout::plain_rollout_path(path.as_path());
    StoredThread {
...
mod tests {
    ...
    async fn read_thread_returns_rollout_path_summary() {
        ...
        let relative_path = active_path
            ...
            .expect("path should be under atlas-agent home")
            .to_path_buf();
    ...
    async fn read_thread_by_rollout_path_preserves_legacy_name_and_sqlite_git_info() {
        ...
        metadata.section = Some(atlas_engine_state::ThreadSection {
            id: atlas_engine_state::PINNED_THREAD_SECTION_ID.to_string(),
            name: atlas_engine_state::PINNED_THREAD_SECTION_NAME.to_string(),
            appearance: None,
        });
        ...
        atlas_engine_rollout::append_thread_name(home.path(), thread_id, "Latest index name")
            .await
        ...
        assert_eq!(
            ...
            Some(atlas_engine_state::PINNED_THREAD_SECTION_ID)
        );
    ...
    async fn read_paginated_thread_prefers_sqlite_display_metadata() {
        ...
        .expect("session file");
        atlas_engine_rollout::append_rollout_item_to_path(
            &rollout_path,
    ...
    async fn read_thread_uses_legacy_thread_name_when_sqlite_title_is_missing() {
        ...
        let thread_id = ThreadId::from_string(&uuid.to_string()).expect("valid thread id");
        write_session_file(home.path(), "2025-01-03T12-00-00", uuid).expect("session file");
        atlas_engine_rollout::append_thread_name(home.path(), thread_id, "Legacy title")
            .await
            .expect("append legacy thread name");
