...
impl LocalThreadStore {
        ...
        paths.extend(
            find_rollout_paths(
                &self
                    .config
                    .atlas_agent_home
                    .join(atlas_engine_rollout::ARCHIVED_SESSIONS_SUBDIR),
            )
            .await?,
        );
    ...
    async fn migrate_rollout_path(
        ...
        let metadata = match atlas_engine_rollout::read_session_meta_line(&path).await {
        ...
        let journal_path = migration_journal_path(&self.config.atlas_agent_home, thread_id);
    ...
    async fn migrate_one_rollout(
        ...
        let bounded_subagent_context = if kind == RolloutMigrationKind::Subagent {
            ...
            let context =
                subagent::select_bounded_context(source_path.to_path_buf(), session_meta.clone())
                    .await?;
        ...
        let (_, expected_ordinal) = if let Some(items) = bounded_subagent_context {
            Self::write_bounded_subagent_rollout(&canonicalization_source, items, limiter).await?
        } else {
            ...
            || current_source_metadata.modified().ok() != source_modified
        {
            return Err(ThreadStoreError::Conflict {
                message: "rollout changed while migration was staging it; close older Atlas Agent processes and retry".to_string(),
            });
        }
    ...
    async fn cleanup_failed_unpublished_migration(
        ...
        let Ok(metadata) = atlas_engine_rollout::read_session_meta_line(rollout_path).await else {
            return Ok(());
        };
        if metadata.meta.history_mode == ThreadHistoryMode::Paginated {
            return Ok(());
        ...
        while let Some(entry) = entries.next_entry().await.map_err(migration_error)? {
                ...
                && (name.ends_with(".jsonl") || name.ends_with(".jsonl.zst"))
            {
                ...
                if name.ends_with(".jsonl.zst")
                    && tokio::fs::try_exists(atlas_engine_rollout::plain_rollout_path(&path))
                        .await
                        .map_err(migration_error)?
                {
                    continue;
