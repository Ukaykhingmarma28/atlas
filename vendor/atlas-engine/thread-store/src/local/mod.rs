...
impl LocalThreadStoreConfig {
    pub fn from_config(config: &impl atlas_engine_rollout::RolloutConfigView) -> Self {
        Self {
    ...
    async fn thread_history_db(&self) -> ThreadStoreResult<&sqlx::SqlitePool> {
        ...
        self.thread_history_db
            .get_or_try_init(|| async {
                atlas_engine_state::open_thread_history_db(&self.config.sqlite).await
            })
            .await
...
mod tests {
    ...
    async fn read_thread_uses_live_writer_rollout_path_for_external_resume() {
        ...
        assert!(error.to_string().contains("must be in Atlas Agent home"));
    }
