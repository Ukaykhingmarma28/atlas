...
use atlas_engine_app_server_protocol::build_turns_from_rollout_items;
...
fn move_to_archived(home: &Path, path: PathBuf) -> PathBuf {
    let directory = home.join(atlas_engine_rollout::ARCHIVED_SESSIONS_SUBDIR);
...
async fn indexed_store(home: &Path) -> LocalThreadStore {
    ...
    let state_db = atlas_engine_rollout::state_db::try_init(&rollout_config)
        .await
...
async fn migration_publishes_canonical_projected_history_and_is_idempotent() {
    let home = TempDir::new().expect("create Atlas Agent home");
    ...
    let replacement_histories = read_rollout(&path)
        ...
        .filter_map(|line| match line.item {
            RolloutItem::Compacted(item) => item.replacement_history.map(|items| {
                items
                    .into_iter()
                    .map(atlas_engine_rollout::ResponseItemEnvelope::into_item)
                    .collect::<Vec<_>>()
            }),
            _ => None,
        })
...
async fn migration_preserves_compressed_rollouts_during_publish_and_recovery() {
    ...
    assert_eq!(
        atlas_engine_rollout::read_session_meta_line(&compressed_path)
            .await
