...
pub(super) async fn list_threads(
    ...
    let sort_key = match params.sort_key {
        ThreadSortKey::CreatedAt => atlas_engine_rollout::ThreadSortKey::CreatedAt,
        ThreadSortKey::UpdatedAt => atlas_engine_rollout::ThreadSortKey::UpdatedAt,
        ThreadSortKey::RecencyAt => atlas_engine_rollout::ThreadSortKey::RecencyAt,
    ...
    let sort_direction = match params.sort_direction {
        SortDirection::Asc => atlas_engine_rollout::SortDirection::Asc,
        SortDirection::Desc => atlas_engine_rollout::SortDirection::Desc,
    };
    let state_db = store.state_db().await;
    let rollout_config = RolloutConfig {
        atlas_agent_home: store.config.atlas_agent_home.clone(),
        sqlite: store.config.sqlite.clone(),
        cwd: store.config.atlas_agent_home.clone(),
...
async fn list_section_threads(
    ...
    let anchor = params
        .cursor
        .as_deref()
        .map(|cursor| -> ThreadStoreResult<atlas_engine_state::Anchor> {
    ...
    let normalized_cwd_filters = params.cwd_filters.as_ref().map(|filters| {
        filters
            .iter()
            .map(|cwd| atlas_engine_rollout::state_db::normalize_cwd_for_state_db(cwd))
            .collect::<Vec<_>>()
    });
    let filters = ThreadFilterOptions {
        ...
        sort_key: atlas_engine_state::SortKey::SectionPosition,
        sort_direction: match params.sort_direction {
            SortDirection::Asc => atlas_engine_state::SortDirection::Asc,
            SortDirection::Desc => atlas_engine_state::SortDirection::Desc,
        },
    ...
    let page = match params.relation_filter {
        Some(ThreadRelationFilter::DirectChildrenOf(thread_id)) => {
            state_db
                .list_threads_by_relation(
                    params.page_size,
                    atlas_engine_state::ThreadRelationFilter::DirectChildrenOf(thread_id),
                    filters,
                )
                .await
        }
        Some(ThreadRelationFilter::DescendantsOf(thread_id)) => {
            state_db
                .list_threads_by_relation(
                    params.page_size,
                    atlas_engine_state::ThreadRelationFilter::DescendantsOf(thread_id),
                    filters,
                )
                .await
        }
    ...
    let atlas_engine_state::ThreadsPage {
...
pub(super) async fn list_rollout_threads(
    state_db: Option<atlas_engine_rollout::StateDbHandle>,
    config: &RolloutConfig,
    default_model_provider_id: &str,
    params: &ListThreadsParams,
    cursor: Option<&atlas_engine_rollout::Cursor>,
    sort_key: atlas_engine_rollout::ThreadSortKey,
    sort_direction: atlas_engine_rollout::SortDirection,
) -> ThreadStoreResult<atlas_engine_rollout::ThreadsPage> {
        ...
        let relation_filter = params
            ...
            .map(|relation_filter| match relation_filter {
                ThreadRelationFilter::DirectChildrenOf(parent_thread_id) => {
                    atlas_engine_state::ThreadRelationFilter::DirectChildrenOf(parent_thread_id)
                }
                ThreadRelationFilter::DescendantsOf(ancestor_thread_id) => {
                    atlas_engine_state::ThreadRelationFilter::DescendantsOf(ancestor_thread_id)
                }
            });
        let page = atlas_engine_rollout::state_db::list_threads_db(
            state_db.as_deref(),
            &config.sqlite,
            params.page_size,
            cursor,
...
mod tests {
    ...
    async fn list_paginated_threads_uses_sqlite_name_over_legacy_compatibility() {
        ...
        runtime
            ...
            .await
            .expect("state db upsert should succeed");
        atlas_engine_rollout::append_thread_name(home.path(), thread_id, "stale index name")
            .await
    ...
    async fn section_listing_uses_sqlite_metadata_without_reading_rollouts() {
        let home = TempDir::new().expect("temp dir");
        let config = test_config(home.path());
        let state = atlas_engine_state::StateRuntime::init(
            config.sqlite.clone(),
            config.default_model_provider_id.clone(),
        )
        .await
        ...
        for index in 0..3 {
            ...
            atlas_engine_rollout::state_db::reconcile_rollout(
                Some(state.as_ref()),
