...
async fn resolve(
    ...
    if let Ok(path) = live_writer::rollout_path(store, thread_id).await
        && atlas_engine_rollout::existing_rollout_path(path.as_path())
            .await
            .is_some()
...
fn location_for_path(store: &LocalThreadStore, path: &std::path::Path) -> RolloutLocation {
    if rollout_path_is_archived(store.config.atlas_agent_home.as_path(), path) {
...
async fn resolve_path(
    ...
    let rollout_id = match atlas_engine_rollout::rollout_id_from_path(path.as_path()) {
        Some(rollout_id) => rollout_id,
        None => {
            let history_mode = atlas_engine_rollout::read_session_meta_line(path.as_path())
                .await
...
pub(super) fn rollout_id_from_path_or_legacy_thread_id(
    ...
    Ok(match atlas_engine_rollout::rollout_id_from_path(path) {
        Some(rollout_id) => rollout_id,
        None => {
