...
/// Separate Atlas Agent and SQLite homes must work together across startup backfill,
...
async fn split_homes_support_backfill_listing_and_paginated_history() {
    ...
    let sqlite = atlas_engine_state::SqliteConfig::new_for_testing(sqlite_home.as_path().abs());
    let rollout_config = RolloutConfig {
        atlas_agent_home: atlas_agent_home.clone(),
        sqlite: sqlite.clone(),
        cwd: atlas_agent_home.clone(),
    ...
    let runtime = atlas_engine_rollout::state_db::try_init(&rollout_config)
        .await
        .expect("backfill state from Atlas Agent home");
    assert!(
        runtime
    ...
    store
        .resume_thread(ResumeThreadParams {
            ...
            metadata: ThreadPersistenceMetadata {
                cwd: Some(atlas_agent_home.clone()),
    ...
    for sqlite_path in [&state_db_path, &thread_history_db_path] {
        ...
        assert!(
            !atlas_agent_home.join(filename).exists(),
            "SQLite database should not be created under Atlas Agent home"
        );
    }
}
...
async fn paginated_live_append_materializes_turn_items_and_state() {
    ...
    let pool = atlas_engine_state::open_thread_history_db(&atlas_engine_state::SqliteConfig::new_for_testing(
        home.path().abs(),
    ))
    .await
...
async fn projection_store(atlas_agent_home: &Path) -> LocalThreadStore {
    let config = test_config(atlas_agent_home);
