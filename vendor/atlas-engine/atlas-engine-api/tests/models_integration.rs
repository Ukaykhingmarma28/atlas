...
use atlas_engine_client::ReqwestTransport;
...
async fn models_client_hits_models_endpoint() {
    ...
    let base_url = format!("{}/api/atlas-agent", server.uri());
    ...
    Mock::given(method("GET"))
        .and(path("/api/atlas-agent/models"))
        .respond_with(
    ...
    assert_eq!(received[0].url.path(), "/api/atlas-agent/models");
}
