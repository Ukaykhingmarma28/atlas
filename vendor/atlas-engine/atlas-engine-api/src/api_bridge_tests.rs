...
fn map_api_error_preserves_retry_delay() {
    ...
    assert!(matches!(
        err.details(),
        AtlasEngineErrorDetails::Stream(message) if message == "retry later"
    ));
...
fn map_api_error_maps_cloudflare_blocked_response_to_user_message() {
    ...
    let AtlasEngineErrorDetails::UnexpectedStatus(err) = err.details() else {
        panic!("expected AtlasEngineErrorDetails::UnexpectedStatus, got {err:?}");
    };
    assert_eq!(
...
fn map_api_error_maps_cyber_policy_from_400_body() {
    ...
    let AtlasEngineErrorDetails::CyberPolicy { message } = err.details() else {
        panic!("expected AtlasEngineErrorDetails::CyberPolicy, got {err:?}");
    };
    assert_eq!(
        message,
...
fn map_api_error_keeps_unknown_400_errors_generic() {
    ...
    let AtlasEngineErrorDetails::InvalidRequest(message) = err.details() else {
        panic!("expected AtlasEngineErrorDetails::InvalidRequest, got {err:?}");
    };
...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    headers.insert(
        ACTIVE_LIMIT_HEADER,
        http::HeaderValue::from_static("atlas_engine_other"),
    );
    headers.insert(
        "x-atlas-engine-other-limit-name",
        http::HeaderValue::from_static("atlas_engine_other"),
    );
    let body = serde_json::json!({
        "error": {
    ...
    let AtlasEngineErrorDetails::UsageLimitReached(usage_limit) = err.details() else {
        panic!("expected AtlasEngineErrorDetails::UsageLimitReached, got {err:?}");
    };
    assert_eq!(
        usage_limit
            .rate_limits
...
fn map_api_error_ignores_unparseable_rate_limit_reached_type_headers() {
    ...
    for value in values {
        let mut headers = HeaderMap::new();
        headers.insert("x-atlas-engine-rate-limit-reached-type", value);
        let body = serde_json::json!({
            "error": {
                "type": "usage_limit_reached",
                "plan_type": "pro",
