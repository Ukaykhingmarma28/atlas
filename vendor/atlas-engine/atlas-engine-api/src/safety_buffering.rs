...
pub(crate) fn treatment_from_headers(headers: &HeaderMap) -> Option<SafetyBufferingTreatment> {
    if !headers.contains_key(X_ATLAS_AGENT_SAFETY_BUFFERING_ENABLED_HEADER)
        && !headers.contains_key(X_ATLAS_AGENT_SAFETY_BUFFERING_FASTER_MODEL_HEADER)
    {
        return None;
    }
    let faster_model = headers
        .get(X_ATLAS_AGENT_SAFETY_BUFFERING_FASTER_MODEL_HEADER)
...
mod tests {
    ...
    fn reads_treatment_from_http_headers() {
        ...
        headers.insert(
            X_ATLAS_AGENT_SAFETY_BUFFERING_ENABLED_HEADER,
            HeaderValue::from_static("true"),
        );
        headers.insert(
            X_ATLAS_AGENT_SAFETY_BUFFERING_FASTER_MODEL_HEADER,
