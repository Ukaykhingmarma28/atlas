...
const X_ATLAS_AGENT_TURN_STATE_HEADER: &str = "x-atlas-engine-turn-state";
...
async fn connect_websocket(
    ...
    if let Some(turn_state) = turn_state
        && let Some(header_value) = response
            .headers()
            .get(X_ATLAS_AGENT_TURN_STATE_HEADER)
            .and_then(|value| value.to_str().ok())
    {
    ...
    loop {
        ...
        match message {
            Message::Text(text) => {
                ...
                if event.kind() == "atlas_agent.response.metadata"
                    && let Some(etag) =
                        event
                            .headers
                            .as_ref()
...
fn emit_responses_websocket_timing_event(
    ...
    // `RUST_LOG='atlas_engine_api::responses_websocket_timing=trace'`.
    tracing::event!(
...
mod tests {
    ...
    fn parse_wrapped_websocket_error_event_maps_to_transport_http() {
        ...
        assert_eq!(
            headers
                .get("x-atlas-engine-primary-used-percent")
                .and_then(|value| value.to_str().ok()),
            Some("100.0")
        );
        assert_eq!(
            headers
                .get("x-atlas-engine-primary-window-minutes")
                .and_then(|value| value.to_str().ok()),
            Some("15")
        );
    ...
    fn websocket_safety_buffering_uses_event_before_header_fallback() {
        let metadata: ResponsesStreamEvent = serde_json::from_value(json!({
            ...
            "headers": {
                "x-atlas-engine-safety-buffering-enabled": "true",
                "x-atlas-engine-safety-buffering-faster-model": "gpt-fast-header"
            }
        }))
    ...
    fn websocket_safety_buffering_event_controls_visibility_when_header_disables_it() {
        let metadata: ResponsesStreamEvent = serde_json::from_value(json!({
            "type": "atlas_agent.response.metadata",
            "headers": {
                "x-atlas-engine-safety-buffering-enabled": "false",
                "x-atlas-engine-safety-buffering-faster-model": "gpt-fast-header"
            }
        }))
        .expect("deserialize treatment metadata");
