...
use atlas_engine_app_server_protocol::ConfigLayer as ApiConfigLayer;
use atlas_engine_app_server_protocol::ConfigLayerMetadata as ApiConfigLayerMetadata;
use atlas_engine_app_server_protocol::ConfigLayerSource as ApiConfigLayerSource;
use atlas_engine_config::ConfigLayer;
use atlas_engine_config::ConfigLayerMetadata;
use atlas_engine_config::ConfigLayerSource;

/// Converts a config-layer source owned by `atlas-engine-config` into the app-server wire type owned by
/// `atlas-engine-app-server-protocol`.
///
...
/// Converts config-layer metadata owned by `atlas-engine-config` into the app-server wire type owned by
/// `atlas-engine-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
...
/// Converts a config layer owned by `atlas-engine-config` into the app-server wire type owned by
/// `atlas-engine-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
