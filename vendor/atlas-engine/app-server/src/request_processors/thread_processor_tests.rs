...
mod thread_processor_behavior_tests {
    async fn forked_from_id_from_rollout(path: &Path) -> Option<String> {
        atlas_engine_core::read_session_meta_line(path)
            .await
            .ok()
    ...
    fn validate_dynamic_tools_rejects_duplicate_name_in_same_namespace() {
        ...
        assert!(err.contains("atlas_engine_app"), "unexpected error: {err}");
        assert!(err.contains("my_tool"), "unexpected error: {err}");
    }
    ...
    fn validate_dynamic_tools_rejects_namespace_not_supported_by_responses() {
        ...
        assert!(err.contains("atlas_agent.app"), "unexpected error: {err}");
        assert!(
            err.contains("Responses API") && err.contains("^[a-zA-Z0-9_-]+$"),
            "unexpected error: {err}"
        );
    ...
    fn summary_from_stored_thread_preserves_millisecond_precision() {
        ...
        let stored_thread = StoredThread {
            ...
            thread_source: Some(atlas_engine_protocol::protocol::ThreadSource::User),
            agent_nickname: None,
            agent_role: None,
            agent_path: None,
            git_info: None,
    ...
    fn collect_resume_override_mismatches_includes_service_tier() {
        ...
        let config_snapshot = ThreadConfigSnapshot {
            ...
            approval_policy: atlas_engine_protocol::protocol::AskForApproval::OnRequest,
            approvals_reviewer: atlas_engine_protocol::config_types::ApprovalsReviewer::User,
            permission_profile: atlas_engine_protocol::models::PermissionProfile::Disabled,
            active_permission_profile: None,
    ...
    fn test_thread_metadata(
        ...
        let mut builder = ThreadMetadataBuilder::new(
            ...
            Utc::now(),
            atlas_engine_protocol::protocol::SessionSource::default(),
        );
    ...
    async fn read_summary_from_rollout_preserves_agent_nickname() -> Result<()> {
        ...
        let session_meta = SessionMeta {
            ...
            thread_source: Some(atlas_engine_protocol::protocol::ThreadSource::Subagent),
    ...
    fn summary_from_state_db_metadata_preserves_agent_nickname() -> Result<()> {
        ...
        let summary = summary_from_state_db_metadata(
            ...
            "0.0.0".to_string(),
            source,
            Some(atlas_engine_protocol::protocol::ThreadSource::Subagent),
