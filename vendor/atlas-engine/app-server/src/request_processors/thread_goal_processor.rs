...
use atlas_engine_goal_extension::GoalServiceError;
...
impl ThreadGoalRequestProcessor {
    ...
    async fn state_db_for_materialized_thread(
        ...
        if let Ok(thread) = self.thread_manager.get_thread(thread_id).await {
            ...
            atlas_engine_rollout::find_thread_path_by_id_str(
    ...
    async fn reconcile_thread_goal_rollout(
        ...
        let rollout_path = match running_thread.as_ref() {
            ...
            None => atlas_engine_rollout::find_thread_path_by_id_str(
                &self.config.atlas_agent_home,
                &thread_id.to_string(),
                self.state_db.as_deref(),
            )
        ...
        if let Ok(Some(metadata)) = state_db.get_thread(thread_id).await
            && atlas_engine_rollout::plain_rollout_path(metadata.rollout_path.as_path())
                == atlas_engine_rollout::plain_rollout_path(rollout_path.as_path())
            && let Some(existing_path) =
                atlas_engine_rollout::existing_rollout_path(metadata.rollout_path.as_path()).await
            && atlas_engine_rollout::read_session_meta_line(existing_path.as_path())
                .await
...
pub(super) fn api_thread_goal_from_state(goal: atlas_engine_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
