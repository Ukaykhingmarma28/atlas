...
//! the configured Atlas Agent executable as a subprocess, accepts only valid JSON from
//! `atlas_engine doctor --json`, derives a small set of Sentry tags, and otherwise
...
use atlas_engine_feedback::DOCTOR_REPORT_ATTACHMENT_FILENAME;
use atlas_engine_feedback::FeedbackAttachment;
...
pub(crate) struct DoctorFeedbackReport {
    /// JSON support report to upload as `atlas-engine-doctor-report.json`.
    pub(crate) attachment: FeedbackAttachment,
    /// Low-cardinality Sentry tags derived from the report status and check ids.
    pub(crate) tags: BTreeMap<String, String>,
}

/// Runs `atlas_engine doctor --json` and returns a best-effort feedback attachment.
///
/// Failure to spawn Atlas Agent, finish before the timeout, or parse JSON means the
