...
fn extract_conversation_summary(
    ...
    let preview = head
        ...
        .find_map(|item| match atlas_engine_core::parse_turn_item(&item) {
...
pub(super) fn with_thread_spawn_agent_metadata(
    source: atlas_engine_protocol::protocol::SessionSource,
    agent_nickname: Option<String>,
    agent_role: Option<String>,
) -> atlas_engine_protocol::protocol::SessionSource {
    if agent_nickname.is_none() && agent_role.is_none() {
        return source;
    }

    match source {
        atlas_engine_protocol::protocol::SessionSource::SubAgent(
...
pub(crate) fn thread_response_active_permission_profile(
    active_permission_profile: Option<atlas_engine_protocol::models::ActivePermissionProfile>,
) -> Option<atlas_engine_app_server_protocol::ActivePermissionProfile> {
...
pub(crate) fn thread_settings_from_core_snapshot(
    snapshot: atlas_engine_protocol::protocol::ThreadSettingsSnapshot,
) -> ThreadSettings {
    let atlas_engine_protocol::protocol::ThreadSettingsSnapshot {
        model,
        model_provider_id,
        service_tier,
        approval_policy,
