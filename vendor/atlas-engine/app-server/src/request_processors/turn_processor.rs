...
use super::*;
use atlas_engine_agent_extension::AgentInvocation;
use atlas_engine_agent_extension::AgentRun;
use atlas_engine_agent_extension::AgentRunner;
...
use atlas_engine_protocol::protocol::AdditionalContextEntry as CoreAdditionalContextEntry;
use atlas_engine_protocol::protocol::AdditionalContextKind as CoreAdditionalContextKind;
...
use atlas_engine_skills::system_cache_root_dir;
...
struct ThreadSettingsBuildParams {
    ...
    sandbox_policy: Option<atlas_engine_app_server_protocol::SandboxPolicy>,
...
impl TurnRequestProcessor {
    ...
    fn review_request_from_target(
        ...
        let hint = atlas_engine_core::review_prompts::user_facing_hint(&core_target);
    ...
    async fn turn_start_inner(
        ...
        if turn_has_input && started {
            ...
            atlas_engine_memories_write::start_memories_startup_task(
    ...
    async fn build_thread_settings_overrides(
    ...
    ) -> Result<atlas_engine_protocol::protocol::ThreadSettingsOverrides, JSONRPCErrorError> {
        ...
        let approval_policy =
            approval_policy.map(atlas_engine_app_server_protocol::AskForApproval::to_core);
        let approvals_reviewer =
            approvals_reviewer.map(atlas_engine_app_server_protocol::ApprovalsReviewer::to_core);
        ...
        Ok(atlas_engine_protocol::protocol::ThreadSettingsOverrides {
            environments,
    ...
    async fn thread_settings_update_inner(
        ...
        if thread_settings != atlas_engine_protocol::protocol::ThreadSettingsOverrides::default() {
            self.submit_core_op(
                request_id,
                thread.as_ref(),
                ...
                let (message, data, error_type) = match reason {
                    ...
                    NotSubmittedReason::ActiveTurnNotSteerable { turn_kind } => {
                        let (message, turn_steer_error) = match turn_kind {
                            atlas_engine_protocol::protocol::NonSteerableTurnKind::Review => (
                                "cannot steer a review turn".to_string(),
                                TurnSteerRequestError::NonSteerableReview,
                            ),
                            atlas_engine_protocol::protocol::NonSteerableTurnKind::Compact => (
    ...
    async fn prepare_realtime_conversation_thread(
        ...
        request_id: &ConnectionRequestId,
        thread_id: &str,
    ) -> Result<Option<(ThreadId, Arc<AtlasEngineThread>)>, JSONRPCErrorError> {
    ...
    async fn start_inline_review(
        &self,
        request_id: &ConnectionRequestId,
        parent_thread: Arc<AtlasEngineThread>,
    ...
    async fn start_detached_review(
        ...
        if matches!(
            ...
            atlas_engine_protocol::protocol::ThreadHistoryMode::Paginated
        ) {
            return Err(invalid_request(
    ...
    async fn review_start_inner(
        ...
        match delivery.unwrap_or(ApiReviewDelivery::Inline).to_core() {
            ...
            CoreReviewDelivery::Detached => {
                let review_skill_path = system_cache_root_dir(&self.config.atlas_agent_home)
                    .join("review-agent")
                    .join("SKILL.md");
                let prompt = format!(
