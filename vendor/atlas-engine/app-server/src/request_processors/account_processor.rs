...
const LOGIN_ISSUER_OVERRIDE_ENV_VAR: &str = "ATLAS_AGENT_APP_SERVER_LOGIN_ISSUER";
#[cfg(debug_assertions)]
const LOGIN_OPEN_APP_URL_OVERRIDE_ENV_VAR: &str = "ATLAS_AGENT_APP_SERVER_DEV_OPEN_APP_URL";
...
impl AccountRequestProcessor {
    ...
    fn current_account_updated_notification(&self) -> AccountUpdatedNotification {
        ...
        AccountUpdatedNotification {
            ...
            plan_type: auth.as_ref().and_then(AtlasEngineAuth::account_plan_type),
        }
    }
            ...
            LoginAccountParams::Chatgpt {
                ...
                let login_success_page = if use_hosted_login_success_page {
                    let app_brand = match app_brand.unwrap_or_default() {
                        LoginAppBrand::AtlasEngine => LoginSuccessPageBrand::AtlasEngine,
                        LoginAppBrand::Chatgpt => LoginSuccessPageBrand::Chatgpt,
                    };
                    LoginSuccessPage::Hosted {
                        url: ATLAS_AGENT_OPEN_APP_URL.parse().map_err(|err| {
    ...
    async fn login_chatgpt_common(
        &self,
        atlas_engine_streamlined_login: bool,
    ...
    async fn login_chatgpt_v2(
        ...
        let result = self
            .login_chatgpt_response(atlas_engine_streamlined_login, login_success_page)
            .await;
        self.outgoing.send_result(request_id, result).await;
    }
    ...
    async fn login_chatgpt_response(
        ...
        let opts = self
            .login_chatgpt_common(atlas_engine_streamlined_login, login_success_page)
            .await?;
    ...
    async fn logout_common(&self) -> std::result::Result<Option<AuthMode>, JSONRPCErrorError> {
        ...
        let managed_bedrock_auth = matches!(
            self.auth_manager.auth_cached(),
            Some(AtlasEngineAuth::BedrockApiKey(_))
        );
    ...
    async fn get_account_rate_limits_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "atlas-agent account authentication required to read rate limits",
            ));
        };
    ...
    async fn get_account_token_usage_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "atlas-agent account authentication required to read token usage",
            ));
        };
        ...
        if let Some(thread_id) = thread_id {
            ...
            let thread_usage = match usage {
                Ok(usage) => Some(atlas_engine_app_server_protocol::ThreadUsage {
                    ...
                    groups: usage
                        ...
                        .into_iter()
                        .map(
                            |group| atlas_engine_app_server_protocol::ThreadUsageBreakdownGroup {
                                model: group.model,
    ...
    async fn get_workspace_messages_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "atlas-agent account authentication required to read workspace messages",
            ));
        };
    ...
    async fn send_add_credits_nudge_email_inner(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "atlas-agent account authentication required to notify workspace owner",
            ));
        };
...
mod tests {
    use super::*;
    use atlas_engine_backend_client::TokenUsageProfileDailyBucket;
    use atlas_engine_backend_client::TokenUsageProfileStats;
