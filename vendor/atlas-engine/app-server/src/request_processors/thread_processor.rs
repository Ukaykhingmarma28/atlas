...
use atlas_engine_app_server_protocol::SelectedCapabilityRoot;
...
use atlas_engine_app_server_protocol::ThreadSectionAppearance;
...
const ATLAS_AGENT_TUI_CLIENT_NAME: &str = "atlas-engine-tui";
...
fn collect_resume_override_mismatches(
    ...
    if let Some(requested_review_policy) = request.approvals_reviewer.as_ref() {
        let active_review_policy: atlas_engine_app_server_protocol::ApprovalsReviewer =
    ...
    if let Some(requested_sandbox) = request.sandbox.as_ref() {
        ...
        let sandbox_matches = matches!(
            ...
            (
                ...
                atlas_engine_protocol::protocol::SandboxPolicy::DangerFullAccess
            ) | (
                SandboxMode::DangerFullAccess,
...
fn latest_persisted_approval_policy(
    history: &[RolloutItem],
) -> Option<atlas_engine_protocol::protocol::AskForApproval> {
    history
        .iter()
        .enumerate()
        .rev()
...
fn validate_dynamic_tools(tools: &[DynamicToolSpec]) -> Result<(), String> {
    ...
    fn validate_dynamic_tool<'a>(
        ...
        if let Err(err) = atlas_engine_tools::parse_tool_input_schema(&tool.input_schema) {
            return Err(format!(
...
impl ThreadRequestProcessor {
    ...
    pub(crate) async fn thread_rollback(
        ...
        if app_server_client_name != Some(ATLAS_AGENT_TUI_CLIENT_NAME) {
    ...
    async fn load_thread(
        &self,
        thread_id: &str,
    ) -> Result<(ThreadId, Arc<AtlasEngineThread>), JSONRPCErrorError> {
    ...
    fn listener_task_context(&self) -> ListenerTaskContext {
        ListenerTaskContext {
            ...
            atlas_agent_home: self.config.atlas_agent_home.to_path_buf(),
    ...
    async fn thread_start_inner(
        ...
        if matches!(
            history_mode,
            Some(atlas_engine_app_server_protocol::ThreadHistoryMode::Paginated)
    ...
    async fn request_trace_context(
        &self,
        request_id: &ConnectionRequestId,
    ) -> Option<atlas_engine_protocol::protocol::W3cTraceContext> {
    ...
    async fn thread_start_task(
        ...
        session_start_source: Option<atlas_engine_app_server_protocol::ThreadStartSource>,
        thread_source: Option<atlas_engine_protocol::protocol::ThreadSource>,
        ...
        let effective_permissions_trust_project = match &effective_permission_profile {
            atlas_engine_protocol::models::PermissionProfile::Disabled
            | atlas_engine_protocol::models::PermissionProfile::External { .. } => true,
            atlas_engine_protocol::models::PermissionProfile::Managed { .. } => {
            ...
            && effective_permissions_trust_project
        {
            ...
            let cli_overrides_for_reload = if let Err(err) =
                atlas_engine_core::config::set_project_trust_level(
                    &listener_task_context.atlas_agent_home,
        ...
        if let Ok(Some(err)) =
            atlas_engine_core::check_execpolicy_for_warnings(&config.config_layer_stack).await
        {
    ...
    fn build_thread_config_overrides(
        ...
        approval_policy: Option<atlas_engine_app_server_protocol::AskForApproval>,
        approvals_reviewer: Option<atlas_engine_app_server_protocol::ApprovalsReviewer>,
        ...
        ConfigOverrides {
            ...
            approval_policy: approval_policy
                .map(atlas_engine_app_server_protocol::AskForApproval::to_core),
            approvals_reviewer: approvals_reviewer
                .map(atlas_engine_app_server_protocol::ApprovalsReviewer::to_core),
    ...
    ) -> Result<(ThreadSetNameResponse, Option<ThreadNameUpdatedNotification>), JSONRPCErrorError>
    {
        ...
        let thread_id = ThreadId::from_string(&thread_id)
            .map_err(|err| invalid_request(format!("invalid thread id: {err}")))?;
        let Some(name) = atlas_engine_core::util::normalize_thread_name(&name) else {
    ...
    async fn memory_reset_response_inner(&self) -> Result<MemoryResetResponse, JSONRPCErrorError> {
        ...
        clear_memory_roots_contents(&self.config.atlas_agent_home)
            .await
            .map_err(|err| {
                internal_error(format!(
                    "failed to clear memory directories under {}: {err}",
                    self.config.atlas_agent_home.display()
                ))
            })?;
    ...
    async fn thread_revert_response(
        ...
        let revert_result = self
            .thread_store
            .revert_thread(atlas_engine_thread_store::RevertThreadParams {
                thread_id,
                before_turn_id,
            })
            .await
    ...
    async fn load_live_thread_view(
        ...
        thread_id: ThreadId,
        include_turns: bool,
        loaded_thread: &AtlasEngineThread,
    ...
    async fn apply_thread_read_store_fields(
        ...
        if include_turns {
            if matches!(
                thread.history_mode,
                atlas_engine_app_server_protocol::ThreadHistoryMode::Paginated
            ) {
                self.thread_store
    ...
    ) -> Result<atlas_engine_app_server_protocol::TurnsPage, JSONRPCErrorError> {
    ...
    async fn thread_resume_inner(
            ...
            .await
        {
            Ok(NewThread {
                ...
                let instruction_sources = atlas_engine_thread.legacy_instruction_sources().await;
                let SessionConfiguredEvent { rollout_path, .. } = session_configured;
                ...
                thread.thread_source = atlas_engine_thread
                    .config_snapshot()
                    .await
                    .thread_source
                    .map(Into::into);
                ...
                let config_snapshot = atlas_engine_thread.config_snapshot().await;
                let (turns_backwards_cursor, items_backwards_cursor) =
                    if matches!(config_snapshot.history_mode, ThreadHistoryMode::Paginated) {
    ...
    async fn read_stored_thread_for_resume(
            ...
            && matches!(stored_thread.history_mode, ThreadHistoryMode::Paginated)
        {
            ...
            if let Some(current_path) = current_thread.rollout_path.as_ref()
                && !path_utils::paths_match_after_normalization(
                    atlas_engine_rollout::plain_rollout_path(requested_path).as_path(),
                    atlas_engine_rollout::plain_rollout_path(current_path).as_path(),
                )
            {
                return Err(invalid_request(format!(
        ...
        if stored_thread.archived_at.is_some() {
            ...
            return Err(invalid_request(format!(
                "session {thread_id} is archived. Run `atlas_engine unarchive {thread_id}` to unarchive it first."
            )));
        }
    ...
    async fn thread_fork_inner(
        ...
        let source_thread_name = source_thread
            .name
            .as_deref()
            .and_then(atlas_engine_core::util::normalize_thread_name);
        let prepared_fork = if paginated_source {
            let boundary = match (last_turn_id.as_deref(), before_turn_id.as_deref()) {
                (Some(turn_id), None) => {
                    atlas_engine_thread_store::ForkBoundary::ThroughTurn(turn_id.to_string())
                }
                (None, Some(turn_id)) => {
                    atlas_engine_thread_store::ForkBoundary::BeforeTurn(turn_id.to_string())
                }
                (None, None) => atlas_engine_thread_store::ForkBoundary::Latest,
                (Some(_), Some(_)) => unreachable!("fork boundaries are mutually exclusive"),
            };
            Some(
                self.thread_store
                    .prepare_fork(atlas_engine_thread_store::PrepareForkParams {
        ...
        let NewThread {
            ...
            AtlasEngineErrorDetails::Io(_) | AtlasEngineErrorDetails::Json(_) => {
...
fn deserialize_stored_thread_item(
    item: atlas_engine_thread_store::StoredThreadItem,
...
fn stored_turn_to_api_turn(
    ...
    let error = turn.error.map(|error| TurnError {
        ...
        atlas_engine_error_info: error.atlas_engine_error_info,
...
pub(super) fn core_thread_write_error(operation: &str, err: AtlasEngineErr) -> JSONRPCErrorError {
    match err.details() {
        AtlasEngineErrorDetails::ThreadNotFound(thread_id) => {
            invalid_request(format!("thread not found: {thread_id}"))
        }
        AtlasEngineErrorDetails::InvalidRequest(message) => invalid_request(message.clone()),
        AtlasEngineErrorDetails::UnsupportedOperation(message) => method_not_found(message.clone()),
...
fn summary_from_state_db_metadata(
    ...
    cli_version: String,
    source: String,
    _thread_source: Option<atlas_engine_protocol::protocol::ThreadSource>,
    ...
    let source = serde_json::from_str(&source)
        ...
        .unwrap_or(atlas_engine_protocol::protocol::SessionSource::Unknown);
...
fn build_thread_from_snapshot(
    thread_id: ThreadId,
    session_id: String,
    multi_agent_version: Option<atlas_engine_protocol::protocol::MultiAgentVersion>,
    config_snapshot: &ThreadConfigSnapshot,
    path: Option<PathBuf>,
) -> Thread {
