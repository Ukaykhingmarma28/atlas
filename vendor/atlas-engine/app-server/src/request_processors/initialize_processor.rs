...
impl InitializeRequestProcessor {
    ...
    pub(crate) async fn initialize(
        ...
        let client_mcp_extensions = atlas_engine_mcp::client_mcp_extensions(
            extensions,
        ...
        let atlas_agent_home = self.config.atlas_agent_home.clone();
        if session
            ...
            if let Err(error) = set_default_originator(originator.clone()) {
                match error {
                    ...
                    SetOriginatorError::AlreadyInitialized => {
                        ...
                        // TODO(owen): Once we remove support for ATLAS_AGENT_INTERNAL_ORIGINATOR_OVERRIDE,
