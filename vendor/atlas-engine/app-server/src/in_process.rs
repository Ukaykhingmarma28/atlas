...
//!
//! # Relationship to `atlas-engine-app-server-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `atlas-engine-app-server-client`,
...
use atlas_engine_app_server_protocol::Result;
...
pub use atlas_engine_rollout::StateDbHandle;
pub use atlas_engine_state::log_db::LogDbLayer;
use tokio::sync::mpsc;
...
/// through `atlas-engine-app-server-client`, which adds worker-task buffering,
...
pub struct InProcessClientHandle {
    ...
    #[cfg(test)]
    _test_atlas_agent_home: Option<tempfile::TempDir>,
}
...
async fn start_uninitialized(
    ...
    let installation_id = resolve_installation_id(&args.config.atlas_agent_home).await?;
    let auth_manager =
        AuthManager::shared_from_config(args.config.as_ref(), args.enable_atlas_engine_api_key_env)
            .await
    ...
    let runtime_handle = tokio::spawn(async move {
        ...
        let config_manager = ConfigManager::new(
            args.config.atlas_agent_home.to_path_buf(),
            args.cli_overrides,
...
mod tests {
    ...
    async fn start_test_client_with_capacity(
        ...
        let config = Arc::new(build_test_config(atlas_agent_home.path()).await);
        let state_db = atlas_engine_rollout::state_db::try_init(config.as_ref())
            .await
        ...
        let args = InProcessStartArgs {
            ...
            initialize: InitializeParams {
                client_info: ClientInfo {
                    name: "atlas-engine-in-process-test".to_string(),
                    title: None,
        ...
        client._test_atlas_agent_home = Some(atlas_agent_home);
        client
    }
