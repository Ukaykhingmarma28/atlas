...
use atlas_engine_extension_api::AgentSpawnFuture;
use atlas_engine_extension_api::AgentSpawner;
...
use atlas_engine_queue_extension::QueuedItemService;
...
pub(crate) struct ThreadExtensionDependencies {
    ...
    pub(crate) executor_skill_provider: Arc<dyn atlas_engine_skills_extension::SkillProvider>,
...
where
    S: AgentSpawner<StartThreadOptions, Spawned = NewThread, Error = AtlasEngineErr> + 'static,
{
    ...
    if let Some(queue_service) = queue_service {
        atlas_engine_queue_extension::install(&mut builder, queue_service);
    }
    if let Some(state_db) = state_db {
        atlas_engine_goal_extension::install_with_backend(
            &mut builder,
            state_db,
            analytics_events_client,
            atlas_engine_otel::global(),
    ...
    atlas_engine_git_attribution::install(
        &mut builder,
        auth_manager.clone(),
        git_attribution_base_url,
        http_client_factory,
    );
    atlas_engine_guardian::install(&mut builder, guardian_agent_spawner);
    atlas_engine_guardian_v2::install(&mut builder, auth_manager.clone(), thread_manager);
    atlas_engine_memories_extension::install(&mut builder, atlas_engine_otel::global());
    ...
    atlas_engine_skills_extension::install_with_providers_and_metrics(
        ...
        skill_providers,
        atlas_engine_otel::global(),
        |config: &Config| atlas_engine_skills_extension::SkillsExtensionConfig {
            include_instructions: config.include_skill_instructions,
            bundled_skills_enabled: config.bundled_skills_enabled(),
            orchestrator_skills_enabled: config.orchestrator_skills_enabled,
            shadow_selection_enabled: config
                .features
                .enabled(atlas_engine_features::Feature::SkillSearch),
        },
    );
...
) -> impl AgentSpawner<StartThreadOptions, Spawned = NewThread, Error = AtlasEngineErr> {
    move |forked_from_thread_id: ThreadId,
          options: StartThreadOptions|
          -> AgentSpawnFuture<'static, NewThread, AtlasEngineErr> {
...
#[cfg(test)]
mod tests {
    use atlas_engine_protocol::protocol::ThreadGoal as CoreThreadGoal;
