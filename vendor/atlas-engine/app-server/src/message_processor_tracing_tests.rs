...
fn init_test_tracing() -> &'static TestTracing {
    ...
    TEST_TRACING.get_or_init(|| {
        ...
        let provider = SdkTracerProvider::builder()
            ...
            .build();
        let tracer = provider.tracer("atlas-engine-app-server-message-processor-tests");
...
impl TracingHarness {
    async fn new() -> Result<Self> {
        ...
        let config = Arc::new(build_test_config(atlas_agent_home.path(), &server.uri()).await?);
...
async fn build_test_config(atlas_agent_home: &Path, server_uri: &str) -> Result<Config> {
        ...
        match envelope {
            crate::outgoing_message::OutgoingEnvelope::ToConnection {
                ...
                if matches!(
                    ...
                    atlas_engine_app_server_protocol::ServerNotification::ThreadStarted(_)
                ) {
                    return;
                }
            }
