...
async fn write_value_preserves_comments_and_order() -> Result<()> {
    ...
    let original = r#"# Atlas Agent user configuration
model = "gpt-5.2"
...
"#;
    ...
    let expected = r#"# Atlas Agent user configuration
model = "gpt-5.2"
approval_policy = "on-request"
...
async fn psp_feature_configures_first_party_routing() -> Result<()> {
    ...
    assert!(config.features.enabled(atlas_engine_features::Feature::Psp));
    assert_eq!(
...
async fn batch_write_rejects_legacy_profile_selector() -> Result<()> {
    ...
    let error = service
        .batch_write(ConfigBatchWriteParams {
            edits: vec![
                atlas_engine_app_server_protocol::ConfigEdit {
...
async fn managed_auth_policy_survives_unusable_requirements_file_changes() -> Result<()> {
    ...
    let auth_manager = atlas_engine_login::AuthManager::shared_from_config(
        &startup, /*enable_atlas_engine_api_key_env*/ false,
    )
    .await?;
    std::fs::write(
        &requirements_path,
