...
use atlas_engine_cloud_config::cloud_config_bundle_loader;
...
use atlas_engine_utils_json_to_toml::json_to_toml;
...
/// Shared app-server entry point for loading effective Atlas Agent configuration.
#[derive(Clone)]
...
impl ConfigManager {
    ...
    pub(crate) fn atlas_agent_home(&self) -> &Path {
    ...
    fn current_thread_config_loader(&self) -> Arc<dyn ThreadConfigLoader> {
        self.thread_config_loader
            ...
            .unwrap_or_else(|_| Arc::new(atlas_engine_config::NoopThreadConfigLoader))
    }
    ...
    pub(crate) async fn load_default_config(&self) -> std::io::Result<Config> {
        ...
        let mut config = ConfigBuilder::default()
            ...
            .fallback_cwd(Some(self.atlas_agent_home.clone()))
    ...
    pub(crate) async fn load_with_cli_overrides(
        ...
        let merged_cli_overrides = cli_overrides
            ...
            .collect::<Vec<_>>();
        let mut config = atlas_engine_core::config::ConfigBuilder::default()
            .atlas_agent_home(self.atlas_agent_home.clone())
    ...
    pub(crate) async fn load_config_layers(
        ...
        load_config_layers_state(
            ...
            cwd,
            &self.current_cli_overrides(),
            atlas_engine_config::ConfigLoadOptions {
                loader_overrides: self.loader_overrides.clone(),
                strict_config: self.strict_config,
                cloud_config_bundle: self.current_cloud_config_bundle(),
            },
    ...
    pub(crate) fn new_for_tests(
        ...
        Self::new(
            ...
            Arc::new(atlas_engine_config::NoopThreadConfigLoader),
        )
    }

    #[cfg(test)]
    pub(crate) fn without_managed_config_for_tests(atlas_agent_home: PathBuf) -> Self {
        Self::new_for_tests(
            atlas_agent_home,
            Vec::new(),
