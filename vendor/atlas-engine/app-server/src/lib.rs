...
use atlas_engine_code_mode::GrpcCodeModeSessionProvider;
use atlas_engine_code_mode::WebSocketCodeModeSessionProvider;
...
use atlas_engine_app_server_protocol::TextPosition as AppTextPosition;
use atlas_engine_app_server_protocol::TextRange as AppTextRange;
use atlas_engine_config::ConfigLayerSource;
use atlas_engine_config::ConfigLoadError;
use atlas_engine_config::TextRange as CoreTextRange;
use atlas_engine_core::ExecPolicyError;
use atlas_engine_core::check_execpolicy_for_warnings;
use atlas_engine_core::config::find_atlas_agent_home;
...
const SQLITE_RECOVERY_CONFIG_WARNING_SUMMARY: &str = "Atlas Agent rebuilt its local database.";
...
const OTEL_SERVICE_NAME: &str = "atlas-engine-app-server";
#[cfg(debug_assertions)]
const TEST_USER_CONFIG_FILE_ENV_VAR: &str = "ATLAS_AGENT_APP_SERVER_TEST_USER_CONFIG_FILE";
...
fn project_config_warning(config: &Config) -> Option<ConfigWarningNotification> {
    ...
    for layer in config.config_layer_stack.all_layers_low_to_high() {
        ...
        disabled_folders.push((
            dot_atlas_agent_folder.as_path().display().to_string(),
...
pub async fn run_main_with_transport_options(
    ...
    let local_runtime_paths = ExecServerRuntimePaths::from_optional_paths(
        arg0_paths.atlas_engine_self_exe.clone(),
        arg0_paths.atlas_engine_linux_sandbox_exe.clone(),
    )?;
    ...
    let otel = atlas_engine_core::otel_init::build_provider(
        &config,
    ...
    atlas_engine_core::otel_init::record_process_start(otel.as_ref(), OTEL_SERVICE_NAME);
    atlas_engine_core::otel_init::install_sqlite_telemetry(otel.as_ref(), OTEL_SERVICE_NAME);
    let unix_socket_startup_lock = match &transport {
        AppServerTransport::UnixSocket { socket_path } => {
            let startup_lock_path = app_server_startup_lock_path(&atlas_agent_home)?;
    ...
    if let Some(warning) =
        atlas_engine_core::config::system_bwrap_warning(config.permissions.permission_profile())
    {
        config_warnings.push(ConfigWarningNotification {
            summary: warning,
            details: None,
    ...
    let feedback = AtlasEngineFeedback::new();
...
async fn init_sqlite_state_db_with_fresh_start_on_corruption(
    ...
    loop {
        ...
        let database_path = atlas_engine_state::runtime_db_path_for_corruption_error(&err)
            .unwrap_or_else(|| config.sqlite_config().state_db_path());
        if !atlas_engine_state::is_sqlite_corruption_error(&err)
        ...
        emit_state_db_backup_warning(&format!(
            "Atlas Agent local database at {} appears damaged. Moving it into a backup folder so the app server can rebuild it from saved data.",
            database_path.display()
        ));
        let backups = atlas_engine_state::backup_runtime_db_for_fresh_start(database_path.as_path())
            .await
            .map_err(|backup_err| {
                anyhow::anyhow!(
        ...
        for backup in &backups {
            emit_state_db_backup_warning(&format!(
                "Moved damaged Atlas Agent local database file {} to {}",
...
mod tests {
    ...
    fn debug_test_user_config_file_overrides_loader_path() {
        let path = std::env::temp_dir().join("atlas-engine-app-server-test-config.toml");
        let loader_overrides = loader_overrides_with_test_user_config_file(
