...
use atlas_engine_app_server_protocol::Config as ApiConfig;
...
use atlas_engine_app_server_protocol::OverriddenMetadata;
...
use atlas_engine_config::ConfigLayerMetadata;
...
use atlas_engine_config::ShellEnvironmentPolicyFilterRepresentation;
use atlas_engine_config::config_toml::ConfigToml;
use atlas_engine_config::merge_toml_values;
use atlas_engine_config::shell_environment_filter_entry;
use atlas_engine_config::validate_shell_environment_policy_filter_config;
use atlas_engine_core::config::deserialize_config_toml_with_base;
use atlas_engine_core::config::edit::ConfigEdit;
use atlas_engine_core::config::edit::ConfigEditsBuilder;
use atlas_engine_core::config::validate_feature_requirements_for_config_toml;
use atlas_engine_core::path_utils;
use atlas_engine_core::path_utils::SymlinkWritePaths;
use atlas_engine_core::path_utils::resolve_symlink_write_paths;
use atlas_engine_core::path_utils::write_atomically;
...
impl ConfigManager {
    ...
    async fn apply_edits(
        ...
        let user_config_toml =
            deserialize_config_toml_with_base(user_config.clone(), self.atlas_agent_home()).map_err(
                |err| {
                    ConfigManagerError::write(
                        ConfigWriteErrorCode::ConfigValidationError,
                        format!("Invalid configuration: {err}"),
    ...
    /// include any in-repo .atlas-agent/ folders because there is no cwd/project root
...
fn override_message(layer: &ConfigLayerSource) -> String {
    match layer {
        ...
        ConfigLayerSource::Project { dot_atlas_agent_folder } => format!(
            "Overridden by project config: {}/{CONFIG_TOML_FILE}",
            dot_atlas_agent_folder.display(),
        ),
