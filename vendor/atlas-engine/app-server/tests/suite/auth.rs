...
fn create_config_toml(atlas_agent_home: &Path) -> std::io::Result<()> {
...
async fn get_auth_status_no_auth() -> Result<()> {
    ...
    create_config_toml(atlas_agent_home.path())?;
...
async fn get_auth_status_omits_token_after_permanent_refresh_failure() -> Result<()> {
    ...
    let second_status: GetAuthStatusResponse =
...
async fn get_auth_status_returns_token_after_proactive_refresh_recovery() -> Result<()> {
    ...
    let failed_status: GetAuthStatusResponse =
        timeout(DEFAULT_READ_TIMEOUT, mcp.read_response(failed_request_id)).await??;
    assert_eq!(
        failed_status,
        GetAuthStatusResponse {
            auth_method: Some(AuthMode::Chatgpt),
    ...
    let recovered_status: GetAuthStatusResponse = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_response(recovered_request_id),
    )
    .await??;
    assert_eq!(
        recovered_status,
...
async fn login_api_key_rejected_when_forced_chatgpt() -> Result<()> {
    let atlas_agent_home = TempDir::new()?;
    create_config_toml_forced_login(atlas_agent_home.path(), "chatgpt")?;
