...
use atlas_engine_app_server_protocol::SessionSource;
use atlas_engine_app_server_protocol::SortDirection;
use atlas_engine_app_server_protocol::ThreadActiveFlag;
...
use atlas_engine_app_server_protocol::ThreadStatusChangedNotification;
...
use atlas_engine_core::ARCHIVED_SESSIONS_SUBDIR;
...
use atlas_engine_protocol::protocol::SessionSource as RolloutSessionSource;
...
use atlas_engine_rollout::append_rollout_item_to_path;
...
const ATLAS_AGENT_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: &str = "You are Atlas Agent, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
-> Result<()> {
    ...
    let live_cwd = normalized_existing_path(atlas_agent_home.path())?;
    mock_responses_config(&server.uri()).write(atlas_agent_home.path())?;
    let config_path = atlas_agent_home.path().join("config.toml");
...
async fn goal_first_live_thread_appears_in_state_db_thread_list() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let atlas_agent_home = TempDir::new()?;
    let atlas_agent_home_path = normalized_existing_path(atlas_agent_home.path())?;
    mock_responses_config(&server.uri()).write(&atlas_agent_home_path)?;
    let config_path = atlas_agent_home_path.join("config.toml");
    let config = std::fs::read_to_string(&config_path)?;
    std::fs::write(
        &config_path,
        config.replace("personality = true\n", "personality = true\ngoals = true\n"),
    ...
    let sqlite_home = atlas_agent_home_path
        .as_path()
        .to_str()
        .expect("test atlas-agent home should be utf-8");
    let mut mcp = TestAppServer::builder()
        .with_atlas_agent_home(&atlas_agent_home_path)
        .without_managed_config()
        .with_env_overrides(&[("ATLAS_AGENT_SQLITE_HOME", Some(sqlite_home))])
        .build_initialized()
        .await?;
...
async fn thread_resume_running_thread_tracks_thread_originator_in_analytics() -> Result<()> {
    ...
    let event = wait_for_matching_analytics_event(&server, DEFAULT_READ_TIMEOUT, |event| {
        event["event_type"] == "atlas_engine_thread_initialized"
...
fn set_session_meta_on_fake_rollout(
    ...
    let path = rollout_path(atlas_agent_home, filename_ts, thread_id);
...
fn append_resume_redaction_history(
    ...
    let rollout_file_path = rollout_path(atlas_agent_home, filename_ts, conversation_id);
...
async fn thread_resume_rejects_archived_session_by_id() -> Result<()> {
    ...
    let active_rollout_path = rollout_path(atlas_agent_home.path(), filename_ts, &conversation_id);
    ...
    assert!(
        message.contains(&format!("session {conversation_id} is archived"))
            && message.contains(&format!(
                "atlas-agent unarchive {conversation_id}` to unarchive it first"
            )),
...
async fn thread_goal_set_edits_objective_without_resetting_usage() -> Result<()> {
    ...
    state_db
        ...
        .account_thread_goal_usage(
            ...
            /*token_delta*/ 50,
            atlas_engine_state::GoalAccountingMode::ActiveOnly,
...
async fn thread_goal_lifecycle_emits_analytics_and_clear_deletes_goal() -> Result<()> {
    ...
    let get_id = mcp
        ...
        .await?;
    let get: atlas_engine_app_server_protocol::ThreadGoalGetResponse =
...
async fn cold_paginated_resume_omits_usage_when_its_turn_is_ambiguous() -> Result<()> {
    ...
    let path = rollout_path(atlas_agent_home.path(), filename_ts, &conversation_id);
    append_rollout_item_to_path(
        &path,
        &RolloutItem::EventMsg(EventMsg::TokenCount(TokenCountEvent {
            info: Some(TokenUsageInfo {
...
async fn thread_resume_token_usage_replay_ignores_stale_interrupted_tail_turn() -> Result<()> {
    ...
    let rollout_file_path = rollout_path(atlas_agent_home.path(), filename_ts, &conversation_id);
    let persisted_rollout = std::fs::read_to_string(&rollout_file_path)?;
...
async fn thread_resume_prefers_persisted_git_metadata_for_local_threads() -> Result<()> {
    ...
    let repo_path = atlas_agent_home.path().join("repo");
    ...
    let rollout_path = rollout_path(atlas_agent_home.path(), "2025-01-05T12-00-00", &thread_id);
    ...
    let session_meta = SessionMeta {
        ...
        cwd: repo_path.clone(),
        originator: "atlas-agent".to_string(),
...
async fn thread_resume_defers_updated_at_until_turn_start() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let atlas_agent_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(atlas_agent_home.path(), &server.uri()).await?;
...
async fn thread_resume_with_overrides_defers_updated_at_until_turn_start() -> Result<()> {
    ...
    let RestartedThreadFixture {
        ...
        rollout_file_path,
        updated_at,
    } = start_materialized_thread_and_restart(atlas_agent_home.path(), "materialize").await?;
...
command = "atlas-engine-definitely-not-a-real-binary"
required = true"#,
        )
        .write(atlas_agent_home.path())?;
...
async fn thread_resume_supports_history_and_overrides() -> Result<()> {
    ...
    let RestartedThreadFixture {
        mut mcp, thread_id, ..
    } = start_materialized_thread_and_restart(atlas_agent_home.path(), "seed history").await?;
...
async fn thread_resume_accepts_personality_override() -> Result<()> {
    ...
    assert!(
        instructions_text.contains(ATLAS_AGENT_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT),
