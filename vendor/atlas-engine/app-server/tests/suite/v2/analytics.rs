...
use atlas_engine_config::types::OtelHttpProtocol;
use atlas_engine_core::config::ConfigBuilder;
use atlas_engine_core_plugins::loader::curated_plugin_cache_version;
...
fn set_metrics_exporter(config: &mut atlas_engine_core::config::Config) {
...
async fn app_server_default_analytics_disabled_without_flag() -> Result<()> {
    ...
    let provider = atlas_engine_core::otel_init::build_provider(
        &config,
        SERVICE_VERSION,
        Some("atlas-engine-app-server"),
...
pub(crate) async fn wait_for_goal_event(
    ...
    wait_for_matching_analytics_event(server, read_timeout, |event| {
        event["event_type"] == "atlas_engine_goal_event"
...
pub(crate) fn thread_initialized_event(payload: &Value) -> Result<&Value> {
    ...
    events
        ...
        .ok_or_else(|| anyhow::anyhow!("atlas_engine_thread_initialized event should be present"))
}
...
fn write_curated_metrics_plugin(atlas_agent_home: &Path) -> Result<PathBuf> {
    let plugin_id = PluginId::parse(METRICS_PLUGIN_ID)?;
    let plugin_root = PluginStore::new(atlas_agent_home.to_path_buf()).plugin_root(
        &plugin_id,
...
"#,
    ...
    std::fs::write(
        &script_path,
        r#"test -n "$ATLAS_AGENT_PLUGIN_METRICS_OUTPUT"
sleep "${1:-0.3}"
printf '%s' '{"version":1,"measurements":[{"name":"findings","value":3,"dimensions":{"severity":"high"}},{"name":"files_scanned","value":17}]}' > "$ATLAS_AGENT_PLUGIN_METRICS_OUTPUT"
"#,
    )?;

    let curated_repo = atlas_agent_home.join(".tmp/plugins");
...
async fn assert_plugin_measurement_analytics(runtime: PluginMetricsRuntime) -> Result<()> {
    ...
    skip_if_remote!(
        Ok(()),
        "trusted plugin metrics fixture uses a local Atlas Agent home cache"
    );
    skip_if_wine_exec!(Ok(()), "plugin metrics fixture is Unix-only");

    let atlas_agent_home = TempDir::new()?;
    let script_path = write_curated_metrics_plugin(atlas_agent_home.path())?.canonicalize()?;
...
"#,
    ...
    mount_analytics_capture(&analytics_server, atlas_agent_home.path()).await?;
    ...
    if remote {
        assert_eq!(
            ...
            atlas_engine_exec_server::REMOTE_ENVIRONMENT_ID
        );
    }
    ...
    for measurement_name in ["findings", "files_scanned"] {
        wait_for_matching_analytics_event(&analytics_server, Duration::from_secs(10), |event| {
            event["event_type"] == "atlas_engine_plugin_measurement_event"
    ...
    let command_event =
        wait_for_matching_analytics_event(&analytics_server, Duration::from_secs(10), |event| {
            event["event_type"] == "atlas_engine_command_execution_event"
                && event["event_params"]["item_id"] == call_id
        })
        .await?;
    assert_eq!(
