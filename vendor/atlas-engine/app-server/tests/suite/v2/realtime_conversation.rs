...
use atlas_engine_app_server_protocol::ThreadRealtimeAudioChunk;
...
use atlas_engine_app_server_protocol::ThreadRealtimeInitialItem;
...
const STARTUP_CONTEXT_HEADER: &str = "Startup context from Atlas Agent.";
...
impl RealtimeE2eHarness {
    ...
    async fn start_webrtc_realtime(&mut self, offer_sdp: &str) -> Result<StartedWebrtcRealtime> {
        self.start_webrtc_realtime_with_atlas_engine_response_routing(
            offer_sdp,
    ...
    async fn start_webrtc_realtime_with_atlas_engine_response_items(
        &mut self,
        offer_sdp: &str,
        ...
        self.start_webrtc_realtime_with_atlas_engine_response_routing(
            offer_sdp,
            /*client_managed_handoffs*/ None,
            /*atlas_engine_responses_as_items*/ Some(true),
            /*atlas_engine_response_handoff_mode*/ None,
            /*delegation_ack_filler*/ None,
            RealtimeConversationVersion::V1,
        )
    ...
    async fn start_webrtc_realtime_with_atlas_engine_response_routing(
        &mut self,
        offer_sdp: &str,
        client_managed_handoffs: Option<bool>,
        atlas_engine_responses_as_items: Option<bool>,
        atlas_engine_response_handoff_mode: Option<AtlasEngineResponseHandoffMode>,
        ...
        let start_request_id = self
            ...
            .send_thread_realtime_start_request(ThreadRealtimeStartParams {
                ...
                atlas_engine_response_item_prefix: atlas_engine_responses_as_items
                    .unwrap_or(false)
    ...
    async fn start_websocket_realtime(&mut self) -> Result<ThreadRealtimeStartedNotification> {
        self.start_websocket_realtime_with_atlas_engine_responses_as_items(
            /*atlas_engine_responses_as_items*/ None,
        )
        .await
    }

    async fn start_websocket_realtime_with_atlas_engine_response_items(
        &mut self,
    ...
    async fn start_websocket_realtime_with_atlas_engine_responses_as_items(
        &mut self,
        atlas_engine_responses_as_items: Option<bool>,
    ) -> Result<ThreadRealtimeStartedNotification> {
        let start_request_id = self
...
async fn realtime_webrtc_start_emits_sdp_notification() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some("multipart/form-data; boundary=atlas-engine-realtime-call-boundary")
    );
    ...
    assert_eq!(
        body,
        format!(
            "--atlas-engine-realtime-call-boundary\r\n\
...
async fn webrtc_v3_start_posts_live_session_and_joins_without_session_update() -> Result<()> {
    ...
    let started = harness
        .start_webrtc_realtime_with_atlas_engine_response_routing(
            "v=offer\r\n",
            /*client_managed_handoffs*/ None,
            /*atlas_engine_responses_as_items*/ None,
            /*atlas_engine_response_handoff_mode*/ None,
...
async fn webrtc_v1_client_managed_handoffs_disable_automatic_output() -> Result<()> {
    ...
    assert!(
        ...
        "automatic Atlas Agent output should not reach realtime in client-managed handoff mode"
    );
...
#[tokio::test]
async fn webrtc_v1_ignores_atlas_engine_response_handoff_mode() -> Result<()> {
    skip_if_no_network!(Ok(()));
...
async fn webrtc_v1_handoff_request_delegates_context_and_manual_append_speaks() -> Result<()> {
    ...
    let started = harness
        .start_webrtc_realtime_with_atlas_engine_response_items("v=offer\r\n")
        .await?;
    assert_eq!(started.started.version, RealtimeConversationVersion::V1);
    assert_call_create_multipart(
        harness.call_capture.single_request(),
...
async fn realtime_automatic_standalone_output_is_item_and_append_speaks() -> Result<()> {
    ...
    let started = harness
        .start_websocket_realtime_with_atlas_engine_response_items()
        .await?;
    assert_eq!(started.version, RealtimeConversationVersion::V2);
    assert_eq!(
...
async fn websocket_v3_routes_handoffs_by_session_mode() -> Result<()> {
    ...
    for (mode, channel_prefixes, texts, expected_channels) in [
        ...
        (
            Some(AtlasEngineResponseHandoffMode::Commentary),
            None,
            [
                "[ANALYSIS]silent context",
                "[COMMENTARY]still working",
...
async fn websocket_v2_background_agent_steering_ack_requests_response_create() -> Result<()> {
    ...
    // Phase 4: release the gated delegated turn. Atlas Agent should then continue
