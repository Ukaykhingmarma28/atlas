...
use atlas_engine_core::exec_env::ATLAS_AGENT_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR;
    ...
    let command_request_id = mcp
        .send_command_exec_request(CommandExecParams {
            command: vec![
                "/bin/sh".to_string(),
                "-lc".to_string(),
                "printf '%s|%s|%s|%s|%s|%s' \"$COMMAND_EXEC_BASELINE\" \"$COMMAND_EXEC_EXTRA\" \"${RUST_LOG-unset}\" \"$ATLAS_AGENT_HOME\" \"$OPENAI_FEDERATION_RULE_ID\" \"$openai_identity_token_file\"".to_string(),
            ],
            process_id: None,
            tty: false,
            stream_stdin: false,
    ...
    let command_request_id = mcp
        .send_command_exec_request(CommandExecParams {
            command: vec![
                "sh".to_string(),
                "-lc".to_string(),
                "printf '%s' \"${ATLAS_AGENT_NETWORK_PROXY_ACTIVE-unset}\"".to_string(),
            ],
            process_id: None,
            tty: false,
            stream_stdin: false,
...
async fn command_exec_permission_profile_does_not_reuse_default_network_proxy() -> Result<()> {
    ...
    let atlas_agent_home = TempDir::new()?;
    create_config_toml(atlas_agent_home.path(), &server.uri(), "never")?;
    insert_networked_permission_profile_config(atlas_agent_home.path(), Some("networked"))?;
    let mut mcp = TestAppServer::builder()
        .with_atlas_agent_home(atlas_agent_home.path())
        .without_auto_env()
        .build_initialized_with_timeout(DEFAULT_READ_TIMEOUT)
...
async fn command_exec_permission_profile_project_roots_use_command_cwd() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let atlas_agent_home = TempDir::new()?;
    let command_dir = atlas_agent_home.path().join("command-cwd");
...
"#,
    ...
    assert!(
        !atlas_agent_home.path().join("parent.txt").exists(),
...
-> Result<()> {
    ...
    create_config_toml(atlas_agent_home.path(), &server.uri(), "never")?;
    let marker = format!(
        "atlas-engine-command-exec-marker-{}",
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)?
            .as_nanos()
    );

    let (mut process, bind_addr) = spawn_websocket_server(atlas_agent_home.path()).await?;
...
"#
    ...
    insert_command_exec_config(atlas_agent_home, &inserted_config)?;
    Ok(())
}

fn insert_command_exec_config(atlas_agent_home: &Path, inserted_config: &str) -> Result<()> {
