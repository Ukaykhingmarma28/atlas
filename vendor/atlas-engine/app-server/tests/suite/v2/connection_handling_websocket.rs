...
async fn thread_start_routes_project_exec_policy_warning_to_requester() -> Result<()> {
    ...
    let rules_dir = project.path().join(".atlas-agent/rules");
    std::fs::create_dir_all(&rules_dir)?;
    let rules_path = rules_dir.join("broken.rules");
    std::fs::write(&rules_path, "prefix_rule(")?;
    set_project_trust_level(atlas_agent_home.path(), project.path(), TrustLevel::Trusted)?;
...
async fn websocket_transport_rejects_missing_and_invalid_capability_tokens() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let atlas_agent_home = TempDir::new()?;
    let token_file = atlas_agent_home.path().join("app-server-token");
...
async fn websocket_transport_verifies_signed_short_lived_bearer_tokens() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let atlas_agent_home = TempDir::new()?;
    let shared_secret_file = atlas_agent_home.path().join("app-server-signing-secret");
    ...
    let auth_args = vec![
        ...
        "--ws-issuer".to_string(),
        "atlas-engine-enroller".to_string(),
        "--ws-audience".to_string(),
        "atlas-engine-app-server".to_string(),
...
pub(super) async fn spawn_websocket_server(atlas_agent_home: &Path) -> Result<(Child, SocketAddr)> {
    spawn_websocket_server_with_args(atlas_agent_home, "ws://127.0.0.1:0", &[]).await
}
...
pub(super) async fn spawn_websocket_server_with_args(
    ...
    let program = atlas_engine_utils_cargo_bin::cargo_bin("atlas-engine-app-server")
    ...
    cmd.arg("--listen")
        ...
        .stdout(Stdio::null())
        .stderr(Stdio::piped())
        .env("ATLAS_AGENT_HOME", atlas_agent_home)
        .env("RUST_LOG", "warn");
    let mut process = cmd
        .kill_on_drop(true)
        .spawn()
