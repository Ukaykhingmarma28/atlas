...
use atlas_engine_app_server::AppServerWebsocketAuthSettings;
use atlas_engine_app_server::PluginStartupTasks;
use atlas_engine_app_server::RemoteControlStartupMode;
use atlas_engine_app_server::run_main_with_transport_options;
...
use atlas_engine_app_server_protocol::RemoteControlClient;
use atlas_engine_app_server_protocol::RemoteControlClientsListOrder;
use atlas_engine_app_server_protocol::RemoteControlClientsListParams;
use atlas_engine_app_server_protocol::RemoteControlClientsListResponse;
use atlas_engine_app_server_protocol::RemoteControlClientsRevokeParams;
use atlas_engine_app_server_protocol::RemoteControlClientsRevokeResponse;
use atlas_engine_app_server_protocol::RemoteControlConnectionStatus;
use atlas_engine_app_server_protocol::RemoteControlDisableResponse;
use atlas_engine_app_server_protocol::RemoteControlEnableResponse;
...
use atlas_engine_app_server_protocol::RemoteControlStatusReadResponse;
...
async fn explicit_remote_control_startup_fails_when_disabled_by_requirements() -> Result<()> {
    ...
    let managed_config_path = atlas_agent_home.path().join("managed_config.toml");
...
async fn listen_off_honors_persisted_remote_control_enable() -> Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let listener = configured_remote_control_listener(atlas_agent_home.path()).await?;
...
async fn remote_control_disable_returns_disabled_status() -> Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let _listener = configured_remote_control_listener(atlas_agent_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_atlas_agent_home(atlas_agent_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_enable_returns_connecting_status() -> Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let mut backend = BlockingRemoteControlBackend::start(atlas_agent_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_atlas_agent_home(atlas_agent_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn stdio_eof_exits_with_remote_control_connection() -> Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let mut backend = ConnectedRemoteControlBackend::start(atlas_agent_home.path()).await?;
    let mut app_server = TestAppServer::builder()
        .with_atlas_agent_home(atlas_agent_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_pairing_start_returns_pairing_artifacts() -> Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let mut backend = PairingRemoteControlBackend::start(atlas_agent_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_atlas_agent_home(atlas_agent_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_client_management_works_while_disabled() -> Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let mut backend = ClientManagementRemoteControlBackend::start(atlas_agent_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_atlas_agent_home(atlas_agent_home.path())
        .without_auto_env()
        .build_initialized()
...
impl ConnectedRemoteControlBackend {
    async fn start(atlas_agent_home: &std::path::Path) -> Result<Self> {
        let listener = configured_remote_control_listener(atlas_agent_home).await?;
