...
async fn cyber_policy_response_emits_typed_error_notification_v2() -> Result<()> {
    ...
    assert_eq!(
        error,
        ErrorNotification {
            error: atlas_engine_app_server_protocol::TurnError {
                message: CYBER_POLICY_MESSAGE.to_string(),
                atlas_engine_error_info: Some(AtlasEngineErrorInfo::CyberPolicy),
...
async fn turn_moderation_metadata_emits_typed_notification_v2() -> Result<()> {
    ...
    let turn_start: TurnStartResponse = mcp
        ...
        .await?;
    let metadata: TurnModerationMetadataNotification = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_notification("turn/moderationMetadata"),
    )
    .await??;
    assert_eq!(
        metadata,
...
fn create_config_toml(atlas_agent_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
