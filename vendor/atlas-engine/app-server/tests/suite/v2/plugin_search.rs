...
use atlas_engine_app_server_protocol::PluginSearchResponse;
use atlas_engine_app_server_protocol::PluginSearchScope;
...
async fn plugin_search_returns_local_matches_for_api_key_auth() -> Result<()> {
    for remote_plugin_enabled in [false, true] {
        let atlas_agent_home = TempDir::new()?;
        let repo_root = TempDir::new()?;
        let curated_root = atlas_agent_home.path().join(".tmp/plugins");
        let bundled_alpha_root = atlas_agent_home
            .path()
        ...
        write_remote_plugin_search_config(atlas_agent_home.path(), &server, remote_plugin_enabled)?;
        login_with_api_key(
            atlas_agent_home.path(),
            "sk-test-key",
            AuthCredentialsStoreMode::File,
...
fn write_chatgpt_search_auth(atlas_agent_home: &std::path::Path) -> Result<()> {
    write_chatgpt_auth(
        atlas_agent_home,
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_installed_plugin(
    ...
    let plugin_manifest = atlas_agent_home
        .join("plugins/cache")
