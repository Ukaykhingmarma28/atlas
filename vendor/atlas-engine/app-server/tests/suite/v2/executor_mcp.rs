...
async fn selected_executor_plugin_exposes_its_mcps_only_to_that_thread() -> Result<()> {
    ...
    let oauth_credentials_path = atlas_agent_home.path().join(".credentials.json");
    std::fs::write(
        &oauth_credentials_path,
        serde_json::to_vec(&json!({"host": host_oauth_credential.clone()}))?,
    )?;
    let atlas_engine_bin = toml::Value::String(
        atlas_engine_utils_cargo_bin::cargo_bin("atlas-agent")?
            .to_string_lossy()
            .into_owned(),
    );
...
"#
    ...
    std::fs::create_dir_all(plugin.path().join(".atlas-agent-plugin"))?;
    std::fs::write(
        plugin.path().join(".atlas-agent-plugin/plugin.json"),
...
"#,
    ...
    assert_eq!(registration_request["client_name"], json!("Atlas Agent"));
    assert_eq!(
