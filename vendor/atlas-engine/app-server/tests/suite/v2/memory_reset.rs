...
use atlas_engine_state::Stage1JobClaimOutcome;
...
async fn memory_reset_clears_memory_files_and_rows_preserves_threads() -> Result<()> {
    ...
    let thread_id = seed_stage1_output(&state_db, atlas_agent_home.path()).await?;
...
async fn seed_stage1_output(state_db: &Arc<StateRuntime>, atlas_agent_home: &Path) -> Result<ThreadId> {
    ...
    let mut builder = ThreadMetadataBuilder::new(
        thread_id,
        atlas_agent_home.join("sessions").join("test.jsonl"),
        now,
        SessionSource::Cli,
    );
