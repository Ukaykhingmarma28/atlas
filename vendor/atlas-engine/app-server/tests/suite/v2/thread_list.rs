...
use atlas_engine_protocol::protocol::SessionSource as CoreSessionSource;
...
async fn init_mcp(atlas_agent_home: &Path) -> Result<TestAppServer> {
...
async fn thread_list_basic_empty() -> Result<()> {
    ...
    create_minimal_config(atlas_agent_home.path())?;

    let mut mcp = init_mcp(atlas_agent_home.path()).await?;
...
async fn thread_list_reports_system_error_idle_flag_after_failed_turn() -> Result<()> {
    ...
    let atlas_agent_home = TempDir::new()?;
    create_runtime_config(atlas_agent_home.path(), &server.uri())?;
    let mut mcp = init_mcp(atlas_agent_home.path()).await?;
...
fn create_minimal_config(atlas_agent_home: &std::path::Path) -> std::io::Result<()> {
...
async fn thread_list_respects_cwd_filters() -> Result<()> {
    ...
    let first_target_cwd = atlas_agent_home.path().join("first-target-cwd");
    let second_target_cwd = atlas_agent_home.path().join("second-target-cwd");
    ...
    let mut mcp = init_mcp(atlas_agent_home.path()).await?;
    let request_id = mcp
        .send_thread_list_request(atlas_engine_app_server_protocol::ThreadListParams {
            cursor: None,
            limit: Some(10),
            sort_key: None,
            sort_direction: None,
...
"#,
    ...
    state_db
        ...
        .await?;
    let rollout_config = atlas_engine_rollout::RolloutConfig {
        atlas_agent_home: atlas_agent_home.path().to_path_buf(),
        sqlite: atlas_engine_state::SqliteConfig::new_for_testing(atlas_agent_home.path().abs()),
        cwd: atlas_agent_home.path().to_path_buf(),
        model_provider_id: "mock_provider".to_string(),
        generate_memories: false,
    };
    let repaired_page = atlas_engine_core::RolloutRecorder::list_threads(
        Some(state_db.clone()),
        &rollout_config,
        /*page_size*/ 10,
        /*cursor*/ None,
        atlas_engine_core::ThreadSortKey::CreatedAt,
        atlas_engine_core::SortDirection::Desc,
        &[],
...
async fn thread_search_returns_content_matches() -> Result<()> {
    ...
    let mut mcp = init_mcp(atlas_agent_home.path()).await?;
    let request_id = mcp
        .send_thread_search_request(atlas_engine_app_server_protocol::ThreadSearchParams {
            cursor: None,
            limit: Some(10),
            sort_key: None,
            sort_direction: None,
    ...
    for thread_id in [&older_match, &newer_match] {
        let request_id = mcp
            .send_thread_section_move_request(ThreadSectionMoveParams {
                ...
                section_id: Some(atlas_engine_state::PINNED_THREAD_SECTION_ID.to_string()),
...
"#,
    ...
    let stale_cwd = atlas_agent_home.path().join("stale-cwd");
...
async fn thread_list_relation_filters_read_spawn_graph_from_state_db() -> Result<()> {
    ...
    for (thread_id, created_at, source, model_provider) in [
        ...
        let mut builder = atlas_engine_state::ThreadMetadataBuilder::new(
            thread_id,
            atlas_agent_home.path().join(format!("{thread_id}.jsonl")),
            created_at,
            source,
        );
        builder.model_provider = Some(model_provider.to_string());
        builder.cwd = atlas_agent_home.path().to_path_buf();
...
async fn thread_list_reports_loaded_subagent_direct_input_capability() -> Result<()> {
    ...
    for (filename_ts, timestamp, version, capability, should_resume) in [
        ...
        let path = rollout_path(atlas_agent_home.path(), filename_ts, &thread_id);
...
async fn thread_list_sort_updated_at_orders_by_mtime() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(atlas_agent_home.path(), "2025-01-01T10-00-00", &id_old).as_path(),
        "2025-01-03T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(atlas_agent_home.path(), "2025-01-01T11-00-00", &id_mid).as_path(),
        "2025-01-02T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(atlas_agent_home.path(), "2025-01-01T12-00-00", &id_new).as_path(),
...
async fn thread_list_sort_recency_at_uses_state_db_order_with_provider_filter() -> Result<()> {
    ...
    atlas_engine_core::RolloutRecorder::list_threads(
        Some(state_db.clone()),
        &rollout_config,
        /*page_size*/ 10,
        /*cursor*/ None,
        atlas_engine_core::ThreadSortKey::CreatedAt,
        atlas_engine_core::SortDirection::Desc,
        atlas_engine_core::INTERACTIVE_SESSION_SOURCES.as_slice(),
        /*model_providers*/ None,
        /*cwd_filters*/ None,
        "mock_provider",
        /*search_term*/ None,
...
async fn thread_list_updated_at_paginates_with_cursor() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(atlas_agent_home.path(), "2025-02-01T10-00-00", &id_a).as_path(),
        "2025-02-03T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(atlas_agent_home.path(), "2025-02-01T11-00-00", &id_b).as_path(),
        "2025-02-02T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(atlas_agent_home.path(), "2025-02-01T12-00-00", &id_c).as_path(),
...
async fn thread_list_backwards_cursor_can_seed_forward_delta_sync() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(atlas_agent_home.path(), "2025-02-01T10-00-00", &id_old).as_path(),
        "2025-02-02T00:00:00Z",
    )?;
    set_rollout_mtime(
    ...
    set_rollout_mtime(
        rollout_path(atlas_agent_home.path(), "2025-02-01T12-00-00", &id_new).as_path(),
...
async fn thread_list_updated_at_uses_mtime() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(atlas_agent_home.path(), "2025-02-01T10-00-00", &thread_id).as_path(),
