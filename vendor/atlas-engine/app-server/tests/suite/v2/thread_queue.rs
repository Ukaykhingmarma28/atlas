...
use atlas_engine_app_server_protocol::QueuedSubmission;
...
use atlas_engine_app_server_protocol::ThreadQueueAddParams;
use atlas_engine_app_server_protocol::ThreadQueueAddResponse;
use atlas_engine_app_server_protocol::ThreadQueueChangedNotification;
use atlas_engine_app_server_protocol::ThreadQueueDeleteParams;
use atlas_engine_app_server_protocol::ThreadQueueDeleteResponse;
use atlas_engine_app_server_protocol::ThreadQueueListParams;
use atlas_engine_app_server_protocol::ThreadQueueListResponse;
use atlas_engine_app_server_protocol::ThreadQueueReorderParams;
use atlas_engine_app_server_protocol::ThreadQueueReorderResponse;
use atlas_engine_app_server_protocol::ThreadQueueStartParams;
use atlas_engine_app_server_protocol::ThreadQueueStartResponse;
use atlas_engine_app_server_protocol::ThreadQueueUpdateParams;
use atlas_engine_app_server_protocol::ThreadQueueUpdateResponse;
...
async fn queue_requires_experimental_handshake() -> Result<()> {
    let (mut app, atlas_agent_home, _server) = queue_app(Vec::new()).await?;
...
async fn queue_crud_preserves_identity_order_and_notifications() -> Result<()> {
    ...
    let (mut app, _atlas_agent_home, _server) = queue_app(responses).await?;
    let thread_id = app
...
async fn queue_list_returns_ordered_pages_and_lightweight_notifications() -> Result<()> {
    ...
    let (mut app, _atlas_agent_home, _server) = queue_app(vec![blocked_turn_response()?]).await?;
    let thread_id = app
        .start_thread(ThreadStartParams::default())
        .await?
        .thread
...
async fn idle_queue_dispatch_preserves_client_id() -> Result<()> {
    ...
    let (mut app, _atlas_agent_home, server) = queue_app(responses).await?;
    let thread_id = app
        .start_thread(ThreadStartParams::default())
        .await?
        .thread
    ...
    let metadata_header = request
        .headers
        .get("x-atlas-engine-turn-metadata")
        .context("queued model request is missing its x-atlas-engine-turn-metadata header")?
        .to_str()
...
async fn cold_thread_resume_dispatches_a_persisted_queued_submission() -> Result<()> {
    ...
    let (mut first, atlas_agent_home, _server) = queue_app(responses).await?;
    let thread_id = first
        .start_thread(ThreadStartParams::default())
        .await?
        .thread
...
async fn interrupt_preserves_queue_and_queue_start_can_resume_a_non_head_item() -> Result<()> {
    ...
    let (mut app, atlas_agent_home, _server) = queue_app(responses).await?;
    let thread_id = app
        .start_thread(ThreadStartParams::default())
        .await?
        .thread
...
async fn queue_start_while_active_returns_busy_and_preserves_the_queue() -> Result<()> {
    ...
    .await;
    let (mut app, _atlas_agent_home, _server) = queue_app_with_server(server).await?;
    let thread_id = app
        .start_thread(ThreadStartParams::default())
        .await?
        .thread
...
async fn queue_app_with_server(server: MockServer) -> Result<(TestAppServer, TempDir, MockServer)> {
    ...
    config.write(atlas_agent_home.path())?;
    let app = TestAppServer::builder()
        .with_atlas_agent_home(atlas_agent_home.path())
        .without_managed_config()
        .build_initialized()
        .await?;
    Ok((app, atlas_agent_home, server))
}
