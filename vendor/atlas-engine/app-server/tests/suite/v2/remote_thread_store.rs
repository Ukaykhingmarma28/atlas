...
//! the temporary `atlas_agent_home` must not contain rollout session files or sqlite
...
use atlas_engine_app_server::in_process::InProcessClientHandle;
...
use atlas_engine_app_server_protocol::ThreadSectionCreateParams;
use atlas_engine_app_server_protocol::ThreadSectionDeleteParams;
use atlas_engine_app_server_protocol::ThreadSectionListParams;
use atlas_engine_app_server_protocol::ThreadSectionUpdateParams;
...
use atlas_engine_thread_store::CreateThreadParams as StoreCreateThreadParams;
...
async fn thread_section_operations_without_sqlite_return_method_not_found() -> Result<()> {
    ...
    create_config_toml_with_thread_store(atlas_agent_home.path(), "http://127.0.0.1:1", &store_id)?;
    let _in_memory_store = InMemoryThreadStoreId { store_id };
    let client = start_in_process_server(atlas_agent_home.path()).await?;
    ...
    assert_no_local_persistence_artifacts(atlas_agent_home.path())?;
...
async fn thread_start_rejects_paginated_history_without_list_support() -> Result<()> {
    ...
    let atlas_agent_home = TempDir::new()?;
    let store_id = Uuid::new_v4().to_string();
    create_config_toml_with_thread_store(atlas_agent_home.path(), &server.uri(), &store_id)?;
...
async fn thread_delete_with_non_local_thread_store_does_not_create_local_persistence() -> Result<()>
{
    ...
    let atlas_agent_home = TempDir::new()?;
    let store_id = Uuid::new_v4().to_string();
    // Plugin startup warmups may create `.tmp` under atlas_agent_home. Disable them
    ...
    let mut client = start_in_process_server(atlas_agent_home.path()).await?;
...
async fn start_in_process_server(atlas_agent_home: &Path) -> Result<InProcessClientHandle> {
    let loader_overrides = LoaderOverrides::without_managed_config_for_tests();
    let config = Arc::new(
        ConfigBuilder::default()
...
fn assert_no_local_persistence_artifacts(atlas_agent_home: &Path) -> Result<()> {
    ...
    // the isolated test atlas_agent_home.
    assert!(
        !atlas_agent_home.join("sessions").exists(),
        "non-local thread persistence should not create local rollout sessions"
    );
    assert!(
        !atlas_agent_home.join("archived_sessions").exists(),
        "non-local thread persistence should not create archived rollout sessions"
    );
    assert!(
        !atlas_engine_state::SqliteConfig::new_for_testing(atlas_agent_home.abs())
            .state_db_path()
            .exists(),
        "non-local thread persistence should not create local thread sqlite"
    );

    let sqlite_artifacts = std::fs::read_dir(atlas_agent_home)?
    ...
    let mut entries = atlas_agent_home_entries(atlas_agent_home)?;
    ...
    assert_eq!(
        ...
        "non-local thread persistence should not create unexpected files in atlas_agent_home"
    );

    Ok(())
}

fn atlas_agent_home_entries(atlas_agent_home: &Path) -> Result<BTreeSet<String>> {
    Ok(std::fs::read_dir(atlas_agent_home)?
        .filter_map(|entry| {
            let entry = entry.ok()?;
