...
use atlas_engine_app_server_protocol::DynamicToolNamespaceSpec;
...
async fn dynamic_tool_call_round_trip_sends_text_content_items_to_model() -> Result<()> {
    ...
    let tool_namespace = "atlas_engine_app";
...
async fn start_function_dynamic_tool_call(call_id: &str) -> Result<PendingDynamicToolCall> {
    ...
    let mut model_info =
        atlas_engine_core::test_support::construct_model_info_offline("mock-model", &config);
    model_info.input_modalities.push(InputModality::Audio);
    write_models_cache_with_models(atlas_agent_home.path(), vec![model_info])?;
