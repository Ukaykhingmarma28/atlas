...
async fn assert_standalone_web_search_round_trips_output(
    ...
    let search_path = match provider {
        WebSearchProvider::ChatGpt => "/api/atlas-agent/alpha/search",
    ...
    let config = match provider {
        WebSearchProvider::ChatGpt => config
            ...
            .with_provider_base_url(&format!("{}/api/atlas-agent", server.uri()))
    ...
    let turn_metadata_header = search_request
        ...
        .context("standalone search should include x-atlas-engine-turn-metadata")?
        .to_str()
        .context("x-atlas-engine-turn-metadata should be valid ASCII")?;
    let turn_metadata: Value = serde_json::from_str(turn_metadata_header)
        .context("x-atlas-engine-turn-metadata should be valid JSON")?;
