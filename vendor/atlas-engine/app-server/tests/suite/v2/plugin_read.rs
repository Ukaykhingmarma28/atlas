...
use atlas_engine_app_server_protocol::HookEventName;
...
use atlas_engine_app_server_protocol::ScheduledTaskSchedule;
use atlas_engine_app_server_protocol::ScheduledTaskSummary;
use atlas_engine_app_server_protocol::ScheduledTaskWeekday;
    ...
    let request_id = mcp
        .send_plugin_read_request(PluginReadParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                atlas_agent_home.path().join("marketplace.json"),
            )?),
...
"#,
    ...
    write_installed_plugin(&atlas_agent_home, "openai-curated", "demo-plugin")?;
...
async fn plugin_read_returns_share_context_for_shared_local_plugin() -> Result<()> {
    ...
    write_plugin_share_local_path_mapping(atlas_agent_home.path(), "plugins_123", &plugin_path)?;
    Mock::given(method("GET"))
...
async fn plugin_read_falls_back_to_local_share_context_without_remote_auth() -> Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    write_plugins_enabled_config(&atlas_agent_home)?;
    write_plugin_marketplace(
        repo_root.path(),
        "atlas-engine-curated",
        "demo-plugin",
...
async fn plugin_read_fails_on_malformed_share_mapping() -> Result<()> {
    ...
    write_plugin_source(repo_root.path(), "demo-plugin", &[])?;
    std::fs::create_dir_all(atlas_agent_home.path().join(".tmp"))?;
    std::fs::write(
        atlas_agent_home
            .path()
...
async fn plugin_read_agent_plugin_excludes_nested_skills() -> Result<()> {
    ...
    assert_eq!(
        ...
        vec![atlas_engine_app_server_protocol::SkillSummary {
...
[plugins."demo-plugin@atlas-engine-curated"]
enabled = true

[hooks.state."demo-plugin@atlas-engine-curated:hooks/hooks.json:pre_tool_use:0:0"]
enabled = false
"#,
    )?;
    write_installed_plugin(&atlas_agent_home, "atlas-engine-curated", "demo-plugin")?;
...
async fn plugin_read_batches_large_app_metadata_requests() -> Result<()> {
    ...
    let atlas_agent_home = TempDir::new()?;
    write_connectors_config(atlas_agent_home.path(), &server_url)?;
    write_chatgpt_auth(
        atlas_agent_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
async fn plugin_read_stops_batching_after_app_metadata_failure() -> Result<()> {
    ...
    let atlas_agent_home = TempDir::new()?;
    write_connectors_config(atlas_agent_home.path(), &server.uri())?;
    write_chatgpt_auth(
        atlas_agent_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
async fn plugin_read_hides_apps_for_api_key_auth() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .without_auto_env()
        .with_env_overrides(&[
            ("ATLAS_AGENT_ACCESS_TOKEN", None),
            ("ATLAS_AGENT_API_KEY", None),
            ("OPENAI_API_KEY", None),
        ])
        .build_initialized_with_timeout(DEFAULT_TIMEOUT)
        .await?;
...
fn write_plugins_enabled_config(atlas_agent_home: &TempDir) -> Result<()> {
    std::fs::write(
        atlas_agent_home.path().join("config.toml"),
        r#"[features]
plugins = true
...
async fn batch_apps(
    ...
    let product_sku_ok = headers
        .get("oai-product-sku")
        .and_then(|value| value.to_str().ok())
        .is_some_and(|value| value == "atlas-agent");
