...
use atlas_engine_app_server_protocol::TextPosition;
use atlas_engine_app_server_protocol::TextRange;
...
async fn thread_start_warns_for_exec_policy_parse_failure_after_initialize() -> Result<()> {
    ...
    let rules_dir = atlas_agent_home.path().join("rules");
...
async fn thread_start_provider_model_fallback_ignores_dynamic_catalog() -> Result<()> {
    ...
    let atlas_agent_home = TempDir::new()?;
    create_config_toml_without_approval_policy(atlas_agent_home.path(), &server.uri())?;
    let mut mcp = TestAppServer::builder()
        .with_atlas_agent_home(atlas_agent_home.path())
        .build_initialized()
        .await?;
...
async fn thread_start_response_includes_loaded_instruction_sources() -> Result<()> {
    ...
    let atlas_agent_home = TempDir::new()?;
    create_config_toml_without_approval_policy(atlas_agent_home.path(), &server.uri())?;
    let global_agents_path = atlas_agent_home.path().join("AGENTS.md");
...
async fn thread_start_fails_when_required_mcp_server_fails_to_initialize() -> Result<()> {
    ...
    let atlas_agent_home = TempDir::new()?;
    create_config_toml_with_required_broken_mcp(atlas_agent_home.path(), &server.uri())?;
...
async fn thread_start_emits_mcp_server_status_updated_notifications() -> Result<()> {
    ...
    let atlas_agent_home = TempDir::new()?;
    create_config_toml_with_optional_broken_mcp(atlas_agent_home.path(), &server.uri())?;
...
"#,
    ...
    if cfg!(windows) {
        assert_eq!(
            ...
            atlas_engine_app_server_protocol::SandboxPolicy::ReadOnly {
...
"#,
    ...
    let config_after = std::fs::read_to_string(atlas_agent_home.path().join("config.toml"))?;
    assert_eq!(config_after, config_before);
