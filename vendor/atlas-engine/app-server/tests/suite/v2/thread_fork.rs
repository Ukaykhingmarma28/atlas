...
async fn thread_fork_creates_new_thread_and_emits_started() -> Result<()> {
    ...
    let original_path = atlas_agent_home
        .path()
        .join("sessions")
        .join("2025")
        .join("01")
...
async fn thread_fork_inherits_explicit_source_name_from_session_index() -> Result<()> {
    ...
    append_thread_name(atlas_agent_home.path(), source_thread_id, source_name).await?;
...
async fn thread_fork_can_cut_before_unfinished_stored_turn() -> Result<()> {
    ...
    let source_path = rollout_path(atlas_agent_home.path(), filename_ts, &conversation_id);
...
async fn assert_thread_fork_freezes_active_paginated_turn_as_interrupted(
    ...
    assert!(matches!(
        ...
        [
            ...
            &response_item.item,
            atlas_engine_protocol::models::ResponseItem::Message { role, .. }
...
#[tokio::test]
async fn pathless_ephemeral_thread_rejects_atlas_agent_home_path_after_reload() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let atlas_agent_home = TempDir::new()?;
    MockResponsesConfig::new(&server.uri()).write(atlas_agent_home.path())?;
    ...
    let mut app_server = TestAppServer::builder()
        ...
        .build_initialized()
        .await?;
    let atlas_agent_home_path = atlas_agent_home.path().to_path_buf();

    let resume_id = app_server
        .send_thread_resume_request(ThreadResumeParams {
            thread_id: side_thread_id.clone(),
            path: Some(atlas_agent_home_path.clone()),
            ..Default::default()
        })
        .await?;
    ...
    let fork_id = app_server
        .send_thread_fork_request(ThreadForkParams {
            ...
            path: Some(atlas_agent_home_path),
            ..Default::default()
        })
        .await?;
    let fork_err: JSONRPCError = timeout(
