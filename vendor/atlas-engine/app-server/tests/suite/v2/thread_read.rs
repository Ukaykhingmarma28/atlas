...
use atlas_engine_protocol::protocol::SessionSource as ProtocolSessionSource;
...
async fn thread_search_occurrences_reads_paginated_projection() -> Result<()> {
    ...
    let atlas_agent_home = TempDir::new()?;
    MockResponsesConfig::new(&server.uri()).write(atlas_agent_home.path())?;
    let thread_id = atlas_engine_protocol::ThreadId::default();
    ...
    store
        .create_thread(CreateThreadParams {
            ...
            history_mode: atlas_engine_protocol::protocol::ThreadHistoryMode::Paginated,
            history_base: None,
...
async fn thread_read_loaded_include_turns_reads_store_history_without_rollout_path() -> Result<()> {
    ...
    let thread_id = atlas_engine_protocol::ThreadId::from_string(&thread.id)?;
    store
        .append_items(AppendThreadItemsParams {
            thread_id,
...
) -> anyhow::Result<atlas_engine_app_server_protocol::Turn> {
    let read_id = mcp
        .send_thread_turns_list_request(ThreadTurnsListParams {
            thread_id: thread_id.to_string(),
            cursor: None,
...
fn turn_user_texts(turns: &[atlas_engine_app_server_protocol::Turn]) -> Vec<&str> {
    turns
        .iter()
...
fn turn_agent_texts(turns: &[atlas_engine_app_server_protocol::Turn]) -> Vec<&str> {
    turns
        .iter()
