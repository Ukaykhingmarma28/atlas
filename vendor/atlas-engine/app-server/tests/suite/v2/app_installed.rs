...
async fn installed_apps_force_refresh_only_refreshes_tools_snapshot() -> Result<()> {
    ...
    let atlas_agent_home = configured_atlas_agent_home(fixture.base_url())?;
    let mut app_server = start_app_server(atlas_agent_home.path()).await?;
...
fn configured_atlas_agent_home(base_url: &str) -> Result<TempDir> {
...
impl InstalledAppsFixture {
    async fn start() -> Result<Self> {
        ...
        synthetic_link
            ...
            .0
            .insert("_atlas_apps".to_string(), json!({ "synthetic_link": true }));
