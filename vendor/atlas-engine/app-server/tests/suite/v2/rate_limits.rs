...
use atlas_engine_app_server_protocol::RateLimitReachedType;
...
use atlas_engine_app_server_protocol::SpendControlLimitSnapshot;
...
async fn get_account_rate_limits_returns_snapshot() -> Result<()> {
    ...
    let response_body = json!({
        ...
        "additional_rate_limits": [
            {
                "limit_name": "atlas_engine_other",
                "metered_feature": "atlas_engine_other",
                "rate_limit": {
                    "allowed": true,
                    "limit_reached": false,
                    "primary_window": {
    ...
    Mock::given(method("GET"))
        .and(path("/api/atlas-agent/usage"))
        ...
        .await;
    Mock::given(method("GET"))
        .and(path("/api/atlas-agent/rate-limit-reset-credits"))
        .and(header("authorization", "Bearer chatgpt-token"))
        .and(header("chatgpt-account-id", "account-123"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "credits": [
                {
                    "id": "credit-1",
                    "reset_type": "atlas_engine_rate_limits",
                    "status": "available",
        ...
        rate_limit_reset_credits: Some(RateLimitResetCreditsSummary {
            ...
            credits: Some(vec![
                RateLimitResetCredit {
                    ...
                    reset_type: RateLimitResetType::AtlasEngineRateLimits,
...
async fn get_account_rate_limits_preserves_count_when_reset_credit_details_fail() -> Result<()> {
    ...
    let server = MockServer::start().await;
    write_chatgpt_base_url(atlas_agent_home.path(), &server.uri())?;
...
fn write_chatgpt_base_url(atlas_agent_home: &Path, base_url: &str) -> std::io::Result<()> {
