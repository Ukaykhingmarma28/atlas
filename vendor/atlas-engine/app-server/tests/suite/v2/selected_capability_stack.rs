...
async fn selected_capability_stack_tracks_environment_availability_and_resume() -> Result<()> {
    ...
    let mut app_server = TestAppServer::builder()
        .with_atlas_agent_home(fixture.atlas_agent_home.path())
...
fn selected_capability_fixture(
    ...
    let local_skill_dir = atlas_agent_home.path().join("skills/local-deploy");
    ...
    let manifest_dir = plugin.path().join(".atlas-agent-plugin");
...
fn assert_selected_plugin_tools_absent(request: &ResponsesRequest) {
    ...
    let connector = request
        .tool_by_name("mcp__atlas_apps__calendar", "connector_calendar")
...
async fn spawn_exec_server(atlas_agent_home: &std::path::Path, url: &str) -> Result<Child> {
    let mut child = Command::new(atlas_engine_utils_cargo_bin::cargo_bin("atlas-agent")?)
