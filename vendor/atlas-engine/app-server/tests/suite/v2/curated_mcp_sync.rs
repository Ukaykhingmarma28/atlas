...
impl CuratedMcpSyncFixture {
    async fn set_up() -> Result<Self> {
        ...
        let atlas_agent_home = fixture_root.path().join("atlas-engine-home");
        ...
        run_git(
            &real_git,
            &curated_repo,
            &["config", "user.email", "atlas-engine-tests@openai.com"],
        )?;
        run_git(
            &real_git,
            &curated_repo,
            &["config", "user.name", "Atlas Agent Tests"],
        )?;
...
fn write_plugin(
    ...
    std::fs::create_dir_all(root.join(".atlas-agent-plugin"))?;
    std::fs::write(
        root.join(".atlas-agent-plugin/plugin.json"),
