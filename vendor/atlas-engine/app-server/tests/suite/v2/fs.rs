...
async fn initialized_mcp(atlas_agent_home: &TempDir) -> Result<TestAppServer> {
...
async fn fs_get_metadata_returns_only_used_fields() -> Result<()> {
    ...
    let file_path = atlas_agent_home.path().join("note.txt");
    std::fs::write(&file_path, "hello")?;

    let mut mcp = initialized_mcp(&atlas_agent_home).await?;
    let request_id = mcp
        .send_fs_get_metadata_request(atlas_engine_app_server_protocol::FsGetMetadataParams {
...
async fn fs_methods_return_error_when_local_environment_is_disabled() -> Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let absolute_file = atlas_agent_home.path().join("absolute.txt");
    ...
    let read_id = mcp
        .send_fs_read_file_request(atlas_engine_app_server_protocol::FsReadFileParams {
...
async fn fs_get_metadata_reports_symlink() -> Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let file_path = atlas_agent_home.path().join("note.txt");
    let symlink_path = atlas_agent_home.path().join("note-link.txt");
    std::fs::write(&file_path, "hello")?;
...
async fn fs_methods_cover_current_fs_utils_surface() -> Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let source_dir = atlas_agent_home.path().join("source");
    let nested_dir = source_dir.join("nested");
    let source_file = source_dir.join("root.txt");
    let copied_dir = atlas_agent_home.path().join("copied");
    let copy_file_path = atlas_agent_home.path().join("copy.txt");
    ...
    let read_directory_request_id = mcp
        .send_fs_read_directory_request(atlas_engine_app_server_protocol::FsReadDirectoryParams {
    ...
    .await??;
    let mut entries =
        to_response::<atlas_engine_app_server_protocol::FsReadDirectoryResponse>(readdir_response)?
            .entries;
    ...
    let remove_request_id = mcp
        .send_fs_remove_request(atlas_engine_app_server_protocol::FsRemoveParams {
...
async fn fs_write_file_accepts_base64_bytes() -> Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let file_path = atlas_agent_home.path().join("blob.bin");
...
async fn fs_copy_rejects_directory_without_recursive() -> Result<()> {
    ...
    let request_id = mcp
        .send_fs_copy_request(FsCopyParams {
            ...
            destination_path: absolute_path(atlas_agent_home.path().join("dest")),
            recursive: false,
        })
        .await?;
    let error = timeout(
...
async fn fs_copy_rejects_standalone_fifo_source() -> Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let fifo_path = atlas_agent_home.path().join("named-pipe");
    let output = Command::new("mkfifo").arg(&fifo_path).output()?;
    if !output.status.success() {
        anyhow::bail!(
            "mkfifo failed: stdout={} stderr={}",
    ...
    let request_id = mcp
        .send_fs_copy_request(FsCopyParams {
            ...
            destination_path: absolute_path(atlas_agent_home.path().join("copied")),
            recursive: false,
        })
        .await?;
    expect_error_message(
...
-> Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let git_dir = atlas_agent_home.path().join("repo").join(".git");
    ...
    let watch_request_id = mcp
        .send_fs_watch_request(atlas_engine_app_server_protocol::FsWatchParams {
