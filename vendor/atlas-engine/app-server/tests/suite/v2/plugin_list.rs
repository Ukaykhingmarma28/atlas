...
use atlas_engine_app_server_protocol::PluginDisabledReason;
use atlas_engine_app_server_protocol::PluginInstallPolicy;
use atlas_engine_app_server_protocol::PluginInstallPolicySource;
...
const TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS: &str =
    "ATLAS_AGENT_TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS";
...
fn write_plugins_enabled_config(atlas_agent_home: &std::path::Path) -> std::io::Result<()> {
    std::fs::write(
...
async fn plugin_list_skips_invalid_marketplace_file_and_reports_error() -> Result<()> {
    ...
    let home = atlas_agent_home.path().to_string_lossy().into_owned();
...
async fn plugin_installed_includes_installed_plugins_and_explicit_install_suggestions() -> Result<()>
{
    ...
    write_installed_plugin(&atlas_agent_home, "openai-api-curated", "linear")?;
    std::fs::write(
...
async fn plugin_installed_prefers_remote_curated_conflicts_when_remote_plugin_enabled() -> Result<()>
{
    ...
    write_openai_curated_marketplace(atlas_agent_home.path(), &["linear", "calendar"])?;
    write_installed_plugin(&atlas_agent_home, "openai-curated", "linear")?;
    write_installed_plugin(&atlas_agent_home, "openai-curated", "calendar")?;
    std::fs::write(
        atlas_agent_home.path().join("config.toml"),
        format!(
...
async fn plugin_installed_prefers_api_curated_conflicts_after_switching_to_api_auth() -> Result<()>
{
    let atlas_agent_home = TempDir::new()?;
    let server = MockServer::start().await;
    write_openai_api_curated_marketplace(atlas_agent_home.path(), &["linear"])?;
    write_installed_plugin(&atlas_agent_home, "openai-api-curated", "linear")?;
    let config = format!(
        r#"chatgpt_base_url = "{}/backend-api/"
...
"#,
    ...
    std::fs::write(atlas_agent_home.path().join("config.toml"), &config)?;
    write_chatgpt_auth(
        atlas_agent_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
async fn plugin_list_keeps_valid_marketplaces_when_another_marketplace_fails_to_load() -> Result<()>
{
    ...
    std::fs::create_dir_all(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.atlas-agent-plugin"),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.atlas-agent-plugin/plugin.json"),
...
#[tokio::test]
async fn plugin_list_returns_empty_when_workspace_atlas_engine_plugins_disabled() -> Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
...
#[tokio::test]
async fn plugin_list_reuses_cached_workspace_atlas_engine_plugins_setting() -> Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    std::fs::create_dir_all(repo_root.path().join("demo-plugin/.atlas-agent-plugin"))?;
    write_plugins_enabled_config_with_base_url(
        atlas_agent_home.path(),
        &format!("{}/backend-api/", server.uri()),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        repo_root
            .path()
            .join("demo-plugin/.atlas-agent-plugin/plugin.json"),
        r#"{"name":"demo-plugin"}"#,
    )?;
        ...
        vec![PluginMarketplaceEntry {
            ...
            plugins: vec![
                PluginSummary {
                    ...
                    disabled_reason: None,
                    eligible_plan_types: None,
                    interface: Some(atlas_engine_app_server_protocol::PluginInterface {
...
async fn plugin_list_force_refetch_waits_for_same_path_local_plugin_upgrade() -> Result<()> {
    ...
    let source_manifest = marketplace_root
        .path()
        .join("sample-plugin/.atlas-agent-plugin/plugin.json");
...
"#,
    ...
    let plugin_cache = atlas_agent_home
        .path()
        .join("plugins/cache/sample-marketplace/sample-plugin");
    let installed_manifest = plugin_cache.join("1.1.0/.atlas-agent-plugin/plugin.json");
    assert!(
...
async fn plugin_list_includes_install_and_enabled_state_from_config() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    write_installed_plugin(&atlas_agent_home, "atlas-engine-curated", "enabled-plugin")?;
    write_installed_plugin(&atlas_agent_home, "atlas-engine-curated", "disabled-plugin")?;
    std::fs::write(
        repo_root.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "atlas-engine-curated",
...
[plugins."enabled-plugin@atlas-engine-curated"]
enabled = true

[plugins."disabled-plugin@atlas-engine-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(marketplace.name, "atlas-engine-curated");
    assert_eq!(
        marketplace
            .interface
            .as_ref()
    ...
    assert_eq!(
        ...
        "uninstalled-plugin@atlas-engine-curated"
    );
...
[plugins."shared-plugin@atlas-engine-curated"]
enabled = true
"#,
    )?;
  ...
  "plugins": [
    ...
    std::fs::create_dir_all(workspace_enabled.path().join(".atlas-agent"))?;
    std::fs::write(
        workspace_enabled.path().join(".atlas-agent/config.toml"),
        r#"[plugins."shared-plugin@atlas-engine-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(shared_plugin.id, "shared-plugin@atlas-engine-curated");
...
async fn plugin_list_returns_plugin_interface_with_absolute_asset_paths() -> Result<()> {
  ...
  "interface": {
    ...
    assert_eq!(plugin.id, "demo-plugin@atlas-engine-curated");
...
async fn app_server_startup_sync_downloads_remote_installed_plugin_bundles() -> Result<()> {
    ...
    let installed_path = atlas_agent_home
        .path()
    ...
    wait_for_path_exists(&installed_path.join(".atlas-agent-plugin/plugin.json")).await?;
...
async fn plugin_list_sync_upgrades_and_removes_remote_installed_plugin_bundles() -> Result<()> {
    ...
    let old_path = atlas_agent_home
        .path()
        .join("plugins/cache/openai-curated-remote/linear/1.0.0");
    let new_path = atlas_agent_home
        .path()
        .join("plugins/cache/openai-curated-remote/linear/1.2.3");
    let stale_path = atlas_agent_home
        .path()
    ...
    wait_for_path_exists(&new_path.join(".atlas-agent-plugin/plugin.json")).await?;
    let installed_plugin_manifest: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(new_path.join(".atlas-agent-plugin/plugin.json"))?,
    )?;
    assert_eq!(
        installed_plugin_manifest["version"],
        serde_json::json!("1.2.3")
...
async fn plugin_list_includes_remote_marketplaces_when_remote_plugin_enabled() -> Result<()> {
  ...
  "pagination": {
    ...
    assert_eq!(
        ...
        atlas_engine_app_server_protocol::PluginAvailability::Available
    );
    assert_eq!(
...
async fn plugin_list_honors_global_remote_catalog_cache_ttl() -> Result<()> {
    ...
    wait_for_cached_remote_catalog_plugin_ids(atlas_agent_home.path(), &[cached_remote_plugin_id])
        .await?;
...
async fn app_server_startup_skips_disabled_remote_plugin_catalog_scopes() -> Result<()> {
    ...
    write_remote_plugin_catalog_config(atlas_agent_home.path(), &base_url)?;
    write_chatgpt_auth(
        atlas_agent_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
    ...
    write_remote_plugins_disabled_config_with_base_url(atlas_agent_home.path(), &base_url)?;
    server.reset().await;
    mount_remote_plugin_list(&server, "GLOBAL", &global_body).await;
    mount_remote_plugin_list(&server, "USER", &user_body).await;
    mount_remote_plugin_list(&server, "WORKSPACE", &workspace_body).await;
...
-> Result<()> {
    ...
    write_openai_api_curated_marketplace(atlas_agent_home.path(), &["api-plugin"])?;
    login_with_api_key(
        atlas_agent_home.path(),
        "sk-test-key",
        AuthCredentialsStoreMode::File,
...
"#,
    ...
    write_openai_curated_marketplace(atlas_agent_home.path(), &["chatgpt-plugin"])?;
    write_openai_api_curated_marketplace(atlas_agent_home.path(), &["api-plugin"])?;
    ...
    assert!(!atlas_agent_home.path().join("auth.json").exists());
    let api_curated_marketplace = response
        .marketplaces
        .iter()
        .find(|marketplace| marketplace.name == "openai-api-curated")
...
async fn plugin_installed_hook_trust_write_failure_stays_untrusted() -> Result<()> {
    ...
    symlink(&config_target, atlas_agent_home.path().join("config.toml"))?;
    write_remote_plugin_test_auth(atlas_agent_home.path())?;
...
async fn assert_disabled_remote_plugin_metadata(
    ...
    assert_eq!(
        plugin.availability,
        atlas_engine_app_server_protocol::PluginAvailability::DisabledByAdmin
    );
...
async fn wait_for_cached_remote_catalog_plugin_ids(
    ...
    timeout(DEFAULT_TIMEOUT, async {
        loop {
            let plugin_ids = cached_remote_catalog_plugin_ids(atlas_agent_home)?;
...
fn cached_remote_catalog_plugin_ids(atlas_agent_home: &std::path::Path) -> Result<Vec<String>> {
    let cache_dir = atlas_agent_home.join("cache/remote_plugin_catalog");
    if !cache_dir.exists() {
        return Ok(Vec::new());
    }
...
fn write_installed_plugin(
    ...
    write_installed_plugin_with_version(atlas_agent_home, marketplace_name, plugin_name, "local")
}
...
fn write_remote_plugin_test_auth(atlas_agent_home: &std::path::Path) -> Result<()> {
    write_chatgpt_auth(
        atlas_agent_home,
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_curated_marketplace(
    ...
    let curated_root = atlas_agent_home.join(".tmp/plugins");
...
{plugins}
    ...
    for plugin_name in plugin_names {
        let plugin_root = curated_root.join(format!("plugins/{plugin_name}/.atlas-agent-plugin"));
        std::fs::create_dir_all(&plugin_root)?;
        std::fs::write(
            plugin_root.join("plugin.json"),
            format!(r#"{{"name":"{plugin_name}"}}"#),
    ...
    std::fs::create_dir_all(atlas_agent_home.join(".tmp"))?;
    std::fs::write(
...
fn write_plugin_share_local_path_mapping(
    ...
    std::fs::create_dir_all(atlas_agent_home.join(".tmp"))?;
    std::fs::write(
        atlas_agent_home.join(".tmp/plugin-share-local-paths-v1.json"),
        format!("{contents}\n"),
    )
}
