...
use atlas_engine_app_server_protocol::ConnectorMetadata;
...
async fn app_read_deduplicates_orders_partial_misses_and_reuses_cached_metadata() -> Result<()> {
    ...
    write_apps_config(atlas_agent_home.path(), &server_url, Some("tpp"))?;
...
async fn app_read_refetches_metadata_only_cache_entries_when_tools_are_requested() -> Result<()> {
    ...
    write_auth(atlas_agent_home.path())?;
    let mut mcp = TestAppServer::builder()
        .with_atlas_agent_home(atlas_agent_home.path())
        .without_auto_env()
        .without_managed_config()
...
"#,
    ...
    write_plugin_app(atlas_agent_home.path(), "alpha-z", "Alpha Z", "alpha")?;
    write_plugin_app(atlas_agent_home.path(), "alpha-a", "Alpha A", "alpha")?;
    write_plugin_app(
        atlas_agent_home.path(),
        "disabled",
        "Disabled Plugin",
...
fn write_auth(atlas_agent_home: &Path) -> Result<()> {
    write_chatgpt_auth(
        atlas_agent_home,
...
async fn start_batch_server(state: BatchServerState) -> Result<(String, JoinHandle<()>)> {
    ...
    let router = Router::new()
        ...
        .route("/api/atlas-agent/ps/mcp", any(unexpected_mcp_request))
        .with_state(state);
