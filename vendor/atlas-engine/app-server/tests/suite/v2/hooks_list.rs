...
use atlas_engine_app_server_protocol::HookExecutionMode;
use atlas_engine_app_server_protocol::HookHandlerType;
use atlas_engine_app_server_protocol::HookMetadata;
use atlas_engine_app_server_protocol::HookSource;
use atlas_engine_app_server_protocol::HookTrustStatus;
use atlas_engine_app_server_protocol::HooksListEntry;
...
fn command_hook_hash(
    ...
    let Ok(value) = atlas_engine_config::TomlValue::try_from(identity) else {
        unreachable!("normalized hook identity should serialize to TOML");
    };
    atlas_engine_config::version_for_toml(&value)
}

fn write_user_hook_config(atlas_agent_home: &std::path::Path) -> Result<()> {
    std::fs::write(
...
fn write_plugin_hook_config(atlas_agent_home: &std::path::Path, hooks_json: &str) -> Result<()> {
    let plugin_root = atlas_agent_home.join("plugins/cache/test/demo/local");
...
async fn hooks_list_shows_discovered_hook() -> Result<()> {
    ...
    let cwd = TempDir::new()?;
    write_user_hook_config(atlas_agent_home.path())?;
...
async fn hooks_list_warms_plugin_capabilities_for_thread_start() -> Result<()> {
  ...
  "hooks": {
    ...
    let plugin_mcp_path = atlas_agent_home
        .path()
...
async fn hooks_list_shows_plugin_hook_load_warnings() -> Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    write_plugin_hook_config(atlas_agent_home.path(), "{ not-json")?;
...
"#,
    ...
    std::fs::create_dir_all(workspace.path().join(".atlas-agent"))?;
    std::fs::write(
        workspace.path().join(".atlas-agent/config.toml"),
        r#"[features]
hooks = true
...
"#,
    ...
    let project_config_path =
        AbsolutePathBuf::try_from(workspace.path().join(".atlas-agent/config.toml"))?;
    assert_eq!(
        data,
        vec![
            HooksListEntry {
...
async fn hooks_list_uses_root_repo_hooks_for_linked_worktrees() -> Result<()> {
    ...
    write_project_hook_config(&repo_root.join(".atlas-agent"), "echo root hook")?;
    write_project_hook_config(&worktree_root.join(".atlas-agent"), "echo worktree hook")?;
    set_project_trust_level(atlas_agent_home.path(), &repo_root, TrustLevel::Trusted)?;
    ...
    let repo_config_path =
        AbsolutePathBuf::from_absolute_path(repo_root.join(".atlas-agent/config.toml"))?;
    ...
    let write_id = mcp
        ...
        .await?;
    let _: atlas_engine_app_server_protocol::ConfigWriteResponse =
...
"#,
    ...
    let hook_list_id = mcp
        .send_hooks_list_request(HooksListParams {
            cwds: vec![atlas_agent_home.path().to_path_buf()],
        })
        .await?;
    let HooksListResponse { data } =
