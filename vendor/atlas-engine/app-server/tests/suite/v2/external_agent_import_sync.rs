...
const NATIVE_USER: &str = "native Atlas Agent message";
const NATIVE_ASSISTANT: &str = "native Atlas Agent answer";
...
impl ImportFixture {
    async fn new() -> Result<Self> {
        ...
        let session_path = atlas_agent_home
            .path()
            .join(concat!(".", "cla", "ude"))
        ...
        let app_server = start_app_server(atlas_agent_home.path()).await?;
        Ok(Self {
    ...
    async fn restart(&mut self) -> Result<()> {
        ...
        self.app_server = start_app_server(self._atlas_agent_home.path()).await?;
        Ok(())
    }
...
async fn start_app_server(atlas_agent_home: &Path) -> Result<TestAppServer> {
    let home = atlas_agent_home.display().to_string();
...
async fn cold_diverged_target_is_deferred_without_checkpoint() -> Result<()> {
    ...
    let mut fixture = ImportFixture::new().await?;
    MockResponsesConfig::new(&model.uri()).write(fixture._atlas_agent_home.path())?;
