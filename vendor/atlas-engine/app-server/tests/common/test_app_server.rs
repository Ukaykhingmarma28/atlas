...
use atlas_engine_app_server_protocol::ClientNotification;
...
use atlas_engine_app_server_protocol::JSONRPCRequest;
...
use atlas_engine_app_server_protocol::ModelProviderCapabilitiesReadParams;
...
use atlas_engine_app_server_protocol::PluginSearchParams;
use atlas_engine_app_server_protocol::PluginSkillReadParams;
use atlas_engine_app_server_protocol::PluginUninstallParams;
use atlas_engine_app_server_protocol::ProcessKillParams;
use atlas_engine_app_server_protocol::ProcessSpawnParams;
use atlas_engine_app_server_protocol::RemoteControlClientsListParams;
use atlas_engine_app_server_protocol::RemoteControlClientsRevokeParams;
use atlas_engine_app_server_protocol::RemoteControlPairingStartParams;
use atlas_engine_app_server_protocol::RemoteControlPairingStatusParams;
...
use atlas_engine_app_server_protocol::ThreadRealtimeListVoicesParams;
...
use atlas_engine_exec_server::ATLAS_AGENT_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR;
...
use core_test_support::test_atlas_engine::TestEnv;
...
pub struct TestAppServer {
    ...
    // removing an owned ATLAS_AGENT_HOME that may still be its cwd on Windows.
    ...
    _owned_atlas_agent_home: Option<TempDir>,
}

pub const DEFAULT_CLIENT_NAME: &str = "atlas-engine-app-server-tests";
pub const DISABLE_PLUGIN_STARTUP_TASKS_ARG: &str = "--disable-plugin-startup-tasks-for-tests";
const DISABLE_MANAGED_CONFIG_ENV_VAR: &str = "ATLAS_AGENT_APP_SERVER_DISABLE_MANAGED_CONFIG";
#[cfg(windows)]
...
impl TestAppServer {
    /// Starts building a server with a temporary ATLAS_AGENT_HOME and the standard
    /// automatic test environment.
    pub fn builder() -> TestAppServerBuilder {
        TestAppServerBuilder {
            atlas_agent_home: None,
    ...
    async fn new_with_program_env_and_args(
        ...
        cmd.current_dir(atlas_agent_home);
        cmd.env("ATLAS_AGENT_HOME", atlas_agent_home);
        cmd.env("RUST_LOG", "warn");
        // Keep integration tests isolated from host managed configuration.
        cmd.env(
            "ATLAS_AGENT_APP_SERVER_MANAGED_CONFIG_PATH",
            atlas_agent_home.join("managed_config.toml"),
        );
        cmd.env_remove(ATLAS_AGENT_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR);
        cmd.args(args);
        ...
        let mut process = loop {
            ...
            if !process
                ...
                || retries == 2
            {
                break process.context("atlas-engine-mcp-server proc should start")?;
            }
            retries += 1;
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: atlas_engine_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
...
impl TestAppServerBuilder {
    /// Uses this existing ATLAS_AGENT_HOME instead of a temporary one.
    pub fn with_atlas_agent_home(mut self, atlas_agent_home: &Path) -> Self {
        self.atlas_agent_home = Some(atlas_agent_home.to_path_buf());
        self
    }
    ...
    /// Builds a server with a temporary ATLAS_AGENT_HOME and automatic environment
    /// by default.
    pub async fn build(self) -> anyhow::Result<TestAppServer> {
        ...
        let (atlas_agent_home, owned_atlas_agent_home) = match atlas_agent_home {
            Some(atlas_agent_home) => (atlas_agent_home, None),
            None => {
                let owned_atlas_agent_home = TempDir::new()?;
                (
                    owned_atlas_agent_home.path().to_path_buf(),
                    Some(owned_atlas_agent_home),
                )
            }
        };
        let attribution_settings_server = if atlas_agent_home.join("auth.json").is_file() {
            let config_path = atlas_agent_home.join("config.toml");
        ...
        let (auto_env, delayed_exec_server) = match environment {
            TestAppServerEnvironment::Auto => {
                let environments_toml = atlas_agent_home.join("environments.toml");
                ensure!(
                ...
                let (auto_env, delayed_exec_server) = match exec_server_delay {
                    Some(added_delay) => {
                        ...
                        let exec_server_program =
                            atlas_engine_utils_cargo_bin::cargo_bin("exec-server")
                ...
                let mut auto_env_overrides = vec![
                    (
                        ATLAS_AGENT_EXEC_SERVER_URL_ENV_VAR.to_string(),
                        auto_env.exec_server_url().map(str::to_string),
                    ),
                    (
                        ATLAS_AGENT_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR.to_string(),
                        None,
                    ),
                    (
        ...
        let mut program = match program {
            ...
            None => atlas_engine_utils_cargo_bin::cargo_bin("atlas-engine-app-server")
                .context("should find binary for atlas-engine-app-server")?,
        };
        let mut owned_install_dir = None;
        if !custom_program
            && atlas_engine_utils_cargo_bin::runfiles_available()
            && let Ok(code_mode_host_program) =
                atlas_engine_utils_cargo_bin::cargo_bin("atlas-engine-code-mode-host")
        {
        ...
        app_server._owned_atlas_agent_home = owned_atlas_agent_home;
...
impl Drop for TestAppServer {
    fn drop(&mut self) {
        // These tests spawn a `atlas-engine-app-server` child process.
        //
