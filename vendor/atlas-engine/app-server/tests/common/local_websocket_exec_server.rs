...
impl LocalWebsocketExecServer {
    pub(crate) async fn start(atlas_agent_home: &Path, exec_server_program: &Path) -> Result<Self> {
        let mut command = Command::new(exec_server_program);
        command.stdin(Stdio::null());
        command.stdout(Stdio::piped());
        command.stderr(Stdio::inherit());
        command.current_dir(atlas_agent_home);
        command.env("ATLAS_AGENT_HOME", atlas_agent_home);
        #[cfg(target_os = "linux")]
        command.env(
            ATLAS_AGENT_LINUX_SANDBOX_EXE_ENV_VAR,
            core_test_support::find_atlas_engine_linux_sandbox_exe()
