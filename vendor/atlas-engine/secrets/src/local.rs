...
const ATLAS_AGENT_AUTH_SECRETS_FILENAME: &str = "atlas_engine_auth.age";
...
pub enum LocalSecretsNamespace {
    ...
    #[default]
    ManagedSecrets,
    /// Atlas Agent authentication credentials used by the CLI, TUI, app server, and other clients.
    AtlasEngineAuth,
...
impl LocalSecretsBackend {
    pub fn new(atlas_agent_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
    ...
    fn secrets_dir(&self) -> PathBuf {
        self.atlas_agent_home.join("secrets")
    }

    fn secrets_path(&self) -> PathBuf {
        let filename = match self.namespace {
            LocalSecretsNamespace::ManagedSecrets => LOCAL_SECRETS_FILENAME,
            LocalSecretsNamespace::AtlasEngineAuth => ATLAS_AGENT_AUTH_SECRETS_FILENAME,
    ...
    fn load_or_create_passphrase(&self) -> Result<SecretString> {
        let account = compute_keyring_account(&self.atlas_agent_home);
        let loaded = self
            .keyring_store
...
mod tests {
    ...
    fn load_file_rejects_newer_schema_versions() -> Result<()> {
        ...
        let backend = LocalSecretsBackend::new(atlas_agent_home.path().to_path_buf(), keyring);
    ...
    fn set_fails_when_keyring_is_unavailable() -> Result<()> {
        let atlas_agent_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let account = compute_keyring_account(atlas_agent_home.path());
        keyring.set_error(
            &account,
    ...
    fn local_namespaces_write_separate_files() -> Result<()> {
        ...
        let atlas_agent_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let atlas_engine_auth_backend = LocalSecretsBackend::new_with_namespace(
        ...
        atlas_engine_auth_backend.set(&scope, &name, "atlas-engine-auth-value")?;
        mcp_backend.set(&scope, &name, "mcp-value")?;

        assert_eq!(
            atlas_engine_auth_backend.get(&scope, &name)?,
            Some("atlas-engine-auth-value".to_string())
        );
        assert!(
            MCP_OAUTH_CACHE
                .lock()
                .unwrap_or_else(PoisonError::into_inner)
                .as_ref()
                .is_none_or(|cached| cached.path != atlas_engine_auth_backend.secrets_path())
        );
        assert_eq!(
