...
const KEYRING_SERVICE: &str = "atlas-agent";
...
impl SecretsManager {
    pub fn new(atlas_agent_home: PathBuf, backend_kind: SecretsBackendKind) -> Self {
        let backend: Arc<dyn SecretsBackend> = match backend_kind {
            SecretsBackendKind::Local => {
                let keyring_store: Arc<dyn KeyringStore> = Arc::new(DefaultKeyringStore);
                Arc::new(LocalSecretsBackend::new(atlas_agent_home, keyring_store))
            }
        };
        Self { backend }
    }
...
pub fn compute_keyring_account(atlas_agent_home: &Path) -> String {
