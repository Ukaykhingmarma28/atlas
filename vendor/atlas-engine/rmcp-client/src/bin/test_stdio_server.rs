...
const MEMO_URI: &str = "memo://atlas-agent/example-note";
const MEMO_CONTENT: &str = "This is a sample MCP resource served by the rmcp test server.";
const SANDBOX_STATE_META_CAPABILITY: &str = "atlas-agent/sandbox-state-meta";
...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of Atlas Agent TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (Atlas Agent TUI):
    /// - Build this binary: `cargo build -p atlas-engine-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `atlas_engine mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in Atlas Agent TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        ResourceTemplate::new("memo://atlas-agent/{slug}", "atlas-engine-memo")
            .with_title("Atlas Agent Memo")
            .with_description("Template for memo://atlas-agent/{slug} resources used in tests.")
            .with_mime_type("text/plain")
    }
...
/// Scenarios for `image_scenario`, intended to exercise Atlas Agent TUI handling of MCP image outputs.
///
    ...
    fn get_info(&self) -> ServerInfo {
        ...
        let server_info = match dynamic_server_process_label() {
            Some(process_label) => server_info
                .with_server_info(
                    Implementation::new("atlas-engine-rmcp-test-server", env!("CARGO_PKG_VERSION"))
        ...
        match request.name.as_ref() {
            "js" => {
                ...
                if args.code == "nodeRepl.fail()" {
                    ...
                    meta.insert("atlas-agent/imageDetail".to_string(), json!("low"));
                    Ok(CallToolResult::success(vec![
            ...
            "encrypted_output" => {
                let mut meta = MetaObject::new();
                meta.insert("atlas-agent/encryptedContent".to_string(), json!(true));
    ...
    fn image_scenario_result(args: ImageScenarioArgs) -> Result<CallToolResult, McpError> {
        ...
        match args.scenario {
            ...
            ImageScenario::ImageOnlyOriginalDetail => {
                let mut meta = MetaObject::new();
                meta.insert(
                    "atlas-agent/imageDetail".to_string(),
