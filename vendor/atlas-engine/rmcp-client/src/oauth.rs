...
//!
//! If the keyring is not available or fails, we fall back to ATLAS_AGENT_HOME/.credentials.json which is consistent with other coding CLI agents.
...
const KEYRING_SERVICE: &str = "Atlas Agent MCP Credentials";
...
fn load_oauth_tokens_from_secrets_keyring_with_lock_held<K: KeyringStore + Clone + 'static>(
    ...
    let atlas_agent_home = find_atlas_agent_home().map_err(anyhow::Error::from)?;
...
fn fallback_file_path() -> Result<PathBuf> {
    Ok(find_atlas_agent_home()?.join(FALLBACK_FILENAME).to_path_buf())
}
...
mod tests {
    ...
    fn save_oauth_tokens_with_secrets_backend_writes_encrypted_storage() -> Result<()> {
        let env = TempAtlasAgentHome::new();
        let store = MockKeyringStore::default();
        let tokens = sample_tokens();
        let key = super::compute_store_key(&tokens.server_name, &tokens.url)?;
        let serialized = serde_json::to_string(&tokens)?;
