...
impl RefreshCredentialLock {
    pub(super) async fn acquire_for_server(server_name: &str, url: &str) -> Result<Self> {
        ...
        Self::acquire_in(&atlas_agent_home, &store_key, REFRESH_LOCK_ACQUIRE_TIMEOUT)
            .await
    ...
    async fn acquire_in(
        ...
        // Scope coordination to ATLAS_AGENT_HOME alongside File and Secrets state. Direct keyring
