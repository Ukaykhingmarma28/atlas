...
impl OAuthStoreLock {
    ...
    fn acquire_with_timeout(
        ...
        // authority. Those stores are ATLAS_AGENT_HOME-backed today: if ATLAS_AGENT_HOME is unset they use
        // the default home (`~/.atlas_engine`), and if an embedder has no local home/filesystem authority
        ...
        let atlas_agent_home = find_atlas_agent_home()
            .map_err(|source| OAuthStoreLockFailure::AtlasAgentHome { store, source })?;
        Self::acquire_in_with_mode(&atlas_agent_home, store, acquire_timeout, mode)
    }
    ...
    fn acquire_in_with_mode(
        ...
        let path = oauth_store_lock_path(atlas_agent_home, store);
...
pub(super) enum OAuthStoreLockFailure {
    #[error("failed to resolve ATLAS_AGENT_HOME for MCP OAuth {store} aggregate-store lock")]
    AtlasAgentHome {
        store: OAuthStore,
        #[source]
        source: io::Error,
