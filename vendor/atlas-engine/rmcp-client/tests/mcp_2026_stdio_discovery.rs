...
fn local_stdio_inherits_ca_certificate_variables() -> anyhow::Result<()> {
    ...
    let output = Command::new(std::env::current_exe()?)
        ...
        .env("ATLAS_AGENT_CA_CERTIFICATE", "")
        .env("REQUESTS_CA_BUNDLE", requests_ca_bundle)
        .env("ATLAS_AGENT_MCP_TEST_SERVER_CWD", &server_dir)
        .output()?;
...
async fn local_stdio_inherits_ca_certificate_variables_child() -> anyhow::Result<()> {
    let server = atlas_engine_utils_cargo_bin::cargo_bin("test_stdio_server")?;
    let source_dir = std::env::current_dir()?;
    let server_dir = std::env::var("ATLAS_AGENT_MCP_TEST_SERVER_CWD")?;
    ...
    for (overrides, env_name, expected) in [
        ...
        (None, "ATLAS_AGENT_CA_CERTIFICATE", None),
        (
...
-> anyhow::Result<()> {
    let server = atlas_engine_utils_cargo_bin::cargo_bin("test_mcp_2026_discovery_stdio_server")?;
            ...
            let result = client
                .call_tool(
                    ...
                    Some(json!({
                        ...
                        "env_var": "ATLAS_AGENT_MCP_PROTOCOL_VERSION",
                    })),
                    /*meta*/ None,
                    Some(Duration::from_secs(10)),
                )
