...
use atlas_engine_rmcp_client::determine_streamable_http_auth_status;
...
async fn assert_expired_token_refresh(with_headers_helper: bool) -> anyhow::Result<()> {
    ...
    Mock::given(method("GET"))
        ...
        .and(header(
            "user-agent",
            concat!("atlas-agent/", env!("CARGO_PKG_VERSION")),
        ))
    ...
    // Credential storage resolves ATLAS_AGENT_HOME from the process environment.
