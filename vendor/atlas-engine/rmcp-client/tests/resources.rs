...
const RESOURCE_URI: &str = "memo://atlas-agent/example-note";
...
fn init_params() -> InitializeRequestParams {
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("atlas-engine-test", "0.0.0-test").with_title("Atlas Agent rmcp resource test"),
    )
...
async fn rmcp_client_can_list_and_read_resources() -> anyhow::Result<()> {
    ...
    let mut expected_templates = ListResourceTemplatesResult::with_all_items(vec![
        rmcp::model::ResourceTemplate::new("memo://atlas-agent/{slug}", "atlas-engine-memo")
