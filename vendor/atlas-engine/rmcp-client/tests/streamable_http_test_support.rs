...
use atlas_engine_exec_server::RemoteExecServerConnectArgs;
...
fn streamable_http_server_bin() -> Result<PathBuf, CargoBinError> {
    atlas_engine_utils_cargo_bin::cargo_bin("test_streamable_http_server")
}
...
fn control_client() -> anyhow::Result<atlas_engine_http_client::HttpClient> {
    Ok(atlas_engine_http_client::HttpClientBuilder::new().build_direct()?)
}
...
/// Reads the websocket URL printed by `atlas_engine exec-server --listen`.
