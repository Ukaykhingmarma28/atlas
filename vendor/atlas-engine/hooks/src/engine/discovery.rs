...
fn append_matcher_groups(
    ...
    event_name: atlas_engine_protocol::protocol::HookEventName,
            ...
            let normalized = match handler {
                HookHandlerConfig::Command {
                    ...
                    let runs_async = r#async
                        && event_name != atlas_engine_protocol::protocol::HookEventName::SessionEnd;
                    ...
                    let additional_context_limit = if matches!(
                        event_name,
                        atlas_engine_protocol::protocol::HookEventName::PreToolUse
                            | atlas_engine_protocol::protocol::HookEventName::PostToolUse
                            | atlas_engine_protocol::protocol::HookEventName::SessionStart
                            | atlas_engine_protocol::protocol::HookEventName::UserPromptSubmit
                            | atlas_engine_protocol::protocol::HookEventName::SubagentStart
                    ) {
                        additional_context_limit
                    } else {
...
fn normalize_command_hook(
    ...
    if event_name != atlas_engine_protocol::protocol::HookEventName::SessionEnd {
...
mod tests {
        ...
        let source = RequirementSource::Composite {
            sources: vec![
                RequirementSource::SystemRequirementsToml {
                    file: test_path_buf("/etc/atlas-agent/requirements.toml").abs(),
                },
    ...
    fn hook_metadata_for_config_layer_source_discards_source_details() {
        let config_file = test_path_buf("/tmp/.atlas-agent/config.toml").abs();
        let dot_atlas_agent_folder = test_path_buf("/tmp/worktree/.atlas-agent").abs();
