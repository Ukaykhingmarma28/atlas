...
use atlas_engine_protocol::approvals::ElicitationRequest as CoreElicitationRequest;
use atlas_engine_protocol::config_types::MultiAgentMode;
use atlas_engine_protocol::items::AgentMessageContent;
use atlas_engine_protocol::items::AgentMessageItem;
use atlas_engine_protocol::items::CollabAgentTool as CoreCollabAgentTool;
use atlas_engine_protocol::items::CollabAgentToolCallItem;
use atlas_engine_protocol::items::CollabAgentToolCallStatus as CoreCollabAgentToolCallStatus;
use atlas_engine_protocol::items::CommandExecutionItem;
...
use atlas_engine_protocol::items::FileChangeItem;
use atlas_engine_protocol::items::ImageViewItem;
use atlas_engine_protocol::items::McpToolCallItem;
use atlas_engine_protocol::items::McpToolCallStatus as CoreMcpToolCallStatus;
...
use atlas_engine_protocol::items::WebSearchItem as CoreWebSearchItem;
use atlas_engine_protocol::mcp::CallToolResult;
use atlas_engine_protocol::mcp::McpServerInfo;
use atlas_engine_protocol::memory_citation::MemoryCitation as CoreMemoryCitation;
use atlas_engine_protocol::memory_citation::MemoryCitationEntry as CoreMemoryCitationEntry;
...
use atlas_engine_protocol::permissions::FileSystemAccessMode as CoreFileSystemAccessMode;
use atlas_engine_protocol::permissions::FileSystemPath as CoreFileSystemPath;
use atlas_engine_protocol::permissions::FileSystemSandboxEntry as CoreFileSystemSandboxEntry;
use atlas_engine_protocol::permissions::FileSystemSpecialPath as CoreFileSystemSpecialPath;
use atlas_engine_protocol::protocol::AgentStatus as CoreAgentStatus;
use atlas_engine_protocol::protocol::AskForApproval as CoreAskForApproval;
use atlas_engine_protocol::protocol::ConversationTextRole;
use atlas_engine_protocol::protocol::ExecCommandSource as CoreExecCommandSource;
use atlas_engine_protocol::protocol::GranularApprovalConfig as CoreGranularApprovalConfig;
use atlas_engine_protocol::protocol::NetworkAccess as CoreNetworkAccess;
use atlas_engine_protocol::protocol::SubAgentActivityKind as CoreSubAgentActivityKind;
use atlas_engine_protocol::request_permissions::RequestPermissionProfile as CoreRequestPermissionProfile;
use atlas_engine_protocol::user_input::UserInput as CoreUserInput;
...
fn thread_sources_round_trip_as_scalar_labels() {
    for (source, label) in [
        ...
        let core_source: atlas_engine_protocol::protocol::ThreadSource = source.clone().into();
...
fn sandbox_policy_round_trips_external_sandbox_network_access() {
    ...
    assert_eq!(
        core_policy,
        atlas_engine_protocol::protocol::SandboxPolicy::ExternalSandbox {
...
fn sandbox_policy_round_trips_read_only_network_access() {
    ...
    assert_eq!(
        core_policy,
        atlas_engine_protocol::protocol::SandboxPolicy::ReadOnly {
            network_access: true,
        }
    );
...
fn config_requirements_read_accepts_foreign_path_uris() {
    let response: ConfigRequirementsReadResponse = serde_json::from_value(json!({
        "requirements": {
            "sqliteHome": "file:///C:/Users/alice/.atlas-agent/state",
            "logDir": "file:///C:/Users/alice/.atlas-agent/logs",
            "modelCatalogJson": "file:///C:/Users/alice/.atlas-agent/models.json"
        }
    }))
    ...
    assert_eq!(
        ...
        Some(PathUri::parse("file:///C:/Users/alice/.atlas-agent/state").expect("valid URI"))
    );
    assert_eq!(
        requirements.log_dir,
        Some(PathUri::parse("file:///C:/Users/alice/.atlas-agent/logs").expect("valid URI"))
    );
    assert_eq!(
        requirements.model_catalog_json,
        Some(PathUri::parse("file:///C:/Users/alice/.atlas-agent/models.json").expect("valid URI"))
    );
}
...
fn sandbox_policy_round_trips_workspace_write_access() {
    ...
    assert_eq!(
        core_policy,
        atlas_engine_protocol::protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: true,
            exclude_tmpdir_env_var: false,
            exclude_slash_tmp: false,
...
fn core_turn_item_into_thread_item_converts_supported_variants() {
    let user_item = TurnItem::UserMessage(UserMessageItem {
        ...
        content: vec![
            ...
            CoreUserInput::Skill {
                ...
                path: PathBuf::from("/repo/.atlas-agent/skills/skill-creator/SKILL.md"),
            },
    ...
    let command_item = TurnItem::CommandExecution(CommandExecutionItem {
        ...
        parsed_cmd: vec![atlas_engine_protocol::parse_command::ParsedCommand::Unknown {
    ...
    let dynamic_tool_call_item = TurnItem::DynamicToolCall(DynamicToolCallItem {
        ...
        content_items: Some(vec![
            atlas_engine_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText {
                text: "ok".to_string(),
            },
    ...
    let sender_thread_id = atlas_engine_protocol::ThreadId::default();
    let receiver_thread_id = atlas_engine_protocol::ThreadId::default();
    ...
    let sub_agent_activity_item = TurnItem::SubAgentActivity(SubAgentActivityItem {
        ...
        agent_path: atlas_engine_protocol::AgentPath::root()
            .join("worker")
            .expect("worker path"),
    });
    ...
    assert_eq!(
        ThreadItem::from(TurnItem::Extension(
            atlas_engine_extension_items::ExtensionItem::WebSearch(expected_search_item.clone()),
        )),
    ...
    let file_change_item = TurnItem::FileChange(FileChangeItem {
        ...
        .into_iter()
        .collect(),
        status: Some(atlas_engine_protocol::protocol::PatchApplyStatus::Completed),
        auto_approved: None,
...
fn plugin_share_params_and_response_serialization_use_camel_case_fields() {
    ...
    assert_eq!(
        serde_json::to_value(PluginShareCheckoutResponse {
            remote_plugin_id: "plugins~Plugin_00000000000000000000000000000000".to_string(),
            plugin_id: "gmail@atlas-engine-curated".to_string(),
            plugin_name: "gmail".to_string(),
            plugin_path,
            marketplace_name: "atlas-engine-curated".to_string(),
            marketplace_path,
        ...
        json!({
            "remotePluginId": "plugins~Plugin_00000000000000000000000000000000",
            "pluginId": "gmail@atlas-engine-curated",
            "pluginName": "gmail",
            "pluginPath": plugin_path_json,
            "marketplaceName": "atlas-engine-curated",
            "marketplacePath": marketplace_path_json,
            "remoteVersion": "1.2.3",
        }),
    );
...
#[test]
fn atlas_engine_error_info_serializes_http_status_code_in_camel_case() {
    let value = AtlasEngineErrorInfo::ResponseTooManyFailedAttempts {
...
#[test]
fn atlas_engine_error_info_serializes_cyber_policy_in_camel_case() {
    assert_eq!(
        serde_json::to_value(AtlasEngineErrorInfo::CyberPolicy).unwrap(),
        json!("cyberPolicy")
    );
}

#[test]
fn atlas_engine_error_info_serializes_active_turn_not_steerable_turn_kind_in_camel_case() {
    let value = AtlasEngineErrorInfo::ActiveTurnNotSteerable {
...
fn turn_start_params_round_trip_multi_agent_mode() {
    ...
    assert_eq!(
        ...
        Some(atlas_engine_protocol::config_types::MultiAgentMode::Proactive)
    );
    assert_eq!(
...
fn thread_settings_update_params_preserve_field_level_experimental_gates() {
    ...
    let collaboration_mode = ThreadSettingsUpdateParams {
        ...
        collaboration_mode: Some(atlas_engine_protocol::config_types::CollaborationMode {
            mode: atlas_engine_protocol::config_types::ModeKind::Plan,
...
fn realtime_start_deserializes_client_handoff_channel_prefixes() {
    let params = serde_json::from_value::<ThreadRealtimeStartParams>(json!({
        "threadId": "thread_123",
        "outputModality": "audio",
        "atlasEngineResponseHandoffChannelPrefixes": {
            "analysis": ["[THINKING]"],
    ...
    assert_eq!(
        params.atlas_engine_response_handoff_channel_prefixes,
        Some(BTreeMap::from([
