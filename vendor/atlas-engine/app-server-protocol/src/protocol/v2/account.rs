...
use atlas_engine_protocol::protocol::CreditsSnapshot as CoreCreditsSnapshot;
use atlas_engine_protocol::protocol::RateLimitReachedType as CoreRateLimitReachedType;
use atlas_engine_protocol::protocol::RateLimitSnapshot as CoreRateLimitSnapshot;
use atlas_engine_protocol::protocol::RateLimitWindow as CoreRateLimitWindow;
use atlas_engine_protocol::protocol::SpendControlLimitSnapshot as CoreSpendControlLimitSnapshot;
...
pub enum Account {
    ...
    AmazonBedrock {
        #[serde(default)]
        uses_atlas_engine_managed_credentials: bool,
    },
}
    ...
    fn from(account: ProviderAccount) -> Self {
        match account {
            ...
            ProviderAccount::AmazonBedrock {
                uses_atlas_engine_managed_credentials,
            } => Self::AmazonBedrock {
                uses_atlas_engine_managed_credentials,
            },
        }
...
pub enum LoginAccountParams {
    ...
    /// The access token must contain the same scopes that Atlas Agent-managed ChatGPT auth tokens have.
    ...
    ChatgptAuthTokens {
        ...
        ///
        /// When `null`, Atlas Agent attempts to derive the plan type from access-token
...
pub enum ChatgptAuthTokensRefreshReason {
    /// Atlas Agent attempted a backend request and received `401 Unauthorized`.
    Unauthorized,
}
...
pub struct ChatgptAuthTokensRefreshParams {
    ...
    /// Workspace/account identifier that Atlas Agent was previously using.
    ///
...
pub struct GetAccountRateLimitsResponse {
    ...
    /// Multi-bucket view keyed by metered `limit_id` (for example, `atlas_engine`).
