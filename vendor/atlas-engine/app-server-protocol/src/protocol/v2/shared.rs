...
use atlas_engine_protocol::config_types::ApprovalsReviewer as CoreApprovalsReviewer;
use atlas_engine_protocol::config_types::SandboxMode as CoreSandboxMode;
...
use atlas_engine_protocol::protocol::NonSteerableTurnKind as CoreNonSteerableTurnKind;
#[cfg(test)]
...
/// This translation layer make sure that we expose atlas-agent error code in camel case.
///
/// When an upstream HTTP status is available (for example, from the Responses API or a provider),
/// it is forwarded in `httpStatusCode` on the relevant `atlasEngineErrorInfo` variant.
    ...
    fn from(value: CoreAtlasEngineErrorInfo) -> Self {
        match value {
            ...
            CoreAtlasEngineErrorInfo::HttpConnectionFailed { http_status_code } => {
                AtlasEngineErrorInfo::HttpConnectionFailed { http_status_code }
            }
            ...
            CoreAtlasEngineErrorInfo::ResponseTooManyFailedAttempts { http_status_code } => {
                AtlasEngineErrorInfo::ResponseTooManyFailedAttempts { http_status_code }
            }
            CoreAtlasEngineErrorInfo::ActiveTurnNotSteerable { turn_kind } => {
                AtlasEngineErrorInfo::ActiveTurnNotSteerable {
                    turn_kind: turn_kind.into(),
                }
            }
            CoreAtlasEngineErrorInfo::Other => AtlasEngineErrorInfo::Other,
        }
    }
}
