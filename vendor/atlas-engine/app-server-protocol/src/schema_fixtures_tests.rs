...
fn write_schema_fixtures_from_env() -> Result<()> {
    let schema_root = std::env::var_os("ATLAS_AGENT_APP_SERVER_SCHEMA_ROOT")
        .map(PathBuf::from)
        .context("ATLAS_AGENT_APP_SERVER_SCHEMA_ROOT must be set")?;
    let prettier = std::env::var_os("ATLAS_AGENT_APP_SERVER_SCHEMA_PRETTIER").map(PathBuf::from);
    let experimental = std::env::var("ATLAS_AGENT_APP_SERVER_SCHEMA_EXPERIMENTAL")
        .context("ATLAS_AGENT_APP_SERVER_SCHEMA_EXPERIMENTAL must be set")?;
