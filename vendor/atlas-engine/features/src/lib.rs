...
pub enum Feature {
    ...
    Chronicle,
    /// Compress request bodies (zstd) when sending streaming requests to atlas-engine-backend.
    ...
    NetworkProxy,
    /// Respect host system proxy settings for Atlas Agent-owned network clients.
    RespectSystemProxy,
    /// Enable collab tools.
    Collab,
    ...
    /// Requirements-only gate: this should be set from requirements, not user config.
    BrowserUseExternal,
    /// Allow Atlas Agent Computer Use.
    ///
    /// Requirements-only gate: this should be set from requirements, not user config.
    ComputerUse,
    ...
    /// Prompt Atlas Agent Apps connector auth failures through MCP URL elicitations.
    AuthElicitation,
...
pub const FEATURES: &[FeatureSpec] = &[
    ...
    FeatureSpec {
        id: Feature::AtlasEngineGitCommit,
        key: "atlas_engine_git_commit",
        stage: Stage::Removed,
        default_enabled: false,
    },
    FeatureSpec {
    ...
    FeatureSpec {
        ...
        stage: Stage::Experimental {
            ...
            announcement: "NEW: Network proxy can now be enabled from /experimental. Restart Atlas Agent after enabling it.",
        },
        default_enabled: false,
    },
    FeatureSpec {
    ...
    FeatureSpec {
        ...
        stage: if cfg!(any(
            ...
            Stage::Experimental {
                ...
                menu_description: "Keep your computer awake while Atlas Agent is running a thread.",
