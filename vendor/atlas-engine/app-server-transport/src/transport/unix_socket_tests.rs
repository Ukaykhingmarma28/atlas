...
fn listen_unix_socket_accepts_absolute_custom_path() {
    assert_eq!(
        AppServerTransport::from_listen_url("unix:///tmp/atlas-agent.sock"),
        Ok(AppServerTransport::UnixSocket {
            socket_path: absolute_path("/tmp/atlas-agent.sock")
        })
    );
}
...
fn listen_unix_socket_accepts_relative_custom_path() {
    assert_eq!(
        AppServerTransport::from_listen_url("unix://atlas-agent.sock"),
        Ok(AppServerTransport::UnixSocket {
            socket_path: AbsolutePathBuf::relative_to_current_dir("atlas_agent.sock")
...
fn default_control_socket_path() -> AbsolutePathBuf {
    let atlas_agent_home = find_atlas_agent_home().expect("atlas-agent home");
    app_server_control_socket_path(&atlas_agent_home).expect("default control socket path")
}
