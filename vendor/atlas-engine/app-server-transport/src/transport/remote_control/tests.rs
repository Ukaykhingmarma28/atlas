...
fn remote_control_auth_manager() -> Arc<AuthManager> {
    auth_manager_from_auth(AtlasEngineAuth::create_dummy_chatgpt_auth_for_testing())
}

fn remote_control_auth_manager_with_home(atlas_agent_home: &TempDir) -> Arc<AuthManager> {
...
async fn remote_control_state_runtime(atlas_agent_home: &TempDir) -> Arc<StateRuntime> {
    StateRuntime::init(
...
async fn plain_start_resolves_persisted_remote_control_preference() {
    ...
    let state_db = remote_control_state_runtime(&atlas_agent_home).await;
...
async fn ephemeral_enable_preserves_durable_preference() {
    ...
    remote_handle.state_db = Some(remote_control_state_runtime(&atlas_agent_home).await);
    remote_handle
        .desired_state_tx
...
async fn remote_control_transport_manages_virtual_clients_and_routes_messages() {
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Notification(
                    atlas_engine_app_server_protocol::JSONRPCNotification {
    ...
    let followup_message =
        JSONRPCMessage::Notification(atlas_engine_app_server_protocol::JSONRPCNotification {
            method: "initialized".to_string(),
            params: None,
        });
    send_client_event(
...
async fn remote_control_transport_reconnects_after_disconnect() {
    ...
    let (remote_task, remote_handle) = start_remote_control(
        ...
        Some(remote_control_state_runtime(&atlas_agent_home).await),
        remote_control_auth_manager(),
        transport_event_tx,
        shutdown_token.clone(),
        /*app_server_client_name_rx*/ None,
...
async fn remote_control_http_mode_enrolls_before_connecting() {
    ...
    assert_eq!(
        handshake_request.headers.get("x-atlas-engine-server-id"),
        Some(&"srv_e_test".to_string())
    );
    assert_eq!(
        handshake_request.headers.get("x-atlas-engine-name"),
    ...
    let writer = {
        let initialize_message =
            JSONRPCMessage::Request(atlas_engine_app_server_protocol::JSONRPCRequest {
                id: atlas_engine_app_server_protocol::RequestId::Integer(11),
                method: "initialize".to_string(),
                params: Some(json!({
                    "clientInfo": {
    ...
    writer
        .send(QueuedOutgoingMessage::new(OutgoingMessage::Response(
            crate::outgoing_message::OutgoingResponse {
                id: atlas_engine_app_server_protocol::RequestId::Integer(11),
                result: Box::new(
                    atlas_engine_app_server_protocol::ClientResponsePayload::Initialize(
                        atlas_engine_app_server_protocol::InitializeResponse {
                            user_agent: "atlas-engine-test-agent".to_string(),
                            atlas_agent_home: atlas_agent_home.path().abs(),
    ...
    assert_eq!(
        ...
        json!({
            ...
            "message": {
                "id": 11,
                "result": {
                    "userAgent": "atlas-engine-test-agent",
                    "atlasAgentHome": atlas_agent_home.path(),
...
async fn remote_control_http_mode_refreshes_persisted_enrollment_before_connecting() {
    ...
    let (remote_task, _remote_handle) = start_remote_control(
        ...
        Some(state_db.clone()),
        remote_control_auth_manager_with_home(&atlas_agent_home),
        transport_event_tx,
        shutdown_token.clone(),
        /*app_server_client_name_rx*/ None,
        RemoteControlStartupMode::EnabledEphemeral,
...
async fn remote_control_http_mode_reenrolls_after_explicit_missing_server_404() {
    ...
    assert_eq!(
        websocket_request.headers.get("x-atlas-engine-server-id"),
