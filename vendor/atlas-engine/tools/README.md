...
`atlas-engine-tools` is the shared support crate for building, adapting, and executing
model-visible tools outside `atlas-engine-core`.
...
compatibility-sensitive orchestration in `atlas-engine-core` until the surrounding
...
- code-mode compatibility shims that do not depend on `atlas-engine-core`
- other narrowly scoped host utilities that multiple crates need

The corresponding non-goals are just as important:

- do not move `atlas-engine-core` orchestration here prematurely
...
1. Keep extension-owned executable-tool authoring in `atlas-engine-extension-api`.
2. Move host-side planning/adaptation helpers here when they no longer need to
   stay coupled to `atlas-engine-core`.
3. Leave compatibility-sensitive adapters in `atlas-engine-core` while downstream
...
`atlas-engine-core`, that is a sign to revisit the extraction boundary before adding
more here.
