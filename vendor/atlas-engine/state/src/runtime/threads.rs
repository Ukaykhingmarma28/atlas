...
ON CONFLICT(id) DO NOTHING
        ...
        .bind(
            metadata
                .thread_source
                .as_ref()
                .map(atlas_engine_protocol::protocol::ThreadSource::as_str),
        )
...
mod tests {
    ...
    async fn upsert_thread_keeps_creation_memory_mode_for_existing_rows() {
        let atlas_agent_home = unique_temp_dir();
    ...
    async fn thread_metadata_history_mode_does_not_downgrade() {
        ...
        let metadata = test_thread_metadata(&atlas_agent_home, thread_id, atlas_agent_home.clone());
    ...
    async fn list_threads_filters_by_cwd() {
        ...
        let first_cwd = atlas_agent_home.join("first");
        let second_cwd = atlas_agent_home.join("second");
        let other_cwd = atlas_agent_home.join("other");
        ...
        for (thread_id, cwd, updated_at) in [
            ...
            let mut metadata = test_thread_metadata(&atlas_agent_home, thread_id, cwd);
            metadata.updated_at =
    ...
    async fn apply_rollout_items_preserves_existing_git_branch_and_fills_missing_git_fields() {
        ...
        let items = vec![RolloutItem::SessionMeta(SessionMetaLine {
            ...
            git: Some(GitInfo {
                commit_hash: Some(atlas_engine_git_utils::GitSha::new("rollout-sha")),
                branch: Some("rollout-branch".to_string()),
                repository_url: Some("git@example.com:openai/atlas-agent.git".to_string()),
            }),
        })];
        ...
        assert_eq!(
            ...
            Some("git@example.com:openai/atlas-agent.git")
        );
    }
    ...
    async fn update_thread_git_info_preserves_newer_non_git_metadata() {
        ...
        let updated = runtime
            .update_thread_git_info(
                ...
                Some(Some("git@example.com:openai/atlas-agent.git")),
            )
            .await
    ...
    async fn thread_updated_at_uses_unique_epoch_millis_and_reads_legacy_seconds() {
        ...
        let mut older = test_thread_metadata(&atlas_agent_home, older_id, atlas_agent_home.clone());
    ...
    async fn apply_rollout_items_uses_override_updated_at_when_provided() {
        ...
        let items = vec![RolloutItem::EventMsg(EventMsg::TokenCount(
            atlas_engine_protocol::protocol::TokenCountEvent {
                info: Some(atlas_engine_protocol::protocol::TokenUsageInfo {
                    total_token_usage: atlas_engine_protocol::protocol::TokenUsage {
                        input_tokens: 0,
                    ...
                    last_token_usage: atlas_engine_protocol::protocol::TokenUsage::default(),
