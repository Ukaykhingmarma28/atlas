...
impl RuntimeDbSpec {
    fn path(self, atlas_agent_home: &Path) -> PathBuf {
        atlas_agent_home.join(self.filename)
    }
}
...
/// Resolved configuration shared by all Atlas Agent SQLite connections.
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl SqliteConfig {
    ...
    /// Open a writable Atlas Agent SQLite database, creating it if necessary.
    ...
    /// Open an existing Atlas Agent SQLite database without creating or modifying it.
