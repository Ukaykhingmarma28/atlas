...
//!
//! # async fn example(state_db: std::sync::Arc<atlas_engine_state::StateRuntime>) {
...
pub fn default_filter() -> Targets {
    Targets::new()
        ...
        .with_target("atlas_engine_rmcp_client", LevelFilter::INFO)
    ...
    S: tracing::Subscriber + for<'a> LookupSpan<'a>,
{
    ...
    fn on_event(&self, event: &Event<'_>, ctx: tracing_subscriber::layer::Context<'_, S>) {
        ...
        // were over 30% of retained logs in measured high-fanout Atlas Agent environments.
...
mod tests {
    ...
    fn temp_atlas_agent_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("atlas-engine-state-log-db-{}", Uuid::new_v4()))
    }
    ...
    async fn sqlite_feedback_logs_match_feedback_formatter_shape() {
        let atlas_agent_home = temp_atlas_agent_home();
