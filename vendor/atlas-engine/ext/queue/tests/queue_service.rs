...
impl ThreadLifecycleContributor<atlas_engine_core::config::Config> for InstalledQueue {
...
fn registered_queue_extensions() -> (
    Arc<InstalledQueue>,
    Arc<ExtensionRegistry<atlas_engine_core::config::Config>>,
) {
...
async fn starting_a_selected_item_while_active_leaves_it_queued() -> anyhow::Result<()> {
    ...
    wait_for_event_match(test.atlas_engine.as_ref(), |event| {
...
async fn invalid_head_is_skipped_and_a_live_user_turn_is_accepted() -> anyhow::Result<()> {
    ...
    let client_id = wait_for_event_match(test.atlas_engine.as_ref(), |event| match event {
