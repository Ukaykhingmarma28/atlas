...
use atlas_engine_extension_api::FunctionCallError;
use atlas_engine_extension_api::NoopTurnItemEmitter;
use atlas_engine_extension_api::ThreadResumeInput;
use atlas_engine_extension_api::ThreadStartInput;
use atlas_engine_extension_api::ThreadStopInput;
use atlas_engine_extension_api::ToolCall;
use atlas_engine_extension_api::ToolCallOutcome;
use atlas_engine_extension_api::ToolCallSource;
use atlas_engine_extension_api::ToolExecutor;
use atlas_engine_extension_api::ToolFinishInput;
use atlas_engine_extension_api::ToolPayload;
use atlas_engine_extension_api::TurnErrorInput;
use atlas_engine_extension_api::TurnStartInput;
use atlas_engine_extension_api::TurnStopInput;
use atlas_engine_goal_extension::GoalExtensionConfig;
use atlas_engine_goal_extension::GoalObjectiveUpdate;
use atlas_engine_goal_extension::GoalRuntimeHandle;
use atlas_engine_goal_extension::GoalService;
use atlas_engine_goal_extension::GoalSetRequest;
use atlas_engine_goal_extension::GoalTokenBudgetUpdate;
use atlas_engine_goal_extension::install_with_backend;
...
use atlas_engine_protocol::protocol::ThreadGoalStatus;
...
async fn budget_limited_goal_keeps_accruing_until_turn_stop() -> anyhow::Result<()> {
    ...
    assert_eq!(atlas_engine_state::ThreadGoalStatus::BudgetLimited, goal.status);
...
async fn turn_error_usage_limit_accounts_progress_and_clears_accounting() -> anyhow::Result<()> {
    ...
    harness
        ...
        .await;
    harness
        .notify_turn_error("turn-1", AtlasEngineErrorInfo::UsageLimitExceeded)
        .await;
    ...
    let goal = runtime
        ...
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(23, goal.tokens_used);
    assert_eq!(atlas_engine_state::ThreadGoalStatus::UsageLimited, goal.status);
    assert_eq!(
        vec![
            CapturedGoalEvent {
...
async fn turn_error_blocks_goal() -> anyhow::Result<()> {
    ...
    harness
        .notify_turn_error("turn-1", AtlasEngineErrorInfo::Other)
        .await;
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(atlas_engine_state::ThreadGoalStatus::Blocked, goal.status);
    Ok(())
}
...
async fn usage_limit_plan_turn_does_not_stop_goal() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(atlas_engine_state::ThreadGoalStatus::Active, goal.status);
...
async fn installed_tools(
    runtime: Arc<atlas_engine_state::StateRuntime>,
    thread_id: ThreadId,
...
struct GoalExtensionHarness {
    registry: atlas_engine_extension_api::ExtensionRegistry<()>,
...
impl GoalExtensionHarness {
    ...
    async fn notify_tool_finish(&self, turn_id: &str, call_id: &str, tool_name: &str) {
        let turn_store = ExtensionData::new(turn_id);
        let tool_name = atlas_engine_extension_api::ToolName::plain(tool_name);
    ...
    async fn notify_turn_error(&self, turn_id: &str, error: AtlasEngineErrorInfo) {
        let turn_store = ExtensionData::new(turn_id);
        for contributor in self.registry.turn_lifecycle_contributors() {
            contributor
...
fn tool_call(tool_name: &str, call_id: &str, arguments: serde_json::Value) -> ToolCall {
    ToolCall {
        ...
        tool_name: atlas_engine_extension_api::ToolName::plain(tool_name),
        model: "gpt-test".to_string(),
        atlas_engine_turn_metadata: None,
        truncation_policy: TruncationPolicy::Bytes(1024),
        conversation_history: atlas_engine_extension_api::ConversationHistory::default(),
...
async fn test_runtime() -> anyhow::Result<Arc<atlas_engine_state::StateRuntime>> {
    let tempdir = TempDir::new()?;
    atlas_engine_state::StateRuntime::init(
        atlas_engine_state::SqliteConfig::new_for_testing(tempdir.keep().as_path().abs()),
...
async fn seed_thread_metadata(
    runtime: &atlas_engine_state::StateRuntime,
    thread_id: ThreadId,
) -> anyhow::Result<()> {
    let builder = atlas_engine_state::ThreadMetadataBuilder::new(
        thread_id,
        runtime
            .sqlite()
            .home()
...
fn protocol_status(status: atlas_engine_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
        ...
        atlas_engine_state::ThreadGoalStatus::Blocked => ThreadGoalStatus::Blocked,
        atlas_engine_state::ThreadGoalStatus::UsageLimited => ThreadGoalStatus::UsageLimited,
        atlas_engine_state::ThreadGoalStatus::BudgetLimited => ThreadGoalStatus::BudgetLimited,
        atlas_engine_state::ThreadGoalStatus::Complete => ThreadGoalStatus::Complete,
    }
}
