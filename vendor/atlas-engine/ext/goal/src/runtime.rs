...
struct GoalRuntimeInner {
    thread_id: ThreadId,
    state_dbs: Arc<atlas_engine_state::StateRuntime>,
...
pub struct PreviousGoalSnapshot {
    pub goal_id: String,
    pub status: atlas_engine_state::ThreadGoalStatus,
    pub objective: String,
}

impl From<&atlas_engine_state::ThreadGoal> for PreviousGoalSnapshot {
    fn from(goal: &atlas_engine_state::ThreadGoal) -> Self {
        Self {
...
impl GoalRuntimeHandle {
    ...
    pub(crate) async fn stop_active_goal_for_turn(
        ...
        let can_stop = active_goal.status == atlas_engine_state::ThreadGoalStatus::Active
            || (active_goal.status == atlas_engine_state::ThreadGoalStatus::BudgetLimited
                && status == atlas_engine_state::ThreadGoalStatus::UsageLimited);
        if !can_stop {
            self.inner.accounting_state.clear_active_goal();
            return Ok(());
        }
    ...
    pub async fn restore_after_resume(&self) -> Result<(), String> {
        ...
        let goal = self
            ...
            .map_err(|err| err.to_string())?;
        match goal {
            Some(goal) if goal.status == atlas_engine_state::ThreadGoalStatus::Active => {
                self.inner
                    .accounting_state
    ...
    pub(crate) async fn continue_if_idle(&self) -> Result<(), String> {
        ...
        if goal.status != atlas_engine_state::ThreadGoalStatus::Active {
            self.inner.accounting_state.clear_active_goal();
            return Ok(());
        }
    ...
    async fn account_idle_goal_progress(
        ...
        Ok(match outcome {
            ...
            atlas_engine_state::GoalAccountingOutcome::Unchanged(_) => {
    ...
    ) -> Result<Option<atlas_engine_state::ThreadGoalStatus>, String> {
        let goal = self
            .inner
            .state_dbs
            .thread_goals()
