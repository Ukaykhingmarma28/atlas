...
use atlas_engine_extension_api::ThreadIdleInput;
...
use atlas_engine_extension_api::TurnAbortInput;
    ...
    C: Send + Sync + 'static,
{
    fn on_turn_start<'a>(&'a self, input: TurnStartInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            if matches!(
                input.collaboration_mode.mode,
                atlas_engine_protocol::config_types::ModeKind::Plan
            ) {
    ...
    fn on_turn_error<'a>(&'a self, input: TurnErrorInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            let reason = match input.error {
                AtlasEngineErrorInfo::UsageLimitExceeded => ActiveGoalStopReason::UsageLimit,
