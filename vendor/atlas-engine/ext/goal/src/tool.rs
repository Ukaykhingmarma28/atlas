...
use atlas_engine_extension_api::JsonToolOutput;
...
use atlas_engine_extension_api::ToolOutput;
use atlas_engine_extension_api::ToolSpec;
...
pub(crate) struct GoalToolExecutor {
    kind: GoalToolKind,
    thread_id: ThreadId,
    state_db: Arc<atlas_engine_state::StateRuntime>,
...
impl ToolExecutor<ToolCall> for GoalToolExecutor {
    ...
    fn handle(&self, invocation: ToolCall) -> atlas_engine_extension_api::ToolExecutorFuture<'_> {
        Box::pin(async move {
            match self.kind {
...
impl GoalToolExecutor {
    ...
    async fn account_active_goal_progress(
        &self,
        mode: atlas_engine_state::GoalAccountingMode,
        event_id: &str,
        ...
        Ok(match outcome {
            atlas_engine_state::GoalAccountingOutcome::Updated(goal) => {
                self.metrics
                    .record_terminal_if_status_changed(previous_status, &goal);
                self.analytics
            ...
            atlas_engine_state::GoalAccountingOutcome::Unchanged(_) => None,
        })
    }
    ...
    ) -> Result<Option<atlas_engine_state::ThreadGoalStatus>, FunctionCallError> {
        let goal = self
            .state_db
            .thread_goals()
            .get_thread_goal(self.thread_id)
...
pub(crate) async fn fill_empty_thread_preview_if_possible(
    state_db: &atlas_engine_state::StateRuntime,
    thread_id: ThreadId,
...
pub(crate) fn protocol_goal_from_state(goal: atlas_engine_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn protocol_status_from_state(status: atlas_engine_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(crate) fn state_status_from_protocol(
    status: ThreadGoalStatus,
) -> atlas_engine_state::ThreadGoalStatus {
    match status {
        ThreadGoalStatus::Active => atlas_engine_state::ThreadGoalStatus::Active,
        ThreadGoalStatus::Paused => atlas_engine_state::ThreadGoalStatus::Paused,
        ThreadGoalStatus::Blocked => atlas_engine_state::ThreadGoalStatus::Blocked,
        ThreadGoalStatus::UsageLimited => atlas_engine_state::ThreadGoalStatus::UsageLimited,
        ThreadGoalStatus::BudgetLimited => atlas_engine_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => atlas_engine_state::ThreadGoalStatus::Complete,
    }
}
