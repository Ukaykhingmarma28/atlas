...
use atlas_engine_api::ImageQuality;
use atlas_engine_api::ImageUrl;
...
use atlas_engine_extension_api::ToolEnvironment;
...
use atlas_engine_extension_api::parse_tool_input_schema;
...
pub(crate) struct ImageGenerationTool {
    backend: AtlasEngineImagesBackend,
    ...
    async fn handle_call(&self, call: ToolCall) -> Result<Box<dyn ToolOutput>, FunctionCallError> {
        ...
        .map_err(|error| {
            (
                ...
                usage_limit_failure(error.atlas_engine_error()),
            )
        })
        .and_then(|response| {
...
fn usage_limit_failure(error: &AtlasEngineErr) -> Option<ImageGenerationFailure> {
    let AtlasEngineErrorDetails::UsageLimitReached(usage_limit) = error.details() else {
        return None;
    };
