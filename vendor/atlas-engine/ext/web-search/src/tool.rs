...
use atlas_engine_api::SearchClient;
use atlas_engine_api::SearchCommands;
use atlas_engine_api::SearchQuery;
use atlas_engine_api::SearchRequest;
use atlas_engine_api::SearchSettings;
use atlas_engine_core::X_ATLAS_AGENT_TURN_METADATA_HEADER;
use atlas_engine_core::web_search_action_detail;
use atlas_engine_extension_api::ExtensionTurnItem;
use atlas_engine_extension_api::FunctionCallError;
use atlas_engine_extension_api::ResponsesApiTool;
...
use atlas_engine_extension_api::parse_tool_input_schema_without_compaction;
use atlas_engine_extension_items::ExtensionItem;
use atlas_engine_extension_items::web_search::WebSearchAction;
...
const RESULTS_PAYLOAD_BYTES_METRIC: &str = "atlas_agent.web_search.results.payload_bytes";
...
impl ToolExecutor<ToolCall> for WebSearchTool {
    ...
    fn handle(&self, call: ToolCall) -> atlas_engine_extension_api::ToolExecutorFuture<'_> {
...
impl WebSearchTool {
    async fn handle_call(&self, call: ToolCall) -> Result<Box<dyn ToolOutput>, FunctionCallError> {
        ...
        let extra_headers = search_request_headers(
            ...
            call.atlas_engine_turn_metadata.as_deref(),
        );
        ...
        if let Some(results) = results.as_ref()
            && let Some(metrics) = atlas_engine_otel::global()
...
fn search_request_headers(originator: Option<&str>, turn_metadata: Option<&str>) -> HeaderMap {
    ...
    {
        headers.insert(X_ATLAS_AGENT_TURN_METADATA_HEADER, header_value);
    }
