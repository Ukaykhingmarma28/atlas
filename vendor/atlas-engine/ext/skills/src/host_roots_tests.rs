...
use atlas_engine_exec_server::ExecutorFileSystemFuture;
use atlas_engine_exec_server::FileMetadata;
use atlas_engine_exec_server::FileSystemReadStream;
use atlas_engine_exec_server::FileSystemSandboxContext;
use atlas_engine_exec_server::LOCAL_FS;
use atlas_engine_exec_server::ReadDirectoryEntry;
use atlas_engine_exec_server::RemoveOptions;
use atlas_engine_exec_server::WalkOptions;
use atlas_engine_exec_server::WalkOutcome;
...
fn user_layer(atlas_agent_home: &AbsolutePathBuf) -> ConfigLayerEntry {
    ConfigLayerEntry::new(
        ConfigLayerSource::User {
            file: atlas_agent_home.join("config.toml"),
            profile: None,
        },
        empty_config(),
    )
...
fn project_layer(dot_atlas_agent_folder: &AbsolutePathBuf) -> ConfigLayerEntry {
    ConfigLayerEntry::new(
...
fn layer_roots_preserve_scope_precedence_and_disabled_projects() {
    ...
    let system_folder = absolute(temp_dir.path().join("etc/atlas-agent"));
    let home_folder = absolute(temp_dir.path().join("home"));
    let user_folder = home_folder.join("atlas-agent");
    let project_folder = absolute(temp_dir.path().join("repo/.atlas-agent"));
    let nested_project_folder = absolute(temp_dir.path().join("repo/nested/.atlas-agent"));
    let config_stack = stack(vec![
        ...
        ConfigLayerEntry::new_disabled(
            ConfigLayerSource::Project {
                dot_atlas_agent_folder: project_folder.clone(),
            },
            empty_config(),
            "untrusted project",
        ),
...
async fn resolved_project_layer_loads_skill_without_git_marker() {
    ...
    let dot_atlas_engine = workspace.join(".atlas-agent");
    let skill_root = dot_atlas_engine.join("skills");
    fs::create_dir_all(&workspace).expect("create workspace");
    let skill_path = write_skill(&skill_root, "local", "local-skill");
    let config_stack = stack(vec![project_layer(&dot_atlas_engine)]);
...
async fn resolved_project_layer_loads_skill_when_cwd_is_file() {
    ...
    let dot_atlas_engine = repository.join(".atlas-agent");
    let skill_root = dot_atlas_engine.join("skills");
...
async fn resolved_config_and_repo_roots_preserve_order_and_dedupe_paths_not_names() {
    let temp_dir = TempDir::new().expect("temp dir");
    let home_folder = absolute(temp_dir.path().join("home"));
    let atlas_agent_home = home_folder.join("atlas-agent");
    let system_folder = absolute(temp_dir.path().join("etc/atlas-agent"));
    let repository = absolute(temp_dir.path().join("repo"));
    ...
    let project_dot_atlas_engine = repository.join(".atlas-agent");
    let nested_project_dot_atlas_engine = repository.join("nested/.atlas-agent");
    let user_skills = atlas_agent_home.join("skills");
    let root_project_skill = write_skill(
        &project_dot_atlas_engine.join("skills"),
        "root-duplicate",
        "duplicate-skill",
    );
    let nested_project_skill = write_skill(
        &nested_project_dot_atlas_engine.join("skills"),
        "nested-duplicate",
        "duplicate-skill",
    );
    ...
    let config_stack = stack(vec![
        ...
        user_layer(&atlas_agent_home),
        project_layer(&project_dot_atlas_engine),
        project_layer(&nested_project_dot_atlas_engine),
    ]);
