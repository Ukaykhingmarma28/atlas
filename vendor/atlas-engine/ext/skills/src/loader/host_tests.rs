...
use atlas_engine_skills::SkillInterface;
use atlas_engine_skills::SkillMetadata;
use atlas_engine_skills::SkillModel;
...
policy:
  ...
  products: [atlas_engine, CHATGPT, atlas]
"##,
    );
    ...
    assert_eq!(
        ...
        vec![SkillMetadata {
            ...
            policy: Some(SkillPolicy {
                ...
                products: vec![Product::AtlasEngine, Product::Chatgpt, Product::Atlas],
            }),
...
async fn discovers_nested_plugin_namespace_without_plugin_identity() {
    let root = TempDir::new().expect("temp dir");
    let plugin_manifest = root.path().join("nested/.atlas-agent-plugin/plugin.json");
