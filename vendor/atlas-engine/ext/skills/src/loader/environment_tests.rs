...
use atlas_engine_exec_server::CapabilityRootDiscoverRequest;
use atlas_engine_exec_server::CapabilityRootsDiscoverParams;
use atlas_engine_exec_server::LOCAL_FS;
use atlas_engine_exec_server::discover_capability_roots;
use atlas_engine_protocol::protocol::Product;
use atlas_engine_skills::EnvironmentSkillMetadata;
use atlas_engine_skills::SkillDependencies;
use atlas_engine_skills::SkillPolicy;
...
async fn direct_environment_loader_preserves_plugin_dependencies_and_product_policy() {
    ...
    fs::create_dir_all(root.path().join(".atlas-agent-plugin")).expect("manifest dir");
    ...
    assert_eq!(
        ...
        vec![EnvironmentSkillMetadata {
            ...
            policy: Some(SkillPolicy {
                ...
                products: vec![Product::AtlasEngine, Product::Atlas],
            }),
        }]
    );
    let atlas =
...
async fn executor_bundle_parser_matches_direct_environment_loader() {
    let root = tempdir().expect("tempdir");
    let plugin_manifest = root.path().join(".atlas-agent-plugin/plugin.json");
...
async fn executor_bundle_preserves_parent_namespace_and_manifest_precedence() {
    ...
    for (relative_path, name) in [
        (".atlas-agent-plugin/plugin.json", "atlas-engine-name"),
    ...
    assert!(
        discovery.namespace_manifests[0]
            .path
            .to_string()
            .ends_with("/.atlas-agent-plugin/plugin.json")
    );
    assert_eq!(bundled.warnings, existing.warnings);
    assert_eq!(
        bundled
    ...
    assert_eq!(bundled.skills[0].metadata.name, "atlas-engine-name:search");
}
