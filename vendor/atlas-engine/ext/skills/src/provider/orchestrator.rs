...
use atlas_engine_protocol::mcp::ResourceContent;
...
const ORCHESTRATOR_SKILL_DISCOVERY_DURATION_METRIC: &str =
    "atlas_agent.skills.orchestrator.discovery.duration_ms";
const ORCHESTRATOR_SKILL_DISCOVERY_RESOURCES_METRIC: &str =
    "atlas_agent.skills.orchestrator.discovery.resources_total";
...
impl SkillProvider for OrchestratorSkillProvider {
    fn list(&self, query: SkillListQuery) -> SkillProviderFuture<'_, SkillCatalog> {
        Box::pin(async move {
            ...
            if !client.has_server(ATLAS_APPS_MCP_SERVER_NAME).await {
                return Ok(SkillCatalog::default());
            }
            ...
            for _ in 0..MAX_RESOURCE_PAGES {
                let page = match tokio::time::timeout_at(
                    discovery_deadline,
                    client.list_resources(ATLAS_APPS_MCP_SERVER_NAME, cursor.clone()),
                )
                .await
                {
    ...
    fn read(&self, request: SkillReadRequest) -> SkillProviderFuture<'_, SkillReadResult> {
        Box::pin(async move {
            ...
            let result = tokio::time::timeout(
                ...
                client.read_resource(ATLAS_APPS_MCP_SERVER_NAME, request.resource.as_str()),
            )
            .await
            .map_err(|_| {
                SkillProviderError::new(format!(
