...
use atlas_engine_extension_api::ConversationHistory;
...
use atlas_engine_extension_api::NoopTurnItemEmitter;
use atlas_engine_extension_api::PreviousWorldStateSection;
...
use atlas_engine_protocol::protocol::SKILLS_INSTRUCTIONS_CLOSE_TAG;
use atlas_engine_protocol::protocol::SKILLS_INSTRUCTIONS_OPEN_TAG;
...
use atlas_engine_skills_extension::HostSkillsSnapshot;
use atlas_engine_skills_extension::InjectedHostSkillPrompts;
use atlas_engine_skills_extension::SkillLoadOutcome;
...
use atlas_engine_skills_extension::install_with_providers_and_metrics;
use atlas_engine_skills_extension::provider::SkillListQuery;
use atlas_engine_skills_extension::provider::SkillProvider;
...
static NEXT_ATLAS_AGENT_HOME_ID: AtomicUsize = AtomicUsize::new(0);
...
async fn skill_world_state_fragments(
    registry: &atlas_engine_extension_api::ExtensionRegistry<TestConfig>,
    ...
    let sections = registry.context_contributors()[0]
        .contribute_world_state(WorldStateContributionInput {
            thread_id: atlas_engine_protocol::ThreadId::new(),
            turn_id,
            environments: &[],
...
async fn installed_extension_uses_host_service_snapshot() -> TestResult {
    ...
    std::fs::remove_dir_all(atlas_agent_home)?;
    Ok(())
}
...
async fn host_world_state_records_catalog_metrics_on_publish_and_change() -> TestResult {
    ...
    let second_skill_path =
        AbsolutePathBuf::try_from(test_atlas_agent_home().join("skills/other/SKILL.md"))?;
    outcome.skills.push(SkillMetadata {
...
async fn shadow_selection_uses_host_catalog_when_instructions_are_disabled() -> TestResult {
    ...
    let catalog_entry_counts = snapshot
        ...
        .find(|metric| metric.name() == "atlas_agent.skills.shadow_selection.catalog_entries")
...
async fn shadow_lru_selector_recovers_a_skill_invoked_on_an_earlier_turn() -> TestResult {
    ...
    let metric = snapshot
        .scope_metrics()
        .flat_map(opentelemetry_sdk::metrics::data::ScopeMetrics::metrics)
        .find(|metric| metric.name() == "atlas_agent.skills.shadow_selection.invocation")
...
async fn skills_list_only_returns_model_visible_bounded_metadata() -> TestResult {
    ...
    let output = list_tool
        .handle(ToolCall {
            ...
            atlas_engine_turn_metadata: None,
...
async fn host_catalog_compacts_shared_paths_under_budget_pressure() -> TestResult {
    let test_root = test_atlas_agent_home();
    ...
    let atlas_agent_home = AbsolutePathBuf::try_from(test_root.clone())?;
    ...
    let snapshot = service
        .snapshot_for_config(
            &HostSkillsLoadInput::new(atlas_agent_home, Vec::new(), config_layer_stack),
...
fn test_atlas_agent_home() -> PathBuf {
    let id = NEXT_ATLAS_AGENT_HOME_ID.fetch_add(1, Ordering::Relaxed);
    std::env::temp_dir().join(format!(
        "atlas-engine-skills-extension-test-{}-{id}",
        std::process::id(),
    ))
}
