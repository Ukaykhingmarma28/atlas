...
use atlas_engine_exec_server::CapabilityTextFile;
use atlas_engine_exec_server::CopyOptions;
use atlas_engine_exec_server::CreateDirectoryOptions;
use atlas_engine_exec_server::DiscoveredSkillFiles;
...
use atlas_engine_skills_extension::ExecutorSkillProvider;
    ...
    let config_layer_stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_atlas_agent_folder: project_folder,
            },
...
async fn executor_discovery_routes_produce_equivalent_catalog_metadata() {
    ...
    let test_root = std::env::temp_dir().join(format!(
        "atlas-engine-executor-skill-parity-{}-{id}",
        std::process::id()
    ));
    let plugin_manifest = test_root.join(".atlas-agent-plugin/plugin.json");
    ...
    let comparable_entries = |catalog: &atlas_engine_skills_extension::catalog::SkillCatalog| {
        catalog
            .entries
            .iter()
            .map(|entry| {
...
async fn pre_discovered_executor_catalog_snapshot() {
    ...
    let manifest_dir = test_root.join(".atlas-agent-plugin");
    ...
    let snapshot_file = atlas_engine_utils_cargo_bin::find_resource!(
...
async fn direct_executor_discovery_preserves_hidden_nested_and_probed_metadata() {
    let id = NEXT_TEST_ROOT_ID.fetch_add(1, Ordering::Relaxed);
    let test_root = std::env::temp_dir().join(format!(
        "atlas-engine-executor-skill-discovery-{}-{id}",
        std::process::id()
    ));
    let outer_manifest = test_root.join(".atlas-agent-plugin/plugin.json");
    ...
    let inner_manifest = test_root.join("nested/.atlas-agent-plugin/plugin.json");
...
fn create_local_skill_root(label: &str) -> io::Result<std::path::PathBuf> {
    let id = NEXT_TEST_ROOT_ID.fetch_add(1, Ordering::Relaxed);
    let test_root = std::env::temp_dir().join(format!(
        "atlas-engine-executor-skill-{label}-{}-{id}",
        std::process::id()
    ));
