...
use atlas_engine_api::ResponsesWebsocketClient;
use atlas_engine_api::ResponsesWebsocketConnection;
...
pub enum LunaSamplerError {
    ...
    Provider(#[source] AtlasEngineErr),
    ...
    async fn open_connection(&self) -> Result<PooledConnection, LunaSamplerError> {
            ...
            && provider_info.aws.is_none()
        {
            ...
            if let Ok(value) = HeaderValue::from_str(&routing_hint) {
                headers.insert("x-atlas-engine-routing-hint", value);
            }
        }
