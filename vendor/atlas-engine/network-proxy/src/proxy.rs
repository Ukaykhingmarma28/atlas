...
impl NetworkProxyBuilder {
    ...
    pub fn managed_by_atlas_engine(mut self, managed_by_atlas_engine: bool) -> Self {
        self.managed_by_atlas_engine = managed_by_atlas_engine;
        self
    }
...
pub const PROXY_ACTIVE_ENV_KEY: &str = "ATLAS_AGENT_NETWORK_PROXY_ACTIVE";
pub const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "ATLAS_AGENT_NETWORK_ALLOW_LOCAL_BINDING";
...
pub fn is_managed_proxy_env_var(key: &str, value: &str) -> bool {
    ...
    #[cfg(target_os = "macos")]
    {
        key == PROXY_GIT_SSH_COMMAND_ENV_KEY
            && value.starts_with(ATLAS_AGENT_PROXY_GIT_SSH_COMMAND_MARKER)
    }
...
#[cfg(target_os = "macos")]
pub const ATLAS_AGENT_PROXY_GIT_SSH_COMMAND_MARKER: &str = "ATLAS_AGENT_PROXY_GIT_SSH_COMMAND=1 ";
#[cfg(target_os = "macos")]
const ATLAS_AGENT_PROXY_GIT_SSH_COMMAND_PREFIX: &str =
    "ATLAS_AGENT_PROXY_GIT_SSH_COMMAND=1 ssh -o ProxyCommand='nc -X 5 -x ";
#[cfg(target_os = "macos")]
const ATLAS_AGENT_PROXY_GIT_SSH_COMMAND_SUFFIX: &str = " %h %p'";
...
#[cfg(target_os = "macos")]
fn is_atlas_engine_proxy_git_ssh_command(command: &str) -> bool {
    command.starts_with(ATLAS_AGENT_PROXY_GIT_SSH_COMMAND_PREFIX)
        && command.ends_with(ATLAS_AGENT_PROXY_GIT_SSH_COMMAND_SUFFIX)
}
...
fn apply_proxy_env_overrides(
    ...
    if socks_enabled {
        ...
        // but refresh a previously injected Atlas Agent fallback so it cannot point
        // at a stale proxy port after the proxy is restarted.
        match env.get(GIT_SSH_COMMAND_ENV_KEY) {
            Some(command) if !is_atlas_engine_proxy_git_ssh_command(command) => {}
            _ => {
                env.insert(
                    GIT_SSH_COMMAND_ENV_KEY.to_string(),
                    atlas_engine_proxy_git_ssh_command(socks_addr),
                );
            }
        }
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn non_atlas_engine_managed_proxy_builder_uses_configured_ports() {
    ...
    fn apply_proxy_env_overrides_sets_common_tool_vars() {
        ...
        assert_eq!(
            ...
            Some(
                &"ATLAS_AGENT_PROXY_GIT_SSH_COMMAND=1 ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
                    .to_string()
            )
        );
        #[cfg(not(target_os = "macos"))]
    ...
    fn apply_proxy_env_overrides_sets_mitm_ca_trust_bundle_vars() {
        let mut env = HashMap::new();
        let mitm_ca_trust_bundle_path = Path::new("/tmp/atlas-engine-proxy/ca-bundle.pem");
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn apply_proxy_env_overrides_refreshes_previous_atlas_engine_proxy_git_ssh_command() {
        let mut env = HashMap::new();
        env.insert(
            GIT_SSH_COMMAND_ENV_KEY.to_string(),
        ...
        assert_eq!(
            env.get(GIT_SSH_COMMAND_ENV_KEY),
            Some(&atlas_engine_proxy_git_ssh_command(SocketAddr::new(
