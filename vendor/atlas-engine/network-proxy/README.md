...
# atlas-engine-network-proxy

`atlas-engine-network-proxy` is Atlas Agent's local network policy enforcement proxy. It runs:
...
`atlas-engine-network-proxy` reads from Atlas Agent's merged `config.toml` (via `atlas-engine-core` config loading).
...
# bundle env vars pointing at immutable public files under $ATLAS_AGENT_HOME/proxy/ so common HTTPS
...
```bash
cargo run -p atlas-engine-network-proxy --
```
...
`atlas-engine-network-proxy` can be embedded as a library with a thin API:

```rust
use atlas_engine_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `atlas-engine-network-proxy` is embedded in managed Atlas Agent runtime, policy decisions emit structured
OTEL-compatible events with `target=atlas_engine_otel.network_proxy`.

Event name:

- `atlas_engine.network_proxy.policy_decision`
...
This section documents the protections implemented by `atlas-engine-network-proxy`, and the boundaries of
