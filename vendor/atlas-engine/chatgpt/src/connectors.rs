...
use atlas_engine_connectors::ConnectorMetadataStore;
use atlas_engine_connectors::ConnectorToolSummary;
use atlas_engine_connectors::DirectoryListResponse;
use atlas_engine_connectors::merge::merge_connectors;
use atlas_engine_connectors::merge::merge_plugin_connectors;
use atlas_engine_core::config::Config;
pub use atlas_engine_core::connectors::list_accessible_connectors_from_mcp_tools;
pub use atlas_engine_core::connectors::list_accessible_connectors_from_mcp_tools_with_environment_manager;
pub use atlas_engine_core::connectors::list_accessible_connectors_from_mcp_tools_with_mcp_manager;
...
const DEFAULT_APPS_PRODUCT_SKU: &str = "atlas-agent";
...
async fn connector_auth(config: &Config) -> anyhow::Result<AtlasEngineAuth> {
    ...
    anyhow::ensure!(
        ...
        "ChatGPT connectors require Atlas Agent backend auth"
    );
    Ok(auth)
}
...
pub async fn list_cached_all_connectors(
    ...
    let connectors = atlas_engine_connectors::cached_directory_connectors(&cache_context)?;
...
pub async fn list_all_connectors_with_options(
    ...
    let cache_context = connector_directory_cache_context(config, &auth);
    let connectors = atlas_engine_connectors::list_all_connectors_with_options(
        cache_context,
...
pub async fn read_connector_metadata(
    ...
    anyhow::ensure!(
        auth.uses_atlas_engine_backend(),
        "ChatGPT backend requests require Atlas Agent backend auth"
    );
    anyhow::ensure!(
        auth.get_account_id().is_some(),
        "ChatGPT account ID not available, please re-run atlas-agent login"
    );
