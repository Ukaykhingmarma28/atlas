...
//! Registry of model providers supported by Atlas Agent.
//!
//! Providers can be defined in two places:
//!   1. Built-in defaults compiled into the binary so Atlas Agent works out-of-the-box.
//!   2. User-defined entries inside `~/.atlas_engine/config.toml` under the `model_providers`
//!      key. These override or extend the defaults at runtime.

use atlas_engine_api::Provider as ApiProvider;
use atlas_engine_api::RetryConfig as ApiRetryConfig;
...
use atlas_engine_protocol::error::EnvVarError;
...
pub const CHATGPT_ATLAS_AGENT_BASE_URL: &str = "https://chatgpt.com/backend-api/codex";
...
const AMAZON_BEDROCK_MANTLE_CLIENT_AGENT_VALUE: &str = "atlas-agent";
...
pub enum WireApi {
    ...
    #[default]
    Responses,
    /// The OpenAI Chat Completions API, as the Atlas gateway serves it (D3).
    ///
    /// **Reinstated by Atlas, not inherited.** Upstream removed this variant
    /// and made the string `"chat"` a deliberate deserialization error. Atlas
    /// needs it back because its gateway speaks exactly this dialect — and the
    /// dialect is authored fresh (`atlas_engine_api::atlas_chat`) rather than
    /// recovered, since upstream deleted the code along with the variant.
    ///
    /// Reusing the same spelling is intentional: a provider written for any
    /// OpenAI-compatible endpoint says `wire_api = "chat"`, and inventing a
    /// second word for it would only make Atlas's config incompatible with
    /// every example of one.
    Chat,
}
...
impl ModelProviderInfo {
    ...
    fn build_header_map(&self) -> AtlasEngineResult<HeaderMap> {
    ...
    pub fn to_api_provider(&self, auth_mode: Option<AuthMode>) -> AtlasEngineResult<ApiProvider> {
    ...
    pub fn api_key(&self) -> AtlasEngineResult<Option<String>> {
        match &self.env_key {
            Some(env_key) => {
                let api_key = std::env::var(env_key)
                    ...
                    .ok_or_else(|| {
                        AtlasEngineErr::EnvVar(EnvVarError {
                            var: env_key.clone(),
...
pub fn built_in_model_providers(
    ...
    // providers are bundled with Atlas Agent CLI, so we only include the OpenAI and
...
pub fn create_oss_provider(default_provider_port: u16, wire_api: WireApi) -> ModelProviderInfo {
    // These ATLAS_AGENT_OSS_ environment variables are experimental: we may
    // switch to reading values from config.toml instead.
    let default_atlas_engine_oss_base_url = format!(
        "http://localhost:{atlas_engine_oss_port}/v1",
        atlas_engine_oss_port = std::env::var("ATLAS_AGENT_OSS_PORT")
            .ok()
    ...
    let atlas_engine_oss_base_url = std::env::var("ATLAS_AGENT_OSS_BASE_URL")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .unwrap_or(default_atlas_engine_oss_base_url);
    create_oss_provider_with_base_url(&atlas_engine_oss_base_url, wire_api)
}
