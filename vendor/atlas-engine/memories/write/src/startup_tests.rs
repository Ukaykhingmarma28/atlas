...
use atlas_engine_state::Phase2JobClaimOutcome;
...
async fn memories_startup_phase2_tracks_workspace_diff_across_runs() -> anyhow::Result<()> {
    ...
    let test = build_test_atlas_engine(&server, home.clone()).await?;
    trigger_memories_startup(&test).await;
...
async fn phase2_retries_when_clean_workspace_is_missing_artifacts() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        "# Memory Workspace Diff\n\nGenerated by Atlas Agent before Phase 2 memory consolidation. Read this file first and do not edit it.\n\n## Status\n- none\n"
    );
...
-> anyhow::Result<()> {
    ...
    context
        .stream_stage_one_prompt(
            &test.config,
            &atlas_engine_core::Prompt::default(),
            &request_context,
        )
        .await?;
...
async fn run_memory_phase_two_model_request_test(
    ...
    let test = build_test_atlas_engine_with_memories_config(server, home.clone(), memories).await?;
    let provider = Arc::new(MockMemoryModelProvider::new(
        test.config.model_provider.clone(),
        Some(test.thread_manager.auth_manager()),
    ));
    ...
    let (context, config) = memory_startup_context_with_provider(&test, provider).await;
    let root = memory_root(&config.atlas_agent_home);
...
async fn build_test_atlas_engine(
    server: &wiremock::MockServer,
    home: Arc<TempDir>,
) -> anyhow::Result<TestAtlasEngine> {
    build_test_atlas_engine_with_memories_config(server, home, startup_test_memories_config()).await
}

async fn build_test_atlas_engine_with_memories_config(
    server: &wiremock::MockServer,
    home: Arc<TempDir>,
    memories: MemoriesConfig,
) -> anyhow::Result<TestAtlasEngine> {
...
async fn trigger_memories_startup(test: &TestAtlasEngine) {
...
impl ModelProvider for MockMemoryModelProvider {
    ...
    ) -> atlas_engine_models_manager::manager::SharedModelsManager {
        self.delegate
            .models_manager(atlas_agent_home, config_model_catalog)
    }
}

async fn seed_stage1_output(
    db: &atlas_engine_state::StateRuntime,
    ...
    let mut metadata_builder = atlas_engine_state::ThreadMetadataBuilder::new(
        thread_id,
        atlas_agent_home.join(format!("rollout-{thread_id}.jsonl")),
        updated_at,
        SessionSource::Cli,
    );
    metadata_builder.cwd = atlas_agent_home.join(format!("workspace-{rollout_slug}"));
...
async fn seed_stage1_candidate(
    ...
    let thread_id = ThreadId::new();
    let rollout_path = atlas_agent_home.join(format!("rollout-{thread_id}.jsonl"));
...
) -> anyhow::Result<atlas_engine_core::ThreadConfigSnapshot> {
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        let config_snapshot = test.atlas_engine.config_snapshot().await;
...
async fn seed_stage1_output_for_existing_thread(
    ...
    let ownership_token = match claim {
        atlas_engine_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
