...
use atlas_engine_state::Stage1Output;
...
mod job {
    ...
    pub(super) async fn claim(
        ...
        let (token, watermark) = match claim {
            atlas_engine_state::Phase2JobClaimOutcome::Claimed {
                ownership_token,
                input_watermark,
            } => {
                context.counter(
            ...
            atlas_engine_state::Phase2JobClaimOutcome::SkippedRetryUnavailable => {
    ...
    pub(super) async fn succeed(
        ...
        selected_outputs: &[atlas_engine_state::Stage1Output],
        reason: &'static str,
    ) -> bool {
        context.counter(MEMORY_PHASE_TWO_JOBS, /*inc*/ 1, &[("status", reason)]);
        db.memories()
...
mod agent {
    ...
    pub(super) fn get_config(
        ...
        // Preserve the parent's explicit choice to skip Atlas Agent-managed sandboxing.
    ...
    pub(super) fn handle(
        ...
        claim: Claim,
        new_watermark: i64,
        selected_outputs: Vec<atlas_engine_state::Stage1Output>,
        memory_root: atlas_engine_utils_absolute_path::AbsolutePathBuf,
        agent: SpawnedConsolidationAgent,
        phase_two_e2e_timer: Option<atlas_engine_otel::Timer>,
    ) {
        let Some(db) = context.state_db() else {
            return;
        };
    ...
    async fn loop_agent(
        ...
        token: String,
        thread_id: ThreadId,
        thread: &atlas_engine_core::AtlasEngineThread,
    ) -> AgentStatus {
...
pub(super) fn get_watermark(
    ...
    latest_memories: &[atlas_engine_state::Stage1Output],
) -> i64 {
    latest_memories
        .iter()
