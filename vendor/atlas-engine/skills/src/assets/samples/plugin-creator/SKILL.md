...
---
name: plugin-creator
description: Create and scaffold plugin directories for Atlas Agent with a required `.atlas_engine-plugin/plugin.json`, optional plugin folders/files, valid manifest defaults, and personal-marketplace entries by default. Use when Atlas Agent needs to create a new personal plugin, add optional plugin structure, generate or update marketplace entries for plugin ordering and availability metadata, or update an existing local plugin during development with the CLI-driven cachebuster and reinstall flow.
---
...
2. Edit `<plugin-path>/.atlas_engine-plugin/plugin.json` when the request gives specific metadata.
   The scaffold starts with valid defaults and must not contain `[TODO: ...]` placeholders.

3. Generate or update the personal marketplace entry when the plugin should appear in Atlas Agent UI ordering:
...
- Always creates `/<parent-plugin-directory>/<plugin-name>/.atlas_engine-plugin/plugin.json`.
...
- If the user specifies a different marketplace path, treat that marketplace as needing explicit installation via `atlas_engine plugin marketplace add`.
...
- Treat plugin order in `plugins[]` as render order in Atlas Agent. Append new entries unless a user explicitly asks to reorder the list.
...
- Do not remove required structure; keep `.atlas_engine-plugin/plugin.json` present.
...
- If Atlas Agent would need approval to write the marketplace file, ask for that approval before
...
- Do not tell the user to run `atlas_engine plugin marketplace add` for the default personal-marketplace
...
- If the user provided a non-default `--marketplace-path`, make sure that marketplace is installed
  before giving reinstall instructions. Use `atlas_engine plugin marketplace add <path-to-marketplace-root>`
...
- When the workflow created or updated a marketplace-backed plugin, end the final user-facing
  response with a short Atlas Agent app handoff. Say `To view this in the AtlasEngine app:` and write
  `View <normalized plugin name>` and `Share <normalized plugin name>` as Markdown links, not raw
  URLs or code spans.
- The View deeplink uses `atlas_engine://plugins/<normalized plugin name>?marketplacePath=<absolute marketplace.json path>`.
...
- Do not add `pluginName` or `hostId` query parameters to these deeplinks. Atlas Agent derives both after
