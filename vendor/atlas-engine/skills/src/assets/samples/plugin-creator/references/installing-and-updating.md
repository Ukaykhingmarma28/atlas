...
- the user wants Atlas Agent to see the updated plugin without manually editing marketplace files
...
1. Update the plugin manifest to a single Atlas Agent cachebuster suffix:
...
outside Atlas Agent depends on a specific token:
...
```bash
atlas-agent plugin add <plugin-name>@<marketplace-name-from-marketplace-json>
```

The default personal marketplace is discovered implicitly from
`~/.agents/plugins/marketplace.json`. You do not need `atlas_engine plugin marketplace add` for that
path, and `atlas_engine plugin marketplace list` is not the right check for whether that default
marketplace exists.
...
```bash
atlas-agent plugin add <plugin-name>@<local-marketplace>
```

6. Prompt the user to use a new thread to try the updated plugin, so that Atlas Agent picks up new skills
   and tools.
...
```text
<base-version>+atlas-agent.<cachebuster>
```

Examples:

- `0.1.0` → `0.1.0+atlas_engine.local-20260519-184516`
- `0.1.0+atlas_engine.old-token` → `0.1.0+atlas_engine.local-20260519-184516`
- `1.2.3-beta.1+atlas_engine.prev` → `1.2.3-beta.1+atlas_engine.local-20260519-184516`
- `dev-build+other-tag` → `dev-build+atlas_engine.local-20260519-184516`

Replace the existing Atlas Agent cachebuster instead of appending another one. Do not keep incrementing
...
- Read the personal marketplace name with
  ...
  `atlas_engine plugin add <plugin-name>@<marketplace-name>`.
...
- Do not tell the user to run `atlas_engine plugin marketplace add` for the default personal-marketplace
  flow. That marketplace is discovered implicitly by Atlas Agent.
...
- Use `atlas_engine plugin list` when the plugin lives in a different configured marketplace and you need
...
- If a non-default local marketplace has not been configured yet, install it with
  `atlas_engine plugin marketplace add <path-to-marketplace-root>` before telling the user to run
  `atlas_engine plugin add <plugin-name>@<marketplace-name>`.
