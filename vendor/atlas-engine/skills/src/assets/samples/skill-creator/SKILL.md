...
Create skills that give Atlas Agent useful, non-obvious guidance without constraining unrelated work.

## Core Principles

**Assume Atlas Agent is already capable.** Include only information that changes its decisions or improves its work. Remove generic advice, repeated instructions, speculative edge cases, and examples that do not materially clarify the task.
...
Ask clarifying questions only when the missing information matters and cannot be reasonably inferred. Respect a user-specified location; otherwise create discoverable skills in `$ATLAS_AGENT_HOME/skills`, or `~/.atlas_engine/skills` when `ATLAS_AGENT_HOME` is unset.
...
```bash
scripts/init_skill.py my-skill --path "${ATLAS_AGENT_HOME:-$HOME/.atlas-agent}/skills"
scripts/init_skill.py my-skill --path "${ATLAS_AGENT_HOME:-$HOME/.atlas-agent}/skills" --resources references
```
