...
use atlas_engine_protocol::openai_models::TruncationMode;
...
pub fn with_config_overrides(mut model: ModelInfo, config: &ModelsManagerConfig) -> ModelInfo {
    ...
    if let Some(base_instructions) = &config.base_instructions {
        ...
        let uses_local_personality_template = model.used_fallback_model_metadata
            && matches!(
                model.slug.as_str(),
                "gpt-5.2-codex" | "exp-atlas-engine-personality"
            );
        if !config.personality_enabled
...
fn local_model_messages_for_slug(slug: &str) -> ModelMessages {
    match slug {
        "gpt-5.2-codex" | "exp-atlas-engine-personality" => ModelMessages {
