...
struct TestModelsEndpoint {
    ...
    uses_atlas_engine_backend: bool,
...
impl TestModelsEndpoint {
    fn new(responses: Vec<Vec<ModelInfo>>) -> Arc<Self> {
        Arc::new(Self {
            ...
            uses_atlas_engine_backend: true,
    ...
    fn without_refresh(responses: Vec<Vec<ModelInfo>>) -> Arc<Self> {
        Arc::new(Self {
            has_command_auth: false,
            uses_atlas_engine_backend: false,
            responses: Mutex::new(responses.into()),
            fetch_count: AtomicUsize::new(0),
            observed_proxy_policy: Mutex::new(None),
        })
...
impl ExternalAuth for TestExternalApiKeyAuth {
    fn resolve(&self) -> atlas_engine_login::ExternalAuthFuture<'_, AtlasEngineAuth> {
        Box::pin(async { Ok(AtlasEngineAuth::from_api_key("test-external-api-key")) })
    }

    fn refresh(
        &self,
        _context: ExternalAuthRefreshContext,
    ) -> atlas_engine_login::ExternalAuthFuture<'_, AtlasEngineAuth> {
        Box::pin(async { Ok(AtlasEngineAuth::from_api_key("test-external-api-key")) })
    }
}
...
impl ModelsEndpointClient for TestModelsEndpoint {
    ...
    fn uses_atlas_engine_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(async { self.uses_atlas_engine_backend })
    }
...
fn openai_manager_for_tests(
    atlas_agent_home: std::path::PathBuf,
...
fn openai_manager_for_tests_with_auth(
    ...
    OpenAiModelsManager::new(atlas_agent_home, endpoint_client, auth_manager)
}

async fn mutate_file_cache_for_test<F>(atlas_agent_home: &Path, f: F)
where
...
async fn file_cache_implements_models_cache_contract() {
    ...
    let cache = FileModelsCache::new(
        atlas_agent_home.path().join(MODEL_CACHE_FILE),
...
async fn chatgpt_auth_tokens_for_tests(atlas_agent_home: &Path) -> AtlasEngineAuth {
    let auth_dot_json = atlas_engine_login::AuthDotJson {
        auth_mode: Some(AuthMode::ChatgptAuthTokens),
        openai_api_key: None,
        tokens: Some(TokenData {
            id_token: atlas_engine_login::token_data::parse_chatgpt_jwt_claims(
...
c2ln",
    ...
    std::fs::create_dir_all(atlas_agent_home).expect("atlas-agent home should be created");
    std::fs::write(
    ...
    AtlasEngineAuth::from_auth_storage(
        atlas_agent_home,
...
async fn dynamic_manager_preserves_requested_model_when_fallback_is_allowed() {
    ...
    let manager = openai_manager_for_tests(atlas_agent_home.path().to_path_buf(), endpoint.clone());
    let requested_model = Some("unsupported".to_string());
...
async fn get_model_info_matches_hyphenated_provider_namespace_suffix() {
    ...
    let namespaced_model = "openai-atlas-agent/gpt-image".to_string();
...
async fn get_model_info_uses_fallback_for_bundled_models_when_chatgpt_remote_is_authoritative() {
    ...
    let manager = openai_manager_for_tests(atlas_agent_home.path().to_path_buf(), endpoint);
...
async fn refresh_available_models_refetches_when_cache_stale() {
    ...
    mutate_file_cache_for_test(atlas_agent_home.path(), |cache| {
...
impl TestAuthAwareModelsEndpoint {
    ...
    async fn uses_atlas_engine_backend(&self) -> bool {
...
impl ModelsEndpointClient for TestAuthAwareModelsEndpoint {
    ...
    fn uses_atlas_engine_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(TestAuthAwareModelsEndpoint::uses_atlas_engine_backend(self))
    }
...
async fn refresh_available_models_fetches_with_chatgpt_auth_tokens() {
    ...
    let auth = chatgpt_auth_tokens_for_tests(atlas_agent_home.path()).await;
    let manager = openai_manager_for_tests_with_auth(
        atlas_agent_home.path().to_path_buf(),
        endpoint.clone(),
