...
use atlas_engine_login::LoginCallbackResult;
...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = atlas_agent_home.clone();

    let opts = ServerOptions {
        atlas_agent_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: atlas_engine_login::test_support::transport_default_auth_route_config(),
        client_id: atlas_engine_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = atlas_agent_home.join("auth.json");
...
#[tokio::test]
async fn creates_missing_atlas_agent_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let atlas_agent_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn oauth_access_denied_missing_entitlement_blocks_login_with_clear_error() -> Result<()> {
    ...
    let body = resp.text().await?;
    assert!(
        body.contains("You do not have access to Atlas Agent"),
        "error body should clearly explain the Atlas Agent access denial"
    );
    assert!(
    ...
    assert!(
        !body.contains("missing_atlas_engine_entitlement"),
...
async fn oauth_access_denied_unknown_reason_uses_generic_error_page() -> Result<()> {
    ...
    assert!(
        body.contains("Return to Atlas Agent to retry"),
    ...
    assert!(
        !body.contains("get access to Atlas Agent"),
...
async fn falls_back_to_registered_fallback_port_when_default_port_is_in_use() -> Result<()> {
    ...
    let default_port_server_handle = {
        ...
        thread::spawn(move || {
            while let Ok(req) = server.recv() {
                let _ = req.respond(tiny_http::Response::from_string("not atlas-agent"));
            }
        })
    };
    ...
    let mut opts = ServerOptions::new(
        ...
        atlas_engine_login::CLIENT_ID.to_string(),
        /*forced_chatgpt_workspace_id*/ None,
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_atlas_agent_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        atlas_agent_home: first_atlas_agent_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: atlas_engine_login::test_support::transport_default_auth_route_config(),
        client_id: atlas_engine_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
    ...
    let second_atlas_agent_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        atlas_agent_home: second_atlas_agent_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: atlas_engine_login::test_support::transport_default_auth_route_config(),
        client_id: atlas_engine_login::CLIENT_ID.to_string(),
        issuer,
