...
// Keep in sync with the Atlas Agent CLI Hydra redirect URI allow-list.
...
pub struct ServerOptions {
    ...
    pub atlas_engine_streamlined_login: bool,
    ...
    match path.as_str() {
            ...
            .await
            {
                Ok(tokens) => {
                    ...
                    let redirect = compose_success_url(
                        ...
                        &tokens.id_token,
                        &tokens.access_token,
                        opts.atlas_engine_streamlined_login,
                    ...
                    match tiny_http::Header::from_bytes(&b"Location"[..], url.as_bytes()) {
                        ...
                        Err(_) => login_error_response(
                            "Sign-in completed but redirecting back to Atlas Agent failed.",
                            io::ErrorKind::Other,
        ...
        "/success" => {
            let use_streamlined_success = parsed_url
                .query_pairs()
                .any(|(key, value)| key == "atlas_engine_streamlined_login" && value == "true");
...
fn redact_sensitive_error_url(
    mut err: atlas_engine_http_client::HttpError,
) -> atlas_engine_http_client::HttpError {
...
pub(crate) async fn persist_tokens_async(
    ...
    let atlas_agent_home = atlas_agent_home.to_path_buf();
...
fn is_missing_atlas_engine_entitlement_error(error_code: &str, error_description: Option<&str>) -> bool {
    error_code == "access_denied"
        && error_description.is_some_and(|description| {
            description
                .to_ascii_lowercase()
                .contains("missing_atlas_engine_entitlement")
        })
}
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_atlas_engine_entitlement_error(error_code, error_description) {
        return "Atlas Agent is not enabled for your workspace. Contact your workspace administrator to request access to Atlas Agent.".to_string();
    }
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_atlas_engine_entitlement_error(code, error_description) {
            (
                "You do not have access to Atlas Agent".to_string(),
                "This account is not currently authorized to use Atlas Agent in this workspace."
                    .to_string(),
                "Contact your workspace administrator to request access to Atlas Agent.".to_string(),
                "Contact your workspace administrator to get access to Atlas Agent, then return to Atlas Agent and try again."
                    .to_string(),
            )
        } else {
            (
                ...
                "Return to Atlas Agent to retry, switch accounts, or contact your workspace admin if access is restricted."
                    .to_string(),
            )
        };
...
mod tests {
    ...
    use super::is_missing_atlas_engine_entitlement_error;
    ...
    fn render_login_error_page_uses_entitlement_copy() {
        let error_description = Some("missing_atlas_engine_entitlement");
        assert!(is_missing_atlas_engine_entitlement_error(
            "access_denied",
            error_description
        ));
        ...
        assert!(body.contains("You do not have access to Atlas Agent"));
        assert!(body.contains("Contact your workspace administrator"));
        assert!(!body.contains("missing_atlas_engine_entitlement"));
    }
}
