...
fn compose_success_url_uses_local_page_by_default() {
    ...
    assert_eq!(
        url.query_pairs()
            .find(|(key, _)| key == "atlas_engine_streamlined_login"),
        None
    );
}
    ...
    assert_eq!(
        compose_success_url(
            ...
            &LoginSuccessPage::Hosted {
                url: Url::parse(ATLAS_AGENT_OPEN_APP_URL).expect("open app URL should parse"),
...
fn compose_success_url_keeps_setup_on_local_page() {
    ...
    let LoginSuccessRedirect::Local(url) = compose_success_url(
        ...
        &LoginSuccessPage::Hosted {
            url: Url::parse(ATLAS_AGENT_OPEN_APP_URL).expect("open app URL should parse"),
            app_brand: LoginSuccessPageBrand::AtlasEngine,
        },
    ) else {
        panic!("expected local success redirect");
    };
