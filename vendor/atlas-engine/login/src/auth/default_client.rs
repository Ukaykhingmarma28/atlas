...
//! Default Atlas Agent HTTP client: shared `User-Agent`, `originator`, optional residency header, and
//! `HttpClient` construction.
//!
//! Use [`crate::default_client`] or [`atlas_engine_login::default_client`] from other crates in this
//! workspace.

use atlas_engine_http_client::BuildRouteAwareHttpClientError;
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(ATLAS_AGENT_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(ATLAS_AGENT_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    // Atlas Agent is the only first-party client of this engine; the
    // upstream's list of its own clients went with the rename (ADR-0011).
...
pub fn get_atlas_engine_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized Atlas Agent user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base Atlas Agent user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default Atlas Agent originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
/// Atlas Agent's route-aware system/PAC resolution.
...
/// Builds the default Atlas Agent HTTP client for a concrete outbound route.
///
/// When route-aware proxy handling is disabled, or the client is running inside the Atlas Agent
...
/// Builds the default Atlas Agent HTTP client for a concrete outbound route without blocking the
...
/// Builds an HTTP client for an auth endpoint without Atlas Agent default headers.
...
/// Builds the default Atlas Agent HTTP client wrapper for an auth endpoint.
...
pub fn default_headers() -> HeaderMap {
    ...
    if let Ok(user_agent) = HeaderValue::from_str(&get_atlas_engine_user_agent()) {
...
fn is_sandboxed() -> bool {
    std::env::var("ATLAS_AGENT_SANDBOX").as_deref() == Ok("seatbelt")
}
