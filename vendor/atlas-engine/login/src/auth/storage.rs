...
use atlas_engine_agent_identity::AgentIdentityJwtClaims;
use atlas_engine_agent_identity::decode_agent_identity_jwt;
use atlas_engine_config::types::AuthCredentialsStoreMode;
pub use atlas_engine_config::types::AuthKeyringBackendKind;
use atlas_engine_keyring_store::DefaultKeyringStore;
...
use atlas_engine_secrets::SecretName;
...
/// Expected structure for $ATLAS_AGENT_HOME/auth.json.
...
pub(super) fn get_auth_file(atlas_agent_home: &Path) -> PathBuf {
    atlas_agent_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(atlas_agent_home: &Path) -> std::io::Result<bool> {
...
impl FileAuthStorage {
    pub(super) fn new(atlas_agent_home: PathBuf) -> Self {
        Self { atlas_agent_home }
    }

    /// Attempt to read and parse the `auth.json` file in the given `ATLAS_AGENT_HOME` directory.
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.atlas_agent_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.atlas_agent_home)
    }
}

static ATLAS_AGENT_AUTH_SECRET_NAME: Lazy<SecretName> =
    Lazy::new(|| match SecretName::new("ATLAS_AGENT_AUTH") {
        Ok(name) => name,
        Err(err) => unreachable!("ATLAS_AGENT_AUTH should be a valid secret name: {err}"),
    });
const KEYRING_SERVICE: &str = "Atlas Agent Auth";

// turns atlas_agent_home path into a stable, short key string
fn compute_store_key(atlas_agent_home: &Path) -> std::io::Result<String> {
    let canonical = atlas_agent_home
        .canonicalize()
        .unwrap_or_else(|_| atlas_agent_home.to_path_buf());
...
impl DirectKeyringAuthStorage {
    fn new(atlas_agent_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            atlas_agent_home,
            keyring_store,
        }
...
impl AuthStorageBackend for DirectKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.atlas_agent_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.atlas_agent_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.atlas_agent_home)?;
...
impl SecretsKeyringAuthStorage {
    fn new(atlas_agent_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        let direct_storage =
            DirectKeyringAuthStorage::new(atlas_agent_home.clone(), Arc::clone(&keyring_store));
        let secrets_manager = SecretsManager::new_with_keyring_store_and_namespace(
            atlas_agent_home.clone(),
            SecretsBackendKind::Local,
            keyring_store,
            LocalSecretsNamespace::AtlasEngineAuth,
        );
        Self {
            atlas_agent_home,
            direct_storage,
...
impl AuthStorageBackend for SecretsKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        match self
            .secrets_manager
            .get(&SecretScope::Global, &ATLAS_AGENT_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
    ...
    fn delete(&self) -> std::io::Result<bool> {
        let keyring_removed = self
            .secrets_manager
            .delete(&SecretScope::Global, &ATLAS_AGENT_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
...
impl AutoAuthStorage {
    fn new(
        ...
        Self {
            ...
            file_storage: Arc::new(FileAuthStorage::new(atlas_agent_home)),
        }
    }
}
...
// A global in-memory store for mapping atlas_agent_home -> AuthDotJson.
...
impl EphemeralAuthStorage {
    fn new(atlas_agent_home: PathBuf) -> Self {
        Self { atlas_agent_home }
    }
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_store(atlas_agent_home, mode, keyring_store, keyring_backend_kind)
}
...
fn create_auth_storage_with_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(atlas_agent_home)),
        AuthCredentialsStoreMode::Keyring => {
            create_keyring_auth_storage(atlas_agent_home, keyring_store, keyring_backend_kind)
        }
