...
#[test]
fn test_get_atlas_engine_user_agent() {
    let user_agent = get_atlas_engine_user_agent();
...
fn is_first_party_originator_matches_known_values() {
    ...
    assert_eq!(is_first_party_originator("atlas_engine_cli"), false);
    assert_eq!(is_first_party_originator("Other"), false);
}

#[test]
fn is_first_party_chat_originator_matches_known_values() {
    assert_eq!(is_first_party_chat_originator(DEFAULT_ORIGINATOR), true);
    assert_eq!(is_first_party_chat_originator("atlas_engine_vscode"), false);
}
...
async fn test_create_client_sets_default_headers() {
    ...
    // User-Agent matches the computed Atlas Agent UA for that originator
    let expected_ua = get_atlas_engine_user_agent();
...
fn test_invalid_suffix_is_sanitized() {
    let prefix = "atlas_agent/0.0.0";
    let suffix = "bad\rsuffix";

    assert_eq!(
        sanitize_user_agent(format!("{prefix} ({suffix})"), prefix),
        "atlas_agent/0.0.0 (bad_suffix)"
    );
}
