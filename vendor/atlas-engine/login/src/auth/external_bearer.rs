...
use super::manager::AtlasEngineAuth;
...
impl BearerTokenRefresher {
    ...
    async fn resolve(&self) -> io::Result<AtlasEngineAuth> {
        let access_token = {
        ...
        Ok(AtlasEngineAuth::from_api_key(access_token.as_str()))
    }

    async fn refresh(&self, _context: ExternalAuthRefreshContext) -> io::Result<AtlasEngineAuth> {
        let access_token = run_provider_auth_command(&self.state.config).await?;
        let mut cached = self.state.cached_token.lock().await;
        *cached = Some(CachedExternalBearerToken {
            access_token: access_token.clone(),
