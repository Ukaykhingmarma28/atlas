...
use atlas_engine_protocol::auth::KnownPlan as InternalKnownPlan;
...
async fn refresh_without_id_token() {
    let atlas_agent_home = tempdir().unwrap();
...
async fn agent_identity_jwt_uses_explicit_staging_endpoint_overrides() -> anyhow::Result<()> {
    ...
    Mock::given(method("GET"))
        .and(path("/api/atlas-agent/agent-identities/jwks"))
        .respond_with(ResponseTemplate::new(200).set_body_json(test_jwks_body()))
        .expect(1)
        .mount(&jwks_server)
        .await;
    ...
    let auth = AtlasEngineAuth::from_agent_identity_jwt(
        &jwt,
    ...
    let AtlasEngineAuth::AgentIdentity(agent_identity) = auth else {
...
fn agent_identity_custom_jwks_base_requires_explicit_authapi_override() {
    let _authapi_guard = EnvVarGuard::remove("ATLAS_AGENT_AGENT_IDENTITY_AUTHAPI_BASE_URL");
...
async fn stored_agent_identity_jwt_keeps_auth_json_unchanged() -> anyhow::Result<()> {
    ...
    let AtlasEngineAuth::AgentIdentity(agent_identity_auth) = auth else {
        panic!("stored JWT should load as agent identity auth");
    };
    assert_eq!(agent_identity_auth.run_task_id(), "task-id");
    let storage = FileAuthStorage::new(atlas_agent_home.path().to_path_buf());
    let auth = storage
        .try_read_auth_json(&get_auth_file(atlas_agent_home.path()))
        .expect("auth.json should parse");
    assert_eq!(
        auth.agent_identity,
        Some(AgentIdentityStorage::Jwt(agent_identity))
...
async fn chatgpt_auth_registers_agent_identity_when_enabled() -> anyhow::Result<()> {
    ...
    Mock::given(method("POST"))
        ...
        .and(body_partial_json(json!({
            "abom": {
                "agent_harness_id": "atlas-engine-cli",
            },
...
async fn chatgpt_auth_task_registration_retry_exhaustion_is_fallback_eligible() -> anyhow::Result<()>
{
    ...
    let storage = FileAuthStorage::new(atlas_agent_home.path().to_path_buf());
    let auth_path = get_auth_file(atlas_agent_home.path());
...
async fn agent_identity_jwt_task_registration_retry_exhaustion_is_strict() -> anyhow::Result<()> {
    ...
    let err = AtlasEngineAuth::from_agent_identity_jwt_with_authapi_base_url(
        &agent_identity,
        Some(&chatgpt_base_url),
        &authapi_base_url,
        &crate::test_support::transport_default_auth_route_config(),
...
async fn refresh_failure_is_scoped_to_the_matching_auth_snapshot() {
    ...
    let updated_auth = AtlasEngineAuth::from_auth_dot_json(
        atlas_agent_home.path(),
...
async fn external_auth_keeps_cached_credentials_after_permanent_reload_failure() {
    let manager = AuthManager::from_auth_for_testing(AtlasEngineAuth::from_api_key("seed"));
    let auth = AtlasEngineAuth::from_api_key("configured-token");
...
async fn replacing_external_auth_clears_permanent_failure() {
    let manager = AuthManager::from_auth_for_testing(AtlasEngineAuth::from_api_key("seed"));
    let auth = AtlasEngineAuth::from_api_key("external-token");
    manager
...
async fn runtime_external_auth_uses_provider_error_classification() {
    ...
    manager
        .set_external_auth(Arc::new(FailingExternalAuth {
            auth: AtlasEngineAuth::from_api_key("runtime-token"),
            resolve_count: AtomicUsize::new(0),
        }))
        .await
...
async fn external_auth_provider_can_install_headers() {
    ...
    let auth = AtlasEngineAuth::Headers(AuthHeaders::new(headers));
    ...
    assert!(
        manager
            .auth_cached()
            .is_some_and(|auth| auth.uses_atlas_engine_backend())
    );
    assert!(
        !manager
            .auth_cached()
...
async fn workload_identity_auth_is_immutable_and_process_local() {
    ...
    let mut manager = AuthManager::from_auth_for_testing_with_home(
        AtlasEngineAuth::from_api_key("seed"),
        atlas_agent_home.path().to_path_buf(),
    );
    ...
    assert!(!get_auth_file(atlas_agent_home.path()).exists());
...
fn write_auth_file(params: AuthFileParams, atlas_agent_home: &Path) -> std::io::Result<String> {
    let fake_jwt = fake_jwt_for_auth_file_params(&params)?;
    let auth_file = get_auth_file(atlas_agent_home);
...
fn remove_access_token_env_var() -> EnvVarGuard {
    EnvVarGuard::remove(ATLAS_AGENT_ACCESS_TOKEN_ENV_VAR)
}
...
fn test_auth_manager_config(atlas_agent_home: &Path) -> TestAuthManagerConfig {
...
fn auth_config_from_preserves_all_fields() {
    let atlas_agent_home = tempdir().expect("tempdir");
    let config = test_auth_manager_config(atlas_agent_home.path());
...
async fn shared_from_config_prefers_workload_identity_to_explicit_access_token() {
    let atlas_agent_home = tempdir().expect("tempdir");
    let config = test_auth_manager_config(atlas_agent_home.path());
    let _access_token_guard = EnvVarGuard::set(ATLAS_AGENT_ACCESS_TOKEN_ENV_VAR, "at-explicit");
...
async fn load_auth_reads_access_token_from_env() {
    ...
    let _access_token_guard = EnvVarGuard::set(ATLAS_AGENT_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    ...
    assert!(
        !get_auth_file(atlas_agent_home.path()).exists(),
...
async fn load_auth_reads_personal_access_token_from_env() {
    ...
    Mock::given(method("GET"))
        ...
        .await;
    let _authapi_guard = EnvVarGuard::set("ATLAS_AGENT_AUTHAPI_BASE_URL", &server.uri());
    let _access_token_guard = EnvVarGuard::set(ATLAS_AGENT_ACCESS_TOKEN_ENV_VAR, "at-env-test");
...
#[tokio::test]
#[serial(atlas_engine_auth_env)]
async fn load_auth_keeps_atlas_engine_api_key_env_precedence() {
    let atlas_agent_home = tempdir().unwrap();
    let record = agent_identity_record(WORKSPACE_ID_ALLOWED);
    let agent_identity = fake_agent_identity_jwt(&record).expect("fake agent identity");
    let _access_token_guard = EnvVarGuard::set(ATLAS_AGENT_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    let _api_key_guard = EnvVarGuard::set(ATLAS_AGENT_API_KEY_ENV_VAR, "sk-env");
...
async fn enforce_login_restrictions_logs_out_for_method_mismatch() {
    ...
    assert!(
        !atlas_agent_home.path().join("auth.json").exists(),
...
async fn auth_manager_rejects_disallowed_stored_and_external_auth() {
    ...
    assert!(
        manager
            .set_external_auth(Arc::new(StaticExternalAuth(AtlasEngineAuth::from_api_key(
                "sk-external",
            ))))
            .await
            .is_err(),
...
async fn api_only_policy_rejects_access_tokens_before_hydration() {
    ...
    let server = MockServer::start().await;
    let _authapi_guard = EnvVarGuard::set("ATLAS_AGENT_AUTHAPI_BASE_URL", &server.uri());
    let _access_token_guard = EnvVarGuard::set(ATLAS_AGENT_ACCESS_TOKEN_ENV_VAR, "at-rejected");
    let mut config = build_config(
        atlas_agent_home.path(),
        /*forced_login_method*/ None,
        /*forced_chatgpt_workspace_id*/ None,
...
async fn workspace_policy_rejects_agent_identity_before_hydration() {
    ...
    let access_token_guard = EnvVarGuard::set(ATLAS_AGENT_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    let mut config = build_config(
        atlas_agent_home.path(),
        /*forced_login_method*/ None,
        /*forced_chatgpt_workspace_id*/ None,
...
async fn workspace_policy_checks_the_selected_request_account() {
    ...
    let auth_path = atlas_agent_home.path().join("auth.json");
...
async fn enforce_login_restrictions_blocks_env_api_key_when_chatgpt_required() {
    let _guard = EnvVarGuard::set(ATLAS_AGENT_API_KEY_ENV_VAR, "sk-env");
    let _access_token_guard = remove_access_token_env_var();
    let atlas_agent_home = tempdir().unwrap();
...
async fn assert_agent_identity_plan_alias(
    ...
    let authapi_base_url = server.uri();
    let chatgpt_base_url = format!("{authapi_base_url}/backend-api");
    let auth = AtlasEngineAuth::from_agent_identity_jwt_with_authapi_base_url(
        &jwt,
        Some(&chatgpt_base_url),
        &authapi_base_url,
        &crate::test_support::transport_default_auth_route_config(),
