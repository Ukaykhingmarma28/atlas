...
use crate::events::AtlasEngineCollabAgentToolCallEventParams;
use crate::events::AtlasEngineCollabAgentToolCallEventRequest;
...
use crate::events::AtlasEngineDynamicToolCallEventParams;
use crate::events::AtlasEngineDynamicToolCallEventRequest;
use crate::events::AtlasEngineFileChangeEventParams;
use crate::events::AtlasEngineFileChangeEventRequest;
use crate::events::AtlasEngineGoalEventRequest;
use crate::events::AtlasEngineHookRunEventRequest;
use crate::events::AtlasEngineImageGenerationEventParams;
use crate::events::AtlasEngineImageGenerationEventRequest;
use crate::events::AtlasEngineMcpToolCallEventParams;
use crate::events::AtlasEngineMcpToolCallEventRequest;
use crate::events::AtlasEngineOnboardingExternalAgentImportCompleteEventRequest;
use crate::events::AtlasEngineOnboardingExternalAgentImportCompleteMetadata;
...
use crate::events::AtlasEnginePluginInstallRequestedEventRequest;
use crate::events::AtlasEnginePluginMeasurementEventParams;
use crate::events::AtlasEnginePluginMeasurementEventRequest;
...
use crate::events::AtlasEngineTurnEventParams;
use crate::events::AtlasEngineTurnEventRequest;
use crate::events::AtlasEngineTurnSteerEventParams;
use crate::events::AtlasEngineTurnSteerEventRequest;
use crate::events::AtlasEngineWebSearchEventParams;
use crate::events::AtlasEngineWebSearchEventRequest;
...
use crate::events::atlas_engine_artifact_operation_event_request;
use crate::events::atlas_engine_compaction_event_params;
use crate::events::atlas_engine_goal_event_params;
use crate::events::atlas_engine_hook_run_metadata;
use crate::events::atlas_engine_plugin_install_requested_metadata;
...
use crate::facts::TurnAtlasEngineError;
...
use atlas_engine_app_server_protocol::NetworkPolicyRuleAction;
...
use atlas_engine_app_server_protocol::WebSearchAction;
...
use atlas_engine_protocol::items::is_safe_plugin_relative_path;
...
impl<'a> AnalyticsDropSite<'a> {
    ...
    fn compaction(input: &'a AtlasEngineCompactionEvent) -> Self {
        Self {
    ...
    fn goal(input: &'a AtlasEngineGoalEvent) -> Self {
        Self {
            event_name: "goal",
            thread_id: &input.thread_id,
    ...
    fn tool_item(
        notification: &'a atlas_engine_app_server_protocol::ItemCompletedNotification,
        item_id: &'a str,
    ) -> Self {
        Self {
...
struct CompletedTurnState {
    ...
    turn_error: Option<AtlasEngineErrorInfo>,
    completed_at: u64,
...
struct TurnState {
    ...
    atlas_engine_error: Option<TurnAtlasEngineError>,
...
impl AnalyticsReducer {
    pub(crate) async fn ingest(&mut self, input: AnalyticsFact, out: &mut Vec<TrackEventRequest>) {
        match input {
            ...
            AnalyticsFact::Custom(input) => match input {
                ...
                CustomAnalyticsFact::TurnAtlasEngineError(input) => {
                    self.ingest_turn_atlas_engine_error(*input);
                }
    ...
    fn ingest_artifact_operation(
        ...
        out.push(TrackEventRequest::ArtifactOperation(
            atlas_engine_artifact_operation_event_request(input.tracking, input.operation),
        ));
    }
    ...
    fn ingest_guardian_review(
        ...
        out.push(TrackEventRequest::GuardianReview(Box::new(
            GuardianReviewEventRequest {
                event_type: "atlas_engine_guardian_review",
    ...
    fn ingest_turn_atlas_engine_error(&mut self, input: TurnAtlasEngineErrorFact) {
        let TurnAtlasEngineErrorFact {
            turn_id,
            thread_id,
            error,
        } = input;
        ...
        turn_state.atlas_engine_error = Some(error);
    }
    ...
    fn ingest_app_mentioned(&mut self, input: AppMentionedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.extend(mentions.into_iter().map(|mention| {
            let event_params = atlas_engine_app_metadata(&tracking, mention);
            TrackEventRequest::AppMentioned(AtlasEngineAppMentionedEventRequest {
    ...
    fn ingest_app_used(&mut self, input: AppUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        let event_params = atlas_engine_app_metadata(&tracking, app);
        out.push(TrackEventRequest::AppUsed(AtlasEngineAppUsedEventRequest {
    ...
    fn ingest_hook_run(&mut self, input: HookRunInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::HookRun(AtlasEngineHookRunEventRequest {
            event_type: "atlas_engine_hook_run",
            event_params: atlas_engine_hook_run_metadata(&tracking, hook),
        }));
    }
    ...
    fn ingest_plugin_install_requested(
        ...
        out.push(TrackEventRequest::PluginInstallRequested(
            AtlasEnginePluginInstallRequestedEventRequest {
                event_type: "atlas_engine_plugin_install_requested",
                event_params: atlas_engine_plugin_install_requested_metadata(&tracking, request),
            },
        ));
    }
    ...
    fn ingest_plugin_state_changed(
        ...
        let event = AtlasEnginePluginEventRequest {
    ...
    fn ingest_external_agent_config_import_completed(
        ...
        out.push(TrackEventRequest::ExternalAgentConfigImportCompleted(
            AtlasEngineOnboardingExternalAgentImportCompleteEventRequest {
                ...
                event_params: AtlasEngineOnboardingExternalAgentImportCompleteMetadata {
        ...
        match notification {
            ServerNotification::ThreadArchived(notification) => {
                out.push(TrackEventRequest::ThreadArchive(ThreadArchiveEvent {
                    event_type: "atlas_engine_thread_archive_event",
            ...
            ServerNotification::TurnCompleted(notification) => {
                ...
                turn_state.completed = Some(CompletedTurnState {
                    ...
                    turn_error: notification
                        .turn
                        .error
                        .and_then(|error| error.atlas_engine_error_info),
    ...
    fn ingest_plugin_measurements(
        ...
        out.extend(
            rows.into_iter()
                ...
                .map(|row| {
                    TrackEventRequest::PluginMeasurement(AtlasEnginePluginMeasurementEventRequest {
                        event_type: "atlas_engine_plugin_measurement_event",
                        event_params: AtlasEnginePluginMeasurementEventParams {
    ...
    fn emit_thread_initialized(
        &mut self,
        connection_id: u64,
        thread: atlas_engine_app_server_protocol::Thread,
        model: String,
        initialization_mode: ThreadInitializationMode,
        thread_originator: Option<String>,
        out: &mut Vec<TrackEventRequest>,
    ...
    fn ingest_compaction(&mut self, input: AtlasEngineCompactionEvent, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::Compaction(Box::new(
            AtlasEngineCompactionEventRequest {
                event_type: "atlas_engine_compaction_event",
                event_params: atlas_engine_compaction_event_params(
                    input,
    ...
    fn ingest_goal(&mut self, input: AtlasEngineGoalEvent, out: &mut Vec<TrackEventRequest>) {
        let Some((connection_state, thread_state, thread_metadata)) =
    ...
    fn ingest_guardian_review_completed(
        &mut self,
        notification: atlas_engine_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification,
        out: &mut Vec<TrackEventRequest>,
    ) {
    ...
    async fn maybe_emit_turn_event(&mut self, turn_id: &str, out: &mut Vec<TrackEventRequest>) {
        ...
        let turn_event = TrackEventRequest::TurnEvent(Box::new(AtlasEngineTurnEventRequest {
            event_type: "atlas_engine_turn_event",
            event_params: atlas_engine_turn_event_params(
                thread_state.app_server_client(connection_state),
                connection_state.runtime.clone(),
                turn_id.to_string(),
                turn_state,
...
fn tool_event_base_mut(event: &mut TrackEventRequest) -> Option<&mut AtlasEngineToolItemEventBase> {
    match event {
    ...
    match item {
        ThreadItem::CommandExecution {
            ...
            Some(TrackEventRequest::CommandExecution(
                AtlasEngineCommandExecutionEventRequest {
        ...
        ThreadItem::McpToolCall {
            ...
            Some(TrackEventRequest::McpToolCall(
                AtlasEngineMcpToolCallEventRequest {
                    event_type: "atlas_engine_mcp_tool_call_event",
                    event_params: AtlasEngineMcpToolCallEventParams {
                        base,
        ...
        ThreadItem::DynamicToolCall {
            ...
            Some(TrackEventRequest::DynamicToolCall(
                AtlasEngineDynamicToolCallEventRequest {
                    event_type: "atlas_engine_dynamic_tool_call_event",
                    event_params: AtlasEngineDynamicToolCallEventParams {
                        base,
        ...
        ThreadItem::CollabAgentToolCall {
            ...
            Some(TrackEventRequest::CollabAgentToolCall(
                AtlasEngineCollabAgentToolCallEventRequest {
                    event_type: "atlas_engine_collab_agent_tool_call_event",
                    event_params: AtlasEngineCollabAgentToolCallEventParams {
                        base,
        ...
        ThreadItem::ImageGeneration(item) => {
            ...
            Some(TrackEventRequest::ImageGeneration(
                AtlasEngineImageGenerationEventRequest {
                    event_type: "atlas_engine_image_generation_event",
                    event_params: AtlasEngineImageGenerationEventParams {
                        base,
...
fn atlas_engine_turn_event_params(
    app_server_client: AtlasEngineAppServerClientMetadata,
    runtime: AtlasEngineRuntimeMetadata,
    turn_id: String,
    turn_state: &TurnState,
    thread_metadata: &ThreadMetadataState,
) -> AtlasEngineTurnEventParams {
    let (
        Some(thread_id),
    ...
    let atlas_engine_error = turn_state.atlas_engine_error.as_ref();
    AtlasEngineTurnEventParams {
        thread_id,
        session_id: thread_metadata.session_id.clone(),
        turn_id,
...
fn analytics_turn_status(status: atlas_engine_app_server_protocol::TurnStatus) -> Option<TurnStatus> {
    match status {
        ...
        atlas_engine_app_server_protocol::TurnStatus::Interrupted => Some(TurnStatus::Interrupted),
        atlas_engine_app_server_protocol::TurnStatus::InProgress => None,
    }
}
    ...
    fn command_execution_script_paths_reject_unsafe_values() {
        assert_eq!(
            safe_plugin_relative_script_path(
                ...
                Some("/home/user/.atlas-agent/plugins/cache/openai-curated/sample/scripts/run.py"),
            ),
            None
        );
        assert_eq!(
