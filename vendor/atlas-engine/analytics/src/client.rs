...
impl AnalyticsEventsClient {
    /// Signature preserved so `atlas-engine-core` compiles unchanged; the auth manager
    ...
    pub fn track_compaction(&self, event: crate::facts::AtlasEngineCompactionEvent) {
        self.record_fact(AnalyticsFact::Custom(CustomAnalyticsFact::Compaction(
            Box::new(event),
        )));
    }

    pub fn track_goal_event(&self, event: AtlasEngineGoalEvent) {
