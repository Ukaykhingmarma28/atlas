...
use atlas_engine_protocol::protocol::CONTEXT_WINDOW_CLOSE_TAG;
use atlas_engine_protocol::protocol::CONTEXT_WINDOW_GUIDANCE_CLOSE_TAG;
...
fn skips_unnamed_image_label_text() {
    let image_url = "data:image/png;base64,abc".to_string();
    let label = atlas_engine_protocol::models::image_open_tag_text();
    let user_text = "Please review this image.".to_string();
    ...
    let item = ResponseItem::Message {
        ...
        content: vec![
            ...
            ContentItem::InputText {
                text: atlas_engine_protocol::models::image_close_tag_text(),
            },
            ContentItem::InputText {
                text: user_text.clone(),
            },
...
fn parses_agent_message() {
    let item = ResponseItem::Message {
        ...
        content: vec![ContentItem::OutputText {
            text: "Hello from Atlas Agent".to_string(),
        }],
        phase: None,
        internal_chat_message_metadata_passthrough: None,
    };
    ...
    match turn_item {
        TurnItem::AgentMessage(message) => {
            ...
            assert_eq!(text, "Hello from Atlas Agent");
        }
        other => panic!("expected TurnItem::AgentMessage, got {other:?}"),
    }
}
