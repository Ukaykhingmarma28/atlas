...
use atlas_engine_analytics::HookRunFact;
use atlas_engine_analytics::build_track_events_context;
use atlas_engine_hooks::PermissionRequestDecision;
use atlas_engine_hooks::PermissionRequestOutcome;
use atlas_engine_hooks::PermissionRequestRequest;
use atlas_engine_hooks::PostToolUseOutcome;
use atlas_engine_hooks::PostToolUseRequest;
use atlas_engine_hooks::PreToolUseOutcome;
use atlas_engine_hooks::PreToolUseRequest;
use atlas_engine_hooks::SessionStartOutcome;
use atlas_engine_hooks::StartHookTarget;
use atlas_engine_hooks::StopHookTarget;
use atlas_engine_hooks::StopOutcome;
use atlas_engine_hooks::SubagentHookContext;
use atlas_engine_hooks::UserPromptSubmitOutcome;
use atlas_engine_hooks::UserPromptSubmitRequest;
use atlas_engine_otel::HOOK_RUN_DURATION_METRIC;
use atlas_engine_otel::HOOK_RUN_METRIC;
...
use atlas_engine_protocol::protocol::HookStartedEvent;
        ...
        let target = match &turn_context.session_source {
            SessionSource::SubAgent(SubAgentSource::ThreadSpawn { agent_role, .. })
                if matches!(
                    ...
                    atlas_engine_hooks::SessionStartSource::Startup
                ) =>
            {
...
pub(crate) async fn run_turn_stop_hooks(
    ...
    let request = atlas_engine_hooks::StopRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_session_end_hooks(sess: &Arc<Session>) {
    ...
    let request = atlas_engine_hooks::SessionEndRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_pre_compact_hooks(
    ...
    let request = atlas_engine_hooks::PreCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_post_compact_hooks(
    ...
    let request = atlas_engine_hooks::PostCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_legacy_after_agent_hook(
    ...
    for hook_outcome in hooks
        .dispatch(atlas_engine_hooks::HookPayload {
            session_id: sess.session_id().into(),
            #[allow(deprecated)]
            cwd: turn_context.cwd.clone(),
            client: turn_context.app_server_client_name.clone(),
            triggered_at: chrono::Utc::now(),
            hook_event: atlas_engine_hooks::HookEvent::AfterAgent {
                event: atlas_engine_hooks::HookEventAfterAgent {
        ...
        .await
    {
        ...
        let (error, should_abort) = match hook_outcome.result {
            atlas_engine_hooks::HookResult::Success => continue,
            atlas_engine_hooks::HookResult::FailedContinue(error) => (error, false),
            atlas_engine_hooks::HookResult::FailedAbort(error) => (error, true),
        };
    ...
    let event = EventMsg::Error(atlas_engine_protocol::protocol::ErrorEvent {
        message,
...
fn hook_run_analytics_payload(
    ...
    completed: &HookCompletedEvent,
) -> (atlas_engine_analytics::TrackEventsContext, HookRunFact) {
    (
...
mod tests {
    ...
    use atlas_engine_protocol::protocol::HookRunStatus;
    use atlas_engine_protocol::protocol::HookScope;
    use atlas_engine_protocol::protocol::HookSource;
    ...
    async fn hook_lifecycle_notifications_only_report_synchronous_runs() {
        ...
        assert!(matches!(
            started.msg,
            atlas_engine_protocol::protocol::EventMsg::HookStarted(event)
        ...
        assert!(matches!(
            completed.msg,
            atlas_engine_protocol::protocol::EventMsg::HookCompleted(event)
                if event.run.id == synchronous_run.id
        ));
        assert!(events.try_recv().is_err());
    }
