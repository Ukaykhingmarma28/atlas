...
use super::X_ATLAS_AGENT_INSTALLATION_ID_HEADER;
use super::X_ATLAS_AGENT_PARENT_THREAD_ID_HEADER;
use super::X_ATLAS_AGENT_TURN_METADATA_HEADER;
use super::X_ATLAS_AGENT_WINDOW_ID_HEADER;
use super::X_OPENAI_SUBAGENT_HEADER;
use crate::AttestationContext;
use crate::AttestationProvider;
use crate::GenerateAttestationFuture;
use crate::responses_metadata::AtlasEngineResponsesMetadata;
use crate::test_support::TestAtlasEngineResponsesRequestKind;
use crate::test_support::responses_metadata as test_responses_metadata;
use atlas_engine_api::AgentIdentityTelemetry;
use atlas_engine_api::ApiError;
use atlas_engine_api::ResponseEvent;
...
use atlas_engine_model_provider_info::CHATGPT_ATLAS_AGENT_BASE_URL;
...
use atlas_engine_rollout_trace::ExecutionStatus;
...
use atlas_engine_rollout_trace::RolloutTrace;
...
async fn compact_uses_bearer_after_agent_identity_session_fallback() -> anyhow::Result<()> {
    ...
    let auth_manager = chatgpt_auth_manager(&atlas_agent_home, server.uri()).await;
    ...
    let output = client
        .compact_conversation_history(
            ...
            CompactConversationRequestSettings {
                effort: None,
                summary: atlas_engine_protocol::config_types::ReasoningSummary::None,
                service_tier: None,
            },
...
fn write_chatgpt_auth_json(atlas_agent_home: &std::path::Path) {
    let auth_json = json!({
        "tokens": {
...
async fn chatgpt_auth_manager(
    ...
    write_chatgpt_auth_json(atlas_agent_home.path());
...
fn started_inference_attempt(temp: &TempDir) -> anyhow::Result<InferenceTraceAttempt> {
    ...
    writer.append(RawTraceEventPayload::AtlasEngineTurnStarted {
...
fn build_ws_client_metadata_includes_window_lineage_and_turn_metadata() {
    ...
    let turn_metadata: serde_json::Value = serde_json::from_str(
        client_metadata
            .get(X_ATLAS_AGENT_TURN_METADATA_HEADER)
...
async fn bedrock_unauthorized_error_uses_provider_mapping() {
    ...
    assert_eq!(
        error.to_string(),
        format!(
            "Amazon Bedrock rejected the request because its AWS signature has expired. Refresh your AWS credentials and retry. If `AWS_BEARER_TOKEN_BEDROCK` is set, update or unset it, then restart Atlas Agent, url: {url}"
        )
    );
}
...
#[tokio::test]
async fn websocket_handshake_includes_attestation_for_chatgpt_atlas_engine_responses() {
...
#[tokio::test]
async fn non_chatgpt_atlas_engine_endpoints_omit_attestation_generation() {
    let (model_client, attestation_calls) =
