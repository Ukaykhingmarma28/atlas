...
fn unified_exec_env_injects_defaults() {
    ...
    let expected = HashMap::from([
        ...
        ("ATLAS_AGENT_CI".to_string(), "1".to_string()),
    ]);
...
fn env_overlay_for_exec_server_keeps_runtime_changes_only() {
    ...
    let request_env = HashMap::from([
        ...
        ("SHELL_SET".to_string(), "policy".to_string()),
        ("ATLAS_AGENT_THREAD_ID".to_string(), "thread-1".to_string()),
        (
            ATLAS_AGENT_PERMISSION_PROFILE_ENV_VAR.to_string(),
            "current-profile".to_string(),
        ),
        ...
        (
            "ATLAS_AGENT_SANDBOX_NETWORK_DISABLED".to_string(),
            "1".to_string(),
        ),
    ]);
...
fn exec_env_policy_excludes_non_inheritable_and_runtime_variables() {
    let policy = ShellEnvironmentPolicy {
        r#set: HashMap::from([
            (
                "atlas_engine_permission_profile".to_string(),
            ...
            (
                "atlas_engine_apply_patch_preserve_line_endings".to_string(),
                "1".to_string(),
            ),
            (
                "atlas_engine_plugin_metrics_output".to_string(),
    ...
    assert_eq!(
        ...
        atlas_engine_exec_server::ExecEnvPolicy {
...
fn exec_server_params_use_path_uri_and_env_policy_overlay_contract() {
    let cwd: atlas_engine_utils_absolute_path::AbsolutePathBuf = std::env::current_dir()
        .expect("current dir")
        .try_into()
        .expect("absolute path");
    let permission_profile = atlas_engine_protocol::models::PermissionProfile::Disabled;
    ...
    let mut request = ExecRequest {
        ...
        exec_server_env_config: Some(ExecServerEnvConfig {
            policy: atlas_engine_exec_server::ExecEnvPolicy {
                inherit: atlas_engine_protocol::config_types::ShellEnvironmentPolicyInherit::Core,
        ...
        sandbox: atlas_engine_sandboxing::SandboxType::None,
    ...
    let proxy_settings_mode = atlas_engine_sandboxing::WindowsSandboxProxySettingsMode::Preserve;
    ...
    assert_eq!(
        ...
        Some(atlas_engine_sandboxing::WindowsSandboxProxySettingsMode::Preserve)
    );
...
async fn network_denial_fallback_message_names_sandbox_network_proxy() {
    ...
    assert_eq!(
        message,
        "Network access was denied by the Atlas Agent sandbox network proxy."
    );
}
...
async fn failed_initial_end_for_unstored_process_uses_fallback_output() {
    ...
    let request = ExecCommandRequest {
        ...
        shell_mode: atlas_engine_tools::UnifiedExecShellMode::Direct,
        network: None,
        tty: true,
    ...
    assert_eq!(
        item.status,
        atlas_engine_protocol::items::CommandExecutionStatus::Failed
    );
...
async fn pruning_does_not_evict_live_process_while_exited_process_is_finalizing() {
    let exited_process = Arc::new(
        crate::unified_exec::process_tests::remote_process(
            atlas_engine_exec_server::WriteStatus::Accepted,
            /*terminate_error*/ None,
            atlas_engine_sandboxing::SandboxType::None,
        )
        .await,
    );
    exited_process
