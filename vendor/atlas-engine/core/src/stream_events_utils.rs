...
use atlas_engine_memories_read::citations::parse_memory_citation;
use atlas_engine_memories_read::citations::thread_ids_from_memory_citation;
use atlas_engine_protocol::error::AtlasEngineErr;
use atlas_engine_protocol::error::Result;
use atlas_engine_protocol::memory_citation::MemoryCitation;
...
use atlas_engine_utils_stream_parser::strip_proposed_plan_blocks;
use futures::Future;
use tracing::debug;
...
fn strip_hidden_assistant_markup_and_parse_memory_citation(
    ...
    String,
    Option<atlas_engine_protocol::memory_citation::MemoryCitation>,
) {
...
pub(crate) async fn handle_output_item_done(
    ...
    match ToolRouter::build_tool_call(item.clone()) {
        ...
        Err(FunctionCallError::Fatal(message)) => {
            return Err(AtlasEngineErr::Fatal(message));
        }
    }
...
pub(crate) async fn finalize_turn_item(
    ...
    if let TurnItem::AgentMessage(agent_message) = &mut *turn_item {
        ...
        agent_message.content =
            vec![atlas_engine_protocol::items::AgentMessageContent::Text { text: stripped }];
