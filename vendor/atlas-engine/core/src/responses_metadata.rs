...
use atlas_engine_analytics::CompactionStrategy;
use atlas_engine_analytics::CompactionTrigger;
...
use crate::client::X_ATLAS_AGENT_INSTALLATION_ID_HEADER;
use crate::client::X_ATLAS_AGENT_PARENT_THREAD_ID_HEADER;
use crate::client::X_ATLAS_AGENT_TURN_METADATA_HEADER;
use crate::client::X_ATLAS_AGENT_WINDOW_ID_HEADER;
...
const RESERVED_METADATA_KEYS: &[&str] = &[
    INSTALLATION_ID_KEY,
    X_ATLAS_AGENT_INSTALLATION_ID_HEADER,
    SESSION_ID_KEY,
    THREAD_ID_KEY,
    AGENT_NAME_KEY,
    TURN_ID_KEY,
    ...
    X_ATLAS_AGENT_TURN_METADATA_HEADER,
    X_ATLAS_AGENT_PARENT_THREAD_ID_HEADER,
...
pub(crate) enum AtlasEngineResponsesRequestKind {
    Turn,
    Prewarm,
...
impl AtlasEngineResponsesRequestKind {
    fn metadata(self) -> (&'static str, Option<CompactionTurnMetadata>) {
        match self {
            AtlasEngineResponsesRequestKind::Turn => ("turn", None),
            AtlasEngineResponsesRequestKind::Prewarm => ("prewarm", None),
            AtlasEngineResponsesRequestKind::Compaction(metadata) => ("compaction", Some(metadata)),
            AtlasEngineResponsesRequestKind::Memory => ("memory", None),
        }
    }

    fn has_turn_identity(self) -> bool {
        !matches!(self, AtlasEngineResponsesRequestKind::Memory)
    }
}
...
/// Caller-owned snapshot of Atlas Agent metadata sent to ResponsesAPI.
///
/// The full Atlas Agent turn metadata blob is transported canonically as
/// `client_metadata["x-atlas-engine-turn-metadata"]`. Flat `client_metadata` keys and direct HTTP/ws
/// headers are generated compatibility projections of this snapshot, not separate sources of
/// truth.
#[derive(Clone, Debug)]
pub struct AtlasEngineResponsesMetadata {
    ...
    pub(crate) request_kind: Option<AtlasEngineResponsesRequestKind>,
...
impl AtlasEngineResponsesMetadata {
    pub(crate) fn new(
    ...
    pub(crate) fn client_metadata(&self) -> HashMap<String, String> {
        let mut client_metadata = HashMap::from([
            (
                X_ATLAS_AGENT_INSTALLATION_ID_HEADER.to_string(),
        ...
        if let Some(parent_thread_id) = self.parent_thread_id {
            client_metadata.insert(
                X_ATLAS_AGENT_PARENT_THREAD_ID_HEADER.to_string(),
    ...
    pub(crate) fn compatibility_headers(&self) -> ApiHeaderMap {
        ...
        if self.has_turn_metadata()
            && let Ok(turn_metadata_json) = to_ascii_json_string(&AtlasEngineTurnMetadataPayload {
                tool_namespaces_info: None,
    ...
    fn turn_metadata_payload(&self) -> AtlasEngineTurnMetadataPayload<'_> {
        ...
        let has_turn_identity =
            request_kind.is_none_or(AtlasEngineResponsesRequestKind::has_turn_identity);
        let has_request_identity =
            request_kind.is_some_and(AtlasEngineResponsesRequestKind::has_turn_identity);
        AtlasEngineTurnMetadataPayload {
            ...
            compaction,
            // Extra metadata enriches the Atlas Agent turn metadata blob, not literal top-level
            // Responses client_metadata. Product metadata is validated while loading config;
            // app-server metadata has reserved Atlas Agent-owned keys filtered when it enters turn state.
            extra: &self.extra,
        }
    }
}
...
#[derive(Serialize)]
struct AtlasEngineTurnMetadataPayload<'a> {
    #[serde(default, skip_serializing_if = "Option::is_none")]
