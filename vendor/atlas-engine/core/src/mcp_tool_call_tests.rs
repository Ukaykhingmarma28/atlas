...
use atlas_engine_config::types::AppConfig;
use atlas_engine_config::types::AppToolConfig;
use atlas_engine_config::types::AppToolsConfig;
use atlas_engine_config::types::ApprovalsReviewer;
use atlas_engine_config::types::AppsConfigToml;
...
fn approval_config(turn_context: &TurnContext) -> atlas_engine_mcp::McpConfig {
...
fn install_mcp_permission_request_hook(
    ...
    std::fs::create_dir_all(&turn_context.config.atlas_agent_home)
        .expect("create atlas-agent home for MCP permission hook");
    let script = format!(
        r#"import json
...
"#,
    ...
    let trusted_config_layer_stack = trusted_config_layer_stack(
        ...
        &turn_context.config.atlas_agent_home,
        hook_list.hooks,
    );
...
#[test]
fn openai_file_params_are_only_honored_for_atlas_apps() {
    let params = HashMap::from([("file".to_string(), Vec::new())]);

    assert_eq!(
        openai_file_input_optional_fields_for_server(ATLAS_APPS_MCP_SERVER_NAME, &params),
        Some(params.clone())
    );
    assert_eq!(
...
async fn mcp_tool_call_span_records_error_type_and_error_code() {
    ...
    let span = mcp_tool_call_span(
        ...
        &turn_context,
        McpToolCallSpanFields {
            server_name: ATLAS_APPS_MCP_SERVER_NAME,
    ...
    assert!(
        logs.contains("error.type=\"tool_result\"")
            && logs.contains("atlas_agent.mcp.error.code=\"RATE_LIMITED\""),
...
async fn mcp_result_telemetry_records_allowlisted_span_fields() {
    ...
    assert!(
        logs.contains("atlas_agent.mcp.target.id=\"com.apple.reminders\"")
            && logs.contains("atlas_agent.mcp.server_user_flow.triggered=false"),
...
async fn mcp_result_telemetry_ignores_invalid_and_missing_values() {
    ...
    .await;
    assert!(
        !invalid_logs.contains("atlas_agent.mcp.target.id=")
            && !invalid_logs.contains("atlas_agent.mcp.server_user_flow.triggered="),
    ...
    .await;
    assert!(
        !missing_logs.contains("atlas_agent.mcp.target.id=")
            && !missing_logs.contains("atlas_agent.mcp.server_user_flow.triggered="),
    ...
    assert!(
        !no_meta_logs.contains("atlas_agent.mcp.target.id=")
            && !no_meta_logs.contains("atlas_agent.mcp.server_user_flow.triggered="),
...
#[test]
fn atlas_apps_tool_question_uses_fallback_app_label() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        ATLAS_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn trusted_atlas_apps_tool_question_offers_always_allow() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        ATLAS_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn atlas_apps_tool_question_without_elicitation_omits_always_allow() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        ATLAS_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn atlas_apps_connectors_support_persistent_approval() {
    let invocation = McpInvocation {
        server: ATLAS_APPS_MCP_SERVER_NAME.to_string(),
...
async fn mcp_tool_call_request_meta_includes_turn_metadata_for_custom_server() {
    ...
    let turn_metadata = meta
        .get(crate::X_ATLAS_AGENT_TURN_METADATA_HEADER)
    ...
    assert_eq!(
        ...
        serde_json::json!({
            "callId": "call-custom",
            crate::X_ATLAS_AGENT_TURN_METADATA_HEADER: expected_turn_metadata,
        })
    );
}
...
#[test]
fn mcp_tool_call_item_metadata_only_trusts_atlas_apps_identity() {
    let mut metadata = approval_metadata(
    ...
    metadata.atlas_apps_meta = Some(
        serde_json::json!({
            "resource_uri": "/asdk_app_0123456789abcdef0123456789abcdef/link_fedcba9876543210fedcba9876543210/create_event",
        })
        .as_object()
        .cloned()
        .expect("_atlas_apps metadata should be an object"),
    );

    assert_eq!(
        McpToolCallItemMetadata::from_tool_metadata(ATLAS_APPS_MCP_SERVER_NAME, Some(&metadata),),
...
#[tokio::test]
async fn atlas_apps_tool_call_request_meta_includes_turn_metadata_and_atlas_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = expected_mcp_turn_metadata(&turn_context);
...
#[tokio::test]
async fn atlas_apps_tool_call_request_meta_includes_call_id_without_existing_atlas_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = expected_mcp_turn_metadata(&turn_context);
...
fn atlas_apps_auth_failure_result() -> CallToolResult {
    CallToolResult {
        content: vec![serde_json::json!({
            "type": "text",
...
fn atlas_apps_auth_failure_metadata() -> McpToolApprovalMetadata {
    approval_metadata(
...
#[tokio::test]
async fn atlas_apps_auth_elicitation_feature_disabled_returns_original_result() {
    ...
    let result = atlas_apps_auth_failure_result();
    let metadata = atlas_apps_auth_failure_metadata();

    let returned = maybe_request_atlas_apps_auth_elicitation(
        &session,
        &turn_context,
...
#[tokio::test]
async fn atlas_apps_auth_elicitation_disallowed_by_policy_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    let mut features = Features::with_defaults();
...
#[tokio::test]
async fn atlas_apps_auth_elicitation_granular_mcp_disabled_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    let mut features = Features::with_defaults();
    features.enable(Feature::AuthElicitation);
    let mutable_turn_context = Arc::get_mut(&mut turn_context).expect("single turn context ref");
...
#[tokio::test]
async fn atlas_apps_auth_elicitation_enabled_by_default_requests_elicitation() {
    let (session, turn_context, rx_event) = make_session_and_context_with_rx().await;
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let request_task = tokio::spawn({
        ...
        async move {
            maybe_request_atlas_apps_auth_elicitation(
                &session,
                &turn_context,
                turn_context.approval_policy(),
                "call_123",
    ...
    assert_eq!(request.server_name, ATLAS_APPS_MCP_SERVER_NAME);
    assert_eq!(
        request.id,
        atlas_engine_protocol::mcp::RequestId::String("atlas_apps_auth_call_123".to_string())
    );
    assert!(matches!(
        request.request,
        atlas_engine_protocol::approvals::ElicitationRequest::Url { .. }
    ));

    session
        .resolve_elicitation(
            ATLAS_APPS_MCP_SERVER_NAME.to_string(),
            rmcp::model::RequestId::String("atlas_apps_auth_call_123".into()),
            ElicitationResponse {
...
#[test]
fn approval_elicitation_meta_includes_connector_source_for_atlas_apps() {
    assert_eq!(
        build_mcp_tool_approval_elicitation_meta(
            ATLAS_APPS_MCP_SERVER_NAME,
            Some(&approval_metadata(
...
#[tokio::test]
async fn persist_atlas_engine_app_tool_approval_writes_tool_override() {
    ...
    persist_atlas_engine_app_tool_approval(&config, "calendar", "calendar/list_events")
        .await
...
async fn custom_mcp_tool_approval_mode_uses_plugin_mcp_policy() {
    ...
    write_sample_plugin_mcp(atlas_agent_home.as_path());
    std::fs::write(
...
async fn maybe_persist_mcp_tool_approval_reloads_session_config_for_custom_server() {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .atlas_agent_home(atlas_agent_home.clone().to_path_buf())
        .build()
        .await
        .expect("load config");
    turn_context.config = Arc::new(config);
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    let project_atlas_engine_dir = project_dir.path().join(".atlas-agent");
    std::fs::create_dir_all(&project_atlas_engine_dir).expect("create project .atlas-agent dir");
    std::fs::write(
        project_atlas_engine_dir.join(CONFIG_TOML_FILE),
        "[mcp_servers.docs]\ncommand = \"docs-server\"\n",
    )
    ...
    ConfigEditsBuilder::new(&atlas_agent_home)
        .set_project_trust_level(
            project_dir.path(),
            atlas_engine_protocol::config_types::TrustLevel::Trusted,
        )
        .apply()
        .await
    ...
    let contents = std::fs::read_to_string(project_atlas_engine_dir.join(CONFIG_TOML_FILE))
...
async fn approve_mode_skips_guardian_in_every_permission_mode() {
    ...
    for approval_policy in [
        ...
        turn_context.auth_manager = Some(crate::test_support::auth_manager_from_auth(
            atlas_engine_login::AtlasEngineAuth::create_dummy_chatgpt_auth_for_testing(),
        ));
        Arc::make_mut(&mut turn_context.config)
            .permissions
            .approval_policy
