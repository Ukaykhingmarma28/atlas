...
use atlas_engine_execpolicy::AmendError;
use atlas_engine_execpolicy::Decision;
use atlas_engine_execpolicy::Error as ExecPolicyRuleError;
...
use atlas_engine_execpolicy::blocking_append_allow_prefix_rule;
use atlas_engine_execpolicy::blocking_append_network_rule;
...
use atlas_engine_protocol::permissions::FileSystemSandboxKind;
use atlas_engine_protocol::protocol::AskForApproval;
use atlas_engine_shell_command::is_dangerous_command::DangerousCommandMatch;
use atlas_engine_shell_command::is_dangerous_command::dangerous_command_match;
use atlas_engine_shell_command::is_safe_command::is_known_safe_command;
use thiserror::Error;
use tokio::fs;
...
pub(crate) fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            .config_layer_stack
            .layers_low_to_high()
            .filter_map(atlas_engine_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
pub enum ExecPolicyError {
    ...
    ParsePolicy {
        path: String,
        source: atlas_engine_execpolicy::Error,
    },
}
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(atlas_agent_home);
        spawn_blocking({
...
fn exec_policy_message_for_display(source: &atlas_engine_execpolicy::Error) -> String {
...
pub(crate) fn render_decision_for_unmatched_command(
    ...
    let is_known_safe = match command_origin {
        ...
        #[cfg(windows)]
        ExecPolicyCommandOrigin::PowerShell => {
            atlas_engine_shell_command::is_safe_command::is_safe_powershell_words(command)
        }
    };
...
/// - Note: we only use this amendment when the command fails to run in sandbox and atlas-agent prompts the user to run outside the sandbox
