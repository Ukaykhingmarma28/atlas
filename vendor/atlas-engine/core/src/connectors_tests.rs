...
use atlas_engine_connectors::merge::plugin_connector_to_app_info;
use atlas_engine_connectors::metadata::connector_install_url;
use atlas_engine_connectors::metadata::sanitize_name;
...
fn atlas_engine_app_tool(
    ...
    let tool_namespace = connector_name
        .map(sanitize_name)
        .map(|connector_name| format!("mcp__{ATLAS_APPS_MCP_SERVER_NAME}__{connector_name}"))
        .unwrap_or_else(|| ATLAS_APPS_MCP_SERVER_NAME.to_string());
...
fn synthetic_links_are_exposed_to_the_agent_but_not_accessible_in_app_list() {
    ...
    let tools = vec![
        synthetic_tool,
        atlas_engine_app_tool("calendar_list_events", "calendar", Some("Calendar"), &[]),
    ];
...
fn accessible_connectors_from_mcp_tools_preserves_description() {
    let mcp_tools = vec![ToolInfo {
        ...
        callable_namespace: "mcp__atlas_apps__calendar".to_string(),
...
"#,
    ...
    let plugins_manager = plugins_manager_for_config(&config, Some(auth.api_auth_mode()));
