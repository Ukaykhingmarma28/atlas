...
use atlas_engine_config::load_global_mcp_servers;
use atlas_engine_login::default_client::is_first_party_originator;
...
use atlas_engine_rmcp_client::perform_oauth_login;
...
use atlas_engine_skills::SkillToolDependency;
...
pub(crate) async fn maybe_prompt_and_install_mcp_dependencies(
    ...
    if mentioned_skills.is_empty()
        || !config
            .features
            .enabled(atlas_engine_features::Feature::SkillMcpDependencyInstall)
    {
        return;
    }
...
pub(crate) async fn maybe_install_mcp_dependencies(
    ...
    let mut servers = match load_global_mcp_servers(&atlas_agent_home).await {
        Ok(servers) => servers,
        Err(err) => {
    ...
    if let Err(err) = ConfigEditsBuilder::new(&atlas_agent_home)
