...
use atlas_engine_analytics::AppInvocation;
use atlas_engine_analytics::CompactionPhase;
use atlas_engine_analytics::CompactionReason;
use atlas_engine_analytics::InvocationType;
use atlas_engine_analytics::TurnResolvedConfigFact;
use atlas_engine_analytics::build_track_events_context;
use atlas_engine_async_utils::OrCancelExt;
use atlas_engine_connectors::AppToolPolicyEvaluator;
...
use atlas_engine_extension_api::TurnInputEnvironment;
use atlas_engine_features::Feature;
use atlas_engine_file_system::FindUpErrorPolicy;
use atlas_engine_file_system::find_nearest_ancestor_with_markers;
use atlas_engine_login::AtlasEngineAuth;
use atlas_engine_model_provider::RemoteCompactionSupport;
...
use atlas_engine_protocol::items::PlanItem;
...
use atlas_engine_protocol::protocol::AgentMessageContentDeltaEvent;
use atlas_engine_protocol::protocol::AgentReasoningSectionBreakEvent;
...
use atlas_engine_protocol::protocol::PlanDeltaEvent;
use atlas_engine_protocol::protocol::RawResponseCompletedEvent;
use atlas_engine_protocol::protocol::ReasoningContentDeltaEvent;
use atlas_engine_protocol::protocol::ReasoningRawContentDeltaEvent;
use atlas_engine_protocol::protocol::SafetyBufferingEvent;
...
use atlas_engine_skills::ToolMentionKind;
use atlas_engine_skills::app_id_from_path;
use atlas_engine_skills::build_skill_name_counts;
use atlas_engine_skills::collect_explicit_skill_mentions;
use atlas_engine_skills::tool_kind_for_path;
use atlas_engine_skills_extension::HostSkillPrompts;
use atlas_engine_skills_extension::InjectedHostSkillPrompts;
use atlas_engine_thread_store::PersistContext;
use atlas_engine_tools::DiscoverableTool;
use atlas_engine_tools::ToolName;
use atlas_engine_tools::filter_request_plugin_install_discoverable_tools_for_client;
use atlas_engine_utils_path_uri::PathUri;
use atlas_engine_utils_stream_parser::AssistantTextChunk;
use atlas_engine_utils_stream_parser::AssistantTextStreamParser;
use atlas_engine_utils_stream_parser::ProposedPlanSegment;
use atlas_engine_utils_stream_parser::extract_proposed_plan_text;
use atlas_engine_utils_stream_parser::strip_citations;
...
const POST_SAMPLING_TOKEN_ESTIMATE_TARGET: &str = "atlas_engine_core::post_sampling_token_estimate";
...
pub(crate) async fn run_turn(
...
) -> AtlasEngineResult<Option<String>> {
    ...
    .await
    {
        if matches!(err.details(), AtlasEngineErrorDetails::TurnAborted) {
            run_hooks_and_record_inputs(&sess, &turn_context, &input, PersistContext::Standard)
                .await;
            return Err(err);
        }
        if matches!(err.details(), AtlasEngineErrorDetails::ToolCollision(_)) {
            return Err(err);
        }
        let error = err.to_atlas_engine_protocol_error();
        ...
        .await
    {
        ...
        Err(err) if matches!(err.details(), AtlasEngineErrorDetails::TurnAborted) => {
            run_hooks_and_record_inputs(&sess, &turn_context, &input, PersistContext::Standard)
                .await;
            return Err(err);
        }
    ...
    // Although from the perspective of atlas-agent.rs, TurnDiffTracker has the lifecycle of a Task which contains
    ...
    loop {
        ...
        let sampling_request_result: AtlasEngineResult<_> = async {
        ...
        match sampling_request_result {
            ...
            Err(atlas_engine_error)
                if matches!(
                    atlas_engine_error.details(),
                    AtlasEngineErrorDetails::InvalidImageRequest()
                ) =>
            {
                sess.track_turn_atlas_engine_error(turn_context.as_ref(), &atlas_engine_error);
                let error = AtlasEngineErrorInfo::BadRequest;
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
                let event = EventMsg::Error(ErrorEvent {
                    message: "Invalid image in your last message. Please remove it and try again."
                        .to_string(),
                    atlas_engine_error_info: Some(error),
                });
            ...
            Err(e) => {
                info!("Turn error: {e:#}");
                let error = e.to_atlas_engine_protocol_error();
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
                sess.track_turn_atlas_engine_error(turn_context.as_ref(), &e);
...
async fn required_mcp_servers_for_input(
    ...
    let connector_slug_counts = if turn_context.apps_enabled() && !mentions.plain_names.is_empty() {
        ...
        build_connector_slug_counts(
            &atlas_engine_connectors::merge::merge_plugin_connectors_with_accessible(
                connector_ids,
...
async fn build_skills_and_plugins(
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = atlas_engine_connectors::merge::merge_plugin_connectors_with_accessible(
            connector_snapshot
                .connector_ids()
                .iter()
                .map(|connector_id| connector_id.0.clone()),
...
///
/// Returns `None` when the active authentication does not use the Atlas Agent backend, the provider is
...
async fn capture_current_model_fallback_step_context(
    ...
    previous_model: &str,
    cancellation_token: &CancellationToken,
) -> AtlasEngineResult<Option<Arc<StepContext>>> {
    let uses_atlas_engine_backend = turn_context
        .auth_manager
        .as_deref()
        .is_some_and(atlas_engine_login::AuthManager::current_auth_uses_atlas_engine_backend);
    if !uses_atlas_engine_backend
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = atlas_engine_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    sess: Arc<Session>,
    step_context: Arc<StepContext>,
    turn_store: Arc<atlas_engine_extension_api::ExtensionData>,
    turn_diff_tracker: SharedTurnDiffTracker,
    client_session: &mut ModelClientSession,
    responses_metadata: &AtlasEngineResponsesMetadata,
    input: Vec<ResponseItem>,
    cancellation_token: CancellationToken,
) -> AtlasEngineResult<(SamplingRequestResult, Vec<ResponseItem>)> {
    ...
    loop {
        ...
        {
            atlas_engine_protocol::models::bound_executed_tool_calls_for_prompt(&mut prompt_input);
        }
        ...
        .await
        {
            ...
            Err(err) => match err.details() {
                ...
                AtlasEngineErrorDetails::UsageLimitReached(e) => {
...
pub(crate) async fn built_tools(
    ...
    mcp: &Arc<atlas_engine_mcp::McpBinding>,
    step_store: &ExtensionData,
    prepared_recommendations: PreparedToolRecommendations,
) -> AtlasEngineResult<Arc<ToolRouter>> {
...
pub(super) fn agent_message_text(item: &atlas_engine_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: atlas_engine_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(atlas_engine_protocol::items::AgentMessageItem {
...
async fn handle_assistant_item_done_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    turn_store: &atlas_engine_extension_api::ExtensionData,
    item: &ResponseItem,
    state: &mut PlanModeStreamState,
    previously_active_item: Option<&TurnItem>,
...
async fn drain_in_flight(
    in_flight: &mut FuturesOrdered<BoxFuture<'static, AtlasEngineResult<ResponseInputItem>>>,
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
) -> AtlasEngineResult<()> {
...
async fn try_run_sampling_request(
    ...
    prompt: &Prompt,
    cancellation_token: CancellationToken,
) -> AtlasEngineResult<SamplingRequestResult> {
    feedback_tags!(
        model = turn_context.model_info.slug.clone(),
        approval_policy = turn_context.approval_policy(),
    ...
    let mut stream = client_session
        ...
        .or_cancel(&cancellation_token)
        .await??;
    let mut in_flight: FuturesOrdered<BoxFuture<'static, AtlasEngineResult<ResponseInputItem>>> =
        FuturesOrdered::new();
    ...
    let outcome: AtlasEngineResult<SamplingRequestResult> = loop {
        let handle_responses = trace_span!(
            ...
            from = field::Empty,
            atlas_engine.request.reasoning_effort = %reasoning_effort,
            gen_ai.usage.input_tokens = field::Empty,
            gen_ai.usage.cache_read.input_tokens = field::Empty,
            gen_ai.usage.cache_write.input_tokens = field::Empty,
            gen_ai.usage.output_tokens = field::Empty,
            atlas_engine.usage.reasoning_output_tokens = field::Empty,
            atlas_engine.usage.total_tokens = field::Empty,
        );
        ...
        {
            Ok(event) => event,
            Err(atlas_engine_async_utils::CancelErr::Cancelled) => {
                break Err(AtlasEngineErr::TurnAborted);
            }
        };
        ...
        let event = match event {
            ...
            None => {
                break Err(AtlasEngineErr::Stream(
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![atlas_engine_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
            ...
            ResponseEvent::Completed {
                ...
                sess.services
                    ...
                    .track_code_mode_tool_call(
                        atlas_engine_analytics::CodeModeToolCallFact::SamplingResponseCompleted {
                            thread_id: sess.thread_id.to_string(),
                            turn_id: turn_context.sub_id.clone(),
    ...
    if cancellation_token.is_cancelled() {
        return Err(AtlasEngineErr::TurnAborted);
    }
