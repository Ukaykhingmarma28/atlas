...
use atlas_engine_config::NetworkConstraints;
use atlas_engine_config::NetworkDomainPermissionToml;
use atlas_engine_config::NetworkDomainPermissionsToml;
...
use atlas_engine_config::types::ToolSuggestDisabledTool;
use core_test_support::test_atlas_engine::TurnInputRequest as ExternalTurnInputRequest;
...
use atlas_engine_models_manager::model_info;
use atlas_engine_models_manager::test_support::construct_model_info_offline_for_tests;
use atlas_engine_models_manager::test_support::get_model_offline_for_tests;
...
use atlas_engine_protocol::config_types::ServiceTier;
use atlas_engine_protocol::config_types::TrustLevel;
use atlas_engine_protocol::exec_output::ExecToolCallOutput;
use atlas_engine_protocol::models::AgentMessageInputContent;
...
use atlas_engine_protocol::openai_models::ModelServiceTier;
...
use atlas_engine_protocol::turn_input::TurnInput as SubmittedTurnInput;
...
use atlas_engine_config::config_toml::ProjectConfig;
use atlas_engine_config::permissions_toml::FilesystemPermissionToml;
use atlas_engine_config::permissions_toml::FilesystemPermissionsToml;
use atlas_engine_config::permissions_toml::NetworkToml;
use atlas_engine_config::permissions_toml::PermissionProfileToml;
use atlas_engine_config::permissions_toml::PermissionsToml;
use atlas_engine_execpolicy::Decision;
use atlas_engine_execpolicy::NetworkRuleProtocol;
...
use atlas_engine_network_proxy::NetworkProxyConfig;
use atlas_engine_otel::MetricsClient;
use atlas_engine_otel::MetricsConfig;
...
use atlas_engine_protocol::items::HookPromptFragment;
use atlas_engine_protocol::items::build_hook_prompt_message;
use atlas_engine_protocol::models::BaseInstructions;
use atlas_engine_protocol::models::ContentItem;
use atlas_engine_protocol::models::InternalChatMessageMetadataPassthrough;
...
use atlas_engine_protocol::protocol::CreditsSnapshot;
...
use atlas_engine_protocol::protocol::NetworkApprovalProtocol;
use atlas_engine_protocol::protocol::RateLimitSnapshot;
use atlas_engine_protocol::protocol::RateLimitWindow;
use atlas_engine_protocol::protocol::RealtimeAudioFrame;
use atlas_engine_protocol::protocol::RealtimeConversationListVoicesResponseEvent;
...
use atlas_engine_protocol::protocol::ThreadRolledBackEvent;
...
use atlas_engine_protocol::mcp::CallToolResult as McpCallToolResult;
...
impl StepContext {
    pub(crate) fn for_test(turn: Arc<TurnContext>) -> Arc<Self> {
        ...
        Arc::new(Self {
            ...
            mcp: Arc::new(atlas_engine_mcp::McpBinding::empty(mcp_config_for_test(
                &turn.config,
            ))),
...
fn extension_metrics_preserve_session_metadata_tags() {
    ...
    let attributes = single_histogram_attributes(&snapshot, "atlas_agent.test.extension");
    assert_eq!(
        attributes,
        BTreeMap::from([
            (
...
async fn world_state_extension_metrics_follow_turn_model_switch() {
    ...
    impl atlas_engine_extension_api::ContextContributor for WorldStateMetricsRecorder {
        fn contribute_world_state<'a>(
            &'a self,
            input: atlas_engine_extension_api::WorldStateContributionInput<'a>,
        ) -> atlas_engine_extension_api::ExtensionFuture<
            'a,
            Vec<atlas_engine_extension_api::WorldStateSectionContribution>,
        > {
            Box::pin(async move {
                input
                    .extension_metrics
                    .expect("turn metrics should be available")
                    .histogram("atlas_agent.test.extension.turn", /*value*/ 1, &[]);
                Vec::new()
            })
        }
    }
    ...
    let snapshot = metrics.snapshot().expect("metrics snapshot");
    let attributes = single_histogram_attributes(&snapshot, "atlas_agent.test.extension.turn");
    assert_eq!(
...
async fn regular_turn_emits_turn_started_with_trace_id_without_waiting_for_startup_prewarm() {
    let _trace_test_context = install_test_tracing("atlas-engine-core-tests");
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    ...
    crate::client::ModelClient::new(
        ...
        atlas_engine_protocol::protocol::SessionSource::Exec,
        "test_originator".to_string(),
...
fn write_project_hooks(dot_atlas_engine: &Path) -> std::io::Result<()> {
    std::fs::create_dir_all(dot_atlas_engine)?;
    std::fs::write(
        dot_atlas_engine.join("hooks.json"),
        r#"{
  "hooks": {
    "SessionStart": [
      {
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        atlas_agent_home.join(atlas_engine_config::CONFIG_TOML_FILE),
...
async fn preview_session_start_hooks(
...
) -> std::io::Result<Vec<atlas_engine_protocol::protocol::HookRunSummary>> {
    ...
    Ok(
        hooks.preview_session_start(&atlas_engine_hooks::SessionStartRequest {
            session_id: thread_id,
            cwd: config.cwd.clone(),
            transcript_path: None,
            model: "gpt-5.2".to_string(),
            permission_mode: "default".to_string(),
            target: atlas_engine_hooks::StartHookTarget::SessionStart {
                source: atlas_engine_hooks::SessionStartSource::Startup,
            },
        }),
    )
}
...
async fn danger_full_access_tool_attempts_do_not_enforce_managed_network() -> anyhow::Result<()> {
    ...
    impl crate::tools::sandboxing::Approvable<TurnEnvironment> for ProbeToolRuntime {
        fn approval_action(
            ...
            Ok(crate::tools::sandboxing::ApprovalAction::Shell {
                ...
                environment_id: atlas_engine_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
                command: Vec::new(),
    ...
    impl crate::tools::sandboxing::Sandboxable for ProbeToolRuntime {
        fn sandbox_preference(&self) -> atlas_engine_sandboxing::SandboxablePreference {
            atlas_engine_sandboxing::SandboxablePreference::Auto
        }
    }
    ...
    let session = make_session_with_config(move |config| {
        ...
        requirements_toml.network = Some(atlas_engine_config::NetworkRequirementsToml {
            enabled: Some(true),
            ..Default::default()
        });
    ...
    let tool_ctx = crate::tools::sandboxing::ToolCtx {
        ...
        tool_name: atlas_engine_tools::ToolName::plain("probe"),
    };
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    let (session, _turn_context) = make_session_and_context().await;
    let atlas_agent_home = session.atlas_agent_home().await;
    std::fs::create_dir_all(&atlas_agent_home).expect("create atlas-agent home");
    let config_toml_path = atlas_agent_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
    ...
    let apps_toml = config
        ...
        .cloned()
        .expect("apps table");
    let apps = atlas_engine_config::types::AppsConfigToml::deserialize(apps_toml)
...
async fn reload_user_config_layer_updates_base_and_selected_profile_layers() {
    ...
    let atlas_agent_home = session.atlas_agent_home().await;
    std::fs::create_dir_all(&atlas_agent_home).expect("create atlas-agent home");
    let base_config_path = atlas_agent_home.join(CONFIG_TOML_FILE);
    let profile_config_path = atlas_agent_home.join("work.config.toml");
    std::fs::write(
        &base_config_path,
    ...
    assert_eq!(
        config
            ...
            .map(atlas_engine_utils_absolute_path::AbsolutePathBuf::as_path),
...
async fn reload_user_config_layer_refreshes_hooks() -> anyhow::Result<()> {
    let session = make_session_with_config(|config| {
        config
            ...
            .expect("enable Atlas Agent hooks");
    })
    .await?;
    let atlas_agent_home = session.atlas_agent_home().await;
    std::fs::create_dir_all(&atlas_agent_home)?;
    let config_toml_path = atlas_agent_home.join(CONFIG_TOML_FILE);
    let user_config: atlas_engine_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
    ...
    let config = session.get_config().await;
    let hook_list = atlas_engine_hooks::list_hooks(atlas_engine_hooks::HooksConfig {
        feature_enabled: true,
...
async fn refresh_runtime_config_refreshes_hooks() -> anyhow::Result<()> {
    ...
    struct NormalizedHookIdentity {
        ...
        #[serde(flatten)]
        group: atlas_engine_config::MatcherGroup,
    }
    let trusted_hash = {
        let identity = NormalizedHookIdentity {
            ...
            group: atlas_engine_config::MatcherGroup {
                matcher: None,
                hooks: vec![atlas_engine_config::HookHandlerConfig::Command {
        ...
        let identity = atlas_engine_config::TomlValue::try_from(identity)?;
...
async fn refresh_mcp_config_replaces_managed_server_and_plugin_requirements() {
    ...
    let requirement = serde_json::from_value::<atlas_engine_config::McpServerRequirement>(json!({
        "identity": { "url": "https://example.com/mcp" }
    }))
    .expect("valid managed MCP requirement");
    let plugin_requirements = std::collections::BTreeMap::from([(
        "example-plugin".to_string(),
        atlas_engine_config::PluginRequirementsToml {
    ...
    next_config.mcp_servers = atlas_engine_config::Constrained::normalized(
...
async fn subagent_activity_emits_matching_start_and_completion() {
    let (session, turn_context, rx) = make_session_and_context_with_rx().await;
    let item = atlas_engine_protocol::items::SubAgentActivityItem {
        id: "activity-1".to_string(),
        kind: atlas_engine_protocol::protocol::SubAgentActivityKind::Started,
...
async fn record_token_usage_info_notifies_extension_contributors() {
    ...
    impl atlas_engine_extension_api::TokenUsageContributor for TokenUsageRecorder {
        fn on_token_usage<'a>(
            &'a self,
            session_store: &'a atlas_engine_extension_api::ExtensionData,
            thread_store: &'a atlas_engine_extension_api::ExtensionData,
            turn_store: &'a atlas_engine_extension_api::ExtensionData,
...
async fn turn_start_lifecycle_exposes_turn_metadata_and_token_baseline() {
    ...
    impl atlas_engine_extension_api::TurnLifecycleContributor for TurnStartRecorder {
        fn on_turn_start<'a>(
            &'a self,
            input: atlas_engine_extension_api::TurnStartInput<'a>,
        ) -> atlas_engine_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_error_lifecycle_exposes_error_and_stores() {
    ...
    impl atlas_engine_extension_api::TurnLifecycleContributor for TurnErrorRecorder {
        fn on_turn_error<'a>(
            &'a self,
            input: atlas_engine_extension_api::TurnErrorInput<'a>,
        ) -> atlas_engine_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
    ...
    let expected = RecordedTurnError {
        ...
        turn_level_id: turn_context.sub_id.clone(),
        turn_id: turn_context.sub_id.clone(),
        error: AtlasEngineErrorInfo::UsageLimitExceeded,
        saw_session_store: true,
        saw_thread_store: true,
    };

    session
        .emit_turn_error_lifecycle(&turn_context, AtlasEngineErrorInfo::UsageLimitExceeded)
        .await;
...
async fn config_change_contributor_observes_effective_config_changes() {
    ...
    impl atlas_engine_extension_api::ConfigContributor<crate::config::Config> for ConfigRecorder {
        fn on_config_changed(
            &self,
            session_store: &atlas_engine_extension_api::ExtensionData,
            thread_store: &atlas_engine_extension_api::ExtensionData,
...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let sandbox_policy = SandboxPolicy::ExternalSandbox {
        network_access: atlas_engine_protocol::protocol::NetworkAccess::Restricted,
    };
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_atlas_engine().with_config(|config| {
        config.permissions.approval_policy =
            atlas_engine_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    fork_config.permissions.approval_policy =
        atlas_engine_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "atlas_engine_core__atlas_engine_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let previous_context_item = TurnContextItem {
        ...
        summary: atlas_engine_protocol::config_types::ReasoningSummary::Auto,
    };
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            atlas_engine_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            atlas_engine_protocol::protocol::UserMessageEvent {
                client_id: None,
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            atlas_engine_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn thread_rollback_fails_without_persisted_thread_history() {
    ...
    assert_eq!(
        error_event.atlas_engine_error_info,
...
async fn set_rate_limits_retains_previous_credits() {
    let atlas_agent_home = tempfile::tempdir().expect("create temp dir");
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(atlas_engine_protocol::account::PlanType::Plus),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(initial.clone());

    let update = RateLimitSnapshot {
        limit_id: Some("atlas_engine_other".to_string()),
        limit_name: Some("atlas_engine_other".to_string()),
        primary: Some(RateLimitWindow {
            used_percent: 40.0,
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        individual_limit: None,
        spend_control_reached: None,
        plan_type: Some(atlas_engine_protocol::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
    ...
    assert_eq!(
        state.latest_rate_limits,
        Some(RateLimitSnapshot {
            limit_id: Some("atlas-agent".to_string()),
            limit_name: None,
...
async fn build_test_config(atlas_agent_home: &Path) -> Config {
...
async fn emit_subagent_session_started_includes_fork_lineage_and_originator() {
    ...
    emit_subagent_session_started(
        ...
        AppServerClientMetadata {
            client_name: Some("atlas-engine-tui".to_string()),
            ...
            if let Some(requests) = server.received_requests().await {
                for request in requests {
                    ...
                    if let Some(event) = payload["events"].as_array().and_then(|events| {
                        events
                            .iter()
                            .find(|event| event["event_type"] == "atlas_engine_thread_initialized")
                    }) {
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let atlas_agent_home = parent_config.atlas_agent_home.clone();
    let skill_dir = atlas_agent_home.join("skills").join("demo");
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&atlas_engine_exec_server::LOCAL_FS));
    ...
    let role_path = atlas_agent_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_new_fails_when_zsh_fork_enabled_without_packaged_zsh() {
    ...
    let result = Session::new(
        ...
        plugins_manager,
        mcp_manager,
        Arc::new(atlas_engine_code_mode::DisabledCodeModeSessionProvider),
        Arc::new(atlas_engine_extension_api::ExtensionRegistryBuilder::new().build()),
        atlas_engine_extension_api::ExtensionDataInit::default(),
        ClientMcpExtensions::default(),
        AgentControl::default(),
        environment_manager,
        /*inherited_environments*/ None,
        /*analytics_events_client*/ None,
        Arc::new(atlas_engine_thread_store::LocalThreadStore::new(
            atlas_engine_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        atlas_engine_rollout_trace::ThreadTraceContext::disabled(),
        /*attestation_provider*/ None,
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let services = SessionServices {
        ...
        rollout_thread_trace: atlas_engine_rollout_trace::ThreadTraceContext::disabled(),
        ...
        plugins_manager,
        mcp_manager,
        extensions: Arc::new(atlas_engine_extension_api::ExtensionRegistryBuilder::new().build()),
        ...
        state_db: None,
        live_thread: None,
        thread_store: Arc::new(atlas_engine_thread_store::LocalThreadStore::new(
            atlas_engine_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        attestation_provider: None,
...
async fn make_session_with_history_source_and_agent_control_and_rx(
    ...
    let session = Session::new(
        ...
        Arc::new(atlas_engine_thread_store::LocalThreadStore::new(
            atlas_engine_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            Some(
                atlas_engine_state::StateRuntime::init(
                    config.sqlite.clone(),
...
async fn notify_request_permissions_response_ignores_unmatched_call_id() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        session
            .granted_turn_permissions(atlas_engine_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
        None
    );
}
    ...
    *session.active_turn.lock().await = Some(current_active_turn);
    ...
    assert_eq!(
        originating_turn_state
            .lock()
            .await
            .granted_permissions(atlas_engine_exec_server::LOCAL_ENVIRONMENT_ID),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
        ...
        async move {
            ...
            session
                .request_permissions_for_environment(
                    turn_context.as_ref(),
                    call_id,
                    atlas_engine_protocol::request_permissions::RequestPermissionsArgs {
                        environment_id: None,
    ...
    assert_eq!(
        ...
        Some(atlas_engine_exec_server::LOCAL_ENVIRONMENT_ID)
    );
    #[allow(deprecated)]
    let turn_cwd = turn_context.cwd.clone();
        ...
        async move {
            handler
                .handle(ToolInvocation {
                    ...
                    tracker,
                    call_id,
                    tool_name: atlas_engine_tools::ToolName::plain("request_permissions"),
...
async fn request_permissions_response_materializes_session_cwd_grants_before_recording() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        session
            .granted_session_permissions(atlas_engine_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
...
async fn primary_environment_uses_first_turn_environment() {
    ...
    let second_cwd_uri = atlas_engine_utils_path_uri::PathUri::from_abs_path(&second_cwd);
    turn_context
        .environments
        .environments
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(atlas_engine_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn atlas_engine_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
    ...
    result_sender
        .try_send(
            serde_json::from_value::<atlas_engine_protocol::protocol::HookCompletedEvent>(json!({
                "turn_id": "turn-1",
                "run": {
    ...
    assert_eq!(
        atlas_engine_thread_store::InMemoryThreadStoreCalls {
            create_thread: 1,
            shutdown_thread: 1,
            ..Default::default()
        },
...
async fn submission_loop_channel_close_aborts_active_turn_before_thread_stop_lifecycle() {
    ...
    impl atlas_engine_extension_api::TurnLifecycleContributor for LifecycleRecorder {
        fn on_turn_abort<'a>(
            &'a self,
            input: atlas_engine_extension_api::TurnAbortInput<'a>,
        ) -> atlas_engine_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
{
    let (tx_event, rx_event) = async_channel::unbounded();
    let mut config = build_test_config(atlas_agent_home).await;
...
async fn refresh_mcp_servers_uses_latest_state_for_existing_turns() {
    ...
    .expect("parse refreshed MCP servers");
    {
        ...
        config.mcp_oauth_credentials_store_mode =
            atlas_engine_config::types::OAuthCredentialsStoreMode::Auto;
        config
            .features
    ...
    let configured_servers = atlas_engine_mcp::configured_mcp_servers(new_step.mcp.config());
    assert_eq!(
        configured_servers.get("refreshed"),
        refreshed_mcp_servers.get("refreshed")
    );
    assert!(
        !atlas_engine_mcp::configured_mcp_servers(old_step.mcp.config()).contains_key("refreshed"),
    ...
    assert!(
        atlas_engine_mcp::configured_mcp_servers(current.config()).contains_key("refreshed"),
...
async fn mcp_elicitation_reviewer_uses_latest_runtime_authority() {
    ...
    let request = atlas_engine_mcp::ElicitationReviewRequest {
        server_name: "browser-use".to_string(),
        request_id: rmcp::model::NumberOrString::Number(7),
        elicitation: atlas_engine_rmcp_client::Elicitation::Mcp(
...
async fn mcp_refresh_updates_plugin_auth_mode_before_checking_pending_state() {
    let atlas_agent_home = tempfile::tempdir().expect("create auth test directory");
    let (mut session, _turn_context) = make_session_and_context().await;
    session.services.auth_manager = AuthManager::from_auth_for_testing_with_home(
        AtlasEngineAuth::from_api_key("old-api-key"),
    ...
    atlas_engine_login::login_with_api_key(
        atlas_agent_home.path(),
        "new-api-key",
        atlas_engine_login::AuthCredentialsStoreMode::File,
        atlas_engine_login::AuthKeyringBackendKind::default(),
    )
...
impl atlas_engine_exec_server::NoiseRendezvousConnectProvider for PendingNoiseConnectProvider {
    fn connect_bundle(
        &self,
...
async fn conflicting_ready_environment_root_ids_keep_first_location() {
    ...
    for selected_root in &selected_roots {
        let atlas_engine_protocol::capabilities::CapabilityRootLocation::Environment {
            environment_id,
            ..
        ...
        environment_manager
            .report_environment_provisioning_status(
                ...
                Ok(atlas_engine_exec_server::EnvironmentReadyInfo {
...
async fn step_context_keeps_its_mcp_runtime_for_tools() -> anyhow::Result<()> {
    ...
    let next_step = session
        ...
        .await
        .expect("a fresh cancellation token cannot be cancelled");
    assert!(atlas_engine_mcp::configured_mcp_servers(next_step.mcp.config()).contains_key("newer"));
    ...
    let current = session
        ...
        .await
        .expect("refreshed runtime should be available");
    assert!(atlas_engine_mcp::configured_mcp_servers(current.config()).contains_key("newer"));
...
impl atlas_engine_extension_api::ContextContributor for PromptExtensionTestContributor {
...
-> Arc<atlas_engine_extension_api::ExtensionRegistry<crate::config::Config>> {
    let mut builder = atlas_engine_extension_api::ExtensionRegistryBuilder::new();
    builder.prompt_contributor(Arc::new(PromptExtensionTestContributor));
    Arc::new(builder.build())
}

impl atlas_engine_extension_api::ContextContributor for TurnContextExtensionTestContributor {
...
async fn attach_in_memory_thread_store(
    session: &mut Session,
) -> Arc<atlas_engine_thread_store::InMemoryThreadStore> {
    let store = Arc::new(atlas_engine_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn atlas_engine_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
...
async fn wait_for_flush_count(
    store: &atlas_engine_thread_store::InMemoryThreadStore,
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    let text_element = atlas_engine_protocol::user_input::TextElement::new(
        atlas_engine_protocol::user_input::ByteRange { start: 5, end: 12 },
...
async fn thread_idle_lifecycle_waits_for_trigger_turn_mailbox_work() {
    ...
    session
        .emit_thread_idle_lifecycle_if_idle(atlas_engine_extension_api::ThreadIdleCause::Completed)
        .await;
...
async fn rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    use crate::turn_diff_tracker::TurnDiffTracker;
    use atlas_engine_protocol::protocol::AskForApproval;
    use atlas_engine_tools::ShellCommandBackendConfig;
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            call_id,
            tool_name: atlas_engine_tools::ToolName::plain(tool_name),
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: atlas_engine_tools::ToolName::plain("exec_command"),
            source: crate::tools::context::ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "cmd": "echo hi",
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let atlas_agent_home = temp.path().join("home");
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let root_dot_atlas_engine = project_root.join(".atlas-agent");
    let nested_dot_atlas_engine = nested.join(".atlas-agent");

    std::fs::create_dir_all(&atlas_agent_home)?;
    std::fs::create_dir_all(&nested_dot_atlas_engine)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&root_dot_atlas_engine)?;
    write_project_hooks(&nested_dot_atlas_engine)?;
    write_project_trust_config(&atlas_agent_home, &[(&nested, TrustLevel::Trusted)]).await?;

    let config = ConfigBuilder::default()
        .atlas_agent_home(atlas_agent_home)
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let dot_atlas_engine = project_root.join(".atlas-agent");
    std::fs::create_dir_all(&nested)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&dot_atlas_engine)?;
    ...
    for (name, trust_entries, expected_hooks) in cases {
        let atlas_agent_home = temp.path().join(format!("home_{name}"));
        std::fs::create_dir_all(&atlas_agent_home)?;
        write_project_trust_config(&atlas_agent_home, &trust_entries).await?;
