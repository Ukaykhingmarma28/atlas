...
use atlas_engine_core_plugins::ResolvedPluginMetricsOperation;
use atlas_engine_core_plugins::TrustedPluginRoots;
...
pub struct TurnContext {
    ...
    pub(crate) extension_data: Arc<atlas_engine_extension_api::ExtensionData>,
...
impl TurnContext {
    ...
    pub(crate) fn apps_enabled(&self) -> bool {
        let uses_atlas_engine_backend = self
            .auth_manager
            .as_deref()
            .is_some_and(AuthManager::current_auth_uses_atlas_engine_backend);
        self.config
            .features
            .apps_enabled_for_auth(uses_atlas_engine_backend)
...
impl Session {
    ...
    pub(crate) async fn new_turn_with_sub_id(
    ...
    ) -> AtlasEngineResult<Arc<TurnContext>> {
        let notify_config_contributors = !self.services.extensions.config_contributors().is_empty();
        let update_result: AtlasEngineResult<_> = {
            ...
            match self.apply_session_settings(&state.session_configuration, &updates) {
                ...
                Err(err) => Err(AtlasEngineErr::InvalidRequest(err.to_string())),
            }
        };
        ...
        let (
            ...
            Err(err) => {
                ...
                .await;
                return Err(AtlasEngineErr::InvalidRequest(message));
            }
        };
    ...
    async fn new_turn_context_from_configuration(
        ...
        let trusted_plugin_roots = TrustedPluginRoots::from_plugin_load_outcome(
            &plugin_outcome,
            per_turn_config.atlas_agent_home.as_path(),
        );
    ...
    pub(crate) async fn maybe_emit_model_warnings_for_turn(&self, tc: &TurnContext) {
        ...
        if !tc.code_mode_available
            && matches!(
                ...
                atlas_engine_protocol::openai_models::ToolMode::CodeMode
                    | atlas_engine_protocol::openai_models::ToolMode::CodeModeOnly
            )
