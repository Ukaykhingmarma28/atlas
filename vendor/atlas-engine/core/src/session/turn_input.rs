...
use atlas_engine_protocol::protocol::NonSteerableTurnKind;
use atlas_engine_protocol::protocol::ThreadSettingsOverrides;
use atlas_engine_protocol::turn_input::NotSubmittedReason;
    ...
    async fn prepare(
        ...
        let thread_settings_update = if thread_settings == ThreadSettingsOverrides::default() {
            ...
            session
                ...
                .await
                .map_err(|error| AtlasEngineErr::InvalidRequest(error.to_string()))?;
            Some(updates)
        };
        Ok(Self {
