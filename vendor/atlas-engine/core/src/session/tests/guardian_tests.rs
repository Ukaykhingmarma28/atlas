...
use core_test_support::atlas_engine_linux_sandbox_exe_or_skip;
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            ...
            if matches!(
                event.msg,
                atlas_engine_protocol::protocol::EventMsg::GuardianAssessment(_)
            ) {
                break;
            }
        }
...
async fn guardian_allows_shell_command_additional_permissions_requests_past_policy_validation() {
    ...
    config
        .permissions
        .set_permission_profile(atlas_engine_protocol::models::PermissionProfile::Disabled)
    ...
    config.atlas_engine_linux_sandbox_exe = atlas_engine_linux_sandbox_exe_or_skip!();
    config
        .features
        .enable(Feature::GuardianApproval)
        .expect("test setup should allow enabling guardian approvals");
    ...
    let handler = crate::tools::handlers::ShellCommandHandler::from(
        atlas_engine_tools::ShellCommandBackendConfig::Classic,
    );
    #[allow(deprecated)]
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: atlas_engine_tools::ToolName::plain("shell_command"),
...
async fn guardian_subagent_does_not_inherit_parent_exec_policy_rules() {
    ...
    config.config_layer_stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_atlas_agent_folder: project_dir.path().abs(),
            },
    ...
    let thread_store = Arc::new(atlas_engine_thread_store::LocalThreadStore::new(
        atlas_engine_thread_store::LocalThreadStoreConfig::from_config(&config),
        /*state_db*/ None,
    ));
    ...
    let (session, io) = Session::spawn(SessionSpawnArgs {
        ...
        parent_rollout_thread_trace: atlas_engine_rollout_trace::ThreadTraceContext::disabled(),
        user_shell_override: None,
        parent_trace: None,
        environment_selections: Vec::new(),
        thread_extension_init: atlas_engine_extension_api::ExtensionDataInit::default(),
