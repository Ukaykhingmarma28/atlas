...
use atlas_engine_mcp::McpPermissionPromptAutoApproveContext;
use atlas_engine_mcp::PreparedMcpCall;
use atlas_engine_mcp::SandboxState;
use atlas_engine_mcp::ToolInfo;
use atlas_engine_mcp::auth_elicitation_completed_result;
use atlas_engine_mcp::build_auth_elicitation_plan;
use atlas_engine_mcp::mcp_permission_prompt_is_auto_approved;
use atlas_engine_protocol::approvals::ElicitationRequest;
use atlas_engine_protocol::items::McpToolCallError;
use atlas_engine_protocol::items::McpToolCallItem;
use atlas_engine_protocol::items::McpToolCallStatus;
use atlas_engine_protocol::items::TurnItem;
use atlas_engine_protocol::mcp::CallToolResult;
use atlas_engine_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_TOOL_APPROVAL_KIND_KEY;
use atlas_engine_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_TOOL_APPROVAL_KIND_MCP_TOOL_CALL;
use atlas_engine_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_CONNECTOR_DESCRIPTION_KEY;
use atlas_engine_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_TOOL_APPROVAL_CONNECTOR_ID_KEY;
use atlas_engine_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_TOOL_APPROVAL_CONNECTOR_NAME_KEY;
use atlas_engine_protocol::mcp_approval_meta::PERSIST_ALWAYS as MCP_TOOL_APPROVAL_PERSIST_ALWAYS;
use atlas_engine_protocol::mcp_approval_meta::PERSIST_KEY as MCP_TOOL_APPROVAL_PERSIST_KEY;
use atlas_engine_protocol::mcp_approval_meta::PERSIST_SESSION as MCP_TOOL_APPROVAL_PERSIST_SESSION;
use atlas_engine_protocol::mcp_approval_meta::SOURCE_CONNECTOR as MCP_TOOL_APPROVAL_SOURCE_CONNECTOR;
use atlas_engine_protocol::mcp_approval_meta::SOURCE_KEY as MCP_TOOL_APPROVAL_SOURCE_KEY;
use atlas_engine_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_TOOL_DESCRIPTION_KEY;
use atlas_engine_protocol::mcp_approval_meta::TOOL_PARAMS_DISPLAY_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_DISPLAY_KEY;
use atlas_engine_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_KEY;
use atlas_engine_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_TOOL_APPROVAL_TOOL_TITLE_KEY;
...
use atlas_engine_protocol::request_user_input::RequestUserInputQuestion;
use atlas_engine_protocol::request_user_input::RequestUserInputQuestionOption;
...
const MCP_RESULT_TELEMETRY_META_KEY: &str = "atlas-agent/telemetry";
...
const MCP_RESULT_TELEMETRY_TARGET_ID_SPAN_ATTR: &str = "atlas_agent.mcp.target.id";
const MCP_RESULT_TELEMETRY_SERVER_USER_FLOW_SPAN_ATTR: &str =
    "atlas_agent.mcp.server_user_flow.triggered";
...
pub(crate) async fn handle_mcp_tool_call(
    ...
    let app_tool_policy = if server == ATLAS_APPS_MCP_SERVER_NAME {
    ...
    let approval_mode = if server == ATLAS_APPS_MCP_SERVER_NAME {
    ...
    if server == ATLAS_APPS_MCP_SERVER_NAME && !app_tool_policy.enabled {
        let result = notify_mcp_tool_call_skip(
            sess.as_ref(),
            turn_context.as_ref(),
            &call_id,
...
impl McpToolCallItemMetadata {
    fn from_tool_metadata(server: &str, metadata: Option<&McpToolApprovalMetadata>) -> Self {
        let trusted_mcp_app_metadata = if server == ATLAS_APPS_MCP_SERVER_NAME {
            metadata
        } else {
            None
        };
        Self {
            ...
            action_name: trusted_mcp_app_metadata
                .and_then(|metadata| metadata.atlas_apps_meta.as_ref())
...
async fn handle_approved_mcp_tool_call(
    ...
    let result = async {
        let result = async {
            ...
            Ok(maybe_request_atlas_apps_auth_elicitation(
                sess,
                turn_context,
    ...
    .await;
    maybe_track_atlas_engine_app_used(sess, turn_context, &server, &metadata).await;
...
fn mcp_tool_call_span(
    ...
    let span = tracing::info_span!(
        ...
        server.port = Empty,
        atlas_engine.mcp.target.id = Empty,
        atlas_engine.mcp.server_user_flow.triggered = Empty,
        error.type = Empty,
        atlas_engine.mcp.error.code = Empty,
    );
...
async fn maybe_request_atlas_apps_auth_elicitation(
    sess: &Arc<Session>,
    turn_context: &TurnContext,
    ...
    if server != ATLAS_APPS_MCP_SERVER_NAME {
        return result;
    }
    ...
    let install_url = connector_id.map(|connector_id| {
        atlas_engine_connectors::metadata::connector_install_url(
    ...
    refresh_atlas_apps_after_connector_auth(sess, turn_context).await;
    auth_elicitation_completed_result(&plan.auth_failure, result.meta)
}

async fn refresh_atlas_apps_after_connector_auth(sess: &Arc<Session>, turn_context: &TurnContext) {
    let mcp_tools_result = sess.hard_refresh_latest_atlas_apps_tools().await;
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    match meta.as_mut() {
        Some(serde_json::Value::Object(map)) => {
            map.insert(
                atlas_engine_mcp::MCP_SANDBOX_STATE_META_CAPABILITY.to_string(),
                sandbox_state,
            );
        }
        Some(_) => {}
...
fn sandbox_cwd_for_mcp_server(step_context: &StepContext, environment_id: &str) -> Option<PathUri> {
    ...
    if environment_id == atlas_engine_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID {
        #[allow(deprecated)]
...
async fn maybe_track_atlas_engine_app_used(
    sess: &Session,
    turn_context: &TurnContext,
    server: &str,
    metadata: &McpToolApprovalMetadata,
...
pub(crate) struct McpToolApprovalMetadata {
    ...
    mcp_app_resource_uri: Option<String>,
    atlas_apps_meta: Option<serde_json::Map<String, serde_json::Value>>,
...
impl Session {
    async fn register_mcp_tool_approval_metadata(
        ...
        turn_state.lock().await.insert_mcp_tool_approval_metadata(
            call_id.to_string(),
            (invocation.server == ATLAS_APPS_MCP_SERVER_NAME).then(|| invocation.clone()),
            metadata,
        );
    }
...
fn build_mcp_tool_call_request_meta(
    ...
    {
        request_meta.insert(
            crate::X_ATLAS_AGENT_TURN_METADATA_HEADER.to_string(),
            turn_metadata,
        );
    }

    if server == ATLAS_APPS_MCP_SERVER_NAME {
        let mut atlas_apps_meta = metadata
            .and_then(|metadata| metadata.atlas_apps_meta.clone())
            .unwrap_or_default();
        atlas_apps_meta.insert(
            "call_id".to_string(),
            serde_json::Value::String(call_id.to_string()),
        );
        request_meta.insert(
            MCP_TOOL_ATLAS_APPS_META_KEY.to_string(),
            serde_json::Value::Object(atlas_apps_meta),
        );
    }
...
async fn maybe_request_mcp_tool_approval(
    ...
    hook_tool_name: &HookToolName,
    metadata: &McpToolApprovalMetadata,
    config: &atlas_engine_mcp::McpConfig,
    policy: McpToolApprovalPolicy,
    ...
    let action = ApprovalAction::McpToolCall {
        ...
        connected_account_email: (invocation.server == ATLAS_APPS_MCP_SERVER_NAME)
    ...
    Some(
        match sess.request_approval(action, approval_context).await {
            ...
            Err(ToolError::AtlasEngine(_)) => ReviewDecision::Abort,
        },
    )
}
...
fn session_mcp_tool_approval_key(
    ...
    if invocation.server == ATLAS_APPS_MCP_SERVER_NAME && connector_id.is_none() {
        return None;
    }
...
fn mcp_tool_metadata(prepared_call: &PreparedMcpCall) -> McpToolApprovalMetadata {
    ...
    let connector_description = (server == ATLAS_APPS_MCP_SERVER_NAME)
        .then(|| tool_info.namespace_description.clone())
        .flatten();

    let atlas_apps_meta = tool_info
        .tool
        .meta
        .as_ref()
        ...
        .cloned();
    let connected_account_email = if server == ATLAS_APPS_MCP_SERVER_NAME {
        atlas_apps_meta
            .as_ref()
...
fn openai_file_input_optional_fields_for_server(
    ...
    (server == ATLAS_APPS_MCP_SERVER_NAME)
...
async fn maybe_persist_mcp_tool_approval(
    ...
    let persist_result = if key.server == ATLAS_APPS_MCP_SERVER_NAME {
...
async fn persist_atlas_engine_app_tool_approval(
    config: &Config,
    connector_id: &str,
    tool_name: &str,
) -> anyhow::Result<()> {
