...
use atlas_engine_analytics::AtlasEngineCompactionEvent;
...
use atlas_engine_protocol::items::ContextCompactionItem;
...
use atlas_engine_utils_output_truncation::approx_token_count;
use atlas_engine_utils_output_truncation::truncate_text;
use futures::prelude::*;
use tracing::error;

pub use atlas_engine_prompts::SUMMARIZATION_PROMPT;
pub use atlas_engine_prompts::SUMMARY_PREFIX;
...
async fn run_compact_task_inner(
    ...
    match pre_compact_outcome {
        ...
        PreCompactHookOutcome::Stopped => {
            let error = AtlasEngineErr::TurnAborted;
            attempt
                .track(
                    sess.as_ref(),
    ...
    let atlas_engine_error = result.as_ref().err();
    if result.is_ok() {
...
async fn run_compact_task_inner_impl(
    ...
    let responses_metadata = turn_context.turn_metadata_state.to_responses_metadata(
        ...
        window_id,
        AtlasEngineResponsesRequestKind::Compaction(compaction_metadata),
    );
    ...
    loop {
        ...
        match attempt_result {
            ...
            Err(err)
                if matches!(
                    err.details(),
                    AtlasEngineErrorDetails::Interrupted | AtlasEngineErrorDetails::TurnAborted
                ) =>
            {
                return Err(err);
            }
            Err(e) if matches!(e.details(), AtlasEngineErrorDetails::SessionBudgetExceeded) => {
                sess.track_turn_atlas_engine_error(turn_context.as_ref(), &e);
                let event = EventMsg::Error(e.to_error_event(/*message_prefix*/ None));
                sess.send_event(&turn_context, event).await;
                return Err(e);
            }
            Err(e) if matches!(e.details(), AtlasEngineErrorDetails::ContextWindowExceeded) => {
                if turn_input_len > 1 {
...
impl CompactionAnalyticsAttempt {
    ...
    pub(crate) async fn track(
        ...
        atlas_engine_error: Option<&AtlasEngineErr>,
        ...
        sess.services
            ...
            .track_compaction(AtlasEngineCompactionEvent {
                ...
                status,
                atlas_engine_error_kind: atlas_engine_error.map(Into::into),
                atlas_engine_error_http_status_code: atlas_engine_error
                    .and_then(AtlasEngineErr::http_status_code_value),
                active_context_tokens_before,
...
pub(crate) fn compaction_status_from_result<T>(result: &AtlasEngineResult<T>) -> CompactionStatus {
    match result {
...
pub(crate) struct CompactedUserMessage {
    ...
    harness_metadata: Option<AtlasEngineHarnessMetadata>,
}
...
async fn drain_to_completed(
    ...
    loop {
        ...
        let Some(event) = maybe_event else {
            return Err(AtlasEngineErr::Stream(
