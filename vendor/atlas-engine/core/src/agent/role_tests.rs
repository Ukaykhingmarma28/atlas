...
async fn test_config_with_cli_overrides(
    ...
    let config = ConfigBuilder::default()
        .atlas_agent_home(home_path.clone())
...
async fn apply_role_preserves_unspecified_keys() {
    ...
    .await;
    config.atlas_engine_linux_sandbox_exe = Some(PathBuf::from("/tmp/atlas-engine-linux-sandbox"));
    config.main_execve_wrapper_exe = Some(PathBuf::from("/tmp/atlas-engine-execve-wrapper"));
    let role_path = write_role_config(
        &home,
    ...
    assert_eq!(
        ...
        Some(PathBuf::from("/tmp/atlas-engine-linux-sandbox"))
    );
    assert_eq!(
        config.main_execve_wrapper_exe,
        Some(PathBuf::from("/tmp/atlas-engine-execve-wrapper"))
    );
...
"#,
    ...
    let snapshot = skills_service
        .snapshot_for_config(
            &skills_input,
            Some(Arc::clone(&atlas_engine_exec_server::LOCAL_FS)),
        )
        .await;
