...
pub(crate) struct AgentControl {
    ...
    /// `ThreadManagerState -> AtlasEngineThread -> Session -> SessionServices -> ThreadManagerState`.
...
impl AgentControl {
    ...
    pub(crate) async fn interrupt_agent(&self, agent_id: ThreadId) -> AtlasEngineResult<String> {
        let state = self.upgrade()?;
    ...
    async fn handle_thread_request_result(
        ...
        agent_id: ThreadId,
        state: &Arc<ThreadManagerState>,
        result: AtlasEngineResult<String>,
    ) -> AtlasEngineResult<String> {
        if result
            .as_ref()
            .is_err_and(|err| matches!(err.details(), AtlasEngineErrorDetails::InternalAgentDied))
        {
    ...
    pub(crate) async fn resolve_agent_reference(
        ...
        Err(AtlasEngineErr::UnsupportedOperation(format!(
    ...
    pub(crate) async fn subscribe_status(
        &self,
        agent_id: ThreadId,
    ) -> AtlasEngineResult<watch::Receiver<AgentStatus>> {
        let state = self.upgrade()?;
        let thread = state.get_thread(agent_id).await?;
    ...
    ) -> AtlasEngineResult<Vec<ListedAgent>> {
        let state = self.upgrade()?;
    ...
    fn prepare_thread_spawn(
        ...
        agent_role: Option<String>,
        preferred_agent_nickname: Option<String>,
    ) -> AtlasEngineResult<(SessionSource, AgentMetadata)> {
        if depth == 1 {
    ...
    async fn open_thread_spawn_children(
        &self,
        parent_thread_id: ThreadId,
    ) -> AtlasEngineResult<Vec<(ThreadId, AgentMetadata)>> {
    ...
    async fn live_thread_spawn_children(
        &self,
    ) -> AtlasEngineResult<HashMap<ThreadId, Vec<(ThreadId, AgentMetadata)>>> {
        let state = self.upgrade()?;
    ...
    async fn persist_thread_spawn_edge_for_source(
        &self,
        child_thread: &crate::AtlasEngineThread,
        child_thread_id: ThreadId,
        session_source: Option<&SessionSource>,
    ) {
        ...
        if let Err(err) = agent_graph_store
            .upsert_thread_spawn_edge(
                parent_thread_id,
                child_thread_id,
                atlas_engine_agent_graph_store::ThreadSpawnEdgeStatus::Open,
            )
            .await
        {
