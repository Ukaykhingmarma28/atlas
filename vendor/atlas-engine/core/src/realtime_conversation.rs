...
use atlas_engine_api::RealtimeContextAppendChannel;
use atlas_engine_api::RealtimeEvent;
use atlas_engine_api::RealtimeEventParser;
use atlas_engine_api::RealtimeSessionConfig;
use atlas_engine_api::RealtimeSessionMode;
use atlas_engine_api::RealtimeWebsocketClient;
use atlas_engine_api::RealtimeWebsocketEvents;
use atlas_engine_api::RealtimeWebsocketWriter;
...
use atlas_engine_login::default_client::default_headers;
use atlas_engine_login::read_openai_api_key_from_env;
...
use atlas_engine_protocol::protocol::RealtimeConversationClosedEvent;
use atlas_engine_protocol::protocol::RealtimeConversationRealtimeEvent;
use atlas_engine_protocol::protocol::RealtimeConversationSdpEvent;
use atlas_engine_protocol::protocol::RealtimeConversationStartedEvent;
use atlas_engine_protocol::protocol::RealtimeHandoffRequested;
use atlas_engine_protocol::protocol::RealtimeOutputModality;
use atlas_engine_protocol::protocol::RealtimeTranscriptEntry;
...
const STANDALONE_HANDOFF_ID: &str = "atlas-agent";
...
struct RealtimeHandoffState {
    ...
    atlas_engine_responses_as_items: bool,
    atlas_engine_response_item_prefix: Option<String>,
    atlas_engine_response_handoff_mode: AtlasEngineResponseHandoffMode,
    atlas_engine_response_handoff_channel_prefixes: Arc<BTreeMap<String, Vec<String>>>,
...
impl RealtimeHandoffState {
    fn streams_handoff_append(&self) -> bool {
        self.event_parser == RealtimeEventParser::FramelessBidi
            ...
            && !self.atlas_engine_responses_as_items
    }

    fn routes_handoff_by_bem(&self) -> bool {
        self.event_parser == RealtimeEventParser::FramelessBidi
            && self.atlas_engine_response_handoff_mode == AtlasEngineResponseHandoffMode::BemTags
    }
}
...
struct RealtimeStart {
    ...
    atlas_engine_response_item_prefix: Option<String>,
    atlas_engine_response_handoff_mode: AtlasEngineResponseHandoffMode,
    atlas_engine_response_handoff_channel_prefixes: Option<BTreeMap<String, Vec<String>>>,
...
impl RealtimeConversationManager {
    ...
    ) -> AtlasEngineResult<RealtimeStartOutput> {
        let previous_state = {
    ...
    async fn start_inner(&self, start: RealtimeStart) -> AtlasEngineResult<RealtimeStartOutput> {
        let RealtimeStart {
            api_provider,
            ...
            atlas_engine_response_item_prefix,
        ...
        let handoff = RealtimeHandoffState {
            ...
            atlas_engine_response_item_prefix,
            atlas_engine_response_handoff_mode,
            atlas_engine_response_handoff_channel_prefixes: Arc::new(
                atlas_engine_response_handoff_channel_prefixes.unwrap_or_default(),
            ),
            session_kind,
            event_parser,
        };
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> AtlasEngineResult<()> {
        let sender = {
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(AtlasEngineErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }
    ...
    pub(crate) async fn handoff_out(
        ...
        let phase = if handoff.routes_handoff_by_bem() {
            match bem_message_phase(
                &output_text,
                &handoff.atlas_engine_response_handoff_channel_prefixes,
            ) {
        ...
        let output = match active_handoff {
            Some(handoff_id) => {
                ...
                *handoff.last_output.lock().await = Some(RealtimeHandoffOutput {
                ...
                if handoff.atlas_engine_responses_as_items {
                    RealtimeOutbound::ConversationItem {
                        text: realtime_backend_item(
                            output_text,
                            handoff.atlas_engine_response_item_prefix.as_deref(),
                        ),
                        phase,
                    }
    ...
    pub(crate) async fn append_speech(&self, text: String) -> AtlasEngineResult<()> {
    ...
    pub(crate) async fn handoff_complete(&self) -> AtlasEngineResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
            guard.as_ref().map(|state| state.handoff.clone())
        };
        ...
        let output = if handoff.atlas_engine_responses_as_items {
    ...
    pub(crate) async fn shutdown(&self) -> AtlasEngineResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> AtlasEngineResult<PreparedRealtimeConversationStart> {
...
) -> AtlasEngineResult<RealtimeSessionConfig> {
...
fn realtime_api_key(auth: Option<&AtlasEngineAuth>, provider: &ModelProviderInfo) -> AtlasEngineResult<String> {
    ...
    if let Some(api_key) = auth.and_then(AtlasEngineAuth::api_key) {
    ...
    Err(AtlasEngineErr::InvalidRequest(
...
fn realtime_request_headers(
    ...
    event_parser: RealtimeEventParser,
    originator: &str,
) -> AtlasEngineResult<Option<HeaderMap>> {
    ...
    if let Some(api_key) = api_key {
        let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
            AtlasEngineErr::InvalidRequest(format!("invalid realtime api key header: {err}"))
        })?;
...
fn v3_output_writer(
    writer: &RealtimeWebsocketWriter,
    phase: Option<&MessagePhase>,
    handoff_mode: AtlasEngineResponseHandoffMode,
        ...
        RealtimeEventParser::FramelessBidi => match handoff_output {
            RealtimeOutbound::StandaloneHandoff { text, phase } => {
                v3_output_writer(
                    writer,
                    phase.as_ref(),
                    handoff_state.atlas_engine_response_handoff_mode,
                )
                .send_standalone_handoff(STANDALONE_HANDOFF_ID.to_string(), text)
                .await
            }
...
async fn send_conversation_error(
    ...
    sub_id: String,
    message: String,
    atlas_engine_error_info: AtlasEngineErrorInfo,
) {
    sess.send_event_raw(Event {
        id: sub_id,
