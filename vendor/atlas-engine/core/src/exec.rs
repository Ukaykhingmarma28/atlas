...
use atlas_engine_protocol::protocol::ExecCommandOutputDeltaEvent;
use atlas_engine_protocol::protocol::ExecOutputStream;
...
pub(crate) use atlas_engine_sandboxing::is_likely_sandbox_denied;
#[cfg(test)]
use atlas_engine_sandboxing::permission_profile_supports_windows_restricted_token_sandbox;
use atlas_engine_sandboxing::record_filesystem_sandbox_violation;
use atlas_engine_sandboxing::resolve_windows_elevated_filesystem_overrides;
use atlas_engine_sandboxing::resolve_windows_restricted_token_filesystem_overrides;
#[cfg(test)]
use atlas_engine_sandboxing::unsupported_windows_restricted_token_sandbox_reason;
use atlas_engine_sandboxing::windows_sandbox_uses_elevated_backend;
...
use atlas_engine_utils_pty::process_group::kill_child_process_group;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: atlas_engine_protocol::config_types::WindowsSandboxLevel,
...
fn select_process_exec_tool_sandbox_type(
    ...
    windows_sandbox_level: atlas_engine_protocol::config_types::WindowsSandboxLevel,
...
pub fn build_exec_request(
    ...
    let (program, args) = command.split_first().ok_or_else(|| {
        AtlasEngineErr::Io(io::Error::new(
    ...
    let mut exec_req = manager
        .transform(SandboxTransformRequest {
            ...
            atlas_engine_linux_sandbox_exe: atlas_engine_linux_sandbox_exe.as_deref(),
            use_legacy_landlock,
            windows_sandbox_level,
            windows_sandbox_private_desktop,
        })
    ...
    .map_err(AtlasEngineErr::UnsupportedOperation)?;
    Ok(exec_req)
}
...
pub(crate) async fn execute_exec_request(
    ...
    let cwd = cwd
        .to_abs_path()
        .map_err(|err| AtlasEngineErr::InvalidRequest(format!("invalid exec cwd: {err}")))?;
    ...
    let windows_sandbox_policy_cwd = windows_sandbox_policy_cwd
        .to_abs_path()
        .map_err(|err| AtlasEngineErr::InvalidRequest(format!("invalid sandbox cwd: {err}")))?;
...
fn record_windows_sandbox_spawn_failure(
    ...
    let level = if matches!(
        windows_sandbox_level,
        atlas_engine_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    ...
    if let Some(metrics) = atlas_engine_otel::global() {
        let _ = metrics.counter(
            "atlas_agent.windows_sandbox.createprocessasuserw_failed",
            /*inc*/ 1,
            &[
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_atlas_agent_home;
    use atlas_engine_windows_sandbox::run_windows_sandbox_capture_for_permission_profile_elevated;
    use atlas_engine_windows_sandbox::run_windows_sandbox_capture_with_filesystem_overrides;
    ...
    let network_proxy_restricting_sid = network
        ...
        .map(|network| {
            network
                ...
                .ok_or_else(|| {
                    AtlasEngineErr::Io(io::Error::other(
    ...
    let (cancellation, timeout_ms) = if capture_policy.uses_expiration() {
        let cancellation = expiration.cancellation_token().map(|token| {
            atlas_engine_windows_sandbox::WindowsSandboxCancellationToken::new(move || {
                token.is_cancelled()
            })
        });
    ...
    let atlas_agent_home = find_atlas_agent_home().map_err(|err| {
        AtlasEngineErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve atlas_agent_home: {err}"
        )))
    })?;
    ...
    let spawn_res = tokio::task::spawn_blocking(move || {
        if use_elevated {
            run_windows_sandbox_capture_for_permission_profile_elevated(
                atlas_engine_windows_sandbox::ElevatedSandboxProfileCaptureRequest {
                    permission_profile: &permission_profile,
                    workspace_roots: workspace_roots.as_slice(),
                    atlas_agent_home: atlas_agent_home.as_ref(),
                    command,
                    cwd: &cwd,
                    env_map: env,
                    timeout_ms,
    ...
    let capture = match spawn_res {
        ...
        Ok(Err(err)) => {
            ...
            return Err(AtlasEngineErr::Io(io::Error::other(format!(
                "windows sandbox: {err}"
            ))));
        }
        Err(join_err) => {
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, AtlasEngineErr>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(AtlasEngineErr::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(AtlasEngineErr::Sandbox(SandboxErr::Timeout {
            ...
            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                ...
                return Err(AtlasEngineErr::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
...
/// ATLAS_AGENT_SANDBOX_NETWORK_DISABLED=1 is added to the environment of the spawned
/// process.
///
                ...
                Some(ExecExpirationOutcome::Cancelled) => {
                    ...
                    .await
                    {
                        Ok(status) => {
                            ...
                            {
                                atlas_engine_utils_pty::process_group::kill_process_group(
                                    process_group_id,
                                )?;
                            }
                        }
