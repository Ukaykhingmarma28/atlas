...
pub(crate) struct SessionState {
    ...
    pub(crate) pending_session_start_sources: VecDeque<atlas_engine_hooks::SessionStartSource>,
...
impl SessionState {
    ...
    pub(crate) fn queue_pending_session_start_source(
        &mut self,
        value: atlas_engine_hooks::SessionStartSource,
    ) {
...
// missing values as the default `"atlas_engine"` bucket.
fn merge_rate_limit_fields(
    previous: Option<&RateLimitSnapshot>,
    mut snapshot: RateLimitSnapshot,
) -> RateLimitSnapshot {
    if snapshot.limit_id.is_none() {
        snapshot.limit_id = Some("atlas-agent".to_string());
    }
