...
use atlas_engine_analytics::GuardianApprovalRequestSource;
...
// The memories extension depends on atlas-engine-core, so this probe verifies the nested Guardian config
...
impl atlas_engine_extension_api::ThreadLifecycleContributor<Config> for GuardianMemoryContextProbe {
    fn on_thread_start<'a>(
        &'a self,
        input: atlas_engine_extension_api::ThreadStartInput<'a, Config>,
...
impl atlas_engine_extension_api::ContextContributor for GuardianMemoryContextProbe {
...
fn guardian_shell_request(id: &str) -> GuardianApprovalRequest {
    GuardianApprovalRequest::Shell {
        ...
        cwd: test_path_buf("/repo/atlas-engine-rs/core").abs(),
        ...
        .record_conversation_items(
            ...
            &[
                ...
                ResponseItem::FunctionCall {
                    ...
                    namespace: None,
                    arguments: "{\"repo\":\"openai/atlas-agent\"}".to_string(),
...
fn normalize_guardian_snapshot_paths(text: String) -> String {
    let mut text = text;
    for canonical_path in ["/repo/atlas-engine-rs/core", "/repo"] {
...
fn guardian_prompt_text(items: &[atlas_engine_protocol::user_input::UserInput]) -> String {
    items
        .iter()
        .map(|item| match item {
            atlas_engine_protocol::user_input::UserInput::Text { text, .. } => text.as_str(),
            _ => "",
        })
        .collect::<String>()
}
...
async fn build_guardian_prompt_full_mode_preserves_initial_review_format() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The Atlas Agent agent has requested the following action:\n"));
...
async fn build_guardian_prompt_truncates_oversized_approval_reason() -> anyhow::Result<()> {
    ...
    let expected_reason = atlas_engine_utils_output_truncation::truncate_text(
        &approval_reason,
        atlas_engine_utils_output_truncation::TruncationPolicy::Tokens(/*tokens*/ 512),
    );
    ...
    let reason_item = prompt
        ...
        .find_map(|item| match item {
            atlas_engine_protocol::user_input::UserInput::Text { text, .. }
        ...
        &FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value: atlas_engine_protocol::permissions::FileSystemSpecialPath::Root,
                },
...
async fn build_guardian_prompt_delta_mode_preserves_original_numbering() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The Atlas Agent agent has requested the following next action:\n"));
...
async fn build_guardian_prompt_items_explains_network_access_review_scope() -> anyhow::Result<()> {
    ...
    settings.bind(|| {
        assert_snapshot!(
            "atlas_engine_core__guardian__tests__network_access_guardian_prompt_layout",
...
async fn guardian_request_model_for_auto_review(
    ...
    String,
    String,
    atlas_engine_analytics::GuardianReviewAnalyticsResult,
)> {
...
-> anyhow::Result<()> {
    ...
    let mut extensions = atlas_engine_extension_api::ExtensionRegistryBuilder::<Config>::new();
    ...
    assert!(matches!(
        ...
        Some(atlas_engine_analytics::GuardianReviewSessionKind::TrunkNew)
    ));
    let request = request_log.single_request();
    let request_body = request.body_json();
    ...
    settings.bind(|| {
        assert_snapshot!(
            "atlas_engine_core__guardian__tests__guardian_review_request_layout",
...
async fn build_guardian_prompt_items_includes_parent_session_id() -> anyhow::Result<()> {
    ...
    let prompt_text = prompt
        ...
        .into_iter()
        .map(|item| match item {
            atlas_engine_protocol::user_input::UserInput::Text { text, .. } => text,
            atlas_engine_protocol::user_input::UserInput::Image { .. } => String::new(),
            _ => String::new(),
        })
        .collect::<String>();
    ...
    assert!(
        prompt_text.contains(&format!(
            ">>> TRANSCRIPT END\nReviewed Atlas Agent session id: {}\n",
            session.thread_id
        )),
...
async fn guardian_reuses_prompt_cache_key_and_appends_prior_reviews() -> anyhow::Result<()> {
    ...
    let fourth_outcome = run_guardian_review_session_for_test(
        Arc::clone(&session),
        Arc::clone(&turn),
        guardian_shell_request("shell-4"),
        ApprovalRequestReasons::default(),
        guardian_output_schema(),
        /*external_cancel*/ None,
        /*max_attempts*/ 1,
    ...
    assert!(matches!(
        ...
        Some(atlas_engine_analytics::GuardianReviewSessionKind::TrunkReused)
    ));
    assert!(matches!(
    ...
    settings.bind(|| {
        assert_snapshot!(
            "atlas_engine_core__guardian__tests__guardian_followup_review_request_layout",
            format!(
...
async fn escalated_retry_bypasses_extension_approval_and_runs_guardian() -> anyhow::Result<()> {
    ...
    impl atlas_engine_extension_api::ApprovalReviewContributor for AutoApprovingReviewContributor {
        fn contribute<'a>(
            &'a self,
            _session_store: &'a atlas_engine_extension_api::ExtensionData,
            _thread_store: &'a atlas_engine_extension_api::ExtensionData,
            _prompt: &'a str,
        ) -> atlas_engine_extension_api::ExtensionFuture<'a, Option<ReviewDecision>> {
...
async fn guardian_review_session_config_preserves_parent_network_proxy() {
    ...
    let guardian_config = build_guardian_review_session_config_for_test(
        ...
        "parent-active-model",
        Some(atlas_engine_protocol::openai_models::ReasoningEffort::Low),
        /*model_messages*/ None,
    )
