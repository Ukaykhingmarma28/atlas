...
use atlas_engine_analytics::GuardianReviewDecision;
use atlas_engine_analytics::GuardianReviewFailureReason;
use atlas_engine_analytics::GuardianReviewTerminalStatus;
use atlas_engine_analytics::GuardianReviewTrackContext;
use atlas_engine_analytics::GuardianReviewedAction;
...
use atlas_engine_protocol::protocol::GuardianAssessmentDecisionSource;
...
impl GuardianReviewError {
    ...
    fn session_with_error_info(err: anyhow::Error, error_info: AtlasEngineErrorInfo) -> Self {
        Self::Session {
            message: err.to_string(),
...
pub(crate) fn is_guardian_reviewer_source(
    session_source: &atlas_engine_protocol::protocol::SessionSource,
) -> bool {
    matches!(
        session_source,
        atlas_engine_protocol::protocol::SessionSource::SubAgent(SubAgentSource::Other(label))
...
pub(crate) fn spawn_approval_request_review(
    ...
    let spawn_result = std::thread::Builder::new()
        .name("atlas-engine-approval-review".to_string())
        .spawn(move || {
    ...
    let (guardian_model, guardian_reasoning_effort) = if let Some(preset) = review_model {
        let reasoning_effort = preferred_reasoning_effort(
            preset
                ...
                .iter()
                .any(|effort| effort.effort == atlas_engine_protocol::openai_models::ReasoningEffort::Low),
        ...
        let reasoning_effort = preferred_reasoning_effort(
            turn.model_info
                ...
                .iter()
                .any(|preset| preset.effort == atlas_engine_protocol::openai_models::ReasoningEffort::Low),
            ...
            GuardianReviewError::Session {
                error_info: Some(
                    AtlasEngineErrorInfo::ServerOverloaded
                        ...
                        | AtlasEngineErrorInfo::ResponseStreamDisconnected { .. }
                ),
                ..
            } | GuardianReviewError::Parse { .. }
        )
