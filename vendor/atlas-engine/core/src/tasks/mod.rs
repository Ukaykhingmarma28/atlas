...
use crate::atlas_engine_thread::BackgroundTerminalInfo;
...
use atlas_engine_analytics::TurnProfileFact;
use atlas_engine_analytics::TurnTokenUsageFact;
use atlas_engine_otel::SessionTelemetry;
use atlas_engine_otel::TURN_E2E_DURATION_METRIC;
use atlas_engine_otel::TURN_MEMORY_METRIC;
use atlas_engine_otel::TURN_NETWORK_PROXY_METRIC;
use atlas_engine_otel::TURN_TOKEN_USAGE_METRIC;
use atlas_engine_otel::TURN_TOOL_CALL_METRIC;
use atlas_engine_otel::TURN_UNIFIED_EXEC_RUNNING_PROCESSES_METRIC;
...
const TASK_COMPACT_METRIC: &str = "atlas_agent.task.compact";
static ACTIVE_TURNS: Gauge = Gauge::new("core.turns.active");

pub(crate) type SessionTaskResult = AtlasEngineResult<Option<String>>;
...
///
/// Implementations encapsulate a specific Atlas Agent workflow (regular chat,
...
pub(crate) trait SessionTask: Send + Sync + 'static {
    ...
    /// [`AtlasEngineErr::TurnAborted`] completes the task through the aborted-turn
    /// lifecycle instead.
    fn run(
        self: Arc<Self>,
        session: Arc<Session>,
...
impl Session {
    ...
    pub(crate) async fn start_task<T: SessionTask>(
        ...
        let task_span = info_span!(
            ...
            atlas_engine.turn.reasoning_effort = %reasoning_effort,
            atlas_engine.turn.token_usage.input_tokens = field::Empty,
            atlas_engine.turn.token_usage.cached_input_tokens = field::Empty,
            atlas_engine.turn.token_usage.cache_write_input_tokens = field::Empty,
            atlas_engine.turn.token_usage.non_cached_input_tokens = field::Empty,
            atlas_engine.turn.token_usage.output_tokens = field::Empty,
            atlas_engine.turn.token_usage.reasoning_output_tokens = field::Empty,
            atlas_engine.turn.token_usage.total_tokens = field::Empty,
        );
                ...
                if let Err(err) = sess.flush_rollout().await {
                    ...
                    sess.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                            message: format!(
                                "Failed to save the conversation transcript; Atlas Agent will continue retrying. Error: {err}"
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) fn has_outstanding_durable_sleep(&self) -> bool {
        self.services
            ...
            .get::<atlas_engine_extension_items::sleep::SleepItem>()
            .is_some()
    }
    ...
    pub async fn on_task_finished(
        ...
        let (last_agent_message, abort_reason) = match task_result {
            ...
            Err(err) => {
                ...
                self.emit_turn_error_lifecycle(
                    turn_context.as_ref(),
                    err.to_atlas_engine_protocol_error(),
                )
                .await;
                self.track_turn_atlas_engine_error(turn_context.as_ref(), &err);
                self.send_event(
                    turn_context.as_ref(),
        ...
        // Emit token usage metrics.
        {
            ...
            current_span.record(
                "atlas_agent.turn.token_usage.input_tokens",
                turn_token_usage.input_tokens,
            );
            current_span.record(
                "atlas_agent.turn.token_usage.cached_input_tokens",
                turn_token_usage.cached_input(),
            );
            current_span.record(
                "atlas_agent.turn.token_usage.cache_write_input_tokens",
                turn_token_usage.cache_write_input_tokens,
            );
            current_span.record(
                "atlas_agent.turn.token_usage.non_cached_input_tokens",
                turn_token_usage.non_cached_input(),
            );
            current_span.record(
                "atlas_agent.turn.token_usage.output_tokens",
                turn_token_usage.output_tokens,
            );
            current_span.record(
                "atlas_agent.turn.token_usage.reasoning_output_tokens",
                turn_token_usage.reasoning_output_tokens,
            );
            current_span.record(
                "atlas_agent.turn.token_usage.total_tokens",
