...
use atlas_engine_config::ConfigError;
use atlas_engine_config::ConfigLayerEntry;
use atlas_engine_config::ConfigLayerSource;
use atlas_engine_config::ConfigLoadError;
use atlas_engine_config::ConfigLoadOptions;
use atlas_engine_config::ConfigRequirements;
use atlas_engine_config::ConfigRequirementsToml;
use atlas_engine_config::ConfigRequirementsWithSources;
use atlas_engine_config::FilesystemDenyReadPattern;
use atlas_engine_config::LoaderOverrides;
use atlas_engine_config::RequirementSource;
use atlas_engine_config::RequirementsLayerEntry;
use atlas_engine_config::SessionThreadConfig;
use atlas_engine_config::StaticThreadConfigLoader;
use atlas_engine_config::ThreadConfigSource;
use atlas_engine_config::compose_requirements;
use atlas_engine_config::config_error_from_ignored_toml_fields;
use atlas_engine_config::config_error_from_toml;
use atlas_engine_config::config_error_from_typed_toml;
...
use atlas_engine_config::loader::load_requirements_toml;
...
async fn write_project_hook_config(
    dot_atlas_agent_folder: &Path,
    foo: Option<&str>,
    command: &str,
) -> std::io::Result<()> {
    tokio::fs::create_dir_all(dot_atlas_agent_folder).await?;
    let foo = foo
        .map(|value| format!("foo = \"{value}\"\n\n"))
        .unwrap_or_default();
    tokio::fs::write(
        dot_atlas_agent_folder.join(CONFIG_TOML_FILE),
        format!(
            r#"{foo}[hooks]
...
invalid = ["#,
    ...
    let user_layer = layers
        ...
        .expect("expected a user layer even when ATLAS_AGENT_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let err = ConfigBuilder::default()
        .atlas_agent_home(tmp.path().to_path_buf())
...
async fn returns_packaged_defaults_when_other_layers_are_missing() {
    ...
    let user_layer = layers
        .get_active_user_layer()
        .expect("expected a user layer even when ATLAS_AGENT_HOME/config.toml does not exist");
...
async fn managed_preferences_requirements_resolve_paths_against_atlas_agent_home() -> anyhow::Result<()> {
    ...
    let tmp = tempdir()?;
    let atlas_agent_home = tmp.path().join("atlas-engine-home");
...
"#
    ...
    assert_eq!(
        ...
        Some(atlas_agent_home.join("state").as_path())
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(atlas_agent_home.join("models.json").as_path())
    );
...
"#,
    ...
    assert_eq!(
        config_requirements_toml
            ...
            .as_deref()
            .cloned(),
        Some(vec![atlas_engine_config::WebSearchModeRequirement::Cached])
    );
    assert_eq!(
        config_requirements_toml
        ...
        Some(atlas_engine_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(atlas_engine_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
    ...
    assert_eq!(
        ...
        Err(ConstraintError::InvalidValue {
            ...
            requirement_source: RequirementSource::MdmManagedPreferences {
                domain: "com.openai.atlas-agent".to_string(),
...
"#,
    ...
    assert_eq!(
        ...
        vec![
            ...
            ConfigLayerSource::User {
                file: AbsolutePathBuf::from_absolute_path(atlas_agent_home.join(CONFIG_TOML_FILE))?,
                profile: None,
            },
        ]
    );
...
#[tokio::test]
async fn load_config_layers_resolves_relative_bundle_requirements_paths_against_atlas_agent_home()
-> anyhow::Result<()> {
    let tmp = tempdir()?;
    let atlas_agent_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&atlas_agent_home).await?;
...
async fn strict_config_rejects_unknown_cloud_config_key() {
    let tmp = tempdir().expect("tempdir");
    let atlas_agent_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&atlas_agent_home)
        .await
        .expect("create atlas-agent home");
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            atlas_engine_config::SandboxModeRequirement::ReadOnly,
            atlas_engine_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
        ...
        ConfigLoadOptions {
            cloud_config_bundle: CloudConfigBundleLoader::new(async {
                Err(CloudConfigBundleLoadError::new(
                    atlas_engine_config::CloudConfigBundleLoadErrorCode::RequestFailed,
                    /*status_code*/ None,
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".atlas-agent")).await?;
    tokio::fs::create_dir_all(project_root.join(".atlas-agent")).await?;
...
-> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_child.join(".atlas-agent")).await?;
    write_linked_worktree_pointer(&repo_root, &worktree_root).await?;
    write_project_hook_config(
        &repo_root.join(".atlas-agent"),
        Some("repo-root"),
        "echo repo root hook",
    )
    .await?;
    write_project_hook_config(
        &repo_child.join(".atlas-agent"),
        Some("repo-child"),
        "echo repo child hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_root.join(".atlas-agent"),
        Some("worktree-root"),
        "echo worktree root hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_child.join(".atlas-agent"),
        Some("worktree-child"),
...
"#;
    ...
    .await?;
    tokio::fs::write(
        nested.join(".atlas-agent").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let atlas_agent_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&atlas_agent_home).await?;
    tokio::fs::write(atlas_agent_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn atlas_agent_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let atlas_agent_home = home_dir.join(".atlas-agent");
    tokio::fs::create_dir_all(&atlas_agent_home).await?;
    tokio::fs::write(
        atlas_agent_home.join(CONFIG_TOML_FILE),
        r#"foo = "user"
...
#[tokio::test]
async fn atlas_agent_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_atlas_engine = project_root.join(".atlas-agent");
    let nested_dot_atlas_engine = nested.join(".atlas-agent");

    tokio::fs::create_dir_all(&nested_dot_atlas_engine).await?;
    tokio::fs::create_dir_all(project_root.join(".git")).await?;
    tokio::fs::write(
        nested_dot_atlas_engine.join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;

    tokio::fs::create_dir_all(&project_dot_atlas_engine).await?;
    make_config_for_test(
        &project_dot_atlas_engine,
        &project_root,
        TrustLevel::Trusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let user_config_path = project_dot_atlas_engine.join(CONFIG_TOML_FILE);
...
"#,
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_atlas_agent_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_atlas_engine)?,
        },
        child_config,
    );
    assert_eq!(vec![&expected_project_layer], project_layers);
...
"#,
    ...
    let atlas_agent_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&atlas_agent_home_untrusted).await?;
    make_config_for_test(
        &atlas_agent_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let untrusted_config_path = atlas_agent_home_untrusted.join(CONFIG_TOML_FILE);
...
{untrusted_config_contents}"#
    ...
    let atlas_agent_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&atlas_agent_home_unknown).await?;
    tokio::fs::write(
        atlas_agent_home_unknown.join(CONFIG_TOML_FILE),
        r#"foo = "user"
"#,
    )
    .await?;
...
async fn project_layer_ignores_unsupported_config_keys() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(&dot_atlas_engine).await?;
    ...
    // `.atlas_engine` folder. The malformed profile value below would fail typed path
    // resolution if `profiles` were not stripped before that pass runs.
    tokio::fs::write(
        dot_atlas_engine.join(CONFIG_TOML_FILE),
        r#"
model = "project-model"
...
responses_api_metadata = { atlas_engine_security_surface = "attacker" }
...
"#,
    ...
    let expected_startup_warnings = vec![format!(
        ...
        dot_atlas_engine.join(CONFIG_TOML_FILE).display(),
    ...
    // still use the project-local `.atlas_engine` folder as its relative-path base.
    assert_eq!(
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".atlas-agent")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".atlas-agent").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let atlas_agent_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&atlas_agent_home).await?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_level, expect_disabled) in cases {
        let atlas_agent_home = tmp.path().join(format!("home_no_config_{name}"));
        tokio::fs::create_dir_all(&atlas_agent_home).await?;
        if let Some(trust_level) = trust_level {
            make_config_for_test(
                &atlas_agent_home,
...
mod requirements_exec_policy_tests {
    ...
    use atlas_engine_config::ConfigRequirementsWithSources;
    use atlas_engine_config::RequirementSource;
    use atlas_engine_config::RequirementsExecPolicyDecisionToml;
    use atlas_engine_config::RequirementsExecPolicyParseError;
    use atlas_engine_config::RequirementsExecPolicyPatternTokenToml;
    use atlas_engine_config::RequirementsExecPolicyPrefixRuleToml;
    use atlas_engine_config::RequirementsExecPolicyToml;
    ...
    fn config_stack_for_dot_atlas_agent_folder_with_requirements(
        dot_atlas_agent_folder: &Path,
