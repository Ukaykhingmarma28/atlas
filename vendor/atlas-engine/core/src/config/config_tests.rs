...
use atlas_engine_config::ConfigLayerEntry;
use atlas_engine_config::ConfigLayerSource;
use atlas_engine_config::ConfigLayerStack;
use atlas_engine_config::McpServerCommandMatcher;
use atlas_engine_config::McpServerIdentity;
use atlas_engine_config::McpServerRequirement;
use atlas_engine_config::McpServerValueMatcher;
use atlas_engine_config::ProfileV2Name;
use atlas_engine_config::RequirementSource;
use atlas_engine_config::Sourced;
use atlas_engine_config::config_toml::AgentRoleToml;
use atlas_engine_config::config_toml::AgentsToml;
use atlas_engine_config::config_toml::AutoReviewToml;
use atlas_engine_config::config_toml::ConfigToml;
use atlas_engine_config::config_toml::ExperimentalRequestUserInput;
use atlas_engine_config::config_toml::ProjectConfig;
use atlas_engine_config::config_toml::RealtimeConfig;
use atlas_engine_config::config_toml::RealtimeToml;
use atlas_engine_config::config_toml::RealtimeTransport;
use atlas_engine_config::config_toml::RealtimeWsMode;
use atlas_engine_config::config_toml::RealtimeWsVersion;
use atlas_engine_config::config_toml::ToolsToml;
...
use atlas_engine_config::permissions_toml::NetworkDomainPermissionToml;
use atlas_engine_config::permissions_toml::NetworkDomainPermissionsToml;
use atlas_engine_config::permissions_toml::NetworkMitmActionToml;
use atlas_engine_config::permissions_toml::NetworkMitmHookToml;
use atlas_engine_config::permissions_toml::NetworkMitmToml;
...
use atlas_engine_config::permissions_toml::WorkspaceRootsToml;
use atlas_engine_config::types::AppToolApproval;
use atlas_engine_config::types::ApprovalsReviewer;
use atlas_engine_config::types::BundledSkillsConfig;
use atlas_engine_config::types::FeedbackConfigToml;
use atlas_engine_config::types::HistoryPersistence;
use atlas_engine_config::types::McpServerEnvVar;
use atlas_engine_config::types::McpServerOAuthConfig;
use atlas_engine_config::types::McpServerToolConfig;
use atlas_engine_config::types::McpServerTransportConfig;
use atlas_engine_config::types::MemoriesConfig;
use atlas_engine_config::types::MemoriesToml;
use atlas_engine_config::types::ModelAvailabilityNuxConfig;
use atlas_engine_config::types::Notice;
use atlas_engine_config::types::NotificationCondition;
use atlas_engine_config::types::NotificationMethod;
use atlas_engine_config::types::Notifications;
use atlas_engine_config::types::OtelConfigToml;
use atlas_engine_config::types::OtelExporterKind;
use atlas_engine_config::types::ResumeCwdMode;
use atlas_engine_config::types::SandboxWorkspaceWrite;
use atlas_engine_config::types::SessionPickerViewMode;
use atlas_engine_config::types::SkillsConfig;
use atlas_engine_config::types::ToolSuggestDisabledTool;
use atlas_engine_config::types::ToolSuggestDiscoverableType;
use atlas_engine_config::types::Tui;
use atlas_engine_config::types::TuiKeymap;
use atlas_engine_config::types::TuiNotificationSettings;
use atlas_engine_config::types::TuiPetAnchor;
use atlas_engine_config::types::WindowsSandboxModeToml;
use atlas_engine_config::types::WindowsToml;
use atlas_engine_exec_server::LOCAL_FS;
use atlas_engine_features::Feature;
use atlas_engine_features::FeaturesToml;
use atlas_engine_model_provider::ProviderCapabilities;
use atlas_engine_model_provider_info::LMSTUDIO_OSS_PROVIDER_ID;
use atlas_engine_model_provider_info::OLLAMA_OSS_PROVIDER_ID;
use atlas_engine_model_provider_info::WireApi;
use atlas_engine_models_manager::bundled_models_response;
use atlas_engine_network_proxy::NetworkMode;
...
use atlas_engine_protocol::openai_models::MultiAgentRoleMessages;
...
use atlas_engine_protocol::protocol::NetworkAccess;
...
excluded_tool_namespaces = ["mcp__atlas_apps", "multi_agent_v1"]
...
"#,
    ...
    assert_eq!(
        ...
        vec!["mcp__atlas_apps".to_string(), "multi_agent_v1".to_string()]
    );
    assert_eq!(
...
"#,
    ...
    assert_eq!(
        ...
        atlas_engine_http_client::OutboundProxyPolicy::RespectSystemProxy
    );
    assert_eq!(
        config
            .auth_route_config()
    ...
    assert_eq!(
        ...
        atlas_engine_core_plugins::remote::RemotePluginServiceConfig::new(
            config.chatgpt_base_url,
            atlas_engine_http_client::HttpClientFactory::new(
                atlas_engine_http_client::OutboundProxyPolicy::RespectSystemProxy,
            ),
        )
    );
...
-> std::io::Result<()> {
    ...
    requirements.network = Some(Sourced::new(
        atlas_engine_config::NetworkConstraints {
            enabled: Some(true),
            ..Default::default()
        },
...
-> std::io::Result<()> {
    ...
    let memories_root = atlas_agent_home.path().join("memories").abs();
    assert!(
        !config
            .permissions
            .file_system_sandbox_policy()
...
-> std::io::Result<()> {
    ...
    let atlas_agent_home = temp_dir.path().join("atlas-engine-home");
    ...
    for root in [&cwd, &runtime_root, &profile_root] {
        ...
        std::fs::create_dir_all(root.join(".atlas-agent"))?;
    }
                        ...
                        filesystem: Some(FilesystemPermissionsToml {
                            ...
                            entries: BTreeMap::from([(
                                ...
                                FilesystemPermissionToml::Scoped(BTreeMap::from([
                                    ...
                                    (".atlas-agent".to_string(), FileSystemAccessMode::Read),
                                ])),
                            )]),
                        }),
                        network: None,
    ...
    for root in [cwd_abs, runtime_root_abs, profile_root_abs.clone()] {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&root.join(".atlas-agent"), cwd.as_path()),
            "expected .atlas-agent carveout under {root:?}, policy: {policy:?}"
        );
    }
    assert_eq!(
...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&cwd.path().join(".atlas-agent"), cwd.path()),
...
-> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    let extra_root = TempDir::new()?;
    for subpath in [".git", ".agents", ".atlas-agent"] {
...
async fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of Atlas Agent and will be ignored."
        )),
        "{:?}",
...
-> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of Atlas Agent and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
async fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `dev` does not define any recognized filesystem entries for this version of Atlas Agent."
        )),
        "{:?}",
        config.startup_warnings
    );
                        ...
                        file_system_policy
                            ...
                            .contains(&FileSystemSandboxEntry {
                                ...
                                missing_path_behavior: Some(
                                    atlas_engine_protocol::permissions::FileSystemSandboxEntryMissingPathBehavior::Skip,
                                ),
                            }),
...
fn filter_plugin_mcp_servers_without_allowlists_does_not_filter_any_plugin() {
    ...
    let requirements = Sourced::new(
        BTreeMap::from([(
            ...
            atlas_engine_config::PluginRequirementsToml { mcp_servers: None },
        )]),
        RequirementSource::LegacyManagedConfigTomlFromMdm,
    );
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    ...
    let requirements_toml = atlas_engine_config::ConfigRequirementsToml {
        mcp_servers: Some(mcp_requirements.clone()),
        ..Default::default()
    };
    let requirements = atlas_engine_config::ConfigRequirements {
        ...
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_atlas_agent_folder: project_dot_atlas_engine.clone(),
                },
                toml::toml! {
    ...
    let refreshed_config = Config::load_config_with_layer_stack(
        ...
        refreshed_toml,
        ConfigOverrides {
            cwd: Some(atlas_agent_home.path().to_path_buf()),
            ..Default::default()
        },
        atlas_agent_home.abs(),
    ...
    let thread_layer_stack = ConfigLayerStack::new(
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_atlas_agent_folder: project_dot_atlas_engine,
                },
                toml::toml! {
                    [mcp_servers.fresh_project]
...
"#,
    ...
    assert_eq!(
        ...
        Some((
            atlas_engine_mcp::McpServerSource::SelectedPlugin(McpPluginAttribution::new(
                "selected-root".to_string(),
                "Selected Plugin".to_string(),
            )),
            selected,
...
async fn default_zsh_path_sets_runtime_zsh_path() -> std::io::Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let default_zsh_path = atlas_agent_home.path().join("packaged-zsh");
...
#[tokio::test]
async fn sqlite_home_defaults_to_atlas_agent_home_for_workspace_write() -> std::io::Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
    ...
    assert_eq!(config.sqlite.home(), atlas_agent_home.path());
...
-> std::io::Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let memories_root = atlas_agent_home.path().join("memories");
    let writable_root = atlas_agent_home.path().join("writable").abs();
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            sandbox_workspace_write: Some(SandboxWorkspaceWrite {
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    ...
    let config_path = atlas_agent_home.path().join(CONFIG_TOML_FILE);
    ...
    let cwd = atlas_agent_home.path().abs();
...
async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
    ...
    let servers = load_global_mcp_servers(atlas_agent_home.path()).await?;
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let expected_cwd = LegacyAppPathString::from_path(atlas_agent_home.path());
    ...
    let loaded = load_global_mcp_servers(atlas_agent_home.path()).await?;
...
"#,
    ...
    assert_eq!(
        ...
        Some(&serde_json::json!("atlas-agent"))
    );
    assert_eq!(
...
async fn to_mcp_config_flows_mcp_2026_feature_from_config() -> std::io::Result<()> {
    ...
    assert_eq!(
        ...
        atlas_engine_mcp::McpProtocolMode::V20260728
    );
...
"#,
    ...
    let err = load_global_mcp_servers(atlas_agent_home.path())
        .await
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    let cwd_path = PathBuf::from("/tmp/atlas-engine-mcp");
    ...
    let serialized = std::fs::read_to_string(&config_path)?;
    assert!(
        serialized.contains(r#"cwd = "/tmp/atlas-engine-mcp""#),
...
async fn replace_mcp_servers_streamable_http_serializes_oauth_resource() -> anyhow::Result<()> {
    ...
    let servers = BTreeMap::from([(
        ...
        McpServerConfig {
            ...
            oauth: Some(McpServerOAuthConfig {
                client_id: Some("eci-prd-pub-atlas-engine-123".to_string()),
                callback_port: None,
            }),
    ...
    assert!(serialized.contains(r#"client_id = "eci-prd-pub-atlas-engine-123""#));
    ...
    assert_eq!(docs.oauth_client_id(), Some("eci-prd-pub-atlas-engine-123"));
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(atlas_agent_home.path())
...
async fn for_config_writes_selected_user_config_file() -> anyhow::Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let base_config = atlas_agent_home.path().join(CONFIG_TOML_FILE);
    let selected_config = atlas_agent_home.path().join("work.config.toml");
...
fn profile_v2_config_path_resolves_validated_names() -> anyhow::Result<()> {
    ...
    assert_eq!(
        resolve_profile_v2_config_path(atlas_agent_home.path(), &profile_name),
        atlas_agent_home.path().join("work.config.toml").abs()
    );
    Ok(())
}
...
impl PrecedenceTestFixture {
    ...
    fn atlas_agent_home(&self) -> AbsolutePathBuf {
        self.atlas_agent_home.abs()
    }
}
...
async fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let workspace = atlas_agent_home.path().join("workspace");
...
async fn load_config_uses_requirements_guardian_policy_config() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        Default::default(),
        atlas_engine_config::ConfigRequirementsToml {
...
"#,
    ...
    let config_layer_stack = atlas_engine_config::ConfigLayerStack::new(
        vec![atlas_engine_config::ConfigLayerEntry::new(
            ConfigLayerSource::User {
                file: atlas_agent_home.path().join(CONFIG_TOML_FILE).abs(),
                profile: None,
            },
            layer_config,
        )],
        Default::default(),
        atlas_engine_config::ConfigRequirementsToml::default(),
    )
    .map_err(std::io::Error::other)?;
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".atlas-agent").join("agents");
...
"#
    ...
    let home_agents_dir = atlas_agent_home.path().join("agents");
...
async fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let catalog_path = atlas_agent_home.path().join("catalog.json");
...
"#;
    ...
    let atlas_agent_home_temp_dir = TempDir::new().unwrap();

    Ok(PrecedenceTestFixture {
        cwd: cwd_temp_dir,
        atlas_agent_home: atlas_agent_home_temp_dir,
        cfg,
    })
}
...
/// in `tests/atlas-engine-no-phone-home.test.ts`.
#[tokio::test]
...
async fn trace_exporter_defaults_to_none_when_log_exporter_is_set() -> std::io::Result<()> {
    ...
    cfg.otel = Some(OtelConfigToml {
        exporter: Some(OtelExporterKind::OtlpHttp {
            ...
            headers: HashMap::new(),
            protocol: atlas_engine_config::types::OtelHttpProtocol::Binary,
            tls: None,
        }),
...
async fn config_toml_priority_service_tier_uses_priority_request_value() -> std::io::Result<()> {
    ...
    let atlas_agent_home = fixture.atlas_agent_home();
...
async fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()>
{
    ...
    let requirements_toml = atlas_engine_config::ConfigRequirementsToml {
        ...
        allowed_web_search_modes: Some(vec![atlas_engine_config::WebSearchModeRequirement::Cached]),
    ...
    let requirement_source = atlas_engine_config::RequirementSource::Unknown;
    ...
    let requirements = atlas_engine_config::ConfigRequirements {
        web_search_mode: atlas_engine_config::ConstrainedWithSource::new(
            constrained,
    ...
    let config_layer_stack =
        atlas_engine_config::ConfigLayerStack::new(Vec::new(), requirements, requirements_toml)
...
projects = { "/Users/mbolin/code/atlas-agent4" = { trust_level = "trusted", foo = "bar" } , "/Users/mbolin/code/atlas-agent3" = { trust_level = "trusted" } }
model = "foo""#;
    let mut doc = initial.parse::<DocumentMut>()?;

    // Approve a new directory
    let new_project = Path::new("/Users/mbolin/code/atlas-agent2");
...
[projects."/Users/mbolin/code/atlas-agent4"]
trust_level = "trusted"
foo = "bar"

[projects."/Users/mbolin/code/atlas-agent3"]
trust_level = "trusted"
...
fn test_set_default_oss_provider() -> std::io::Result<()> {
    let temp_dir = TempDir::new()?;
    let atlas_agent_home = temp_dir.path();
    let config_path = atlas_agent_home.join(CONFIG_TOML_FILE);

    // Test setting valid provider on empty config
    set_default_oss_provider(atlas_agent_home, OLLAMA_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    assert!(content.contains("oss_provider = \"ollama\""));

    // Test updating existing config
    std::fs::write(&config_path, "model = \"gpt-4\"\n")?;
    set_default_oss_provider(atlas_agent_home, LMSTUDIO_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    ...
    let result = set_default_oss_provider(atlas_agent_home, "invalid_provider");
...
fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(atlas_agent_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
    assert!(
...
"#,
    ...
    assert_eq!(
        ...
        Some(atlas_engine_config::types::WindowsSandboxModeToml::Elevated)
    );
    assert!(
...
"#,
    ...
    assert_eq!(
        err.to_string(),
        "`approval_policy = \"never\"` cannot be used because requirements do not allow `sandbox_mode = \"danger-full-access\"`; Atlas Agent would fall back to read-only permissions with approvals disabled. Choose an `approval_policy` based on what you need, such as `on-request`, or choose an allowed sandbox mode."
    );
    Ok(())
}
...
async fn permission_profile_override_preserves_split_write_roots() -> std::io::Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let cwd = atlas_agent_home.path().join("workspace");
    let outside_root = atlas_agent_home.path().join("outside-write");
...
-> std::io::Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let selected_config = atlas_agent_home.path().join("default.config.toml");
    std::fs::write(
        &selected_config,
        r#"approvals_reviewer = "user"
"#,
...
async fn exact_requirements_apply_to_runtime_config() -> std::io::Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let catalog_path = atlas_agent_home.path().join("required-models.json");
    let mut catalog = bundled_models_response()
        .unwrap_or_else(|err| panic!("bundled models.json should parse: {err}"));
    catalog.models = catalog.models.into_iter().take(1).collect();
    std::fs::write(
...
"#,
    ...
    let required_sqlite_home = atlas_agent_home.path().join("required-state");
    let required_log_dir = atlas_agent_home.path().join("required-logs");
...
"#,
    ...
    let config = load_with_enterprise_requirement(&atlas_agent_home, requirements).await?;
...
fn sqlite_home_env_conflict_reports_an_override() -> std::io::Result<()> {
    let atlas_agent_home = TempDir::new()?;
    let required = AbsolutePathBuf::try_from(atlas_agent_home.path().join("required-state"))?;
    let environment = atlas_agent_home.path().join("environment-state");
    ...
    assert_eq!(
        warnings,
        vec![format!(
            "Environment value for `$ATLAS_AGENT_SQLITE_HOME` is overridden by the required `sqlite_home` value {required:?} from {}.",
            RequirementSource::Unknown
        )]
    );
