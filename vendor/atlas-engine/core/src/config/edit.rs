...
pub fn apply_blocking(atlas_agent_home: &Path, edits: &[ConfigEdit]) -> anyhow::Result<()> {
...
impl ConfigEditsBuilder {
    pub fn new(atlas_agent_home: &Path) -> Self {
        Self::for_config_path(&atlas_agent_home.join(CONFIG_TOML_FILE))
    }
    ...
    pub fn for_config(config: &crate::config::Config) -> Self {
        let config_path = config
            ...
            .map(atlas_engine_utils_absolute_path::AbsolutePathBuf::to_path_buf)
            .unwrap_or_else(|| config.atlas_agent_home.join(CONFIG_TOML_FILE).to_path_buf());
