...
use atlas_engine_config::permissions_toml::NetworkUnixSocketPermissionToml;
use atlas_engine_config::permissions_toml::NetworkUnixSocketPermissionsToml;
...
use atlas_engine_features::NetworkProxyDomainPermissionToml;
use atlas_engine_features::NetworkProxyModeToml;
use atlas_engine_features::NetworkProxyUnixSocketPermissionToml;
use atlas_engine_network_proxy::NetworkMode;
use atlas_engine_network_proxy::NetworkProxyConfig;
#[cfg(test)]
use atlas_engine_network_proxy::NetworkUnixSocketPermission as ProxyNetworkUnixSocketPermission;
...
use atlas_engine_protocol::permissions::project_roots_glob_pattern;
...
/// for Atlas Agent to function. These should always be added to the
/// `FileSystemSandboxPolicy` for a thread.
pub(crate) fn get_readable_roots_required_for_atlas_engine_runtime(
    atlas_agent_home: &Path,
    zsh_path: Option<&PathBuf>,
    main_execve_wrapper_exe: Option<&PathBuf>,
) -> Vec<AbsolutePathBuf> {
    let arg0_root = AbsolutePathBuf::from_absolute_path(atlas_agent_home.join("tmp").join("arg0")).ok();
...
// Adding a new `:special_path` must not make older Atlas Agent versions reject the
...
fn missing_filesystem_entries_warning(profile_name: &str) -> String {
    format!(
        "Permissions profile `{profile_name}` does not define any recognized filesystem entries for this version of Atlas Agent. Filesystem access will remain restricted. Upgrade Atlas Agent if this profile expects filesystem permissions."
    )
}
...
fn maybe_push_unknown_special_path_warning(
    ...
    push_warning(
        ...
        match subpath.as_deref() {
            Some(subpath) => format!(
                "Configured filesystem path `{path}` with nested entry `{subpath}` is not recognized by this version of Atlas Agent and will be ignored. Upgrade Atlas Agent if this path is required."
            ),
            None => format!(
                "Configured filesystem path `{path}` is not recognized by this version of Atlas Agent and will be ignored. Upgrade Atlas Agent if this path is required."
            ),
        },
    );
}
