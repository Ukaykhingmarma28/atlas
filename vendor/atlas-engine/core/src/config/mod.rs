...
use atlas_engine_config::ConstrainedWithSource;
use atlas_engine_config::FeatureRequirementsToml;
use atlas_engine_config::ManagedAuthPolicy;
use atlas_engine_config::McpServerRequirement;
use atlas_engine_config::PluginRequirementsToml;
use atlas_engine_config::ProfileV2Name;
use atlas_engine_config::ResidencyRequirement;
use atlas_engine_config::SandboxModeRequirement;
use atlas_engine_config::Sourced;
use atlas_engine_config::ThreadConfigLoader;
use atlas_engine_config::config_toml::ConfigToml;
use atlas_engine_config::config_toml::DEFAULT_PROJECT_DOC_MAX_BYTES;
use atlas_engine_config::config_toml::ProjectConfig;
use atlas_engine_config::config_toml::RealtimeAudioConfig;
use atlas_engine_config::config_toml::RealtimeConfig;
use atlas_engine_config::config_toml::ThreadStoreToml;
use atlas_engine_config::config_toml::validate_model_providers;
use atlas_engine_config::loader::load_config_layers_state;
use atlas_engine_config::loader::project_trust_key;
use atlas_engine_config::permissions_toml::PermissionsToml;
use atlas_engine_config::sandbox_mode_requirement_for_permission_profile;
...
use atlas_engine_config::types::History;
use atlas_engine_config::types::McpServerConfig;
use atlas_engine_config::types::McpServerDisabledReason;
...
use atlas_engine_config::types::ToolSuggestConfig;
use atlas_engine_config::types::ToolSuggestDisabledTool;
use atlas_engine_config::types::ToolSuggestDiscoverable;
...
use atlas_engine_config::types::UriBasedFileOpener;
use atlas_engine_config::types::WindowsSandboxModeToml;
use atlas_engine_core_plugins::PluginLoadOutcome;
use atlas_engine_core_plugins::PluginsConfigInput;
use atlas_engine_exec_server::ExecutorFileSystem;
use atlas_engine_exec_server::LOCAL_FS;
use atlas_engine_features::CodeModeConfigToml;
use atlas_engine_features::CurrentTimeReminderConfigToml;
...
use atlas_engine_features::FeatureToml;
use atlas_engine_features::Features;
use atlas_engine_features::FeaturesToml;
use atlas_engine_features::MultiAgentV2ConfigToml;
use atlas_engine_features::NetworkProxyConfigToml;
use atlas_engine_features::TokenBudgetConfigToml;
...
use atlas_engine_install_context::InstallContext;
use atlas_engine_login::AuthManagerConfig;
use atlas_engine_login::AuthRouteConfig;
use atlas_engine_mcp::McpConfig;
use atlas_engine_mcp::McpPluginAttribution;
use atlas_engine_mcp::McpProtocolMode;
use atlas_engine_mcp::McpServerRegistration;
use atlas_engine_mcp::ResolvedMcpCatalog;
use atlas_engine_memories_read::memory_root;
use atlas_engine_model_provider::ProviderCapabilities;
use atlas_engine_model_provider_info::LEGACY_OLLAMA_CHAT_PROVIDER_ID;
use atlas_engine_model_provider_info::ModelProviderInfo;
use atlas_engine_model_provider_info::OLLAMA_CHAT_PROVIDER_REMOVED_ERROR;
use atlas_engine_model_provider_info::built_in_model_providers;
use atlas_engine_model_provider_info::merge_configured_model_providers;
use atlas_engine_models_manager::ModelsManagerConfig;
use atlas_engine_protocol::config_types::AltScreenMode;
...
use atlas_engine_protocol::config_types::Verbosity;
use atlas_engine_protocol::config_types::WebSearchConfig;
...
pub use atlas_engine_thread_store::ExtraConfig;
use atlas_engine_utils_absolute_path::AbsolutePathBuf;
use atlas_engine_utils_absolute_path::AbsolutePathBufGuard;
...
use crate::config::permissions::get_readable_roots_required_for_atlas_engine_runtime;
...
pub use atlas_engine_config::ConstraintResult;
pub use atlas_engine_config::LoaderOverrides;
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<AbsolutePathBuf> {
    let raw = std::env::var(atlas_engine_state::SQLITE_HOME_ENV).ok()?;
...
impl Permissions {
    ...
    /// selection. Internal Atlas Agent-only writable roots are intentionally excluded.
...
// when Atlas Agent is still responsible for the network policy. `Disabled` means no
...
fn build_network_proxy_spec(
    ...
    network_requirements: Option<Sourced<atlas_engine_config::NetworkConstraints>>,
    permission_profile: &PermissionProfile,
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, Atlas Agent will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Atlas Agent
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.atlas_engine/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Atlas Agent"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Atlas Agent '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the Atlas Agent home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that Atlas Agent can reach out to for tool calls.
    ...
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by Atlas Agent unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/openai/codex/blob/main/codex-rs/rmcp-client/src/oauth.rs#L2
    /// file: ATLAS_AGENT_HOME/.credentials.json
    ///       This file will be readable to Atlas Agent and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, Atlas Agent will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all Atlas Agent state (defaults to `~/.atlas_engine` but can be
    /// overridden by the `ATLAS_AGENT_HOME` environment variable).
    pub atlas_agent_home: AbsolutePathBuf,

    /// Resolved configuration shared by all Atlas Agent SQLite databases.
    pub sqlite: atlas_engine_state::SqliteConfig,

    /// Directory where Atlas Agent writes log files (defaults to `$ATLAS_AGENT_HOME/log`).
    pub log_dir: PathBuf,

    /// Settings that govern if and what will be written to `~/.atlas_engine/history.jsonl`.
    pub history: History,
    ...
    /// Path to the current Atlas Agent executable. This cannot be set in the config
    /// file: it must be set in code via [`ConfigOverrides`].
    pub atlas_engine_self_exe: Option<PathBuf>,

    /// Path to the `atlas-engine-linux-sandbox` executable. This must be set if
    /// [`atlas_engine_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `atlas-engine-linux-sandbox`.
    pub atlas_engine_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `atlas-engine-execve-wrapper` executable used for shell
    ...
    /// Whether Atlas Agent-owned clients should respect host system proxy settings.
    ...
    /// When `true`, checks for Atlas Agent updates on startup and surfaces update prompts.
    /// Set to `false` only if your Atlas Agent updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across Atlas Agent product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across Atlas Agent product surfaces.
    /// Defaults to `true`.
    ...
    pub otel: atlas_engine_config::types::OtelConfig,
}
...
impl AuthManagerConfig for Config {
    fn atlas_agent_home(&self) -> PathBuf {
        self.atlas_agent_home.to_path_buf()
    }
...
pub struct ConfigBuilder {
    atlas_agent_home: Option<PathBuf>,
...
impl ConfigBuilder {
    pub fn atlas_agent_home(mut self, atlas_agent_home: PathBuf) -> Self {
        self.atlas_agent_home = Some(atlas_agent_home);
        self
    }
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let atlas_agent_home = match atlas_agent_home {
            Some(atlas_agent_home) => AbsolutePathBuf::from_absolute_path(atlas_agent_home)?,
            None => find_atlas_agent_home()?,
        };
        ...
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&atlas_engine_config::NoopThreadConfigLoader),
        )
        .await?;
...
impl Config {
    pub fn sqlite_config(&self) -> &atlas_engine_state::SqliteConfig {
        &self.sqlite
    }
    ...
    pub async fn to_mcp_config(
        &self,
        plugins_manager: &atlas_engine_core_plugins::PluginsManager,
    ) -> McpConfig {
    ...
    pub(crate) fn to_mcp_config_with_loaded_plugins(
        ...
        McpConfig {
            ...
            atlas_engine_linux_sandbox_exe: self.atlas_engine_linux_sandbox_exe.clone(),
    ...
    pub async fn rebuild_preserving_session_layers(
        ...
        Self::load_config_with_layer_stack(
            ...
            refreshed_config.atlas_agent_home.clone(),
            config_layer_stack,
        )
        .await
    }
    ...
    pub async fn load_default_with_cli_overrides(
        ...
        Self::load_default_with_cli_overrides_for_atlas_agent_home(
    ...
    /// Load a default configuration for a specific Atlas Agent home without reading
    /// user, project, or system config layers.
    pub async fn load_default_with_cli_overrides_for_atlas_agent_home(
        ...
        let cli_layer = atlas_engine_config::build_cli_overrides_layer(&cli_overrides);
        atlas_engine_config::merge_toml_values(&mut merged, &cli_layer);
        let atlas_agent_home = AbsolutePathBuf::from_absolute_path_checked(atlas_agent_home)?;
        let config_toml = deserialize_config_toml_with_base(merged, &atlas_agent_home)?;
        Self::load_config_with_layer_stack(
            LOCAL_FS.as_ref(),
            config_toml,
            ConfigOverrides::default(),
    ...
    /// ignores user settings. For example, the `atlas_engine exec` subcommand is
    /// designed to use [AskForApproval::Never] exclusively.
    ///
    /// Further, [ConfigOverrides] contains some options that are not supported
    /// in [ConfigToml], such as `cwd`, `atlas_engine_self_exe`, `atlas_engine_linux_sandbox_exe`, and
...
pub async fn load_config_as_toml_with_cli_and_load_options(
    ...
    load_config_toml_with_layer_stack(atlas_agent_home, cwd, cli_overrides, options)
        .await
...
pub async fn load_config_toml_with_layer_stack(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, atlas_agent_home).map_err(|e| {
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .atlas-agent/ folders.
...
/// Patch `ATLAS_AGENT_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
pub fn set_default_oss_provider(atlas_agent_home: &Path, provider: &str) -> std::io::Result<()> {
    atlas_engine_config::config_toml::validate_oss_provider(provider)?;
    use toml_edit::value;
...
fn apply_managed_filesystem_constraints(
    ...
    filesystem_constraints: &atlas_engine_config::FilesystemConstraints,
) {
    for deny_read in &filesystem_constraints.deny_read {
        let deny_entry = if deny_read.contains_glob() {
            atlas_engine_protocol::permissions::FileSystemSandboxEntry {
                path: atlas_engine_protocol::permissions::FileSystemPath::GlobPattern {
                    pattern: deny_read.as_str().to_string(),
                },
                access: atlas_engine_protocol::permissions::FileSystemAccessMode::Deny,
...
fn resolve_orchestrator_feature_enabled(
    feature: Option<&atlas_engine_config::config_toml::OrchestratorFeatureToml>,
) -> bool {
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let memories_root = memory_root(&atlas_agent_home);
        ...
        let forced_chatgpt_workspace_id = cfg
            ...
            .clone()
            .map(atlas_engine_config::config_toml::ForcedChatgptWorkspaceIds::into_vec)
            .map(|values| {
                values
                    .into_iter()
        ...
        let log_dir = cfg
            ...
            .unwrap_or_else(|| atlas_agent_home.join("log").to_path_buf());
        ...
        let sqlite_home = cfg
            ...
            .cloned()
            .or(sqlite_home_env)
            .unwrap_or_else(|| atlas_agent_home.clone());
        ...
        let mut helper_readable_roots = get_readable_roots_required_for_atlas_engine_runtime(
            &atlas_agent_home,
            zsh_path.as_ref(),
        ...
        let config = Self {
            ...
            atlas_agent_home,
            sqlite: atlas_engine_state::SqliteConfig::from_sqlite_home(sqlite_home),
            log_dir,
            config_layer_stack,
            history,
    ...
    pub fn bundled_skills_enabled(&self) -> bool {
        atlas_engine_config::bundled_skills_enabled_from_stack(&self.config_layer_stack)
    }
...
pub fn find_atlas_agent_home() -> std::io::Result<AbsolutePathBuf> {
    atlas_engine_utils_home_dir::find_atlas_agent_home()
}

/// Returns the path to the folder where Atlas Agent logs are stored. Does not verify
