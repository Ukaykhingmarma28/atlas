...
fn blocking_set_model_writes_through_symlink_chain() {
    ...
    let link_path = atlas_agent_home.join("config-link.toml");
    let config_path = atlas_agent_home.join(CONFIG_TOML_FILE);
...
fn blocking_set_model_replaces_symlink_on_cycle() {
    let tmp = tempdir().expect("tmpdir");
    let atlas_agent_home = tmp.path();
    let link_a = atlas_agent_home.join("a.toml");
    let link_b = atlas_agent_home.join("b.toml");
    let config_path = atlas_agent_home.join(CONFIG_TOML_FILE);
...
"#;
    std::fs::write(atlas_agent_home.join(CONFIG_TOML_FILE), original).expect("seed config");
...
client_id = \"eci-prd-pub-atlas-engine-123\"
callback_port = 9876
...
fn blocking_clear_path_noop_when_missing() {
    ...
    assert!(
        !atlas_agent_home.join(CONFIG_TOML_FILE).exists(),
