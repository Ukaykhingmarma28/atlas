...
async fn thread_id_generator_applies_to_roots_children_and_forks() {
    ...
    config.atlas_agent_home = temp_dir.path().join("atlas-engine-home").abs();
...
impl atlas_engine_agent_graph_store::AgentGraphStore for FakeAgentGraphStore {
    fn upsert_thread_spawn_edge(
        &self,
        _parent_thread_id: ThreadId,
        _child_thread_id: ThreadId,
        _status: atlas_engine_agent_graph_store::ThreadSpawnEdgeStatus,
    ) -> atlas_engine_agent_graph_store::AgentGraphStoreFuture<'_, ()> {
    ...
    fn list_thread_spawn_children(
        &self,
        _parent_thread_id: ThreadId,
        _status_filter: Option<atlas_engine_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> atlas_engine_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
    ...
    fn list_thread_spawn_descendants(
        &self,
        root_thread_id: ThreadId,
        status_filter: Option<atlas_engine_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> atlas_engine_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
...
fn effective_originator_prefers_thread_scoped_sources_before_env_originator() {
    for (metrics_service_name, persisted_originator, inherited_originator, expected_originator) in [
        ...
        (
            Some("atlas_engine_work_web"),
            Some("persisted_originator"),
            Some("inherited_originator"),
            "atlas_engine_work_web",
        ),
        (
            Some("atlas_engine_work_mobile"),
            Some("persisted_originator"),
            Some("inherited_originator"),
            "atlas_engine_work_mobile",
        ),
        (
            Some("atlas_engine_work_cca"),
            Some("persisted_originator"),
            Some("inherited_originator"),
...
async fn mcp_invalidation_refreshes_threads_that_are_still_starting() {
    ...
    impl atlas_engine_extension_api::ThreadLifecycleContributor<Config> for BlockingThreadStartup {
        fn on_thread_start<'a>(
            &'a self,
            _input: atlas_engine_extension_api::ThreadStartInput<'a, Config>,
    ...
    impl atlas_engine_extension_api::McpServerContributor<Config> for BlockingThreadStartup {
...
async fn start_thread_keeps_internal_threads_hidden_from_normal_lookups() {
    ...
    assert!(
        atlas_engine_diagnostics::snapshot()
            .gauges
            .iter()
...
async fn start_thread_seeds_extension_data_for_mcp_and_lifecycle_contributors() {
    ...
    impl atlas_engine_extension_api::ThreadLifecycleContributor<Config> for InitialDataRecorder {
        fn on_thread_start<'a>(
            &'a self,
    ...
    impl atlas_engine_extension_api::McpServerContributor<Config> for InitialDataRecorder {
        fn id(&self) -> &'static str {
            "selected_root_test"
        }
        ...
        ) -> atlas_engine_extension_api::ExtensionFuture<'a, Vec<atlas_engine_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
                ...
                let mut server = atlas_engine_mcp::atlas_apps_mcp_server_config(
    ...
    let selected_root_init = |id: &str, environment_id: &str| {
        let mut init = atlas_engine_extension_api::ExtensionDataInit::new();
    ...
    let first_thread = manager
        .start_thread(StartThreadOptions {
            metrics_service_name: Some("atlas_engine_work_desktop".to_string()),
            environments: Some(Vec::new()),
    ...
    let selected_servers = |config: &atlas_engine_mcp::McpConfig| {
        atlas_engine_mcp::configured_mcp_servers(config)
            .into_iter()
    ...
    let atlas_apps_server = atlas_engine_mcp::configured_mcp_servers(&first_resolved.config)
    ...
    assert_eq!(
        atlas_apps_headers
            .expect("Atlas Agent Apps headers should be configured")
            .get("originator"),
        Some(&"atlas_engine_work_desktop".to_string())
    );
}
...
#[tokio::test]
async fn explicit_installation_id_skips_atlas_agent_home_file() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.atlas_agent_home = temp_dir.path().join("atlas-engine-home").abs();
    config.cwd = config.atlas_agent_home.abs();
...
async fn injected_models_manager_controls_refresh_policy() {
    ...
    assert!(!config.atlas_agent_home.join("models_cache.json").exists());
}
