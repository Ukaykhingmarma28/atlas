...
fn test_responses_metadata_json(
    ...
    window_id: &str,
    request_kind: AtlasEngineResponsesRequestKind,
) -> String {
    state
...
fn test_turn_responses_metadata_json(state: &TurnMetadataState, window_id: &str) -> String {
    test_responses_metadata_json(state, window_id, AtlasEngineResponsesRequestKind::Turn)
}
...
fn test_compaction_responses_metadata_json(
    ...
    test_responses_metadata_json(
        state,
        window_id,
        AtlasEngineResponsesRequestKind::Compaction(compaction),
    )
}
...
fn turn_metadata_state_merges_client_metadata_without_replacing_reserved_fields() {
    ...
    state.set_responsesapi_client_metadata(HashMap::from([
        ...
        (
            "x-atlas-engine-installation-id".to_string(),
            "client-supplied".to_string(),
        ),
        (
            "x-atlas-engine-parent-thread-id".to_string(),
            "client-supplied".to_string(),
        ),
        (
    ...
    assert_eq!(json["atlas_engine_security_surface"].as_str(), Some("sdk"));
    ...
    assert!(json.get("x-atlas-engine-installation-id").is_none());
    assert!(json.get("x-atlas-engine-parent-thread-id").is_none());
    ...
    assert_eq!(
        model_request_json["atlas_engine_security_surface"].as_str(),
        Some("sdk")
    );
    assert_eq!(
    ...
    assert!(meta.get("atlas_engine_security_surface").is_none());
