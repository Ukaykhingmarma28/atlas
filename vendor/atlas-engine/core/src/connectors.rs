...
pub use atlas_engine_connectors::AppBranding;
pub use atlas_engine_connectors::AppInfo;
pub use atlas_engine_connectors::AppMetadata;
use atlas_engine_connectors::ConnectorDirectoryCacheContext;
use atlas_engine_connectors::ConnectorDirectoryCacheKey;
use atlas_engine_connectors::apps_config_from_layer_stack;
...
use atlas_engine_mcp::ToolPluginProvenance;
use atlas_engine_mcp::effective_mcp_servers;
use atlas_engine_mcp::tool_plugin_provenance;
...
pub(crate) async fn list_tool_suggest_discoverable_tools_with_auth(
    ...
    let directory_connectors = atlas_engine_connectors::merge::merge_plugin_connectors(
    ...
    let discoverable_connectors =
        atlas_engine_connectors::filter::filter_tool_suggest_discoverable_connectors(
...
pub async fn list_accessible_connectors_from_mcp_tools_with_options_and_status(
    ...
    let environment_manager = EnvironmentManager::from_atlas_agent_home(
...
pub async fn list_accessible_connectors_from_mcp_tools_with_environment_manager(
    ...
    let plugins_manager = Arc::new(plugins_manager_for_config(
        config,
        auth.as_ref().map(AtlasEngineAuth::api_auth_mode),
    ));
...
pub async fn list_accessible_connectors_from_mcp_tools_with_mcp_manager(
    ...
    let atlas_apps_auth_manager =
        atlas_engine_mcp::host_owned_atlas_apps_enabled(&mcp_config, auth.as_ref())
    ...
    let mcp_runtime = McpRuntime::new(McpRuntimeInput {
        ...
        atlas_apps_tools_cache_key: connector_runtime_context_key(auth.as_ref()),
    ...
    let refreshed_tools = if force_refetch {
        match mcp_runtime
            .latest_hard_refresh_atlas_apps_tools_cache()
            .await
        {
            Ok(tools) => Some(tools),
            Err(err) => {
                warn!(
                    "failed to force-refresh tools for MCP server '{ATLAS_APPS_MCP_SERVER_NAME}', using cached/startup tools: {err:#}"
                );
                None
            }
        }
    ...
    let atlas_apps_ready = if refreshed_tools_succeeded {
        true
    } else if let Some(cfg) = mcp_servers.get(ATLAS_APPS_MCP_SERVER_NAME) {
        let immediate_ready = mcp_runtime
            .latest_wait_for_server_ready(ATLAS_APPS_MCP_SERVER_NAME, Duration::ZERO)
            .await;
        if immediate_ready {
            true
            ...
            let ready = mcp_runtime
                .latest_wait_for_server_ready(ATLAS_APPS_MCP_SERVER_NAME, timeout)
                .await;
    ...
    if atlas_apps_ready || !accessible_connectors.is_empty() {
        write_cached_accessible_connectors(cache_key, &accessible_connectors);
    }
...
fn accessible_connectors_cache_key(
    ...
    let chatgpt_user_id = auth.and_then(AtlasEngineAuth::get_chatgpt_user_id);
    let is_workspace_account = auth.is_some_and(AtlasEngineAuth::is_workspace_account);
    AccessibleConnectorsCacheKey {
...
fn write_cached_accessible_connectors(
    ...
    *cache_guard = Some(CachedAccessibleConnectors {
        key: cache_key,
        expires_at: Instant::now() + atlas_engine_connectors::CONNECTORS_CACHE_TTL,
...
async fn cached_directory_connectors_for_tool_suggest_with_auth(
    ...
    let Some(auth) = auth.filter(|auth| auth.uses_atlas_engine_backend()) else {
        return Vec::new();
    };
    ...
    atlas_engine_connectors::cached_directory_connectors(&cache_context).unwrap_or_default()
}
...
fn collect_accessible_connectors_from_mcp_tools<'a>(
    ...
    let tools = mcp_tools.filter_map(|tool| {
        if tool.server_name != ATLAS_APPS_MCP_SERVER_NAME {
            return None;
        }
...
pub(crate) fn mcp_approvals_reviewer_from_layers(
    config_layer_stack: &atlas_engine_config::ConfigLayerStack,
    ...
    let app_reviewer = if server_name == ATLAS_APPS_MCP_SERVER_NAME {
