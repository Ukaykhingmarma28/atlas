...
fn bash_snapshot_filters_invalid_exports() -> Result<()> {
    ...
    assert!(!stdout.contains("NEXTEST_BIN_EXE_atlas-engine-write-config-schema"));
...
fn zsh_snapshot_restores_tied_path() -> Result<()> {
    ...
    let readonly_restored = Command::new("/bin/zsh")
        .arg("-f")
        .arg("-c")
        .arg("set -e; . \"$1\"; export PATH='/atlas-engine-path':\"$PATH\"; print -r -- \"$PATH\"")
        .arg("zsh")
        .arg(&snapshot_path)
        .env_clear()
        .env("PATH", "/usr/bin:/bin")
...
async fn write_rollout_stub(atlas_agent_home: &Path, session_id: ThreadId) -> Result<PathBuf> {
    let dir = atlas_agent_home
        .join("sessions")
        .join("2025")
        .join("01")
        .join("01");
...
async fn cleanup_stale_snapshots_removes_orphans_and_keeps_live() -> Result<()> {
    let dir = tempdir()?;
    let atlas_agent_home = dir.path().abs();
    let snapshot_dir = atlas_agent_home.join(SNAPSHOT_DIR);
    ...
    write_rollout_stub(&atlas_agent_home, live_session).await?;
    fs::write(&live_snapshot, "live").await?;
    fs::write(&orphan_snapshot, "orphan").await?;
    fs::write(&invalid_snapshot, "invalid").await?;

    cleanup_stale_snapshots(&atlas_agent_home, ThreadId::new(), /*state_db*/ None).await?;
...
async fn cleanup_stale_snapshots_removes_stale_rollouts() -> Result<()> {
    ...
    let rollout_path = write_rollout_stub(&atlas_agent_home, stale_session).await?;
...
async fn cleanup_stale_snapshots_skips_active_session() -> Result<()> {
    ...
    let rollout_path = write_rollout_stub(&atlas_agent_home, active_session).await?;
    fs::write(&active_snapshot, "active").await?;

    set_file_mtime(&rollout_path, SNAPSHOT_RETENTION + Duration::from_secs(60))?;

    cleanup_stale_snapshots(&atlas_agent_home, active_session, /*state_db*/ None).await?;
