...
use atlas_engine_rollout_trace::CompactionCheckpointTracePayload;
...
async fn run_remote_compact_task_inner(
    ...
    match pre_compact_outcome {
        ...
        PreCompactHookOutcome::Stopped => {
            ...
            attempt
                .track(
                    sess.as_ref(),
                    atlas_engine_analytics::CompactionStatus::Interrupted,
                    Some(&error),
                    analytics_details,
                )
                .await;
    ...
    if result.is_ok() {
        ...
        if let PostCompactHookOutcome::Stopped = post_compact_outcome {
            attempt
                .track(sess.as_ref(), status, atlas_engine_error, analytics_details)
                .await;
    ...
    match result {
        Ok(()) => Ok(()),
        Err(err) if matches!(err.details(), AtlasEngineErrorDetails::TurnAborted) => Err(err),
        Err(err) => {
            sess.track_turn_atlas_engine_error(turn_context, &err);
...
) -> AtlasEngineResult<RemoteCompactionV2Output> {
...
async fn collect_compaction_output(
    ...
    if compaction_count != 1 {
        return Err(AtlasEngineErr::Fatal(format!(
...
fn build_v2_compacted_history(
    ...
    prompt_input_metadata: Vec<Option<AtlasEngineHarnessMetadata>>,
    compaction_output: ResponseItem,
...
pub(crate) fn truncate_retained_messages_for_remote_compaction(
        ...
        .rev()
    {
        ...
        if token_count <= remaining {
            ...
            let text_budget = if client_developer {
                available_tokens.saturating_sub(
                    source_tokens.saturating_sub(message_text_token_count(&group.source.item)),
                )
            } else {
                available_tokens
            };
            let Some(mut truncated_item) =
                truncate_message_text_to_token_budget(group.source, text_budget)
            else {
                continue;
            };
            if client_developer {
                ...
                if item_tokens > available_tokens {
                    let adjusted_budget = text_budget
...
mod tests {
    ...
    fn build_without_metadata(
        ...
        build_v2_compacted_history(
            input, metadata, output, /*retain_client_developer_messages*/ false,
        )
    }
    ...
    fn response_stream(events: Vec<AtlasEngineResult<ResponseEvent>>) -> ResponseStream {
    ...
    fn build_v2_compacted_history_preserves_retained_metadata_sidecar() {
        ...
        for enabled in [false, true] {
            let (history, _) = build_v2_compacted_history(
                ...
                vec![
                    None,
                    Some(AtlasEngineHarnessMetadata {
            ...
            if enabled {
                expected.insert(
                    ...
                    ResponseItemEnvelope {
                        item: client.clone(),
                        metadata: Some(AtlasEngineHarnessMetadata {
                            client_authored: true,
                        }),
                    },
                );
