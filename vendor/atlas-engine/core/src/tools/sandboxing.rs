    ...
    Fut: Future<Output = ReviewDecision>,
{
    ...
    services.session_telemetry.counter(
        "atlas_agent.approval.requested",
        /*inc*/ 1,
        &[
            ("tool", tool_name),
...
pub(crate) enum ExecApprovalRequirement {
    ...
    Skip {
        ...
        /// Only applies if the command fails to run in sandbox and atlas-agent prompts the user to run outside the sandbox.
...
pub(crate) struct SandboxAttempt<'a> {
    ...
    pub permissions: &'a atlas_engine_protocol::models::PermissionProfile,
    /// Canonical permissions before this host materializes workspace roots.
    pub exec_server_permissions: &'a atlas_engine_protocol::models::PermissionProfile,
    pub enforce_managed_network: bool,
    pub(crate) manager: &'a SandboxManager,
    pub(crate) sandbox_cwd: &'a PathUri,
    pub(crate) workspace_roots: &'a [PathUri],
    pub atlas_engine_linux_sandbox_exe: Option<&'a std::path::PathBuf>,
...
impl<'a> SandboxAttempt<'a> {
    ...
    ) -> Result<crate::sandboxing::ExecRequest, AtlasEngineErr> {
