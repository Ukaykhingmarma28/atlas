...
use atlas_engine_extension_api::ToolCall as ExtensionToolCall;
...
impl atlas_engine_extension_api::ToolContributor for ExtensionEchoContributor {
    fn tools(
        &self,
...
impl ToolExecutor<ExtensionToolCall> for ExtensionEchoExecutor {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Namespace(ResponsesApiNamespace {
            ...
            tools: vec![ResponsesApiNamespaceTool::Function(ResponsesApiTool {
                ...
                strict: true,
                parameters: atlas_engine_extension_api::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
    ...
    fn handle(&self, call: ExtensionToolCall) -> atlas_engine_tools::ToolExecutorFuture<'_> {
...
async fn build_tool_call_uses_namespace_for_registry_name() -> anyhow::Result<()> {
    ...
    let call = ToolRouter::build_tool_call(ResponseItem::FunctionCall {
        ...
        namespace: Some("mcp__atlas_apps__calendar".to_string()),
        arguments: "{}".to_string(),
    ...
    assert_eq!(
        call.tool_name,
        ToolName::namespaced("mcp__atlas_apps__calendar", tool_name)
    );
...
async fn specs_filter_deferred_dynamic_tools() -> anyhow::Result<()> {
    ...
    assert_eq!(
        namespace_function_names(&visible_specs, "atlas_engine_app"),
    ...
    assert!(namespace_function_names(&updated_specs, "atlas_engine_app").is_empty());
...
fn mcp_runtime(tool_info: atlas_engine_mcp::ToolInfo) -> RegisteredTool {
...
async fn extension_tool_executors_are_model_visible_and_dispatchable() -> anyhow::Result<()> {
    ...
    let router = test_tool_router(
        ...
        extension_tool_executors(
            &session,
            &atlas_engine_extension_api::ExtensionData::new(turn.sub_id.clone()),
        ),
        &turn.dynamic_tools,
    );
