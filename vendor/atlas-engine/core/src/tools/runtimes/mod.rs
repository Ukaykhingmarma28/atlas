...
*/
use crate::exec_env::ATLAS_AGENT_PERMISSION_PROFILE_ENV_VAR;
use crate::exec_env::ATLAS_AGENT_SESSION_ID_ENV_VAR;
use crate::exec_env::ATLAS_AGENT_THREAD_ID_ENV_VAR;
use crate::sandboxing::SandboxPermissions;
use crate::shell::Shell;
use crate::shell::ShellType;
use crate::tools::sandboxing::ToolError;
use atlas_engine_apply_patch::ATLAS_AGENT_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR;
use atlas_engine_core_plugins::PLUGIN_METRICS_OUTPUT_ENV_VAR;
#[cfg(unix)]
...
pub(crate) use atlas_engine_network_proxy::is_managed_proxy_env_var;
pub(crate) use atlas_engine_network_proxy::strip_managed_proxy_env;
...
/// PATH entries owned by Atlas Agent runtime setup.
///
...
/// runtime-only vars like `ATLAS_AGENT_THREAD_ID` without pretending they came from
/// the explicit override policy.
///
/// `runtime_path_prepends` contains Atlas Agent-owned PATH entries already applied to
...
pub(crate) fn maybe_wrap_shell_lc_with_snapshot(
    ...
    for key in [
        ATLAS_AGENT_SESSION_ID_ENV_VAR,
        ATLAS_AGENT_THREAD_ID_ENV_VAR,
        ATLAS_AGENT_PERMISSION_PROFILE_ENV_VAR,
...
fn build_override_exports(
    ...
    build_override_exports_for_keys("__ATLAS_AGENT_SNAPSHOT_OVERRIDE", &keys)
}
...
fn build_proxy_env_exports() -> (String, String) {
    ...
    let (captures, restores) =
        build_override_exports_for_keys("__ATLAS_AGENT_SNAPSHOT_PROXY_OVERRIDE", &keys);
    let key = PROXY_ACTIVE_ENV_KEY;
    let proxy_blocks = (
        format!("{captures}\n__ATLAS_AGENT_SNAPSHOT_PROXY_ENV_SET=\"${{{key}+x}}\""),
        format!(
            "if [ -n \"$__ATLAS_AGENT_SNAPSHOT_PROXY_ENV_SET\" ] || [ -n \"${{{key}+x}}\" ]; then\n{restores}\nfi"
        ),
    );
    let git_blocks = build_atlas_engine_proxy_git_ssh_command_exports();
    (
...
fn build_atlas_engine_proxy_git_ssh_command_exports() -> (String, String) {
    ...
    (
        ...
        format!(
            "case \"${{{key}-}}\" in\n  {marker_pattern}) __ATLAS_AGENT_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED=1 ;;\n  *) __ATLAS_AGENT_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED= ;;\nesac\nif [ -n \"$__ATLAS_AGENT_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED\" ]; then\n  if [ -z \"${{{key}+x}}\" ] || [ -n \"$__ATLAS_AGENT_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n    export {key}=\"$__ATLAS_AGENT_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  fi\nelif [ -n \"$__ATLAS_AGENT_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n  if [ -n \"$__ATLAS_AGENT_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET\" ]; then\n    export {key}=\"$__ATLAS_AGENT_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  else\n    unset {key}\n  fi\nfi"
        ),
    )
}
