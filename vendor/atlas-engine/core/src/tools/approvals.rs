...
#[cfg(unix)]
use atlas_engine_protocol::approvals::GuardianCommandSource;
    ...
    pub(crate) fn permission_request_payload(&self) -> PermissionRequestPayload {
        match self {
            ...
            Self::Execve { command, .. } => PermissionRequestPayload::bash(
                atlas_engine_shell_command::parse_command::shlex_join(command),
                /*description*/ None,
            ),
...
fn guardian_cwd(environment_id: &str, cwd: PathUri) -> std::io::Result<AbsolutePathBuf> {
    match cwd.to_abs_path() {
        Ok(cwd) => Ok(cwd),
        Err(err) if environment_id != atlas_engine_exec_server::LOCAL_ENVIRONMENT_ID => Err(err),
        Err(_) => {
...
impl ApprovalResolution {
    fn into_tool_result(self) -> Result<ReviewDecision, ToolError> {
        ...
        match self.decision {
            ...
            ReviewDecision::Abort => Err(ToolError::AtlasEngine(AtlasEngineErr::TurnAborted)),
