...
use atlas_engine_tools::TOOL_SEARCH_TOOL_NAME;
use atlas_engine_tools::ToolCall as ExtensionToolCall;
use atlas_engine_tools::ToolEnvironmentMode;
use atlas_engine_tools::ToolExecutor;
use atlas_engine_tools::ToolExposures;
...
use atlas_engine_tools::can_request_original_image_detail;
use atlas_engine_tools::collect_code_mode_exec_prompt_tool_definitions;
use atlas_engine_tools::collect_request_plugin_install_entries;
use atlas_engine_tools::default_namespace_description;
use atlas_engine_tools::request_user_input_available_modes;
use atlas_engine_tools::shell_command_backend_for_features;
use atlas_engine_tools::shell_type_for_model_and_features;
...
struct CoreToolPlanContext<'a> {
    ...
    mcp: &'a atlas_engine_mcp::McpBinding,
...
fn apply_mcp_tool_exposure_policy(
    turn_context: &TurnContext,
    mcp: &atlas_engine_mcp::McpBinding,
...
pub(crate) fn finalize_tool_router(
    ...
    if code_mode_enabled {
        for tool_name in [
            ToolName::plain(atlas_engine_code_mode::PUBLIC_TOOL_NAME),
            ToolName::plain(atlas_engine_code_mode::WAIT_TOOL_NAME),
        ] {
    ...
    if turn_context.config.tool_registry.error_on_tool_collisions {
        if let Some(tool_name) = registry.first_collision() {
            ...
            return Err(AtlasEngineErrorDetails::ToolCollision(name).into());
        }
        ...
        for tool in registry.entries() {
            ...
            match namespace_descriptions.entry(namespace.name.clone()) {
                ...
                Entry::Occupied(entry) if entry.get() != &namespace.description => {
                    return Err(AtlasEngineErrorDetails::ToolCollision(entry.key().clone()).into());
                }
                Entry::Occupied(_) => {}
            }
        }
...
fn spec_for_model_request(
    ...
    if matches!(tool_mode, ToolMode::CodeMode | ToolMode::CodeModeOnly)
        ...
        && atlas_engine_code_mode::is_code_mode_nested_tool(spec.name())
        && code_mode_tool_names
            .get(&atlas_engine_code_mode::normalize_code_mode_identifier(
                &atlas_engine_tools::code_mode_name_for_tool_name(tool_name),
            ))
            .is_some_and(|winner| winner == tool_name)
    {
        atlas_engine_tools::augment_tool_spec_for_code_mode(spec)
    } else {
        spec
    }
}
    ...
    provider.uses_openai_actor_authorization()
        || (provider.requires_openai_auth
            && turn_context
                .auth_manager
                .as_deref()
                .is_some_and(AuthManager::current_auth_uses_atlas_engine_backend))
}
...
fn register_code_mode_executors(
    ...
    for tool in registry.entries() {
        ...
        if !atlas_engine_code_mode::is_code_mode_nested_tool(&code_mode_name) {
            continue;
        }
        match code_mode_tool_names.entry(atlas_engine_code_mode::normalize_code_mode_identifier(
            &code_mode_name,
        )) {
            Entry::Vacant(entry) => {
    ...
    let execute_handler = CodeModeExecuteHandler::new(
        create_code_mode_tool(
            ...
            {
                atlas_engine_code_mode::ImageDetailVisibility::Hidden
            } else {
                atlas_engine_code_mode::ImageDetailVisibility::Visible
            },
        ),
...
fn code_mode_namespace_descriptions(
    specs: &[ToolSpec],
) -> BTreeMap<String, atlas_engine_code_mode::ToolNamespaceDescription> {
    let mut namespace_descriptions = BTreeMap::new();
    for spec in specs {
        let ToolSpec::Namespace(namespace) = spec else {
            continue;
        ...
        let entry = namespace_descriptions
            ...
            .or_insert_with(|| atlas_engine_code_mode::ToolNamespaceDescription {
...
fn compare_code_mode_tools(
    left: &atlas_engine_code_mode::ToolDefinition,
    right: &atlas_engine_code_mode::ToolDefinition,
    namespace_descriptions: &BTreeMap<String, atlas_engine_code_mode::ToolNamespaceDescription>,
) -> std::cmp::Ordering {
...
fn code_mode_namespace_name<'a>(
    tool: &atlas_engine_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, atlas_engine_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()
