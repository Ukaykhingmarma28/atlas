...
async fn wait_for_recorded_user_input(thread: &crate::AtlasEngineThread, expected: &[UserInput]) {
...
async fn multi_agent_v2_spawn_fork_turns_all_applies_agent_type_override() {
    ...
    let turn = TurnContext {
        ...
        multi_agent_version: atlas_engine_protocol::protocol::MultiAgentVersion::V2,
        ..turn
    };
...
async fn build_agent_spawn_config_uses_turn_context_values() {
    ...
    config.atlas_engine_linux_sandbox_exe = Some(PathBuf::from("/bin/echo"));
    turn.config = Arc::new(config);
