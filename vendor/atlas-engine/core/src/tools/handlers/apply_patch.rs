...
use atlas_engine_apply_patch::ApplyPatchFileUpdateMode;
use atlas_engine_apply_patch::Hunk;
use atlas_engine_apply_patch::StreamingPatchParser;
...
use atlas_engine_protocol::protocol::PatchApplyUpdatedEvent;
use atlas_engine_sandboxing::policy_transforms::effective_file_system_sandbox_policy;
use atlas_engine_sandboxing::policy_transforms::merge_permission_profiles;
use atlas_engine_sandboxing::policy_transforms::normalize_additional_permissions;
...
fn write_permissions_for_paths(
    ...
    file_system_sandbox_policy: &atlas_engine_protocol::permissions::FileSystemSandboxPolicy,
    cwd: &AbsolutePathBuf,
...
async fn effective_patch_permissions(
    ...
    atlas_engine_protocol::permissions::FileSystemSandboxPolicy,
)> {
...
fn patch_permissions_without_path_matching(
    ...
    (
        ...
        atlas_engine_protocol::permissions::FileSystemSandboxPolicy::unrestricted(),
    )
}
...
impl ApplyPatchHandler {
    async fn handle_call(
        ...
        let args = match atlas_engine_apply_patch::parse_patch(&patch_input) {
            Ok(args) => args,
            Err(parse_error) => {
        ...
        match atlas_engine_apply_patch::verify_apply_patch_args_with_mode(
            args,
        ...
        .await
        {
            atlas_engine_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
            ...
            atlas_engine_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                Err(FunctionCallError::RespondToModel(format!(
                    "apply_patch verification failed: {parse_error}"
                )))
            }
            atlas_engine_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                Err(FunctionCallError::RespondToModel(
                    "apply_patch handler received invalid patch input".to_string(),
                ))
            }
            atlas_engine_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                Err(FunctionCallError::RespondToModel(
...
pub(crate) async fn intercept_apply_patch(
    ...
    match atlas_engine_apply_patch::maybe_parse_apply_patch_verified_with_mode(
        command,
        cwd,
        apply_patch_file_update_mode(turn),
        fs,
    ...
    .await
    {
        ...
        atlas_engine_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}
