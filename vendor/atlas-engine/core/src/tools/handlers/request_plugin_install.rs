...
use atlas_engine_analytics::PluginInstallRequestSource;
use atlas_engine_analytics::PluginInstallRequested;
use atlas_engine_analytics::PluginInstallRequestedPlugin;
...
use atlas_engine_tools::DiscoverableToolAction;
use atlas_engine_tools::DiscoverableToolType;
use atlas_engine_tools::LIST_AVAILABLE_PLUGINS_TO_INSTALL_TOOL_NAME;
use atlas_engine_tools::REQUEST_PLUGIN_INSTALL_PERSIST_ALWAYS_VALUE;
use atlas_engine_tools::REQUEST_PLUGIN_INSTALL_PERSIST_KEY;
use atlas_engine_tools::REQUEST_PLUGIN_INSTALL_TOOL_NAME;
use atlas_engine_tools::RequestPluginInstallArgs;
use atlas_engine_tools::RequestPluginInstallResult;
use atlas_engine_tools::ToolName;
use atlas_engine_tools::ToolSpec;
use atlas_engine_tools::all_requested_connectors_picked_up;
use atlas_engine_tools::build_request_plugin_install_elicitation_request;
use atlas_engine_tools::filter_request_plugin_install_discoverable_tools_for_client;
use atlas_engine_tools::verified_connector_install_completed;
...
impl RequestPluginInstallHandler {
    async fn handle_call(
        ...
        if (requested_tool_type == Some(DiscoverableToolType::Plugin)
            ...
            && turn.app_server_client_name.as_deref() == Some("atlas-engine-tui")
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin install requests are not available in atlas-engine-tui yet".to_string(),
            ));
        }
...
async fn maybe_persist_disabled_install_request(
    ...
    if let Err(err) = persist_disabled_install_request(&turn.config.atlas_agent_home, tool).await {
        warn!(
            error = %err,
            tool_id = tool.id(),
...
async fn persist_disabled_install_request(
    atlas_agent_home: &atlas_engine_utils_absolute_path::AbsolutePathBuf,
    tool: &DiscoverableTool,
) -> anyhow::Result<()> {
...
async fn verify_request_plugin_install_completed(
    ...
    tool: &DiscoverableTool,
    auth: Option<&atlas_engine_login::AtlasEngineAuth>,
) -> bool {
    match tool {
...
async fn refresh_missing_requested_connectors(
    ...
    match session.hard_refresh_latest_atlas_apps_tools().await {
        Ok(mcp_tools) => {
        ...
        Err(err) => {
            warn!(
                "failed to refresh atlas-agent apps tools cache after plugin install request for {tool_id}: {err:#}"
            );
            None
        }
    }
