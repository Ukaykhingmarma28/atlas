...
impl ToolCallRuntime {
    ...
    pub(crate) fn handle_tool_call(
    ...
    ) -> impl std::future::Future<Output = Result<ResponseInputItem, AtlasEngineErr>> {
        ...
        async move {
            match future.await {
                ...
                Err(FunctionCallError::Fatal(message)) => Err(AtlasEngineErr::Fatal(message)),
    ...
    pub(crate) fn handle_tool_call_with_source(
        ...
        if self
            ...
            .config
            .features
            .enabled(atlas_engine_features::Feature::ExecutedToolCallMetadata)
    ...
    fn failure_response(call: ToolCall, err: FunctionCallError) -> ResponseInputItem {
        ...
        match call.payload {
            ...
            ToolPayload::Custom { .. } => ResponseInputItem::CustomToolCallOutput {
                call_id: call.call_id,
                name: None,
                output: atlas_engine_protocol::models::FunctionCallOutputPayload {
                    body: atlas_engine_protocol::models::FunctionCallOutputBody::Text(message),
                    success: Some(false),
                },
            },
    ...
    fn drop(&mut self) {
        ...
        macro_rules! log_tool_call {
            ($dispatch_duration_ms:expr, $handler_duration_ms:expr, $total_duration_ms:expr) => {
                info!(
                    event.name = "atlas_agent.tool_call",
                    trace_id = %atlas_engine_otel::current_span_trace_id().unwrap_or_default(),
...
mod tests {
    ...
    fn tool_call_timing_guard_ignores_code_mode_source() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            let call = ToolCall {
                tool_name: atlas_engine_tools::ToolName::plain("test_tool"),
    ...
    async fn cancellation_before_dispatch_admission_logs_dispatch_only_timing() -> anyhow::Result<()>
    {
        ...
        let tool_name = atlas_engine_tools::ToolName::plain("test_tool");
    ...
    impl atlas_engine_extension_api::ToolLifecycleContributor for FinishRecorder {
        fn on_tool_finish<'a>(
            &'a self,
    ...
    impl atlas_engine_extension_api::ToolLifecycleContributor for BlockingFinishContributor {
        fn on_tool_finish<'a>(
            &'a self,
            input: atlas_engine_extension_api::ToolFinishInput<'a>,
        ) -> atlas_engine_extension_api::ToolLifecycleFuture<'a> {
    ...
    -> anyhow::Result<()> {
        ...
        let session = Arc::new(session);
        let turn_context = Arc::new(turn_context);
        let tool_name = atlas_engine_tools::ToolName::plain("cleanup_tool");
