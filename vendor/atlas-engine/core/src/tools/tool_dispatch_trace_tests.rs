...
use atlas_engine_rollout_trace::ToolCallRequester;
...
impl atlas_engine_code_mode::CodeModeSessionProvider for MissingCellCodeModeSessionProvider {
    fn create_session<'a>(
        &'a self,
...
impl atlas_engine_code_mode::CodeModeSession for MissingCellCodeModeSession {
    fn execute<'a>(
        &'a self,
    ...
    fn shutdown<'a>(&'a self) -> atlas_engine_code_mode::CodeModeSessionResultFuture<'a, ()> {
        Box::pin(async { Ok(()) })
    }
}
...
async fn dispatch_lifecycle_trace_records_direct_and_code_mode_requesters() -> anyhow::Result<()> {
    ...
    let replayed = atlas_engine_rollout_trace::replay_bundle(single_bundle_dir(temp.path())?)?;
    assert_eq!(
    ...
    let result = registry
        .dispatch_any_with_terminal_outcome(
            test_invocation_with_payload(
                ...
                turn,
                "incompatible-call",
                atlas_engine_tools::ToolName::plain("test_tool"),
                ToolCallSource::Direct,
                ToolPayload::Custom {
                    input: "{}".to_string(),
                },
...
fn test_invocation(
    ...
    test_invocation_with_payload(
        ...
        turn,
        call_id,
        atlas_engine_tools::ToolName::plain(tool_name),
        source,
...
fn attach_test_trace(session: &mut Session, turn: &TurnContext, root: &Path) -> anyhow::Result<()> {
    ...
    let rollout_thread_trace =
        atlas_engine_rollout_trace::ThreadTraceContext::start_root_in_root_for_test(
            root,
    ...
    rollout_thread_trace.record_atlas_engine_turn_started(turn.sub_id.as_str());
