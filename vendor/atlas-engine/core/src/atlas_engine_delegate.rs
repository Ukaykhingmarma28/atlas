...
/// Start an interactive sub-Atlas Agent thread and return its runtime and IO channels.
///
...
pub(crate) async fn run_atlas_engine_thread_interactive(
    mut config: Config,
...
) -> Result<(Arc<Session>, SessionIo), AtlasEngineErr> {
    if config.permissions.approval_policy.value() != AskForApproval::Never {
        return Err(AtlasEngineErr::InvalidRequest(
            "Atlas Agent delegates require approval policy `never`".to_string(),
        ));
    }
    ...
    let extensions = if crate::guardian::is_guardian_reviewer_source(&session_source) {
        atlas_engine_extension_api::empty_extension_registry()
    } else {
...
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_atlas_engine_thread_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: SharedModelsManager,
    input: Vec<UserInput>,
...
#[cfg(test)]
#[path = "atlas_engine_delegate_tests.rs"]
mod tests;
