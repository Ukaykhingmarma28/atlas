...
fn foreign_agents_md_uses_environment_native_paths() {
    let (cwd, rendered_cwd) = if cfg!(windows) {
        (
            PathUri::parse("file:///atlas-agent%20runtime").expect("POSIX cwd URI"),
            "/atlas-agent runtime",
        )
    } else {
        (
            PathUri::parse("file:///C:/atlas-agent%20runtime").expect("Windows cwd URI"),
            r"C:\atlas-agent runtime",
        )
    };
...
async fn make_config(root: &TempDir, limit: usize, instructions: Option<&str>) -> TestConfig {
    ...
    let user_instructions = instructions.map(|text| UserInstructions {
        text: text.to_owned(),
        source: config.atlas_agent_home.join(DEFAULT_AGENTS_MD_FILENAME),
    });
    TestConfig {
        config,
        user_instructions,
...
async fn project_root_markers_are_honored_for_agents_discovery() {
    ...
    fs::write(root.path().join(".atlas-agent-root"), "").unwrap();
...
async fn instruction_sources_include_global_before_agents_md_docs() {
    ...
    let global_agents = cfg.atlas_agent_home.join(DEFAULT_AGENTS_MD_FILENAME);
    fs::create_dir_all(&cfg.atlas_agent_home).unwrap();
...
async fn skills_are_not_appended_to_agents_md() {
    ...
    let cfg = make_config(&tmp, /*limit*/ 4096, /*instructions*/ None).await;
    create_skill(
        cfg.atlas_agent_home.to_path_buf(),
        "pdf-processing",
        "extract from pdfs",
    );
...
fn create_skill(atlas_agent_home: PathBuf, name: &str, description: &str) {
    let skill_dir = atlas_agent_home.join(format!("skills/{name}"));
