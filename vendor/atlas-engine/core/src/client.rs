...
//!
//! `ModelClient` is intended to live for the lifetime of a Atlas Agent session and holds the stable
...
//! per-turn state such as the `x-atlas-engine-turn-state` token used for sticky routing.
//!
...
use atlas_engine_api::AgentIdentityTelemetry;
use atlas_engine_api::ApiError;
use atlas_engine_api::AuthProvider;
use atlas_engine_api::ChatCompletionsClient as ApiChatCompletionsClient;
use atlas_engine_api::ChatDialect;
use atlas_engine_api::ChatRequestInput;
use atlas_engine_api::CompactClient as ApiCompactClient;
use atlas_engine_api::CompactionInput as ApiCompactionInput;
use atlas_engine_api::Compression;
use atlas_engine_api::MemoriesClient as ApiMemoriesClient;
use atlas_engine_api::MemorySummarizeInput as ApiMemorySummarizeInput;
use atlas_engine_api::MemorySummarizeOutput as ApiMemorySummarizeOutput;
use atlas_engine_api::Provider as ApiProvider;
use atlas_engine_api::RawMemory as ApiRawMemory;
use atlas_engine_api::RealtimeCallClient as ApiRealtimeCallClient;
use atlas_engine_api::RealtimeSessionConfig as ApiRealtimeSessionConfig;
use atlas_engine_api::Reasoning;
use atlas_engine_api::ReasoningContext;
use atlas_engine_api::RequestTelemetry;
use atlas_engine_api::ReqwestTransport;
use atlas_engine_api::ResponseCreateWsRequest;
use atlas_engine_api::ResponsesApiRequest;
use atlas_engine_api::ResponsesClient as ApiResponsesClient;
use atlas_engine_api::ResponsesOptions as ApiResponsesOptions;
use atlas_engine_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use atlas_engine_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use atlas_engine_api::ResponsesWsRequest;
use atlas_engine_api::SharedAuthProvider;
use atlas_engine_api::SseTelemetry;
use atlas_engine_api::StreamOptions;
use atlas_engine_api::TransportError;
use atlas_engine_api::WebsocketTelemetry;
use atlas_engine_api::auth_header_telemetry;
use atlas_engine_api::build_chat_request;
use atlas_engine_api::build_session_headers;
use atlas_engine_api::create_text_param_for_request;
use atlas_engine_api::gateway_prompt_usage;
...
use atlas_engine_login::default_client::create_client_for_route;
...
use atlas_engine_protocol::config_types::Verbosity as VerbosityConfig;
...
use atlas_engine_rollout_trace::CompactionTraceContext;
use atlas_engine_rollout_trace::InferenceTraceAttempt;
use atlas_engine_rollout_trace::InferenceTraceContext;
use atlas_engine_tools::create_tools_json_for_responses_api;
use atlas_engine_tools::create_tools_json_for_responses_lite;
use atlas_engine_tools::create_tools_raw_json_for_responses_api;
...
use crate::responses_metadata::AtlasEngineResponsesMetadata;
use crate::responses_metadata::subagent_header_value;
use crate::util::emit_feedback_auth_recovery_tags;
use atlas_engine_feedback::FeedbackRequestTags;
use atlas_engine_feedback::emit_feedback_request_tags_with_auth_env;
use atlas_engine_login::auth::AgentIdentityAuthPolicy;
use atlas_engine_login::auth_env_telemetry::AuthEnvTelemetry;
use atlas_engine_login::auth_env_telemetry::collect_auth_env_telemetry;
use atlas_engine_model_provider::AgentIdentitySessionFallback;
use atlas_engine_model_provider::ProviderAuthScope;
use atlas_engine_model_provider::SharedModelProvider;
use atlas_engine_model_provider::create_model_provider;
#[cfg(test)]
use atlas_engine_model_provider_info::DEFAULT_WEBSOCKET_CONNECT_TIMEOUT_MS;
...
use atlas_engine_response_debug_context::extract_response_debug_context;
use atlas_engine_response_debug_context::extract_response_debug_context_from_api_error;
use atlas_engine_response_debug_context::telemetry_api_error_message;
use atlas_engine_response_debug_context::telemetry_transport_error_message;

pub const OPENAI_BETA_HEADER: &str = "OpenAI-Beta";
pub const X_ATLAS_AGENT_INSTALLATION_ID_HEADER: &str = "x-atlas-engine-installation-id";
pub const X_ATLAS_AGENT_ROUTING_HINT_HEADER: &str = "x-atlas-engine-routing-hint";
pub const X_ATLAS_AGENT_TURN_STATE_HEADER: &str = "x-atlas-engine-turn-state";
pub const X_ATLAS_AGENT_TURN_METADATA_HEADER: &str = "x-atlas-engine-turn-metadata";
pub const X_ATLAS_AGENT_PARENT_THREAD_ID_HEADER: &str = "x-atlas-engine-parent-thread-id";
pub const X_ATLAS_AGENT_WINDOW_ID_HEADER: &str = "x-atlas-engine-window-id";
...
const X_ATLAS_AGENT_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY: &str =
...
const X_OPENAI_INTERNAL_ATLAS_AGENT_RESPONSES_LITE_HEADER: &str =
    "x-openai-internal-atlas-engine-responses-lite";
...
///
/// This holds configuration and state that should be shared across turns within a Atlas Agent session
...
/// - The `x-atlas-engine-turn-state` sticky-routing token, which must be replayed for all requests within
///   the same turn.
///
/// Create a fresh `ModelClientSession` for each Atlas Agent turn. Reusing it across turns would replay
...
pub struct ModelClientSession {
    ...
    /// on turn start via the `x-atlas-engine-turn-state` response header. Once set, this value
    /// should be sent back to the server in the `x-atlas-engine-turn-state` request header for
...
impl ModelClient {
    ...
    ///
    /// All arguments are expected to be stable for the lifetime of a Atlas Agent session. Per-turn values
    ...
    pub fn new(
        ...
        let atlas_engine_api_key_env_enabled = model_provider
            .auth_manager()
            .as_ref()
            .is_some_and(|manager| manager.atlas_engine_api_key_env_enabled());
        let auth_env_telemetry =
            collect_auth_env_telemetry(model_provider.info(), atlas_engine_api_key_env_enabled);
    ...
    fn prompt_cache_key(&self, responses_metadata: &AtlasEngineResponsesMetadata) -> String {
    ...
    pub(crate) fn force_http_fallback(
        ...
        if activated {
            ...
            session_telemetry.counter(
                "atlas_agent.transport.fallback_to_http",
                /*inc*/ 1,
    ...
    pub(crate) async fn compact_conversation_history(
        ...
        responses_metadata: &AtlasEngineResponsesMetadata,
        ...
        let request_telemetry = Self::build_request_telemetry(
            ...
            AuthRequestTelemetryContext::new(
                client_setup.auth.as_ref().map(AtlasEngineAuth::auth_mode),
        ...
        if let Ok(header_value) = HeaderValue::from_str(&responses_metadata.installation_id) {
            extra_headers.insert(X_ATLAS_AGENT_INSTALLATION_ID_HEADER, header_value);
        }
        ...
        if let Some(header_value) = self.build_routing_hint_header(
            ...
            extra_headers.insert(X_ATLAS_AGENT_ROUTING_HINT_HEADER, header_value);
        }
    ...
    fn build_routing_hint_header(
        ...
        if !auth.is_some_and(AtlasEngineAuth::uses_atlas_engine_backend)
            || !provider.is_openai()
    ...
    async fn connect_websocket(
        &self,
        session_telemetry: &SessionTelemetry,
        api_provider: atlas_engine_api::Provider,
        api_auth: SharedAuthProvider,
        responses_metadata: &AtlasEngineResponsesMetadata,
        auth_context: AuthRequestTelemetryContext,
        request_route_telemetry: RequestRouteTelemetry,
        ...
        let result = match tokio::time::timeout(
            ...
            ApiWebSocketResponsesClient::new(api_provider, api_auth).connect(
                &self.http_client_factory,
                headers,
                atlas_engine_login::default_client::default_headers(),
                /*turn_state*/ None,
    ...
    async fn build_websocket_headers(
        ...
        if let Some(routing_hint) = &responses_metadata.routing_hint {
            headers.insert(X_ATLAS_AGENT_ROUTING_HINT_HEADER, routing_hint.clone());
        }
        if let Some(header_value) = self.generate_attestation_header_for().await {
...
impl ModelClientSession {
    ...
    fn responses_request_compression(&self, auth: Option<&AtlasEngineAuth>) -> Compression {
    ...
    async fn stream_chat_completions(
        ...
        loop {
            ...
            let built = build_chat_request(ChatRequestInput {
                ...
                items: &items,
                tools: &tools,
                max_output_tokens: atlas_engine_api::DEFAULT_MAX_OUTPUT_TOKENS,
    ...
    async fn stream_responses_api(
        ...
        loop {
            ...
            if let Some(header_value) = self.client.build_routing_hint_header(
                ...
                options
                    .extra_headers
                    .insert(X_ATLAS_AGENT_ROUTING_HINT_HEADER, header_value);
            }
            self.client
    ...
    /// Permanently disables WebSockets for this Atlas Agent session and resets WebSocket state.
    ///
...
fn stamp_ws_stream_request_start_ms(request: &mut ResponsesWsRequest<'_>) {
    ...
    payload
        ...
        .insert(
            X_ATLAS_AGENT_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY.to_string(),
...
///
/// These headers implement Atlas Agent-specific conventions:
///
/// - `x-atlas-engine-beta-features`: comma-separated beta feature keys enabled for the session.
/// - `x-atlas-engine-turn-state`: sticky routing token captured earlier in the turn.
fn build_responses_headers(
    ...
    {
        headers.insert("x-atlas-engine-beta-features", header_value);
    }
    if let Some(turn_state) = turn_state
        && let Some(state) = turn_state.get()
        && let Ok(header_value) = HeaderValue::from_str(state)
    {
        headers.insert(X_ATLAS_AGENT_TURN_STATE_HEADER, header_value);
    }
    headers
}
...
fn add_responses_lite_header(headers: &mut ApiHeaderMap, use_responses_lite: bool) {
    if use_responses_lite {
        headers.insert(
            X_OPENAI_INTERNAL_ATLAS_AGENT_RESPONSES_LITE_HEADER,
            HeaderValue::from_static("true"),
        );
    }
}
...
fn map_response_stream(
    api_stream: atlas_engine_api::ResponseStream,
    session_telemetry: SessionTelemetry,
    inference_trace_attempt: InferenceTraceAttempt,
    provider: SharedModelProvider,
) -> (ResponseStream, oneshot::Receiver<LastResponse>) {
    let atlas_engine_api::ResponseStream {
        rx_event,
        upstream_request_id,
    } = api_stream;
    let api_stream = atlas_engine_api::ResponseStream {
        rx_event,
...
/// the mapped `AtlasEngineErr` is returned to the caller.
...
struct WebsocketConnectParams<'a> {
    ...
    api_provider: atlas_engine_api::Provider,
    api_auth: SharedAuthProvider,
    responses_metadata: &'a AtlasEngineResponsesMetadata,
    auth_context: AuthRequestTelemetryContext,
    request_route_telemetry: RequestRouteTelemetry,
}
        ...
        && recovery.has_next()
    {
        ...
        return match recovery.next().await {
            ...
            Err(RefreshTokenError::Permanent(failed)) => {
                ...
                Err(AtlasEngineErr::RefreshTokenFailed(failed))
            }
