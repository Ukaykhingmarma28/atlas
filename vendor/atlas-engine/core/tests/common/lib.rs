...
use atlas_engine_utils_cargo_bin::CargoBinError;
use ctor::ctor;
...
pub use atlas_engine_core::test_support::TestAtlasEngineResponsesRequestKind;
pub use atlas_engine_core::test_support::responses_metadata;
use atlas_engine_utils_absolute_path::AbsolutePathBuf;
pub use atlas_engine_utils_absolute_path::test_support::PathBufExt;
pub use atlas_engine_utils_absolute_path::test_support::PathExt;
...
pub mod test_atlas_engine;
pub mod test_atlas_engine_exec;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    atlas_engine_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    atlas_engine_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(atlas_engine_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = atlas_engine_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("atlas-engine-rs"));
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\atlas-agent\AppData\Local\Temp"))
}
...
/// avoids clobbering a developer’s real `~/.atlas_engine`.
pub async fn load_default_config_for_test(atlas_agent_home: &TempDir) -> Config {
...
#[cfg(target_os = "linux")]
pub fn find_atlas_engine_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    atlas_engine_utils_cargo_bin::cargo_bin("atlas-engine-linux-sandbox")
}

pub async fn wait_for_event<F>(
    atlas_engine: &AtlasEngineThread,
    predicate: F,
) -> atlas_engine_protocol::protocol::EventMsg
where
    F: FnMut(&atlas_engine_protocol::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(atlas_engine, predicate, Duration::from_secs(1)).await
}
...
pub async fn submit_thread_settings(
    atlas_engine: &AtlasEngineThread,
    thread_settings: atlas_engine_protocol::protocol::ThreadSettingsOverrides,
) -> anyhow::Result<()> {
    use atlas_engine_protocol::protocol::EventMsg;
...
pub async fn wait_for_event_match<T, F>(atlas_engine: &AtlasEngineThread, matcher: F) -> T
where
    F: Fn(&atlas_engine_protocol::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(atlas_engine, |ev| matcher(ev).is_some()).await;
...
pub fn sandbox_env_var() -> &'static str {
    atlas_engine_core::spawn::ATLAS_AGENT_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    atlas_engine_core::spawn::ATLAS_AGENT_SANDBOX_NETWORK_DISABLED_ENV_VAR
}
...
macro_rules! skip_if_no_network {
    () => {{
        if ::std::env::var($crate::sandbox_network_env_var()).is_ok() {
            println!(
                "Skipping test because it cannot execute when network is disabled in a Atlas Agent sandbox."
            );
            return;
        }
    }};
...
#[macro_export]
macro_rules! atlas_engine_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
