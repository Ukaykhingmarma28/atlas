...
pub use atlas_engine_core::TurnInputRequest;
use atlas_engine_core::config::Config;
use atlas_engine_core::resolve_installation_id;
use atlas_engine_core::shell::Shell;
use atlas_engine_core::shell::get_shell_by_model_provided_path;
...
use atlas_engine_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
...
#[derive(Debug)]
pub struct TestEnv {
    environment: atlas_engine_exec_server::Environment,
...
impl TestEnv {
    ...
    pub async fn local_with_exec_server_url(exec_server_url: Option<String>) -> Result<Self> {
        ...
        let environment =
            atlas_engine_exec_server::Environment::create_for_tests(exec_server_url.clone())?;
        Ok(Self {
            environment,
            exec_server_url,
            cwd,
    ...
    pub fn environment(&self) -> &atlas_engine_exec_server::Environment {
        &self.environment
    }
...
pub struct TestAtlasEngineBuilder {
...
impl TestAtlasEngineBuilder {
    ...
    pub fn with_auth(mut self, auth: AtlasEngineAuth) -> Self {
        self.auth = auth;
        self
    }
    ...
    ///
    /// With no remote test configuration, or with `ATLAS_AGENT_TEST_ENVIRONMENT=local`, this uses a
    /// temporary local environment just like [`Self::build`]. `ATLAS_AGENT_TEST_ENVIRONMENT=docker` or
    /// `ATLAS_AGENT_TEST_ENVIRONMENT=wine-exec` selects the remote exec server configured by
    /// `ATLAS_AGENT_TEST_REMOTE_EXEC_SERVER_URL`; the legacy `ATLAS_AGENT_TEST_REMOTE_ENV` Docker-container
    ...
    pub async fn restart(
        ...
        previous.atlas_engine.shutdown_and_wait().await?;
    ...
    async fn build_with_home_and_base_url(
        ...
        #[cfg(target_os = "linux")]
        let atlas_engine_linux_sandbox_exe = Some(
            crate::find_atlas_engine_linux_sandbox_exe()
                .context("should find binary for atlas-engine-linux-sandbox")?,
        );
        #[cfg(not(target_os = "linux"))]
        let atlas_engine_linux_sandbox_exe = None;
        let local_runtime_paths = atlas_engine_exec_server::ExecServerRuntimePaths::new(
            std::env::current_exe()?,
            atlas_engine_linux_sandbox_exe,
        )?;
        let environment_manager = Arc::new(if include_local_environment {
            atlas_engine_exec_server::EnvironmentManager::create_for_tests_with_local(
                exec_server_url,
                local_runtime_paths,
            )
            .await
        } else {
            atlas_engine_exec_server::EnvironmentManager::create_for_tests(
                exec_server_url,
    ...
    async fn build_from_config(
        ...
        resume_from: Option<PathBuf>,
        test_env: TestEnv,
        environment_manager: Arc<atlas_engine_exec_server::EnvironmentManager>,
    ) -> anyhow::Result<TestAtlasEngine> {
        let auth = self.auth.clone();
        let state_db = atlas_engine_core::init_state_db(&config).await;
        ...
        let user_instructions_provider =
            self.user_instructions_provider.clone().unwrap_or_else(|| {
                Arc::new(AtlasAgentHomeUserInstructionsProvider::new(
        ...
        let thread_manager = ThreadManager::new(
            ...
            thread_store,
            atlas_engine_core::local_agent_graph_store_from_state_db(state_db.as_ref()),
            installation_id,
        ...
        {
            atlas_engine_core::test_support::with_code_mode_host_program(
                thread_manager,
                code_mode_host_program,
                &config,
            )
        ...
        let new_conversation = match (resume_from, user_shell_override) {
            (Some(path), Some(user_shell_override)) => {
                ...
                Box::pin(
                    atlas_engine_core::test_support::resume_thread_from_rollout_with_user_shell_override(
            ...
            (None, Some(user_shell_override)) => {
                Box::pin(
                    atlas_engine_core::test_support::start_thread_with_user_shell_override(
                        thread_manager.as_ref(),
                        config.clone(),
                        user_shell_override,
                        self.supports_openai_form_elicitation,
        ...
        Ok(TestAtlasEngine {
            ...
            cwd,
            config,
            atlas_engine: new_conversation.thread,
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = atlas_engine_utils_cargo_bin::cargo_bin("atlas-agent") {
            config.atlas_engine_self_exe = Some(path);
        } else if let Ok(path) = atlas_engine_utils_cargo_bin::cargo_bin("atlas-engine-exec") {
            // `atlas-engine-exec` also supports `--atlas-engine-run-as-apply-patch`, so use it
        ...
        {
            let atlas_engine = bin_dir.join("atlas-agent");
            let atlas_engine_exec = bin_dir.join("atlas-engine-exec");
            if atlas_engine.is_file() {
                config.atlas_engine_self_exe = Some(atlas_engine);
            } else if atlas_engine_exec.is_file() {
                config.atlas_engine_self_exe = Some(atlas_engine_exec);
            }
        }
...
pub struct TestAtlasEngine {
    pub home: Arc<TempDir>,
    pub cwd: Arc<TempDir>,
    pub atlas_engine: Arc<AtlasEngineThread>,
...
impl TestAtlasEngine {
    pub fn cwd_path(&self) -> &Path {
        self.cwd.path()
    }

    pub fn atlas_agent_home_path(&self) -> &Path {
        self.config.atlas_agent_home.as_path()
    }
    ...
    async fn submit_turn_with_context(
        ...
        let turn_id = wait_for_event_match(&self.atlas_engine, |event| match event {
...
pub struct TestAtlasEngineHarness {
    server: MockServer,
    test: TestAtlasEngine,
}

impl TestAtlasEngineHarness {
    pub async fn with_builder(mut builder: TestAtlasEngineBuilder) -> Result<Self> {
        let server = start_mock_server().await;
        let test = builder.build(&server).await?;
        Ok(Self { server, test })
    }

    pub async fn with_auto_env_builder(mut builder: TestAtlasEngineBuilder) -> Result<Self> {
        let server = start_mock_server().await;
    ...
    pub fn test(&self) -> &TestAtlasEngine {
        &self.test
    }
...
pub fn test_atlas_engine() -> TestAtlasEngineBuilder {
    TestAtlasEngineBuilder {
        ...
        auth: AtlasEngineAuth::from_api_key("dummy"),
        pre_build_hooks: vec![],
