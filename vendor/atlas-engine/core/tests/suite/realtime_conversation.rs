...
use atlas_engine_login::OPENAI_API_KEY_ENV_VAR;
...
use atlas_engine_protocol::protocol::RealtimeNoopRequested;
...
const REALTIME_BACKEND_PROMPT: &str = atlas_engine_prompts::BACKEND_PROMPT;
...
const REALTIME_CONVERSATION_TEST_SUBPROCESS_ENV_VAR: &str =
    "ATLAS_AGENT_REALTIME_CONVERSATION_TEST_SUBPROCESS";
...
async fn seed_recent_thread(
    ...
    let db = test.atlas_engine.state_db().context("state db enabled")?;
    ...
    let mut metadata_builder = atlas_engine_state::ThreadMetadataBuilder::new(
        thread_id,
        rollout_path,
        updated_at,
        SessionSource::Cli,
...
async fn conversation_start_audio_text_close_round_trip() -> Result<()> {
    ...
    let started = wait_for_event_match(&test.atlas_engine, |msg| match msg {
    ...
    let session_updated = wait_for_event_match(&test.atlas_engine, |msg| match msg {
    ...
    let audio_out = wait_for_event_match(&test.atlas_engine, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
...
async fn conversation_webrtc_frameless_chatgpt_sends_codex_headers_to_backend() -> Result<()> {
    ...
    Mock::given(method("POST"))
        .and(path_regex(".*/backend-api/atlas-agent/realtime/calls$"))
        .and(capture.clone())
        .respond_with(
    ...
    let backend_base_url = format!("{}/backend-api/atlas-agent", server.uri());
    ...
    let created = wait_for_event_match(&test.atlas_engine, |msg| match msg {
    ...
    assert_eq!(
        ...
        (
            "/backend-api/atlas-agent/realtime/calls",
...
async fn conversation_webrtc_close_while_sideband_connecting_drops_pending_join() -> Result<()> {
    ...
    let sdp = wait_for_event_match(&test.atlas_engine, |msg| match msg {
    ...
    timeout(Duration::from_secs(10), async {
        ...
        loop {
            match test.atlas_engine.next_event().await?.msg {
                EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
                    payload: RealtimeEvent::Error(message),
                }) => {
        ...
        test.atlas_engine.wait_until_terminated().await;
        Ok::<(), anyhow::Error>(())
    })
    .await
...
async fn conversation_webrtc_sideband_connect_failure_closes_with_error() -> Result<()> {
    ...
    let err = wait_for_event_match(&test.atlas_engine, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::Error(message),
        }) => Some(message.clone()),
        _ => None,
...
async fn conversation_start_preflight_failure_emits_realtime_error_only() -> Result<()> {
    ...
    let closed = timeout(Duration::from_millis(200), async {
        wait_for_event_match(&test.atlas_engine, |msg| match msg {
            EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
            _ => None,
        })
        .await
...
async fn conversation_second_start_replaces_runtime() -> Result<()> {
    ...
    test.atlas_engine
        ...
        .await?;
    let _ = wait_for_event_match(&test.atlas_engine, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::AudioOut(frame),
...
async fn conversation_startup_context_current_thread_selects_many_turns_by_budget() -> Result<()> {
    ...
    let resumed_thread = test
        ...
        .resume_thread_with_history(
            ...
            auth_manager_from_auth(AtlasEngineAuth::from_api_key("dummy")),
            /*parent_trace*/ None,
            ClientMcpExtensions::default(),
        )
        .await?;
    let atlas_engine = resumed_thread.thread;
...
async fn conversation_startup_context_falls_back_to_workspace_map() -> Result<()> {
    ...
    let test = builder.build_with_websocket_server(&startup_server).await?;
    fs::create_dir_all(test.workspace_path("atlas-engine-rs/core"))?;
    ...
    assert!(startup_context.contains("atlas-engine-rs/"));
...
async fn conversation_user_text_turn_is_not_sent_to_realtime() -> Result<()> {
    ...
    let turn_complete = wait_for_event_match(&test.atlas_engine, |event| match event {
...
async fn conversation_flushes_assistant_deltas_every_200ms_for_v3_handoff() -> Result<()> {
    ...
    test.atlas_engine
        .submit(Op::RealtimeConversationStart(ConversationStartParams {
            ...
            atlas_engine_response_item_prefix: None,
            atlas_engine_response_handoff_mode:
                atlas_engine_protocol::protocol::AtlasEngineResponseHandoffMode::BemTags,
            atlas_engine_response_handoff_channel_prefixes: Some(BTreeMap::from([
                (
...
async fn inbound_handoff_request_starts_turn() -> Result<()> {
    ...
    let turn_id = loop {
        let event = test.atlas_engine.next_event().await?;
...
async fn delegated_turn_user_role_echo_does_not_redelegate_and_still_forwards_audio() -> Result<()>
{
    ...
    let mut builder = test_atlas_engine().with_model("gpt-5.4").with_config({
        let realtime_base_url = realtime_server.uri().to_string();
        move |config| {
            config.experimental_realtime_ws_base_url = Some(realtime_base_url);
            config.realtime.version = RealtimeWsVersion::V1;
