...
async fn atlas_engine_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    atlas_engine_returns_json_result("gpt-5.4".to_string()).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn atlas_engine_returns_json_result_for_gpt5_atlas_engine() -> anyhow::Result<()> {
    atlas_engine_returns_json_result("gpt-5.4".to_string()).await
}

async fn atlas_engine_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let message = wait_for_event(&atlas_engine, |ev| matches!(ev, EventMsg::AgentMessage(_))).await;
