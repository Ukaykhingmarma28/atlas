...
async fn assert_compaction_uses_turn_lifecycle_id(atlas_engine: &std::sync::Arc<atlas_engine_core::AtlasEngineThread>) {
    ...
    while turn_completed_id.is_none() {
        let event = atlas_engine.next_event().await.expect("next event");
        match event.msg {
...
async fn summarize_context_three_requests_and_instructions() {
    ...
    // Build config pointing to the mock server and spawn Atlas Agent.
    ...
    // Shut down Atlas Agent to flush rollout entries before inspecting the file.
    atlas_engine.submit(Op::Shutdown).await.unwrap();
...
async fn manual_compact_emits_api_and_local_token_usage_events() {
    ...
    let atlas_engine = builder.build(&server).await.unwrap().atlas_engine;
    ...
    let first = wait_for_event_match(&atlas_engine, |ev| match ev {
    ...
    let last = wait_for_event_match(&atlas_engine, |ev| match ev {
        EventMsg::TokenCount(tc) => tc
            .info
            .as_ref()
            .map(|info| info.last_token_usage.total_tokens),
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        ...
        atlas_engine_protocol::models::ResponseItem::Compaction {
            id: None,
...
async fn pre_sampling_compact_runs_on_switch_to_smaller_context_model() {
    ...
    assert_compaction_uses_turn_lifecycle_id(&test.atlas_engine).await;
...
async fn pre_sampling_compact_falls_back_from_retired_previous_model_after_rename() {
    ...
    let request_log = mount_response_sequence(
        ...
        vec![
            ...
            invalid_request_response(format!(
                "The '{retired_model}' model is not supported when using Atlas Agent with a ChatGPT account."
            )),
            sse_response(sse(vec![
                json!({
                    "type": "response.output_item.done",
    ...
    let initial = initial_builder
        .build(&server)
        .await
        .expect("build initial test atlas-agent");
    ...
    let model_provider = openai_model_provider(&server);
    let mut resumed_builder = test_atlas_engine()
        .with_auth(AtlasEngineAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(retired_model)
        .with_config(move |config| {
            config.model_provider = model_provider;
    ...
    assert_compaction_uses_turn_lifecycle_id(&resumed.atlas_engine).await;
...
async fn manual_compact_non_context_failure_retries_then_emits_task_error() {
    ...
    let task_error_message = wait_for_event_match(&atlas_engine, |event| match event {
...
async fn manual_compact_twice_preserves_latest_user_messages() {
    ...
    assert_eq!(
        ...
        requests[1].header("x-atlas-engine-window-id").as_deref()
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        requests[2].header("x-atlas-engine-window-id").as_deref()
    );
    assert_ne!(
...
-> Result<()> {
    ...
    let mut resume_builder = test_atlas_engine()
        .with_home(Arc::clone(&home))
        .with_auth(AtlasEngineAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(move |config| {
    ...
    assert_eq!(
        resumed.atlas_engine.instruction_sources().await,
        vec![PathUri::from_abs_path(&source)],
