...
use atlas_engine_utils_output_truncation::approx_tokens_from_byte_count;
...
async fn wait_for_raw_unified_exec_output(
    ...
    let content = wait_for_event_match(&test.atlas_engine, |event| match event {
...
async fn unified_exec_intercepts_apply_patch_exec_command() -> Result<()> {
    ...
    let builder = test_atlas_engine().with_config(|config| {
        config.use_experimental_unified_exec_tool = true;
        config
            .features
            .enable(Feature::UnifiedExec)
...
async fn unified_exec_emits_exec_command_begin_event() -> Result<()> {
    ...
    let begin_event = wait_for_event_match(&test.atlas_engine, |msg| match msg {
...
async fn unified_exec_emits_exec_command_end_event() -> Result<()> {
    ...
    let end_event = wait_for_event_match(&test.atlas_engine, |msg| match msg {
...
async fn unified_exec_emits_output_delta_for_exec_command() -> Result<()> {
    ...
    let event = wait_for_event_match(&test.atlas_engine, |msg| match msg {
        EventMsg::ExecCommandEnd(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
...
async fn unified_exec_full_lifecycle_with_background_end_event() -> Result<()> {
    ...
    loop {
        let msg = wait_for_event(&test.atlas_engine, |_| true).await;
        match msg {
...
) -> Result<(TestAtlasEngine, PermissionProfile)> {
...
async fn wait_for_unified_exec_end(
...
) -> (atlas_engine_protocol::protocol::ExecCommandEndEvent, bool) {
    ...
    let end_event = loop {
        ...
        let event = tokio::time::timeout(remaining, test.atlas_engine.next_event())
            .await
...
async fn unified_exec_emits_one_begin_and_one_end_event() -> Result<()> {
    ...
    let mut task_completed = false;
    loop {
        let event_msg = wait_for_event(&test.atlas_engine, |_| true).await;
        match event_msg {
...
async fn unified_exec_keeps_long_running_session_after_turn_end() -> Result<()> {
    ...
    let begin_event = wait_for_event_match(&atlas_engine, |msg| match msg {
...
async fn unified_exec_interrupt_preserves_long_running_session() -> Result<()> {
    ...
    let _begin_event = wait_for_event_match(&atlas_engine, |msg| match msg {
        EventMsg::ExecCommandBegin(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
    ...
    atlas_engine.submit(Op::CleanBackgroundTerminals).await?;
    wait_for_process_exit(&pid).await?;
...
async fn unified_exec_timeout_and_followup_poll() -> Result<()> {
    ...
    loop {
        let event = test.atlas_engine.next_event().await.expect("event");
...
"#
    ...
    let end_event = wait_for_event_match(&test.atlas_engine, |event| match event {
