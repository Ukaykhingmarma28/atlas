...
use atlas_engine_protocol::protocol::ExecPolicyAmendment;
...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestAtlasEngine) -> (PathBuf, String) {
        match self {
    ...
    async fn prepare(
        ...
        match self {
            ...
            ActionKind::ApplyPatchShell { target, content } => {
                ...
                // Bazel may need to launch the configured Atlas Agent helper binary
...
impl Expectation {
    fn verify(&self, test: &TestAtlasEngine, result: &CommandResult) -> Result<()> {
        match self {
...
fn assert_active_workspace_permission_profile(test: &TestAtlasEngine) {
    assert_eq!(
...
async fn wait_for_completion_without_approval(test: &TestAtlasEngine) {
    let event = wait_for_event(&test.atlas_engine, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_completion(test: &TestAtlasEngine) {
...
async fn wait_for_spawned_thread(test: &TestAtlasEngine) -> Result<Arc<AtlasEngineThread>> {
...
/// same sandbox with `atlas_engine sandbox -P`. The nested invocation must also pass
...
printenv ATLAS_AGENT_PERMISSION_PROFILE
touch {outside_path:?}
"#
        ),
    )?;
    ...
    assert_eq!(
        ...
        "the unsandboxed script should inherit ATLAS_AGENT_PERMISSION_PROFILE from the shell command"
    );
    assert!(
        outside_path.exists(),
...
/// profile names before passing one to `atlas_engine sandbox -P`.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[cfg(unix)]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Print an ssh command that recreates the current Atlas Agent sandbox remotely."
    )
...
def main():
    args = parse_args()
    profile_name = os.environ.get("ATLAS_AGENT_PERMISSION_PROFILE")
    if not profile_name:
        raise SystemExit("ATLAS_AGENT_PERMISSION_PROFILE must not be empty")
    if profile_name not in ALLOWED_PROFILES:
        raise SystemExit("ATLAS_AGENT_PERMISSION_PROFILE is not allowlisted")
...
"#
    ...
    assert_eq!(
        sandbox_argv.len(),
        9,
        "expected atlas-agent sandbox ... bash -lc CMD"
    );
    assert_eq!(
        sandbox_argv[..8],
        [
        ...
        "remote_bash.py should use the allowlisted inherited profile and managed configuration to reconstruct the Atlas Agent sandbox"
    );
