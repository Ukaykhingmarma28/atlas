    ...
    S: Subscriber,
{
    fn on_event(&self, event: &Event<'_>, _context: Context<'_, S>) {
        if event.metadata().target() == "atlas_engine_http_client::transport" {
            self.record_request_after_retry();
            return;
        }

        if event.metadata().target() != "atlas_engine_otel.trace_safe" {
            return;
        }

        let mut visitor = RetryTelemetryVisitor::default();
        event.record(&mut visitor);
        if visitor.name.as_deref() != Some("atlas_agent.retry") {
            return;
        }
...
async fn submit_user_input(test: &TestAtlasEngine, text: &str) -> Result<()> {
    test.atlas_engine
...
async fn wait_for_turn_completion(test: &TestAtlasEngine) {
    let EventMsg::TurnComplete(completed) = wait_for_event(&test.atlas_engine, |event| {
        ...
        match wait_for_event(&test.atlas_engine, |_| true).await {
            EventMsg::Error(error) => {
                ...
                assert_eq!(
                    ...
                    Some(AtlasEngineErrorInfo::ServerOverloaded)
                );
                assert_eq!(
                    error.message,
            ...
            EventMsg::TurnComplete(event) => {
                assert_eq!(
                    event.error.and_then(|error| error.atlas_engine_error_info),
                    Some(AtlasEngineErrorInfo::ServerOverloaded)
                );
                break;
            }
    ...
    loop {
        match wait_for_event(&test.atlas_engine, |_| true).await {
            EventMsg::Error(error) => {
                ...
                assert_eq!(
                    error.atlas_engine_error_info,
                    Some(AtlasEngineErrorInfo::ResponseTooManyFailedAttempts {
