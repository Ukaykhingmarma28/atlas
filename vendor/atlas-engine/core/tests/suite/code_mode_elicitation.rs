...
await tools.exec_command({
  ...
  justification: "test command approval",
    ...
    .await?;
    let approval = wait_for_event_match(&harness.test.atlas_engine, |event| match event {
...
await tools.request_permissions({
  ...
  permissions: { network: { enabled: true } },
    ...
    .await?;
    let request = wait_for_event(&harness.test.atlas_engine, |event| {
        matches!(
            event,
