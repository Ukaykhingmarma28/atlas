...
const REMOTE_MODEL: &str = "atlas-engine-test-ttl";
const VERSIONED_MODEL: &str = "atlas-engine-test-versioned";
const MISSING_VERSION_MODEL: &str = "atlas-engine-test-missing-version";
const DIFFERENT_VERSION_MODEL: &str = "atlas-engine-test-different-version";
...
async fn renews_cache_ttl_on_matching_models_etag() -> Result<()> {
    ...
    let cache_path = config.atlas_agent_home.join(CACHE_FILE);
    ...
    atlas_engine
        .start_or_steer_turn(
            ...
            .with_thread_settings(ThreadSettingsOverrides {
                ...
                approval_policy: Some(atlas_engine_protocol::protocol::AskForApproval::Never),
...
async fn matching_models_etag_does_not_rewrite_recent_cache() -> Result<()> {
    ...
    let cache_path = test.config.atlas_agent_home.join(CACHE_FILE);
