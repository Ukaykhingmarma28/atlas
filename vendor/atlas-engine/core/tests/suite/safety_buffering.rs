...
-> anyhow::Result<()> {
    ...
    mount_response_once(
        ...
        .insert_header("x-atlas-engine-safety-buffering-enabled", "true")
        .insert_header("x-atlas-engine-safety-buffering-faster-model", FASTER_MODEL),
    )
    .await;
    ...
    let event = wait_for_event_match(&test.atlas_engine, |event| match event {
