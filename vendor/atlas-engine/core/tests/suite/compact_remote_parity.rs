...
const FIXED_CWD: &str = "/tmp/atlas_engine_remote_compaction_parity_workspace";
...
impl AuthCase {
    fn build(self) -> AtlasEngineAuth {
        match self {
            AuthCase::ChatGpt => AtlasEngineAuth::create_dummy_chatgpt_auth_for_testing(),
            AuthCase::ApiKey => AtlasEngineAuth::from_api_key("dummy"),
        }
    }
}
...
async fn run_manual_hook_session(mode: Mode) -> Result<Value> {
    ...
    let home = harness.test().atlas_agent_home_path();
...
async fn build_auto_harness(mode: Mode) -> Result<TestAtlasEngineHarness> {
    build_harness_inner(
        mode,
        RunSettings::default(),
        /*hooks*/ false,
...
fn rollout_path(harness: &TestAtlasEngineHarness) -> PathBuf {
    harness
        .test()
        .session_configured
        .rollout_path
...
fn normalize_tmp_prefix_before_marker(text: &mut String, marker: &str) {
    ...
    while let Some(relative_marker_index) = text[search_start..].find(marker) {
        ...
        if let Some(start_index) = start {
            text.replace_range(start_index..marker_index, "<ATLAS_AGENT_HOME>");
            search_start = start_index + "<ATLAS_AGENT_HOME>".len() + marker.len();
        } else {
...
fn normalize_string_rewrites_linux_temp_skill_paths() {
    ...
    assert_eq!(
        text,
        "file: <ATLAS_AGENT_HOME>/skills/.system/imagegen/SKILL.md and \
         <ATLAS_AGENT_HOME>/skills/custom/SKILL.md"
    );
}
