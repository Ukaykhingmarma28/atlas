...
async fn submit_user_turn(
    test: &core_test_support::test_atlas_engine::TestAtlasEngine,
    prompt: &str,
...
"#;
    ...
    let mut migrated_builder = test_atlas_engine().with_config(|config| {
        let policy_path = config.atlas_agent_home.join("rules/default.rules");
    ...
    let mut ignored_builder = test_atlas_engine().with_config(|config| {
        let policy_path = config.atlas_agent_home.join("rules/default.rules");
        fs::create_dir_all(policy_path.parent().expect("rules directory"))
            .expect("create rules directory");
        fs::write(policy_path, LEGACY_POLICY).expect("write legacy policy");
    ...
    let ignored_policy_path = ignored.atlas_agent_home_path().join("rules/default.rules");
...
async fn granular_complex_forced_rm_requests_approval_when_allowed() -> Result<()> {
    ...
    let approval_event = wait_for_event(&test.atlas_engine, |event| {
        matches!(
            event,
...
async fn execpolicy_blocks_shell_invocation() -> Result<()> {
    let mut builder = test_atlas_engine().with_config(|config| {
        let policy_path = config.atlas_agent_home.join("rules").join("policy.rules");
        fs::create_dir_all(
            policy_path
                .parent()
    ...
    let EventMsg::ExecCommandEnd(end) = wait_for_event(&test.atlas_engine, |event| {
...
"#,
        ...
        .with_config(|config| {
            config
                ...
                .enable(Feature::UnifiedExec)
                .expect("test config should allow feature update");
            let policy_path = config.atlas_agent_home.join("rules").join("broken.rules");
            fs::create_dir_all(
                policy_path
                    .parent()
                    .expect("policy directory must have a parent"),
