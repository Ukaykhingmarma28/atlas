...
use atlas_engine_models_manager::model_info::BASE_INSTRUCTIONS;
...
const REMOTE_MODEL_SLUG: &str = "atlas-engine-test";
...
async fn unknown_model_sends_builtin_instructions() -> Result<()> {
    ...
    .await;
    let mut builder = test_atlas_engine().with_model("future-custom-model");
...
async fn remote_models_get_model_info_uses_longest_matching_prefix() -> Result<()> {
    ...
    let specific = ModelInfo {
        display_name: "GPT 5.3 Atlas Agent".to_string(),
    ...
    let manager = atlas_engine_core::test_support::models_manager_with_provider(
        atlas_agent_home.path().to_path_buf(),
        atlas_engine_core::test_support::auth_manager_from_auth(auth),
        provider,
    );
...
async fn remote_models_config_context_window_override_clamps_to_max_context_window() -> Result<()> {
    ...
    let turn_started_event = wait_for_event(&atlas_engine, |event| {
        matches!(
            event,
...
async fn namespaced_model_slug_uses_catalog_metadata_without_fallback_warning() -> Result<()> {
    ...
    loop {
        let event = wait_for_event(&atlas_engine, |_| true).await;
        match event {
...
async fn remote_models_truncation_policy_without_override_preserves_remote() -> Result<()> {
    ...
    let slug = "atlas-engine-test-truncation-policy";
...
async fn remote_models_truncation_policy_with_tool_output_override() -> Result<()> {
    ...
    let slug = "atlas-engine-test-truncation-override";
    let remote_model = test_remote_model_with_policy(
        slug,
        ModelVisibility::List,
        /*priority*/ 1,
...
async fn remote_models_hide_picker_only_models() -> Result<()> {
    ...
    let hidden = available
        .iter()
        .find(|model| model.model == "atlas-engine-auto-balanced")
...
fn bundled_default_model_slug() -> String {
    atlas_engine_core::test_support::all_model_presets()
        .iter()
