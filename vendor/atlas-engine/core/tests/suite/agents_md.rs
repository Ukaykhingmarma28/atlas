...
use core_test_support::test_atlas_engine::RecordingUserInstructionsProvider;
...
async fn agents_instructions(mut builder: TestAtlasEngineBuilder) -> Result<String> {
...
async fn loads_user_instructions_without_a_primary_environment() -> Result<()> {
    ...
    let provider = Arc::new(RecordingUserInstructionsProvider::new(Arc::new(
        AtlasAgentHomeUserInstructionsProvider::new(AbsolutePathBuf::try_from(
...
async fn invalid_utf8_global_instructions_are_lossy() -> Result<()> {
    ...
    let mut builder = test_atlas_engine().with_home(home);
    let test = builder.build(&server).await?;
...
async fn cold_resume_invalidates_deleted_legacy_agents_md_once() -> Result<()> {
    ...
    let mut initial_builder = test_atlas_engine().with_home(Arc::clone(&home));
    let initial = initial_builder.build(&server).await?;

    // Assert the pre-resume thread reports the source used to create its snapshot.
    assert_eq!(
        initial.atlas_engine.instruction_sources().await,
    ...
    let mut resume_builder = test_atlas_engine().with_home(Arc::clone(&home));
...
async fn fork_injects_changed_agents_md_once() -> Result<()> {
    ...
    let mut builder = test_atlas_engine().with_home(Arc::clone(&home));
    let parent = builder.build(&server).await?;

    // Assert the parent reports the source used to create its snapshot.
    assert_eq!(
        parent.atlas_engine.instruction_sources().await,
        vec![PathUri::from_abs_path(&source)],
        "parent reports the creation-time global source"
    );
    parent.submit_turn("persist instructions").await?;
    parent.atlas_engine.ensure_rollout_materialized().await;
    parent.atlas_engine.flush_rollout().await?;
    let rollout_path = parent.atlas_engine.rollout_path().expect("rollout path");
    ...
    fork_config.atlas_engine_self_exe = parent.config.atlas_engine_self_exe.clone();
    let forked = parent
        .thread_manager
        .fork_thread(
