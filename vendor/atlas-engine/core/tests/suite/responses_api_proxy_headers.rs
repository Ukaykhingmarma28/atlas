...
-> Result<()> {
    ...
    let parent_window_id = parent
        ...
        .ok_or_else(|| anyhow!("parent request missing x-atlas-engine-window-id"))?;
    let child_window_id = child
        .header("x-atlas-engine-window-id")
        .ok_or_else(|| anyhow!("child request missing x-atlas-engine-window-id"))?;
    ...
    assert_eq!(
        child.header("x-atlas-engine-parent-thread-id").as_deref(),
    ...
    let child_turn_metadata: serde_json::Value = serde_json::from_str(
        &child
            .header("x-atlas-engine-turn-metadata")
            .ok_or_else(|| anyhow!("child request missing x-atlas-engine-turn-metadata"))?,
    )?;
...
async fn submit_turn_with_timeout(test: &TestAtlasEngine, prompt: &str) -> Result<()> {
