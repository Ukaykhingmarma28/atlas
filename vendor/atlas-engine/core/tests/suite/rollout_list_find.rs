...
use atlas_engine_core::find_thread_meta_by_name_str;
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(atlas_agent_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(atlas_agent_home, "sessions", id)
}
...
async fn upsert_thread_metadata(
    ...
    builder.cwd = atlas_agent_home.to_path_buf();
...
#[tokio::test]
async fn find_handles_gitignore_covering_atlas_agent_home_directory() {
    let repo = TempDir::new().unwrap();
    let atlas_agent_home = repo.path().join(".atlas-agent");
    std::fs::create_dir_all(&atlas_agent_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".atlas-agent/**\n").unwrap();
    let id = Uuid::new_v4();
