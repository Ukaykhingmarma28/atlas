...
async fn workspace_roots_test(server: &MockServer) -> Result<TestAtlasEngine> {
...
async fn submit_workspace_turn(test: &TestAtlasEngine, prompt: &str) -> Result<()> {
    test.submit_turn_with_permission_profile(prompt, workspace_roots_profile())
        .await
}

async fn read_file(test: &TestAtlasEngine, path: &PathUri) -> Result<String> {
    Ok(String::from_utf8(
        test.fs().read_file(path, /*sandbox*/ None).await?,
    )?)
}

async fn remove_files(test: &TestAtlasEngine, paths: &[&PathUri]) -> Result<()> {
    for path in paths {
        test.fs()
            .remove(
                path,
...
async fn workspace_roots_allow_patches_but_protect_metadata_directories() -> Result<()> {
    const PATCH_CONTENTS: &str = "workspace root patch access";
    const PROTECTED_METADATA_DIRECTORIES: [&str; 3] = [".git", ".agents", ".atlas-agent"];
