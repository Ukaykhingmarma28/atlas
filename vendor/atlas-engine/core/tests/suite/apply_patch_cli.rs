...
use core_test_support::test_atlas_engine::ApplyPatchModelOutput;
...
pub async fn apply_patch_harness() -> Result<TestAtlasEngineHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestAtlasEngineBuilder) -> TestAtlasEngineBuilder,
) -> Result<TestAtlasEngineHarness> {
    let builder = configure(test_atlas_engine());
    // Box harness construction so apply_patch_cli tests do not inline the
    // full test-thread startup path into each test future.
    Box::pin(TestAtlasEngineHarness::with_auto_env_builder(builder)).await
}

async fn submit_without_wait(harness: &TestAtlasEngineHarness, prompt: &str) -> Result<()> {
...
async fn apply_patch_cli_uses_atlas_engine_self_exe_with_linux_sandbox_helper_alias() -> Result<()> {
    ...
    let atlas_engine_linux_sandbox_exe = harness
        .test()
        .config
        .atlas_engine_linux_sandbox_exe
        .as_ref()
        .expect("linux test config should include atlas-engine-linux-sandbox helper");
    assert_eq!(
        atlas_engine_linux_sandbox_exe
            .file_name()
            .and_then(|name| name.to_str()),
        Some(ATLAS_AGENT_LINUX_SANDBOX_ARG0),
    );
...
async fn apply_patch_custom_tool_streaming_emits_updated_changes() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some(&atlas_engine_protocol::protocol::FileChange::Add {
            content: String::new(),
        })
    );
    assert_eq!(
...
async fn apply_patch_turn_diff_tracks_local_and_remote_environment_paths() -> Result<()> {
    ...
    let shared_cwd = PathBuf::from(format!(
        "/tmp/atlas-engine-remote-turn-diff-{}",
