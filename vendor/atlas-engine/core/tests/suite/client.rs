...
use atlas_engine_core::thread_store_from_config;
...
use atlas_engine_protocol::models::LocalShellAction;
use atlas_engine_protocol::models::LocalShellExecAction;
use atlas_engine_protocol::models::LocalShellStatus;
use atlas_engine_protocol::models::MessagePhase;
use atlas_engine_protocol::models::ReasoningItemContent;
use atlas_engine_protocol::models::ReasoningItemReasoningSummary;
use atlas_engine_protocol::models::ResponseItem;
use atlas_engine_protocol::models::WebSearchAction;
...
use core_test_support::TestAtlasEngineResponsesRequestKind;
...
fn test_turn_responses_metadata(
    _client: &ModelClient,
    thread_id: ThreadId,
) -> atlas_engine_core::AtlasEngineResponsesMetadata {
    ...
    test_responses_metadata(
        ...
        TestAtlasEngineResponsesRequestKind::Turn,
    )
}
...
fn assert_atlas_engine_client_metadata(
    ...
    assert_eq!(
        client_metadata["x-atlas-engine-installation-id"].as_str(),
        Some(installation_id)
    );
    assert_eq!(client_metadata["session_id"].as_str(), Some(session_id));
    assert_eq!(client_metadata["thread_id"].as_str(), Some(thread_id));
    let turn_metadata_str = client_metadata["x-atlas-engine-turn-metadata"]
        .as_str()
        .expect("missing x-atlas-engine-turn-metadata client metadata");
    let turn_metadata = serde_json::from_str::<serde_json::Value>(turn_metadata_str)
        .expect("invalid x-atlas-engine-turn-metadata json");
    assert_eq!(
    ...
    assert_eq!(
        client_metadata["x-atlas-engine-window-id"].as_str(),
...
async fn openai_stateless_responses_requests_preserve_item_turn_metadata_across_turns() {
    ...
    .await;
    let test = test_atlas_engine().build(&server).await.unwrap();
...
async fn response_item_ids_persist_across_resume_and_preserve_server_ids() -> anyhow::Result<()> {
    ...
    wait_for_event(&initial.atlas_engine, |event| {
...
async fn synthetic_call_output_id_is_stable_across_resumes() -> anyhow::Result<()> {
    ...
    let mut builder = test_atlas_engine();
    let first = builder
        .resume(&server, Arc::clone(&atlas_agent_home), session_path.clone())
        .await?;

    first.submit_turn("first resume").await?;
    first.atlas_engine.submit(Op::Shutdown).await?;
    wait_for_event(&first.atlas_engine, |event| {
        matches!(event, EventMsg::ShutdownComplete)
    })
    .await;
    assert!(
...
/// Writes an `auth.json` into the provided `atlas_agent_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        atlas_agent_home.path().join("auth.json"),
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: atlas_engine_utils_absolute_path::AbsolutePathBuf::try_from(self.tempdir.path())
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = atlas_engine_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
    ...
    let prior_system = atlas_engine_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![atlas_engine_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = atlas_engine_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure Atlas Agent to resume from our file
    let atlas_agent_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_atlas_engine()
        .with_home(atlas_agent_home.clone())
        .with_pre_build_hook(|home| {
            std::fs::write(home.join("AGENTS.md"), "be nice").expect("write global instructions");
        });
    let test = builder
        .resume(&server, atlas_agent_home, session_path.clone())
        .await
...
async fn includes_session_id_thread_id_and_model_headers_in_request() {
    ...
    let mut builder = test_atlas_engine().with_auth(AtlasEngineAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
    ...
    assert_eq!(request.header(X_ATLAS_AGENT_ROUTING_HINT_HEADER), None);
    ...
    let request_body = request.body_json();
    let installation_id =
        std::fs::read_to_string(test.atlas_agent_home_path().join(INSTALLATION_ID_FILENAME))
    ...
    assert_atlas_engine_client_metadata(
        &request_body,
...
async fn send_request_with_provider(provider: ModelProviderInfo) {
    ...
    let mut config = load_default_config_for_test(&atlas_agent_home).await;
    config.model_provider_id = provider.name.clone();
    config.model_provider = provider.clone();
    let effort = config.model_reasoning_effort.clone();
    let summary = config.model_reasoning_summary;
    let model = atlas_engine_core::test_support::get_model_offline(config.model.as_deref());
    ...
    let mut stream = client_session
        .stream(
            ...
            /*service_tier*/ None,
            &responses_metadata,
            &atlas_engine_rollout_trace::InferenceTraceContext::disabled(),
        )
        .await
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut model_provider =
        built_in_model_providers(/* openai_base_url */ /*openai_base_url*/ None)["openai"].clone();
    model_provider.base_url = Some(format!("{}/api/atlas-agent", server.uri()));
    model_provider.supports_websockets = false;
    let mut builder = test_atlas_engine()
        .with_auth(create_dummy_atlas_engine_auth())
        .with_config(move |config| {
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/api/atlas-agent/responses");
    let request_authorization = request
        .header("authorization")
        .expect("authorization header");
    let request_originator = request.header("originator").expect("originator header");
    ...
    assert_eq!(
        request.header(X_ATLAS_AGENT_ROUTING_HINT_HEADER),
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth = AtlasEngineAuth::from_auth_storage(
        atlas_agent_home.path(),
        AuthCredentialsStoreMode::File,
        /*chatgpt_base_url*/ None,
        AuthKeyringBackendKind::default(),
        &atlas_engine_login::test_support::transport_default_auth_route_config(),
    )
    .await
    .expect("Failed to load AtlasEngineAuth")
    .expect("No AtlasEngineAuth found in atlas_agent_home");
    let auth_manager = atlas_engine_core::test_support::auth_manager_from_auth(auth);
    let installation_id = resolve_installation_id(&config.atlas_agent_home)
        .await
        .expect("resolve installation id");
    let thread_manager = ThreadManager::new(
        &config,
        auth_manager.clone(),
        atlas_engine_core::build_models_manager(&config, auth_manager),
        atlas_engine_core::AtlasAppsToolsCache::default(),
        SessionSource::Exec,
        Arc::new(atlas_engine_exec_server::EnvironmentManager::default_for_tests()),
        empty_extension_registry(),
        Arc::new(atlas_engine_core::test_support::EmptyUserInstructionsProvider),
...
async fn omits_apps_guidance_when_orchestrator_mcp_is_disabled() {
    ...
    assert!(
        !request.body_contains_text("mcp__atlas_apps"),
        "did not expect atlas_apps MCP tools when orchestrator MCP is disabled, got {:?}",
    ...
    assert!(
        ...
        "did not expect atlas_apps resource calls: {resource_methods:?}"
    );
}
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-atlas-engine-primary-used-percent", "12.5")
        .insert_header("x-atlas-engine-secondary-used-percent", "40.0")
        .insert_header("x-atlas-engine-primary-window-minutes", "10")
        .insert_header("x-atlas-engine-secondary-window-minutes", "60")
        .insert_header("x-atlas-engine-primary-reset-at", "1704069000")
        .insert_header("x-atlas-engine-secondary-reset-at", "1704074400")
    ...
    let mut builder = test_atlas_engine()
        .with_auth(AtlasEngineAuth::from_api_key("test"))
        .with_config(move |config| {
            config.model_provider = provider;
        });
    let atlas_engine = builder
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        ...
        .insert_header("x-atlas-engine-credits-unlimited", "false")
        .insert_header("x-atlas-engine-credits-balance", "")
        .insert_header(
            "x-atlas-engine-rate-limit-reached-type",
    ...
    let mut builder = test_atlas_engine();
    let atlas_engine_fixture = builder.build(&server).await?;
    let atlas_engine = atlas_engine_fixture.atlas_engine.clone();
    ...
    let error_event = wait_for_event(&atlas_engine, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&atlas_engine, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&atlas_engine, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = AtlasEngineErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
async fn env_var_overrides_loaded_auth() {
    ...
    assert_eq!(request.headers.get(X_ATLAS_AGENT_ROUTING_HINT_HEADER), None);
}

fn create_dummy_atlas_engine_auth() -> AtlasEngineAuth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Atlas Agent sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
