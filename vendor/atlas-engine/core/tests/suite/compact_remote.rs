...
use atlas_engine_core::compact::SUMMARY_PREFIX;
...
use atlas_engine_login::auth::AgentIdentityAuth;
...
use core_test_support::test_atlas_engine::TestAtlasEngineHarness;
use core_test_support::test_atlas_engine::test_atlas_engine as base_test_atlas_engine;
...
const TURN_STATE_HEADER: &str = "x-atlas-engine-turn-state";
...
fn test_atlas_engine() -> TestAtlasEngineBuilder {
    base_test_atlas_engine().with_config(|config| {
        let _ = config.features.disable(Feature::RemoteCompactionV2);
    })
}

fn remote_realtime_test_atlas_engine_builder(
    realtime_server: &responses::WebSocketTestServer,
) -> TestAtlasEngineBuilder {
    let realtime_base_url = realtime_server.uri().to_string();
    test_atlas_engine()
        .with_auth(AtlasEngineAuth::from_api_key("dummy"))
    ...
    atlas_engine
        .submit(Op::RealtimeConversationStart(ConversationStartParams {
            ...
            atlas_engine_response_handoff_mode:
                atlas_engine_protocol::protocol::AtlasEngineResponseHandoffMode::Thinking,
...
async fn wait_for_turn_complete(atlas_engine: &atlas_engine_core::AtlasEngineThread) {
...
fn amazon_bedrock_test_atlas_engine() -> TestAtlasEngineBuilder {
    let auth = AtlasEngineAuth::BedrockApiKey(BedrockApiKeyAuth {
...
async fn amazon_bedrock_uses_remote_compaction_endpoint() -> Result<()> {
    ...
    harness.test().atlas_engine.submit(Op::Compact).await?;
    wait_for_turn_complete(&harness.test().atlas_engine).await;
...
async fn remote_compact_v2_retains_metadata_from_resumed_history() -> Result<()> {
    ...
    initial.atlas_engine.shutdown_and_wait().await?;
    annotate_retained_user_in_rollout(&rollout_path, retained_text)?;

    let resumed = builder()
        .resume(&server, home, rollout_path.clone())
        .await?;
    resumed.atlas_engine.submit(Op::Compact).await?;
    wait_for_turn_complete(&resumed.atlas_engine).await;
    resumed.atlas_engine.shutdown_and_wait().await?;
...
async fn remote_compact_v2_retains_only_client_developer_messages_when_enabled(
    ...
    let atlas_engine = &harness.test().atlas_engine;
    let rollout_path = atlas_engine.rollout_path().context("rollout path")?;
    atlas_engine
    ...
    atlas_engine.shutdown_and_wait().await?;
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    let harness = TestAtlasEngineHarness::with_builder(
        test_atlas_engine().with_auth(AtlasEngineAuth::create_dummy_chatgpt_auth_for_testing()),
    )
    .await?;
    let atlas_engine = harness.test().atlas_engine.clone();
...
async fn remote_compact_uses_agent_identity_assertion() -> Result<()> {
    ...
    let harness = TestAtlasEngineHarness::with_builder(
        test_atlas_engine().with_auth(AtlasEngineAuth::AgentIdentity(
    ...
    assert_eq!(
        compact_request.header(X_ATLAS_AGENT_ROUTING_HINT_HEADER),
...
async fn assert_remote_manual_compact_request_parity(
    ...
    let uses_atlas_engine_backend = auth.uses_atlas_engine_backend();
    let mut builder = test_atlas_engine().with_auth(auth);
    ...
    assert_eq!(
        normal_request.header(X_ATLAS_AGENT_ROUTING_HINT_HEADER),
...
-> Result<()> {
    ...
    assert_remote_manual_compact_request_parity(
        AtlasEngineAuth::from_api_key("dummy"),
...
async fn remote_compact_v2_reuses_compaction_trigger_for_followups() -> Result<()> {
    ...
    assert!(
        compact_request
            .header("x-atlas-engine-beta-features")
            .as_deref()
...
async fn remote_compact_filters_deferred_dynamic_tools() -> Result<()> {
    ...
    let dynamic_tools = vec![DynamicToolSpec::Namespace(DynamicToolNamespaceSpec {
        name: "atlas_engine_app".to_string(),
        description: "Atlas Agent app tools.".to_string(),
        tools: vec![
    ...
    let new_thread = test
        ...
        .await?;
    test.atlas_engine = new_thread.thread;
    ...
    assert_eq!(
        namespace_child_tool_names(&first_response_body, "atlas_engine_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        namespace_child_tool_names(&compact_body, "atlas_engine_app"),
        vec![visible_tool.to_string()]
    );
...
async fn remote_compact_does_not_charge_inline_audio_payload_as_text() -> Result<()> {
    ...
    test.atlas_engine
        ...
        .await?;
    let EventMsg::DynamicToolCallRequest(request) = wait_for_event(&test.atlas_engine, |event| {
    ...
    test.atlas_engine
        ...
        .await?;
    wait_for_turn_complete(&test.atlas_engine).await;
...
async fn remote_compact_runs_automatically() -> Result<()> {
    ...
    let message = wait_for_event_match(&atlas_engine, |event| match event {
...
async fn auto_remote_compact_failure_stops_agent_loop() -> Result<()> {
    ...
    let error_message = wait_for_event_match(&atlas_engine, |event| match event {
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    let baseline_harness = TestAtlasEngineHarness::with_builder(
        test_atlas_engine()
            .with_auth(AtlasEngineAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
    ...
    .await?;
    let baseline_atlas_engine = baseline_harness.test().atlas_engine.clone();
    ...
    baseline_atlas_engine
        .start_or_steer_turn(TurnInputRequest::user_input(vec![UserInput::Text {
            text: first_user_message.into(),
            text_elements: Vec::new(),
        }]))
        .await?;
    wait_for_event(&baseline_atlas_engine, |event| {
    ...
    baseline_atlas_engine.submit(Op::Compact).await?;
    wait_for_event(&baseline_atlas_engine, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let override_harness = TestAtlasEngineHarness::with_builder(
        test_atlas_engine()
            .with_auth(AtlasEngineAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config({
    ...
    .await?;
    let override_atlas_engine = override_harness.test().atlas_engine.clone();
    ...
    override_atlas_engine
        .start_or_steer_turn(TurnInputRequest::user_input(vec![UserInput::Text {
            text: first_user_message.into(),
            text_elements: Vec::new(),
        }]))
        .await?;
    wait_for_event(&override_atlas_engine, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    override_atlas_engine.submit(Op::Compact).await?;
    wait_for_event(&override_atlas_engine, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn remote_manual_compact_emits_context_compaction_items() -> Result<()> {
    ...
    {
        let event = atlas_engine.next_event().await.unwrap();
        match event.msg {
...
async fn snapshot_request_shape_remote_pre_turn_compaction_restates_realtime_start() -> Result<()> {
    ...
    start_realtime_conversation(test.atlas_engine.as_ref()).await?;
...
async fn remote_request_uses_custom_experimental_realtime_start_instructions() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_atlas_engine_builder(&realtime_server).with_config({
...
async fn active_realtime_does_not_diff_changed_start_instructions_after_resume() -> Result<()> {
    ...
    let mut initial_builder = remote_realtime_test_atlas_engine_builder(&initial_realtime_server)
        .with_config({
    ...
    start_realtime_conversation(initial.atlas_engine.as_ref()).await?;
    initial.submit_turn("USER_ONE").await?;
    close_realtime_conversation(initial.atlas_engine.as_ref()).await?;
    initial.atlas_engine.submit(Op::Shutdown).await?;
    wait_for_event(&initial.atlas_engine, |ev| {
        matches!(ev, EventMsg::ShutdownComplete)
    })
    ...
    let mut resume_builder = remote_realtime_test_atlas_engine_builder(&resumed_realtime_server)
        .with_config({
    ...
    start_realtime_conversation(resumed.atlas_engine.as_ref()).await?;
    ...
    close_realtime_conversation(resumed.atlas_engine.as_ref()).await?;
...
async fn snapshot_request_shape_remote_manual_compact_restates_realtime_start() -> Result<()> {
    ...
    let server = wiremock::MockServer::start().await;
    let realtime_server = start_remote_realtime_server().await;
    let mut builder = remote_realtime_test_atlas_engine_builder(&realtime_server);
    let test = builder.build(&server).await?;
