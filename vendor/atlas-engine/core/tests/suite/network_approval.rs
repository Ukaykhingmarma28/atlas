...
use atlas_engine_protocol::approvals::NetworkApprovalContext;
...
const NETWORK_TEST_HOST: &str = "atlas-engine-network-test.invalid";
...
-> Result<()> {
    ...
    .await?;
    let EventMsg::RequestPermissions(request) = wait_for_event(&test.atlas_engine, |event| {
...
async fn cancelled_guardian_network_review_fails_closed_without_rewriting_turn_state() -> Result<()>
{
    ...
    while !saw_turn_aborted || !saw_guardian_aborted {
        let event = tokio::time::timeout(Duration::from_secs(5), test.atlas_engine.next_event())
            .await
...
async fn background_network_approval_uses_active_turn_after_original_turn_completes() -> Result<()>
{
    ...
    .await?;
    let EventMsg::TurnComplete(first_turn) = wait_for_event(&test.atlas_engine, |event| {
    ...
    assert_eq!(test.atlas_engine.list_background_terminals().await.len(), 1);
    ...
    .await?;
    let EventMsg::TurnStarted(active_turn) = wait_for_event(&test.atlas_engine, |event| {
    ...
    test.atlas_engine
        ...
        .await?;
    let assessment = wait_for_event(&test.atlas_engine, |event| {
        matches!(
            event,
            EventMsg::GuardianAssessment(assessment)
    ...
    assert_eq!(
        test.atlas_engine.list_background_terminals().await.len(),
        1,
...
async fn user_network_approval_once_session_and_denial_semantics() -> Result<()> {
    ...
    assert!(
        approval
            .call_id
            .starts_with("network#local#http#atlas-engine-network-test.invalid#80#")
    );
    ...
    assert_eq!(
        ...
        Some("atlas-engine-network-test.invalid is not in the allowed_domains")
    );
    test.atlas_engine
...
async fn allowing_network_policy_amendment_persists_context_and_bypasses_prompt() -> Result<()> {
    ...
    assert!(policy.contains(
        r#"network_rule(host="atlas-engine-network-test.invalid", protocol="http", decision="allow""#
    ));
    assert!(first_responses.requests().iter().any(|request| {
        request.body_contains_text(
            "Allowed network rule saved in execpolicy (allowlist): atlas-engine-network-test.invalid",
        )
    }));
    mount_exec_network_turn(
        &server,
...
async fn denying_network_policy_amendment_persists_and_blocks_request() -> Result<()> {
    ...
    let policy = fs::read_to_string(test.home.path().join("rules/default.rules"))?;
    assert!(policy.contains(
        r#"network_rule(host="atlas-engine-network-test.invalid", protocol="http", decision="deny""#
    ));
    let output = responses
        .requests()
        .iter()
...
async fn approved_network_host_for_one_environment_still_prompts_in_another() -> Result<()> {
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/atlas-engine-network-approval-{}",
...
async fn wait_for_completion_without_network_prompt(test: &TestAtlasEngine) {
...
async fn wait_for_turn_complete(test: &TestAtlasEngine) {
    wait_for_event(&test.atlas_engine, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
