...
//! request payload that Atlas Agent would send to the model and assert that the
...
use atlas_engine_core::spawn::ATLAS_AGENT_SANDBOX_NETWORK_DISABLED_ENV_VAR;
...
fn network_disabled() -> bool {
    std::env::var(ATLAS_AGENT_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
fn seed_first_checkpoint_harness_metadata(path: &Path, retained_text: &str) -> Result<()> {
    ...
    for envelope in replacement_history {
        envelope.metadata = Some(AtlasEngineHarnessMetadata::default());
    }
...
async fn compact_conversation(conversation: &Arc<AtlasEngineThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<AtlasEngineThread>) -> std::path::PathBuf {
    conversation.rollout_path().expect("rollout path")
}

async fn shutdown_conversation(conversation: &Arc<AtlasEngineThread>) {
    conversation
        .shutdown_and_wait()
        .await
