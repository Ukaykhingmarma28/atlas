...
use atlas_engine_protocol::openai_models::MultiAgentModeMessages;
...
async fn ultra_reasoning_uses_max_and_proactive_mode() -> Result<()> {
    ...
    submit_turn(&test.atlas_engine, "hello", /*effort*/ None).await?;
...
async fn empty_configured_mode_hint_emits_no_mode_message() -> Result<()> {
    ...
    submit_turn(&test.atlas_engine, "hello", Some(ReasoningEffort::High)).await?;
...
async fn changing_configured_mode_hint_to_empty_emits_no_update() -> Result<()> {
    ...
    .await;
    let initial = test_atlas_engine()
        .with_config(configure_custom_mode_hint)
        .build(&server)
        .await?;

    submit_turn(&initial.atlas_engine, "before resume", /*effort*/ None).await?;
    ...
    drop(initial);
    submit_turn(&resumed.atlas_engine, "after resume", /*effort*/ None).await?;
...
async fn live_mode_change_appends_mode_without_reappending_usage_hint() -> Result<()> {
    ...
    submit_turn(&test.atlas_engine, "proactive", /*effort*/ None).await?;
    submit_turn(&test.atlas_engine, "explicit", Some(ReasoningEffort::High)).await?;
