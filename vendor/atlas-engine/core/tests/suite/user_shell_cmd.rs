...
async fn user_shell_command_without_local_environment_emits_error() -> anyhow::Result<()> {
    ...
    assert_eq!(error.atlas_engine_error_info, None);
...
async fn user_shell_cmd_can_be_interrupted() {
    ...
    let fixture = builder
        ...
        .await
        .expect("create new conversation");
    let atlas_engine = &fixture.atlas_engine;
    ...
    let _begin = wait_for_event_match(atlas_engine, |ev| match ev {
...
async fn user_shell_command_does_not_replace_active_turn() -> anyhow::Result<()> {
    ...
    let _ = wait_for_event_match(&fixture.atlas_engine, |ev| match ev {
    ...
    for _ in 0..200 {
        let event = timeout(Duration::from_secs(20), fixture.atlas_engine.next_event())
            .await
...
async fn user_shell_command_history_is_persisted_and_shared_with_model() -> anyhow::Result<()> {
    ...
    #[cfg(windows)]
    let command = r#"$val = $env:ATLAS_AGENT_SANDBOX; if ([string]::IsNullOrEmpty($val)) { $val = 'not-set' } ; [System.Console]::Write($val)"#.to_string();
    #[cfg(not(windows))]
    let command = r#"sh -c "printf '%s' \"${ATLAS_AGENT_SANDBOX:-not-set}\"""#.to_string();
    ...
    let begin_event = wait_for_event_match(&test.atlas_engine, |ev| match ev {
    ...
    let delta_event = wait_for_event_match(&test.atlas_engine, |ev| match ev {
    ...
    let end_event = wait_for_event_match(&test.atlas_engine, |ev| match ev {
...
async fn user_shell_command_does_not_set_network_sandbox_env_var() -> anyhow::Result<()> {
    ...
    #[cfg(windows)]
    let command = r#"$val = $env:ATLAS_AGENT_SANDBOX_NETWORK_DISABLED; if ([string]::IsNullOrEmpty($val)) { $val = 'not-set' } ; [System.Console]::Write($val)"#.to_string();
    #[cfg(not(windows))]
    let command =
        r#"sh -c "printf '%s' \"${ATLAS_AGENT_SANDBOX_NETWORK_DISABLED:-not-set}\"""#.to_string();
    ...
    let ExecCommandEndEvent {
        ...
        stderr,
        ..
    } = wait_for_event_match(&test.atlas_engine, |ev| match ev {
        EventMsg::ExecCommandEnd(event) => Some(event.clone()),
        _ => None,
    })
    .await;
...
async fn user_shell_command_output_is_truncated_in_history() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let builder = core_test_support::test_atlas_engine::test_atlas_engine();
    let test = builder
        .with_config(|config| {
