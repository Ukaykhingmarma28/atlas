...
use atlas_engine_api::WS_REQUEST_HEADER_TRACEPARENT_CLIENT_METADATA_KEY;
use atlas_engine_api::WS_REQUEST_HEADER_TRACESTATE_CLIENT_METADATA_KEY;
use atlas_engine_core::AtlasEngineResponsesMetadata;
use atlas_engine_core::ModelClient;
use atlas_engine_core::ModelClientSession;
...
use atlas_engine_core::X_RESPONSESAPI_INCLUDE_TIMING_METRICS_HEADER;
use atlas_engine_core::test_support::with_parent_turn;
...
use atlas_engine_rollout_trace::ConversationPart;
use atlas_engine_rollout_trace::InferenceTraceContext;
use atlas_engine_rollout_trace::RawTraceEventPayload;
use atlas_engine_rollout_trace::TraceWriter;
use atlas_engine_rollout_trace::replay_bundle;
...
fn responses_metadata(
    ...
    turn_id: Option<&str>,
    request_kind: TestAtlasEngineResponsesRequestKind,
...
fn websocket_connection_metadata(harness: &WebsocketTestHarness) -> AtlasEngineResponsesMetadata {
    responses_metadata(
        harness,
        /*turn_id*/ None,
        TestAtlasEngineResponsesRequestKind::WebsocketConnection,
    )
}
...
async fn responses_websocket_streams_request() {
    ...
    let harness = websocket_harness_for_atlas_engine_backend(&server).await;
    ...
    assert_eq!(
        handshake.header(X_ATLAS_AGENT_ROUTING_HINT_HEADER),
    ...
    assert_eq!(
        ...
        Some(atlas_engine_login::default_client::get_atlas_engine_user_agent())
    );
    assert_eq!(
        body["client_metadata"]["x-atlas-engine-installation-id"].as_str(),
        Some(TEST_INSTALLATION_ID)
    );
    let stream_request_start_ms = body["client_metadata"]
        [X_ATLAS_AGENT_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY]
        .as_str()
...
async fn responses_websocket_omits_routing_hint_for_provider_with_own_credentials() {
    ...
    assert_eq!(handshake.header(X_ATLAS_AGENT_ROUTING_HINT_HEADER), None);
...
async fn responses_websocket_preconnect_reuses_connection() {
    ...
    assert_eq!(
        server.single_handshake().header("x-atlas-engine-window-id"),
...
async fn responses_websocket_request_prewarm_reuses_connection() {
    ...
    let warmup_turn_metadata: serde_json::Value = serde_json::from_str(
        warmup["client_metadata"]["x-atlas-engine-turn-metadata"]
            .as_str()
...
async fn responses_websocket_request_prewarm_traces_logical_request() {
    ...
    writer
        .append(RawTraceEventPayload::AtlasEngineTurnStarted {
            atlas_engine_turn_id: "turn-1".to_string(),
            thread_id: harness.thread_id.to_string(),
        })
        .expect("turn started");
...
async fn responses_websocket_preconnect_runs_when_only_v2_feature_enabled() {
    ...
    assert_eq!(
        server
            .single_handshake()
            .header(X_ATLAS_AGENT_ROUTING_HINT_HEADER),
        None
    );
...
async fn responses_websocket_emits_rate_limit_events() {
    ...
    let server = start_websocket_server_with_headers(vec![WebSocketConnectionConfig {
        requests: vec![vec![
            json!({
                "type": "atlas_agent.response.metadata",
...
async fn responses_websocket_usage_limit_error_emits_rate_limit_event() {
    ...
    let usage_limit_error = json!({
        ...
        "headers": {
            "x-atlas-engine-primary-used-percent": "100.0",
            "x-atlas-engine-secondary-used-percent": "87.5",
            "x-atlas-engine-primary-over-secondary-limit-percent": "95.0",
            "x-atlas-engine-primary-window-minutes": "15",
            "x-atlas-engine-secondary-window-minutes": "60",
            "x-atlas-engine-credits-has-credits": "true",
            "x-atlas-engine-credits-unlimited": "false",
            "x-atlas-engine-credits-balance": "",
            "x-atlas-engine-rate-limit-reached-type": "workspace_member_usage_limit_reached"
        }
    });
    ...
    let test = builder
        ...
        .await
        .expect("build websocket atlas-agent");
...
async fn responses_websocket_forwards_turn_metadata_on_initial_and_incremental_create() {
    ...
    let first_metadata: serde_json::Value = serde_json::from_str(
        first["client_metadata"]["x-atlas-engine-turn-metadata"]
            .as_str()
    ...
    let second_metadata: serde_json::Value = serde_json::from_str(
        second["client_metadata"]["x-atlas-engine-turn-metadata"]
            .as_str()
...
async fn responses_websocket_sends_canonical_turn_metadata() {
    ...
    let turn_metadata: serde_json::Value = serde_json::from_str(
        body["client_metadata"]["x-atlas-engine-turn-metadata"]
            .as_str()
...
async fn websocket_harness_with_provider_options_and_auth(
    ...
    let model_info = atlas_engine_core::test_support::construct_model_info_offline(MODEL, &config);
