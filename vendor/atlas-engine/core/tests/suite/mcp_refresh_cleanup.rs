...
async fn refresh_keeps_superseded_mcp_server_alive_for_in_flight_calls() -> anyhow::Result<()> {
    ...
    let fixture = test_atlas_engine()
        ...
        .build(&server)
        .await?;
    wait_for_mcp_server(&fixture.atlas_engine, "refresh_cleanup").await?;
    ...
    let long_call = tokio::spawn({
        let atlas_engine = Arc::clone(&fixture.atlas_engine);
    ...
    .await;
    fixture.atlas_engine.submit(Op::RefreshMcpServers).await?;
