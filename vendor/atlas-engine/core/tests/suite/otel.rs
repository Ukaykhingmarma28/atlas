...
async fn responses_api_emits_api_request_event() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| line.contains("atlas_agent.api_request"))
            .map(|_| Ok(()))
            .unwrap_or_else(|| Err("expected atlas-agent.api_request event".to_string()))
    });
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| line.contains("atlas_agent.conversation_starts"))
            .map(|_| Ok(()))
            .unwrap_or_else(|| Err("expected atlas-agent.conversation_starts event".to_string()))
    });
}
...
async fn process_sse_emits_tracing_for_output_item() {
    ...
    let TestAtlasEngine { atlas_engine, .. } = test_atlas_engine().build(&server).await.unwrap();
...
async fn turn_and_completed_response_spans_record_token_usage() {
    ...
    let TestAtlasEngine { atlas_engine, .. } = test;
    ...
    assert!(
        logs.lines().any(|line| {
            line.contains("handle_responses{")
                ...
                && line.contains("atlas_agent.request.reasoning_effort=high")
                ...
                && line.contains("atlas_agent.usage.reasoning_output_tokens=2")
                && line.contains("atlas_agent.usage.total_tokens=9")
        }),
    ...
    assert!(
        logs.lines().any(|line| {
            line.contains("turn{otel.name=\"session_task.turn\"")
                && line.contains("atlas_agent.turn.reasoning_effort=high")
                && line.contains("atlas_agent.turn.token_usage.input_tokens=3")
                && line.contains("atlas_agent.turn.token_usage.cached_input_tokens=1")
                && line.contains("atlas_agent.turn.token_usage.cache_write_input_tokens=2")
                && line.contains("atlas_agent.turn.token_usage.non_cached_input_tokens=2")
                && line.contains("atlas_agent.turn.token_usage.output_tokens=5")
                && line.contains("atlas_agent.turn.token_usage.reasoning_output_tokens=2")
                && line.contains("atlas_agent.turn.token_usage.total_tokens=9")
        }),
...
async fn handle_response_item_records_tool_result_for_function_call() {
    ...
    wait_for_event(&atlas_engine, |ev| matches!(ev, EventMsg::TokenCount(_))).await;

    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("atlas_agent.tool_result") && line.contains("call_id=function-call")
            })
            .ok_or_else(|| "missing atlas-agent.tool_result event".to_string())?;
...
fn sandbox_outcome_assertion<'a>(
    ...
    move |lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("atlas_agent.sandbox_outcome")
                    && line.contains(&format!("call_id={call_id}"))
            })
            .ok_or_else(|| format!("missing atlas-agent.sandbox_outcome event for {call_id}"))?;
...
async fn handle_shell_command_user_approved_records_tool_decision() {
    let server = start_mock_server().await;
    let command = touch_command("atlas-engine-otel-approval-test");
    mount_sse_once(
        &server,
        sse(vec![
