...
use anyhow::Result;
use atlas_engine_config::types::Personality;
...
async fn first_turn_after_empty_prefix_fork_preserves_inherited_base_instructions(
    ...
    let source_rollout_path = test.atlas_engine.rollout_path().expect("rollout path");
    let source_history =
        atlas_engine_rollout::RolloutRecorder::get_rollout_history(&source_rollout_path).await?;
...
async fn rollback_first_turn_model_change_removes_its_instructions(
    ...
    let mut builder = test_atlas_engine().with_model(initial_model);
    let test = builder.build_with_auto_env(&server).await?;
    ...
    let test = match followup {
        RollbackFollowup::ColdResume => {
            let mut resume_builder = test_atlas_engine().with_model(switched_model);
...
async fn model_change_appends_model_instructions_developer_message() -> Result<()> {
    ...
    let mut builder = test_atlas_engine().with_model("gpt-5.2");
...
async fn model_switch_to_smaller_model_updates_token_context_window() -> Result<()> {
    ...
    let large_window_event = wait_for_event(&test.atlas_engine, |event| {
        matches!(
            event,
    ...
    let smaller_turn_started_event = wait_for_event(&test.atlas_engine, |event| {
        matches!(
            event,
    ...
    let smaller_window_event = wait_for_event(&test.atlas_engine, |event| {
        matches!(
            event,
            EventMsg::TokenCount(token_count)
                if token_count
