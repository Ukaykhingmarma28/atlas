...
use atlas_engine_core::config::RolloutBudgetConfig;
                ...
                json!({
                    ...
                    "response": {
                        ...
                        "usage": {
                            ...
                            "total_tokens": 75,
                            "atlas_engine_rollout_budget_units": rollout_budget_units
                        }
                    }
                }),
            ]),
...
async fn invalid_provider_rollout_budget_units_fail_without_retry() -> Result<()> {
    ...
    completed["response"]["usage"]["atlas_engine_rollout_budget_units"] = json!(-1.0);
    let responses = mount_sse_sequence(
        &server,
        ...
        wait_for_event(&test.atlas_engine, |event| {
            matches!(
                ...
                EventMsg::Error(error)
                    if error.atlas_engine_error_info == Some(AtlasEngineErrorInfo::SessionBudgetExceeded)
            )
        })
        .await;
        wait_for_event(&test.atlas_engine, |event| {
...
async fn compaction_budget_exhaustion_fails_without_retry(
    ...
    if provider_units {
        completed["response"]["usage"]["atlas_engine_rollout_budget_units"] = json!(10.0);
    }
