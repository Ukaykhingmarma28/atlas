...
//! `just test -p atlas-engine-core --test all --run-ignored only live_cli` provided they set a valid
//! `OPENAI_API_KEY`.
...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let atlas_agent_home = home.path().join(".atlas-agent");
    ...
    let mut cmd = Command::new(atlas_engine_utils_cargo_bin::cargo_bin("atlas-engine-rs").unwrap());
    ...
    let mut child = cmd.spawn().expect("failed to spawn atlas-engine-rs");
