...
use atlas_engine_skills_extension::HostSkillProvider;
use atlas_engine_skills_extension::OrchestratorSkillProvider;
use atlas_engine_skills_extension::SkillProvider;
use atlas_engine_skills_extension::SkillProviderSource;
use atlas_engine_skills_extension::SkillProviders;
use atlas_engine_skills_extension::SkillsExtensionConfig;
use atlas_engine_skills_extension::catalog::SkillAuthority;
use atlas_engine_skills_extension::catalog::SkillCatalog;
use atlas_engine_skills_extension::catalog::SkillCatalogEntry;
use atlas_engine_skills_extension::catalog::SkillPackageId;
use atlas_engine_skills_extension::catalog::SkillProviderError;
use atlas_engine_skills_extension::catalog::SkillReadResult;
use atlas_engine_skills_extension::catalog::SkillResourceId;
use atlas_engine_skills_extension::catalog::SkillSearchResult;
use atlas_engine_skills_extension::catalog::SkillSourceKind;
use atlas_engine_skills_extension::install;
use atlas_engine_skills_extension::install_with_providers;
use atlas_engine_skills_extension::provider::SkillListQuery;
use atlas_engine_skills_extension::provider::SkillProviderFuture;
use atlas_engine_skills_extension::provider::SkillReadRequest;
use atlas_engine_skills_extension::provider::SkillSearchRequest;
...
fn write_host_skills(atlas_agent_home: &std::path::Path, skills: &[(&str, &str)]) -> Result<()> {
...
fn catalog_extensions(
    ...
    Arc<atlas_engine_extension_api::ExtensionRegistry<Config>>,
...
fn configure_catalog_test(config: &mut Config) {
    ...
    let system_config_path = config.atlas_agent_home.join("config.toml");
...
async fn rendered_catalogs_for_turns(
    ...
    if !host_skills.is_empty() {
        write_host_skills(atlas_agent_home.path(), host_skills)?;
    }
    ...
    for _ in 0..turn_count {
        test.atlas_engine
            ...
            .await?;
        loop {
            match core_test_support::wait_for_event(&test.atlas_engine, |_| true).await {
    ...
    // while core warnings are delivered through the TestAtlasEngine event stream.
...
async fn capability_sections_render_in_order_with_host_repo_and_plugin_skills() -> Result<()> {
    ...
    let atlas_agent_home = Arc::new(TempDir::new()?);
    let host_skill_dir = atlas_agent_home.path().join("skills/host-search");
    ...
    core_test_support::wait_for_event(&test.atlas_engine, |event| {
...
async fn agent_plugin_skill_prompt_stays_bounded_without_skills_extension() -> Result<()> {
    ...
    let mut builder = test_atlas_engine().with_home(atlas_agent_home);
    let test = builder.build_with_auto_env(&server).await?;
    ...
    test.atlas_engine
        ...
        .await?;
    let warning = core_test_support::wait_for_event(&test.atlas_engine, |event| {
        matches!(
            event,
...
async fn explicit_only_orchestrator_skill_is_hidden_but_can_be_invoked() -> Result<()> {
    ...
    Mock::given(method("POST"))
        .and(path_regex("^/api/atlas-agent/ps/mcp/?$"))
        .and(|request: &Request| {
    ...
    let test = builder.build_with_auto_env(&server).await?;
    wait_for_mcp_server(&test.atlas_engine, ATLAS_APPS_MCP_SERVER_NAME).await?;
...
async fn executor_skill_invocation_is_environment_scoped_and_deduplicated() -> Result<()> {
    ...
    let server = responses::start_mock_server().await;
    let atlas_agent_home = Arc::new(TempDir::new()?);
    let skill_path = atlas_agent_home.path().join("executor-skill/SKILL.md");
...
async fn production_turn_aliases_combined_skill_catalogs_under_shared_budget() -> Result<()> {
    ...
    .await;
    let atlas_agent_home = Arc::new(
        tempfile::Builder::new()
            .prefix("atlas-engine-integration-shared-skill-catalog-roots-")
            .tempdir()?,
    );
    write_host_skills(
    ...
    let developer_text = response
        ...
        .message_input_texts("developer")
        .join("\n");
    let host_root = dunce::canonicalize(atlas_agent_home.path().join("skills"))?
        .to_string_lossy()
        .replace('\\', "/");
    for (alias, root) in [
        ("e0", EXECUTOR_ROOT),
...
async fn production_turn_preserves_host_alias_root_order_across_turns() -> Result<()> {
    ...
    let long_parent = temp_parent
        .path()
        .join("atlas-engine-home-with-long-shared-prefix-for-production-alias-order-test");
    std::fs::create_dir_all(&long_parent)?;
    let atlas_agent_home = Arc::new(TempDir::new_in(&long_parent)?);
...
async fn production_turn_suppresses_only_the_superseded_host_skill_prompt() -> Result<()> {
    ...
    let first_skill_path = atlas_agent_home.path().join("skills/first-host/SKILL.md");
    let second_skill_path = atlas_agent_home.path().join("skills/second-host/SKILL.md");
...
async fn production_turn_keeps_full_snapshot_host_skill_prompt() -> Result<()> {
    ...
    .await;
    let atlas_agent_home = Arc::new(TempDir::new()?);
    let skill_dir = atlas_agent_home.path().join("skills").join("long-host");
    std::fs::create_dir_all(&skill_dir)?;
...
async fn production_turn_keeps_orchestrator_world_state_incremental_across_turns() -> Result<()> {
    ...
    let skill_resource = "skill://atlas_apps/orchestrator-search/SKILL.md";
    let catalog = SkillCatalog {
        entries: vec![
            SkillCatalogEntry::new(
                SkillPackageId("orchestrator/orchestrator-search".to_string()),
                SkillAuthority::new(SkillSourceKind::Orchestrator, ATLAS_APPS_MCP_SERVER_NAME),
                skill_name,
                skill_description,
...
async fn production_turn_fairly_shortens_extension_catalog_descriptions() -> Result<()> {
    ...
    assert_eq!(
        warning.message,
        "Skill descriptions were shortened to fit the skills context budget. Atlas Agent can still see every skill, but some descriptions are shorter. Disable unused skills or plugins to leave more room for the rest."
    );
