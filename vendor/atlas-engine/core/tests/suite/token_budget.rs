...
use atlas_engine_core::config::TokenBudgetConfig;
...
async fn token_budget_context_injects_plain_thread_hint_text() -> Result<()> {
    ...
    let test = test_atlas_engine()
        ...
        .build(&server)
        .await?;
    wait_for_mcp_server(&test.atlas_engine, "notes").await?;
    let responses = mount_sse_sequence(
        &server,
        vec![sse(vec![
            ev_response_created("resp-1"),
...
async fn token_budget_context_uses_new_window_after_compaction(
    ...
    assert_context_compaction_item_lifecycle(&test.atlas_engine).await;
...
async fn token_budget_compaction_runs_compact_hooks() -> Result<()> {
    ...
    let pre_compact = wait_for_event_match(&test.atlas_engine, |event| match event {
    ...
    let post_compact = wait_for_event_match(&test.atlas_engine, |event| match event {
        EventMsg::HookCompleted(completed)
