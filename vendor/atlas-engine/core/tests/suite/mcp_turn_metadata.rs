...
use atlas_engine_protocol::protocol::ElicitationAction;
...
fn set_calendar_approval_mode(config: &mut Config, approval_mode: AppToolApproval) {
    ...
    let user_config_path = config.atlas_agent_home.join("config.toml").abs();
...
async fn wait_for_mcp_tool_call_item(
    ...
    wait_for_event_match(&test.atlas_engine, |event| {
...
async fn approved_mcp_tool_call_metadata_records_prior_user_input_request(
    ...
    if !strict_auto_review {
        let EventMsg::ElicitationRequest(request) = wait_for_event(&test.atlas_engine, |event| {
            matches!(
                event,
    ...
    let mcp_turn_metadata = apps_tool_call
        .pointer("/params/_meta/x-atlas-engine-turn-metadata")
    ...
    assert_eq!(
        apps_tool_call
            .pointer("/params/_meta/x-atlas-engine-turn-metadata/user_input_requested_during_turn"),
...
async fn apps_default_prompt_with_auto_review_routes_actual_mcp_approval_to_guardian(
    ...
    let route_event = wait_for_event(&test.atlas_engine, |event| {
        matches!(
            event,
            EventMsg::ElicitationRequest(_) | EventMsg::TurnComplete(_)
        )
...
async fn apps_default_writes_prompts_for_writes_but_not_reads() -> Result<()> {
    ...
    let EventMsg::McpToolCallBegin(read_begin) = wait_for_event(&test.atlas_engine, |event| {
        matches!(event, EventMsg::McpToolCallBegin(_))
    })
    .await
    else {
    ...
    let next_route = wait_for_event(&test.atlas_engine, |event| {
        matches!(
            event,
