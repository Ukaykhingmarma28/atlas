...
fn disabled_text_turn(test: &TestAtlasEngine, text: &str) -> TurnInputRequest {
...
async fn openai_model_header_mismatch_emits_warning_event() -> Result<()> {
    ...
    let mut builder = test_atlas_engine().with_model(REQUESTED_MODEL);
    ...
    let reroute = wait_for_event(&test.atlas_engine, |event| {
    ...
    let _ = wait_for_event(&test.atlas_engine, |event| {
...
async fn model_verification_emits_structured_event_without_reroute_or_warning() -> Result<()> {
    ...
    let mut builder = test_atlas_engine().with_model(SERVER_MODEL);
    let test = builder.build(&server).await?;
