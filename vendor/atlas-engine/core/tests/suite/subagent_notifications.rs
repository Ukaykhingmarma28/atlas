...
use atlas_engine_core::config::AgentRoleConfig;
...
use atlas_engine_protocol::openai_models::MultiAgentMessages;
...
async fn wait_for_spawned_thread_id(test: &TestAtlasEngine) -> Result<String> {
    let deadline = Instant::now() + Duration::from_secs(2);
    loop {
...
async fn setup_turn_one_with_custom_spawned_child(
    ...
    configure_test: impl FnOnce(
        core_test_support::test_atlas_engine::TestAtlasEngineBuilder,
    ) -> core_test_support::test_atlas_engine::TestAtlasEngineBuilder,
) -> Result<(
    TestAtlasEngine,
    String,
    ...
    let mut builder = configure_test(test_atlas_engine().with_config(|config| {
        config
            .features
...
async fn subagent_stop_replaces_stop_and_skips_internal_subagents() -> Result<()> {
    ...
    let stop_inputs = read_hook_log(test.atlas_agent_home_path(), "stop_hook_log.jsonl")?;
    assert!(
        stop_inputs
            .iter()
...
async fn spawned_full_history_v2_child_uses_model_precedence_without_dropping_context(
    ...
    if matches!(selection, FullHistoryV2ModelSelection::WorldStateIdentity) {
        ...
        let child_rollout = atlas_engine_rollout::RolloutRecorder::get_rollout_history(
            &child_thread
                .rollout_path()
...
async fn spawn_agent_role_overrides_requested_model_and_reasoning_settings() -> Result<()> {
    ...
    let child_snapshot = spawn_child_and_capture_snapshot(
        ...
        |builder| {
            builder.with_config(|config| {
                let role_path = config.atlas_agent_home.join("custom-role.toml");
                std::fs::write(
                    &role_path,
                    format!(
...
async fn spawn_agent_preserves_configured_defaults_through_unrelated_role() -> Result<()> {
    ...
    let child_snapshot = spawn_child_and_capture_snapshot(
        ...
        |builder| {
            builder.with_config(|config| {
                let role_path = config.atlas_agent_home.join("instructions-only-role.toml");
    ...
    let test = test_atlas_engine()
        .with_config(|config| {
            config
                ...
                .enable(Feature::Collab)
                .expect("test config should allow feature update");
            let role_path = config.atlas_agent_home.join("model-only-role.toml");
