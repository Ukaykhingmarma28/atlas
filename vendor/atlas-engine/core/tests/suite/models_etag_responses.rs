...
async fn refresh_models_on_models_etag_mismatch_and_avoid_duplicate_models_fetch() -> Result<()> {
    ...
    // 1) On spawn, Atlas Agent fetches /models and stores the ETag.
    ...
    // 2) If the server sends a different X-Models-Etag on /responses, Atlas Agent refreshes /models.
    ...
    // Second /responses request (tool output) includes the same X-Models-Etag; Atlas Agent should not
    ...
    let refresh_req = refresh_models_mock
        ...
        .next()
        .expect("one request");
    // Ensure Atlas Agent includes client_version on refresh. (This is a stable signal that we're using the /models client.)
    assert!(
        refresh_req
            .url
            .query_pairs()
