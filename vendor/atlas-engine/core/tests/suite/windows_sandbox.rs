...
enum TestAtlasAgentHome {
    Persistent(PathBuf),
    Temporary(TempDir),
}

impl TestAtlasAgentHome {
    fn path(&self) -> &Path {
        match self {
...
fn atlas_agent_home_for_windows_sandbox_test(name: &str) -> anyhow::Result<TestAtlasAgentHome> {
    ...
    Ok(TestAtlasAgentHome::Temporary(TempDir::new()?))
}
...
fn stage_windows_sandbox_helpers() -> anyhow::Result<()> {
    ...
    let resources_dir = test_exe_dir.join("atlas-engine-resources");
...
async fn windows_elevated_enforces_deny_read_and_protects_setup_marker() -> anyhow::Result<()> {
    ...
    assert!(
        sandbox_setup_is_complete(atlas_agent_home.path()),
