...
fn configure_multi_agent_v2_with_role(
    ...
    let role_path = config.atlas_agent_home.join("durable-worker-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn cold_root_resume_restores_agent_identity_and_role_on_followup() -> Result<()> {
    ...
    let mut initial_builder = test_atlas_engine().with_config(move |config| {
    ...
    let body_for = |text: &str, thread: atlas_engine_protocol::ThreadId| {
        requests
            .iter()
            .find_map(|request| {
    ...
    for (body, parent_thread, parent_turn) in [
        ...
        if let Some(parent_thread) = parent_thread {
            assert_eq!(
                body["client_metadata"]["x-atlas-engine-parent-thread-id"],
                json!(parent_thread)
            );
        }
