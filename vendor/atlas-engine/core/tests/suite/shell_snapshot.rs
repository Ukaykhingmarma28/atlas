...
const POLICY_PATH_FOR_TEST: &str = "/atlas-agent/policy/path";
const SNAPSHOT_PATH_FOR_TEST: &str = "/atlas-agent/snapshot/path";
const SNAPSHOT_MARKER_VAR: &str = "ATLAS_AGENT_SNAPSHOT_POLICY_MARKER";
...
async fn wait_for_snapshot(atlas_agent_home: &Path) -> Result<PathBuf> {
    let snapshot_dir = atlas_agent_home.join("shell_snapshots");
...
async fn run_snapshot_command_with_options(
    ...
    let builder = test_atlas_engine().with_config(move |config| {
    ...
    let atlas_agent_home = test.home.path().to_path_buf();
    ...
    let begin = wait_for_event_match(&atlas_engine, |ev| match ev {
        EventMsg::ExecCommandBegin(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
    let snapshot_path = wait_for_snapshot(&atlas_agent_home).await?;
    let snapshot_content = fs::read_to_string(&snapshot_path).await?;

    let end = wait_for_event_match(&atlas_engine, |ev| match ev {
...
async fn run_tool_turn_on_harness(
    harness: &TestAtlasEngineHarness,
    prompt: &str,
    call_id: &str,
    tool_name: &str,
    ...
    wait_for_event_match(&atlas_engine, |ev| match ev {
        EventMsg::ExecCommandBegin(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
...
async fn linux_unified_exec_uses_shell_snapshot() -> Result<()> {
    ...
    assert!(run.snapshot_path.starts_with(&run.atlas_agent_home));
...
async fn shell_command_snapshot_preserves_shell_environment_policy_set() -> Result<()> {
    ...
    let harness = TestAtlasEngineHarness::with_builder(builder).await?;
    let atlas_agent_home = harness.test().home.path().to_path_buf();
    ...
    assert!(snapshot_path.starts_with(atlas_agent_home));
...
async fn shell_command_snapshot_still_intercepts_apply_patch() -> Result<()> {
    ...
    wait_for_event(&atlas_engine, |ev| match ev {
...
async fn shell_snapshot_deleted_after_shutdown_with_skills() -> Result<()> {
    ...
    let atlas_agent_home = home.path().to_path_buf();
