...
use atlas_engine_core::REVIEW_PROMPT;
...
use atlas_engine_protocol::openai_models::ModelTokenBudgetConfig;
...
use atlas_engine_protocol::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
...
use atlas_engine_protocol::protocol::ReviewCodeLocation;
use atlas_engine_protocol::protocol::ReviewFinding;
use atlas_engine_protocol::protocol::ReviewLineRange;
use atlas_engine_protocol::protocol::ReviewOutputEvent;
...
use atlas_engine_protocol::review_format::render_review_output_text;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under Atlas Agent sandbox network restrictions.
    skip_if_no_network!();
    ...
    let atlas_engine = new_conversation_for_server(&server, atlas_agent_home.clone(), |_| {}).await;
    ...
    let entered_started = wait_for_event(&atlas_engine, |ev| {
        matches!(
            ev,
    ...
    let entered_completed = wait_for_event(&atlas_engine, |ev| {
        matches!(
            ev,
    ...
    let exited_started = wait_for_event(&atlas_engine, |ev| {
        matches!(
            ev,
            EventMsg::ItemStarted(event)
    ...
    let exited_completed = wait_for_event(&atlas_engine, |ev| {
        matches!(
            ev,
            EventMsg::ItemCompleted(event)
                if matches!(event.item, TurnItem::ExitedReviewMode(_))
...
async fn cancelled_review_does_not_forward_delegate_mcp_startup() {
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            match atlas_engine.next_event().await.expect("review event").msg {
...
async fn review_filters_agent_message_related_events() {
    ...
    wait_for_event(&atlas_engine, |event| match event {
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let atlas_engine = Arc::clone(&test.atlas_engine);
    std::fs::remove_file(atlas_agent_home.path().join("models_cache.json"))
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = atlas_agent_home.path().join("resume.jsonl");
    {
        ...
        let user = atlas_engine_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = atlas_engine_protocol::models::ResponseItem::Message {
            id: None,
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, atlas_agent_home, resume_path)
        .await
