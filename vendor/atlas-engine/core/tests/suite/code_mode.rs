...
use atlas_engine_tools::FreeformTool;
use atlas_engine_tools::FreeformToolFormat;
use atlas_engine_tools::FunctionCallError;
use atlas_engine_tools::JsonToolOutput;
use atlas_engine_tools::ResponsesApiNamespace;
use atlas_engine_tools::ResponsesApiNamespaceTool;
use atlas_engine_tools::ToolCall;
use atlas_engine_tools::ToolExecutor;
use atlas_engine_tools::ToolExecutorFuture;
...
async fn run_code_mode_turn_with_builder(
    ...
    prompt: &str,
    code: &str,
    mut builder: TestAtlasEngineBuilder,
) -> Result<(TestAtlasEngine, ResponseMock)> {
...
async fn missing_process_host_falls_back_to_direct_tools_and_warns_once() -> Result<()> {
    ...
    let mut builder = test_atlas_engine()
        ...
        .with_code_mode_host_program("atlas-engine-code-mode-host-does-not-exist".into())
        .with_config(|config| {
            config
                .features
    ...
    assert!(
        first_warnings.iter().any(|warning| {
            warning.contains("Code Mode is unavailable")
                && warning.contains("atlas-engine-code-mode-host-does-not-exist")
        }),
...
async fn missing_process_host_keeps_code_mode_only_and_fails_closed() -> Result<()> {
    ...
    assert!(
        output.contains("atlas-engine-code-mode-host-does-not-exist"),
...
"#,
    ...
    let auth = AtlasEngineAuth::from_api_key("dummy");
...
const tool = ALL_TOOLS.find(
  ({ name }) => name === "mcp__atlas_apps__calendar_timezone_option_99"
);
if (!tool) {
...
"#,
        visible_tool_name = "mcp__atlas_apps__calendar_timezone_option_99",
    ...
    fn on_tool_finish<'a>(&'a self, input: ToolFinishInput<'a>) -> ToolLifecycleFuture<'a> {
        Box::pin(async move {
            if input.tool_name.name != "test_sync_tool"
                || !matches!(
                    input.source,
                    atlas_engine_extension_api::ToolCallSource::CodeMode { .. }
                )
            {
                return;
            }
...
image({{
  ...
  data,
  mimeType: "image/png",
  _meta: {{ "atlas-agent/imageDetail": "original" }}
}});
"#,
    ...
    assert!(!exec_description.contains("atlas-agent/imageDetail"));
...
async fn code_mode_node_repl_text_evidence_is_visible_only_to_guardian(
            ...
            if reviewer_constraint.is_some_and(|value| value != "large_prompt") || check_detail {
                ...
                if check_detail {
                    ...
                    reviewer.input_modalities =
                        vec![atlas_engine_protocol::openai_models::InputModality::Text];
                } else {
    ...
    let test = builder.build_with_auto_env(&server).await?;
    wait_for_mcp_server(&test.atlas_engine, "node_repl").await?;
...
"#,
        ...
        let turn_id = wait_for_event_match(&test.atlas_engine, |event| match event {
        ...
        if use_responses_lite {
            let metadata: Value = serde_json::from_str(
                first_body["client_metadata"]["x-atlas-engine-turn-metadata"]
                    .as_str()
...
async fn code_mode_can_call_hidden_dynamic_tools() -> Result<()> {
    ...
    let code = r#"
const tool = ALL_TOOLS.find(({ name }) => name === "atlas_engine_app__hidden_dynamic_tool");
const out = await tools.atlas_engine_app__hidden_dynamic_tool({ city: "Paris" });
text(
  JSON.stringify({
    name: tool?.name ?? null,
    description: tool?.description ?? null,
...
"#;
    ...
    .await;
    assert_eq!(request.namespace.as_deref(), Some("atlas_engine_app"));
    ...
    assert!(
        parsed
            ...
            .and_then(Value::as_str)
            .is_some_and(|description| {
                description.contains("Atlas Agent app tools.")
                    && description.contains("A hidden dynamic tool.")
                    && description.contains("declare const tools:")
                    && description.contains("atlas_engine_app__hidden_dynamic_tool(args:")
            })
    );
