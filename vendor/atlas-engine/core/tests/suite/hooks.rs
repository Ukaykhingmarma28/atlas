...
use atlas_engine_core::config::ThreadStoreConfig;
...
async fn managed_hook_discovery_failure_rejects_session_startup_when_hooks_enabled() -> Result<()> {
    ...
    let mut builder = test_atlas_engine().with_cloud_config_bundle(
...
async fn stop_hook_can_block_multiple_times_in_same_turn() -> Result<()> {
    ...
    let hook_inputs = read_stop_hook_inputs(test.atlas_agent_home_path())?;
...
async fn session_start_hook_sees_materialized_transcript_path() -> Result<()> {
    ...
    let hook_inputs = read_session_start_hook_inputs(test.atlas_agent_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
...
async fn session_start_runs_before_user_prompt_submit_on_first_turn() -> Result<()> {
    ...
    let hook_inputs = read_hook_order_inputs(test.atlas_agent_home_path())?;
    assert_eq!(
        hook_inputs
            .iter()
    ...
    test.atlas_engine
        .start_or_steer_turn(
            ...
            .with_thread_settings(
                atlas_engine_protocol::protocol::ThreadSettingsOverrides {
...
async fn async_hook_finishing_while_idle_waits_for_the_next_turn() -> Result<()> {
    ...
    assert!(
        timeout(Duration::from_millis(150), test.atlas_engine.next_event())
            .await
            .is_err(),
...
async fn resumed_thread_runs_resume_then_compact_session_start_hooks() -> Result<()> {
    ...
    let mut resume_builder = test_atlas_engine().with_config(move |config| {
        config.model_auto_compact_token_limit = Some(limit);
        trust_discovered_hooks(config);
    });
    ...
    let hook_inputs = read_session_start_hook_inputs(resumed.atlas_agent_home_path())?;
    assert_eq!(
        hook_inputs
            .iter()
            .filter_map(|input| input.get("source").and_then(Value::as_str))
...
async fn resumed_thread_keeps_stop_continuation_prompt_in_history() -> Result<()> {
    ...
    let mut initial_builder = test_atlas_engine()
        .with_pre_build_hook(|home| {
    ...
    let mut resume_builder = test_atlas_engine().with_config(trust_discovered_hooks);
    let resumed = resume_builder.restart(&server, &initial).await?;
...
async fn blocked_user_prompt_submit_persists_additional_context_for_next_turn() -> Result<()> {
    ...
    let hook_inputs = read_user_prompt_submit_hook_inputs(test.atlas_agent_home_path())?;
...
async fn permission_request_hook_allow_bypasses_strict_auto_review() -> Result<()> {
    ...
    let request = wait_for_event(&test.atlas_engine, |event| {
...
"#,
    ...
    let test = test_atlas_engine()
        ...
        .with_config(move |config| {
            trust_discovered_hooks(config);
            config.approvals_reviewer = atlas_engine_config::types::ApprovalsReviewer::AutoReview;
    ...
    if expected_denial.is_none() {
        ...
        assert!(
            timeout(
                ...
                wait_for_event(&test.atlas_engine, |event| matches!(
                    event,
...
async fn pre_tool_use_blocks_shell_command_before_execution() -> Result<()> {
    ...
    let hook_inputs = read_pre_tool_use_hook_inputs(test.atlas_agent_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
...
"#
    ...
    let hook_inputs = read_post_tool_use_hook_inputs(test.atlas_agent_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
    assert_eq!(hook_inputs[0]["tool_input"]["command"], command);
    assert_eq!(
...
"#,
  ...
  "hooks": {
    ...
    let plugin_hook_sources = vec![PluginHookSource {
        ...
        hooks: serde_json::from_str::<atlas_engine_config::HooksFile>(plugin_hooks_json)
...
async fn pre_tool_use_merges_hooks_json_and_config_toml() -> Result<()> {
    ...
    let json_hook_inputs = read_pre_tool_use_hook_inputs(test.atlas_agent_home_path())?
        .into_iter()
        .map(|hook_input| {
            serde_json::json!({
...
async fn post_tool_use_blocks_when_exec_session_completes_via_write_stdin() -> Result<()> {
    ...
    let pre_hook_inputs = read_pre_tool_use_hook_inputs(test.atlas_agent_home_path())?;
    ...
    let post_hook_inputs = read_post_tool_use_hook_inputs(test.atlas_agent_home_path())?;
