...
async fn window_id_advances_after_compact_persists_on_resume_and_resets_on_fork() -> Result<()> {
    ...
    let initial_thread = Arc::clone(&initial.atlas_engine);
    ...
    let resumed = builder
        ...
        .await?;
    submit_user_turn(&resumed.atlas_engine, "after resume").await?;
    shutdown_thread(&resumed.atlas_engine).await?;
...
async fn submit_user_turn(atlas_engine: &Arc<AtlasEngineThread>, text: &str) -> Result<()> {
    atlas_engine
...
fn window_id_parts(request: &ResponsesRequest) -> (String, u64) {
    let window_id = request
        ...
        .expect("missing x-atlas-engine-window-id header");
