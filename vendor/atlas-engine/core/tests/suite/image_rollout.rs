    ...
    let expected = ResponseItem::Message {
        ...
        content: vec![
            ContentItem::InputText {
                text: atlas_engine_protocol::models::local_image_open_tag_text_with_path(
...
async fn resumed_history_only_emits_resize_notices_for_new_images() -> anyhow::Result<()> {
    ...
    let server = start_mock_server().await;
    let initial = test_atlas_engine().build_with_auto_env(&server).await?;
    ...
    let replayed = test_atlas_engine()
        .with_config(|config| {
            let _ = config.features.enable(Feature::ImageResizeNotice);
            let _ = config
                .features
