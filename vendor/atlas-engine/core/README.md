...
This crate implements the business logic for Atlas Agent. It is designed to be used by the various Atlas Agent UIs written in Rust.

## Wine-exec integration tests

On x86-64 Linux, run the shared suite against the Windows exec server with
`bazel test //atlas-engine-rs/core:core-all-wine-exec-test`.
...
Note that `atlas-engine-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
pointer file), the resolved `gitdir:` target, and `.atlas_engine` read-only.
...
Expects the binary containing `atlas-engine-core` to run the equivalent of `atlas_engine sandbox` when `arg0` is `atlas-engine-linux-sandbox`. See the `atlas-engine-arg0` crate for details.
...
`bwrap` is missing, it falls back to the bundled `atlas-engine-resources/bwrap`
binary shipped with Atlas Agent and Atlas Agent surfaces a startup warning through its
normal notification path instead of printing directly from the sandbox helper.
Atlas Agent also surfaces a startup warning when bubblewrap cannot create user
...
namespaces, so Atlas Agent rejects sandboxed shell commands that would enter the
...
Expects the binary containing `atlas-engine-core` to simulate the virtual
`apply_patch` CLI when `arg1` is `--atlas-engine-run-as-apply-patch`. See the
`atlas-engine-arg0` crate for details.
