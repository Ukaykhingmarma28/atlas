...
pub(crate) fn environment_provider_from_atlas_agent_home(
    atlas_agent_home: &Path,
) -> Result<Box<dyn EnvironmentProvider>, ExecServerError> {
    let path = atlas_agent_home.join(ENVIRONMENTS_TOML_FILE);
...
mod tests {
        ...
        let provider = TomlEnvironmentProvider::new(EnvironmentsToml {
            ...
            environments: vec![
                ...
                EnvironmentToml {
                    ...
                    args: Some(vec![
                        "dev".to_string(),
                        "atlas-agent exec-server --listen stdio".to_string(),
                    ]),
                    env: Some(HashMap::from([(
                        "ATLAS_AGENT_LOG".to_string(),
                        "debug".to_string(),
                    )])),
                    ..Default::default()
                },
        ...
        let cases = [
            ...
            (
                EnvironmentToml {
                    id: "devbox".to_string(),
                    url: Some("ws://127.0.0.1:8765".to_string()),
                    program: Some("atlas-agent".to_string()),
                    ..Default::default()
                },
    ...
    fn load_environments_toml_reads_root_environment_list() {
        ...
        let path = atlas_agent_home.path().join(ENVIRONMENTS_TOML_FILE);
        std::fs::write(
            &path,
            r#"
default = "ssh-dev"
...
id = "ssh-dev"
program = "ssh"
args = ["dev", "atlas-agent exec-server --listen stdio"]
cwd = "/tmp"
[environments.env]
ATLAS_AGENT_LOG = "debug"
"#,
        )
...
"#,
    ...
    #[tokio::test]
    async fn environment_provider_from_atlas_agent_home_uses_present_environments_file() {
        let atlas_agent_home = tempdir().expect("tempdir");
        std::fs::write(
            atlas_agent_home.path().join(ENVIRONMENTS_TOML_FILE),
            r#"
default = "none"
include_local = false
"#,
