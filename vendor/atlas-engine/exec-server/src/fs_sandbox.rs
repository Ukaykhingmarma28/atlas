...
use crate::fs_helper::ATLAS_AGENT_FS_HELPER_ARG1;
...
impl FileSystemSandboxRunner {
    ...
    fn sandbox_exec_request(
        ...
        let helper = &self.runtime_paths.atlas_engine_self_exe;
        ...
        let command = SandboxCommand {
            ...
            args: vec![ATLAS_AGENT_FS_HELPER_ARG1.to_string()],
            cwd: cwd.uri.clone(),
...
fn helper_read_roots(runtime_paths: &ExecServerRuntimePaths) -> Vec<AbsolutePathBuf> {
    let mut roots = vec![runtime_paths.atlas_engine_self_exe.clone()];
    if let Some(path) = &runtime_paths.atlas_engine_linux_sandbox_exe
        && !roots.contains(path)
    {
...
pub(crate) fn spawn_command(
    ...
    unsafe {
        command.pre_exec(|| {
            atlas_engine_utils_pty::pty::close_inherited_fds_except(&[]);
            Ok(())
        });
    }
...
mod tests {
    ...
    fn helper_permissions_preserve_existing_writes() {
        ...
        let readable = runtime_paths.atlas_engine_self_exe.clone();
    ...
    fn helper_env_preserves_path_for_system_bwrap_discovery_without_leaking_secrets() {
        ...
        assert_eq!(
            ...
            HashMap::from([
                ...
                ("TMPDIR".to_string(), "/tmp/atlas-agent".to_string()),
    ...
    fn sandbox_exec_request_carries_helper_env() {
        ...
        let atlas_engine_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths =
            ExecServerRuntimePaths::new(atlas_engine_self_exe.clone(), Some(atlas_engine_self_exe))
                .expect("runtime paths");
    ...
    fn sandbox_context_with_cwd(
        ...
        atlas_engine_file_system::FileSystemSandboxContext::from_permission_profile_with_cwd(
            PermissionProfile::from_runtime_permissions(policy, NetworkSandboxPolicy::Restricted),
            cwd,
        )
    }
