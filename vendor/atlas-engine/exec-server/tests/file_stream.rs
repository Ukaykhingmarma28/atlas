...
use atlas_engine_exec_server::FsCloseParams;
use atlas_engine_exec_server::FsOpenParams;
use atlas_engine_exec_server::FsReadBlockParams;
use atlas_engine_exec_server::FsReadBlockResponse;
...
async fn file_reads_reject_named_pipes() -> Result<()> {
    ...
    let read_path = format!(r"\\.\pipe\atlas-engine-fs-read-{}", Uuid::new_v4());
    ...
    let stream_path = format!(r"\\.\pipe\atlas-engine-fs-stream-{}", Uuid::new_v4());
