...
use atlas_engine_exec_server::ExecServerTelemetry;
use atlas_engine_exec_server::RequestDispatchMode;
...
use atlas_engine_test_binary_support::TestBinaryDispatchGuard;
use atlas_engine_test_binary_support::TestBinaryDispatchMode;
use atlas_engine_test_binary_support::configure_test_binary_dispatch;
use ctor::ctor;

pub(crate) mod exec_server;

pub(crate) const DELAYED_OUTPUT_AFTER_EXIT_PARENT_ARG: &str =
    "--atlas-engine-test-delayed-output-after-exit-parent";
pub(crate) const SYSTEM_PROXY_REQUEST_URL_ENV: &str =
    "ATLAS_AGENT_EXEC_SERVER_TEST_SYSTEM_PROXY_REQUEST_URL";
...
pub(crate) fn current_test_binary_helper_paths() -> anyhow::Result<(PathBuf, Option<PathBuf>)> {
    ...
    let atlas_engine_linux_sandbox_exe = if cfg!(target_os = "linux") {
    ...
    Ok((current_exe, atlas_engine_linux_sandbox_exe))
}
...
fn maybe_run_exec_server_from_test_binary(guard: Option<&TestBinaryDispatchGuard>) {
    ...
    let http_client_factory = match (
        ...
        (Ok(request_url), Ok(proxy_url)) => {
            atlas_engine_http_client::cache_system_proxy_route_for_test(&request_url, proxy_url);
    ...
    let exit_code = match runtime.block_on(atlas_engine_exec_server::run_main_with_telemetry(
        &listen_url,
        runtime_paths,
