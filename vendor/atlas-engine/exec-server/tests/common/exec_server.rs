...
use atlas_engine_exec_server_protocol::JSONRPCNotification;
use atlas_engine_exec_server_protocol::JSONRPCRequest;
...
pub(crate) struct ExecServerHarness {
    ...
    _helper_paths: TestAtlasEngineHelperPaths,
    child: Child,
...
pub(crate) struct TestAtlasEngineHelperPaths {
    pub(crate) atlas_engine_exe: PathBuf,
    pub(crate) atlas_engine_linux_sandbox_exe: Option<PathBuf>,
}
    ...
    V: AsRef<std::ffi::OsStr>,
{
    ...
    let mut child = Command::new(&helper_paths.atlas_engine_exe);
    ...
    child.env("ATLAS_AGENT_HOME", atlas_agent_home.path());
    child.envs(env);
...
impl ExecServerHarness {
    pub(crate) fn atlas_agent_home(&self) -> &std::path::Path {
        self.atlas_agent_home.path()
    }
