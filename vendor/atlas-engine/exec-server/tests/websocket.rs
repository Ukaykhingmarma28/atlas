...
async fn exec_server_accepts_binary_websocket_json() -> anyhow::Result<()> {
    let mut server = exec_server().await?;
    let initialize_id = atlas_engine_exec_server_protocol::RequestId::Integer(1);
    let initialize = JSONRPCMessage::Request(atlas_engine_exec_server_protocol::JSONRPCRequest {
