...
# atlas-engine-exec-server

`atlas-engine-exec-server` is the library backing `atlas_engine exec-server`, a small
JSON-RPC server for spawning and controlling subprocesses through
`atlas-engine-utils-pty`.

It provides:

- a CLI entrypoint: `atlas_engine exec-server`
- a Rust client: `ExecServerClient`
- a small protocol module with shared request/response types

This crate owns the transport, protocol, and filesystem/process handlers. The
top-level `atlas_engine` binary owns hidden helper dispatch for sandboxed
filesystem operations and `atlas-engine-linux-sandbox`.

## Transport

The server speaks the exec-specific `atlas-engine-exec-server-protocol` message
...
must support it.
It uses the standard Atlas Agent ChatGPT sign-in state; run `atlas_engine login` first when
remote registration needs authentication. Containerized callers that receive an
Agent Identity JWT in `ATLAS_AGENT_ACCESS_TOKEN` can opt into that auth path with
`--use-agent-identity-auth`; Atlas Agent then registers an Agent task and sends the
derived AgentAssertion headers on the registry request.

Alternatively, API users can instead use `ATLAS_AGENT_API_KEY`;
Atlas Agent sends it as a bearer token on the registration request. For example:

```sh
ATLAS_AGENT_API_KEY="$OPENAI_API_KEY" \
atlas-agent exec-server \
  --remote ... \
...
`atlas_engine.exec_server.relay.v1.RelayMessageFrame`; the checked-in schema is in
`src/proto/atlas_engine.exec_server.relay.v1.proto`. The relay frame carries stream
...
- `arg0`: optional argv0 override forwarded to `atlas-engine-utils-pty`.
...
hidden helper process launched from the top-level `atlas_engine` executable and
...
`HttpClientFactory` to `run_main()`. The top-level `atlas_engine exec-server` command
builds these paths from the `atlas_engine` arg0 dispatch state and resolves its HTTP
client factory from the effective Atlas Agent configuration.
...
provider from Atlas Agent auth state before starting remote mode.
