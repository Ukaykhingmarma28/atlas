...
use atlas_engine_aws_auth::AwsAuthContext;
use atlas_engine_aws_auth::AwsAuthError;
use atlas_engine_aws_auth::AwsRequestToSign;
use atlas_engine_http_client::Request;
use atlas_engine_http_client::RequestBody;
use atlas_engine_http_client::RequestCompression;
...
pub(super) async fn resolve_auth_method(
    ...
    let context = AwsAuthContext::load(config)
        .await
        .map_err(aws_auth_error_to_atlas_engine_error)?;
...
fn aws_auth_error_to_atlas_engine_error(error: AwsAuthError) -> AtlasEngineErr {
    AtlasEngineErr::Fatal(format!("failed to resolve Amazon Bedrock auth: {error}"))
}
...
fn remove_headers_not_preserved_by_bedrock_mantle(headers: &mut HeaderMap) {
    ...
    // richer Atlas Agent agent requests fail even though raw Responses requests work.
...
impl AuthProvider for BedrockSigV4AuthProvider {
    ...
    fn apply_auth(&self, request: Request) -> atlas_engine_api::AuthProviderFuture<'_> {
