...
use atlas_engine_api::ModelsClient;
...
impl OpenAiModelsEndpoint {
    ...
    async fn list_models(
        ...
        .await
        .map_err(|_| AtlasEngineErr::Timeout)?
    }

    fn auth_env(&self) -> AuthEnvTelemetry {
        let atlas_engine_api_key_env_enabled = self
            .auth_manager
            .as_ref()
            .is_some_and(|auth_manager| auth_manager.atlas_engine_api_key_env_enabled());
        collect_auth_env_telemetry(&self.provider_info, atlas_engine_api_key_env_enabled)
    }
}
...
impl ModelsEndpointClient for OpenAiModelsEndpoint {
    ...
    fn uses_atlas_engine_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(OpenAiModelsEndpoint::uses_atlas_engine_backend(self))
    }
...
impl RequestTelemetry for ModelsRequestTelemetry {
    fn on_request(
        ...
        tracing::event!(
            target: "atlas_engine_otel.log_only",
            tracing::Level::INFO,
            event.name = "atlas_agent.api_request",
            ...
            auth.env_atlas_engine_api_key_present = self.auth_env.atlas_engine_api_key_env_present,
            auth.env_atlas_engine_api_key_enabled = self.auth_env.atlas_engine_api_key_env_enabled,
        ...
        tracing::event!(
            target: "atlas_engine_otel.trace_safe",
            tracing::Level::INFO,
            event.name = "atlas_agent.api_request",
            duration_ms = %duration.as_millis(),
            http.response.status_code = status,
