...
use atlas_engine_agent_identity::AgentIdentityKey;
use atlas_engine_agent_identity::authorization_header_for_agent_task;
use atlas_engine_api::AgentIdentityTelemetry;
use atlas_engine_api::AuthError;
use atlas_engine_api::AuthHeadersFuture;
...
use atlas_engine_login::auth::AgentIdentityAuthError;
...
struct AuthManagerAuthProvider {
    ...
    expected_auth: AtlasEngineAuth,
}

impl AuthManagerAuthProvider {
    fn is_expected_auth(&self, auth: &AtlasEngineAuth) -> bool {
        auth.uses_atlas_engine_backend()
    ...
    fn current_auth(&self) -> Option<AtlasEngineAuth> {
        self.auth_manager
            .auth_cached()
...
pub(crate) fn resolve_provider_auth(
    ...
    provider: &ModelProviderInfo,
) -> atlas_engine_protocol::error::Result<SharedAuthProvider> {
    if matches!(auth, Some(AtlasEngineAuth::BedrockApiKey(_))) {
        return Err(AtlasEngineErr::UnsupportedOperation(
...
) -> atlas_engine_protocol::error::Result<ResolvedProviderAuth> {
    let ProviderAuthScope {
        agent_identity_policy,
        session_source,
        agent_identity_session_fallback,
    } = scope;
    if let Some(AtlasEngineAuth::AgentIdentity(agent_identity_auth)) = auth {
...
fn should_bootstrap_chatgpt_agent_identity(
    ...
    agent_identity_policy == AgentIdentityAuthPolicy::ChatGptAuth
        && matches!(auth, Some(AtlasEngineAuth::Chatgpt(_)))
}

fn bearer_auth_for_provider(
    provider: &ModelProviderInfo,
) -> atlas_engine_protocol::error::Result<Option<BearerAuthProvider>> {
...
/// Builds request-header auth for a first-party Atlas Agent auth snapshot.
pub fn auth_provider_from_auth(auth: &AtlasEngineAuth) -> SharedAuthProvider {
    match auth {
        AtlasEngineAuth::AgentIdentity(auth) => {
            Arc::new(AgentIdentityAuthProvider { auth: auth.clone() })
        }
        AtlasEngineAuth::Headers(auth) => Arc::new(HeaderAuthProvider { auth: auth.clone() }),
        AtlasEngineAuth::BedrockApiKey(_) => unreachable!("{BEDROCK_API_KEY_UNSUPPORTED_MESSAGE}"),
        AtlasEngineAuth::ApiKey(_)
        | AtlasEngineAuth::Chatgpt(_)
        | AtlasEngineAuth::ChatgptAuthTokens(_)
        | AtlasEngineAuth::PersonalAccessToken(_) => Arc::new(BearerAuthProvider {
...
#[cfg(test)]
mod tests {
    use atlas_engine_agent_identity::generate_agent_key_material;
    ...
    use atlas_engine_login::auth::login_with_chatgpt_auth_tokens;
    ...
    fn test_atlas_agent_home() -> PathBuf {
        ...
        let path = std::env::temp_dir().join(format!(
            "atlas-engine-model-provider-agent-identity-{pid}-{id}",
            pid = std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&path);
        std::fs::create_dir_all(&path).expect("create temp atlas-agent home");
        path
    }

    fn write_chatgpt_auth_json(atlas_agent_home: &Path) {
        let auth_json = json!({
            "tokens": {
    ...
    async fn chatgpt_auth_manager(
    ...
    ) -> (PathBuf, Arc<AuthManager>, AtlasEngineAuth) {
        let atlas_agent_home = test_atlas_agent_home();
        write_chatgpt_auth_json(&atlas_agent_home);
        ...
        (atlas_agent_home, auth_manager, auth)
    }
    ...
    fn header_auth_adds_predefined_headers() {
        ...
        let auth = AtlasEngineAuth::Headers(AuthHeaders::new(expected.clone()));
    ...
    async fn first_party_run_scope_uses_agent_assertion_and_exposes_telemetry() {
        let auth = AtlasEngineAuth::AgentIdentity(
    ...
    async fn agent_identity_auth_provider_preserves_account_routing_headers() {
        ...
        let provider = auth_provider_from_auth(&AtlasEngineAuth::AgentIdentity(auth));
    ...
    async fn chatgpt_bootstrap_unavailable_uses_session_bearer_fallback() {
        ...
        .await;
        let (_atlas_agent_home, auth_manager, auth) = chatgpt_auth_manager(server.uri()).await;
        let provider = ModelProviderInfo::create_openai_provider(/*base_url*/ None);
