...
async fn backfill_sessions_resumes_from_watermark_and_marks_complete() {
    let dir = tempdir().expect("tempdir");
    let atlas_agent_home = dir.path().to_path_buf();
    ...
    let runtime = atlas_engine_state::StateRuntime::init(
        atlas_engine_state::SqliteConfig::new_for_testing(atlas_agent_home.as_path().abs()),
...
async fn backfill_sessions_normalizes_cwd_before_upsert() {
    ...
    let atlas_agent_home = dir.path().to_path_buf();
    let thread_uuid = Uuid::new_v4();
    let session_cwd = atlas_agent_home.join(".");
    let rollout_path = write_rollout_in_sessions_with_cwd(
        atlas_agent_home.as_path(),
        "2026-01-27T12-34-56",
        "2026-01-27T12:34:56Z",
...
fn write_rollout_in_sessions_with_cwd(
    ...
    let sessions_dir = atlas_agent_home.join("sessions");
