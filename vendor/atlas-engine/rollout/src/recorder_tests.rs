...
fn test_config(atlas_agent_home: &Path) -> RolloutConfig {
    RolloutConfig {
        atlas_agent_home: atlas_agent_home.to_path_buf(),
        sqlite: atlas_engine_state::SqliteConfig::new_for_testing(atlas_agent_home.abs()),
        cwd: atlas_agent_home.to_path_buf(),
...
async fn state_db_init_backfills_before_returning() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        atlas_engine_state::BackfillStatus::Complete
    );
...
-> std::io::Result<()> {
    ...
    let valid_created_at = chrono::Utc
        ...
        .single()
        .expect("valid datetime");
    let mut valid_builder = atlas_engine_state::ThreadMetadataBuilder::new(
        valid_thread_id,
        valid_path.clone(),
        valid_created_at,
        SessionSource::Cli,
...
fn fill_missing_thread_item_metadata_preserves_identity_and_prefers_state_git_fields() {
    ...
    let state_item = ThreadItem {
        ...
        section: Some(atlas_engine_state::ThreadSection {
