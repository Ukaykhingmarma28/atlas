...
pub async fn append_thread_name(
    ...
    append_session_index_entry(atlas_agent_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(atlas_agent_home);
...
fn session_index_path(atlas_agent_home: &Path) -> PathBuf {
    atlas_agent_home.join(SESSION_INDEX_FILE)
}
