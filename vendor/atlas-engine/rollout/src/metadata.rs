...
use atlas_engine_state::BackfillState;
use atlas_engine_state::BackfillStats;
use atlas_engine_state::BackfillStatus;
use atlas_engine_state::DB_ERROR_METRIC;
use atlas_engine_state::DB_METRIC_BACKFILL;
use atlas_engine_state::DB_METRIC_BACKFILL_DURATION_MS;
use atlas_engine_state::ExtractionOutcome;
use atlas_engine_state::ThreadMetadataBuilder;
use atlas_engine_state::apply_rollout_item;
use std::path::Path;
...
pub(crate) async fn backfill_sessions_with_lease(
    ...
    let metric_client = atlas_engine_otel::global();
    ...
    let sessions_root = atlas_agent_home.join(SESSIONS_SUBDIR);
    let archived_root = atlas_agent_home.join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(atlas_agent_home, &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(atlas_agent_home: &Path, path: &Path) -> String {
    path.strip_prefix(atlas_agent_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
