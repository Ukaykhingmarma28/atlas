...
use crate::state_db;
use atlas_engine_file_search as file_search;
...
pub struct ThreadItem {
    ...
    pub section: Option<atlas_engine_state::ThreadSection>,
...
impl From<atlas_engine_state::Anchor> for Cursor {
    fn from(anchor: atlas_engine_state::Anchor) -> Self {
        let ts = anchor
            .ts
            .timestamp_nanos_opt()
...
pub async fn get_threads(
    ...
    let root = atlas_agent_home.join(SESSIONS_SUBDIR);
    get_threads_in_root(
        root,
        page_size,
        cursor,
...
///
/// Directory layout: `~/.atlas_engine/sessions/YYYY/MM/DD/rollout-YYYY-MM-DDThh-mm-ss-<uuid>.jsonl`
...
async fn find_thread_path_by_id_str_in_subdir(
    ...
    let mut root = atlas_agent_home.to_path_buf();
    root.push(subdir);
    if !root.exists() {
...
pub async fn find_rollout_path_by_rollout_id(
    ...
    for subdir in [SESSIONS_SUBDIR, ARCHIVED_SESSIONS_SUBDIR] {
        let path = find_rollout_path_by_rollout_id_from_filenames(
            atlas_agent_home.join(subdir).as_path(),
            rollout_id,
        )
        .await?;
        if path.is_some() {
