...
//! Persist Atlas Agent session rollouts (.jsonl) so sessions can be replayed or inspected later.
...
use atlas_engine_protocol::protocol::GitInfo as ProtocolGitInfo;
use atlas_engine_protocol::protocol::HistoryPosition;
use atlas_engine_protocol::protocol::MultiAgentVersion;
use atlas_engine_protocol::protocol::SessionContextWindow;
...
use atlas_engine_utils_path as path_utils;
...
///
/// ```ignore
/// $ jq -C . ~/.atlas-agent/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// $ fx ~/.atlas-agent/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// ```
#[derive(Clone)]
...
impl RolloutRecorder {
    /// List threads (rollout files) under the provided Atlas Agent home directory.
    #[allow(clippy::too_many_arguments)]
    ...
    async fn list_threads_with_db_fallback(
        ...
        let atlas_agent_home = config.atlas_agent_home();
...
async fn list_threads_from_files_desc_unfiltered(
    ...
    if archived {
        let root = atlas_agent_home.join(ARCHIVED_SESSIONS_SUBDIR);
        get_threads_in_root(
            root,
            page_size,
            cursor,
...
async fn list_threads_from_files_asc(
    ...
    filter_thread_items_by_search_term(atlas_agent_home, &mut all_items, search_term).await?;
...
async fn filter_thread_items_by_search_term(
    ...
    let thread_ids = items
        ...
        .filter_map(|item| item.thread_id)
        .collect::<HashSet<_>>();
    let thread_names = find_thread_names_by_ids(atlas_agent_home, &thread_ids).await?;
    items.retain(|item| {
        item.thread_id
...
fn precompute_new_rollout_path(
    ...
    // Resolve ~/.atlas-agent/sessions/YYYY/MM/DD path.
    let timestamp = OffsetDateTime::now_local()
        .map_err(|e| IoError::other(format!("failed to get local time: {e}")))?;
    let mut dir = config.atlas_agent_home().to_path_buf();
...
impl From<atlas_engine_state::ThreadsPage> for ThreadsPage {
    fn from(db_page: atlas_engine_state::ThreadsPage) -> Self {
...
fn thread_item_from_state_metadata(
    item: atlas_engine_state::ThreadMetadata,
    parent_thread_id: Option<ThreadId>,
) -> ThreadItem {
    ThreadItem {
...
async fn select_resume_path_from_db_page(
    page: &atlas_engine_state::ThreadsPage,
    filter_cwd: Option<&Path>,
    default_provider: &str,
) -> Option<PathBuf> {
    match filter_cwd {
