...
//! existing rollout path. They must not do that at the same time for one Atlas Agent home, so they share
...
/// Try to exclude rollout compression and migration for one Atlas Agent home.
pub fn try_acquire_rollout_maintenance_lock(
    atlas_agent_home: &Path,
) -> io::Result<Option<RolloutMaintenanceGuard>> {
    let directory = atlas_agent_home.join(".tmp");
