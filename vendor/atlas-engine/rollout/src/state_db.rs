...
pub use atlas_engine_state::LogEntry;
...
pub type StateDbHandle = Arc<atlas_engine_state::StateRuntime>;
...
async fn try_init_with_roots_inner(
    ...
    let runtime =
        atlas_engine_state::StateRuntime::init(sqlite.clone(), default_model_provider_id.clone())
            .await
            .with_context(|| {
                format!(
    ...
    .await;
    atlas_engine_state::record_backfill_gate(
        /*telemetry*/ None,
...
async fn wait_for_backfill_gate(
    ...
    loop {
        ...
        if backfill_state.status == atlas_engine_state::BackfillStatus::Complete {
            return Ok(());
        }
        ...
        if let Some(backfill_lease_seconds) = backfill_lease_seconds {
            ...
            metadata::backfill_sessions(runtime, atlas_agent_home, default_model_provider_id).await;
        }
        let backfill_state = runtime.get_backfill_state().await.map_err(|err| {
            anyhow::anyhow!(
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    ...
    let runtime = match atlas_engine_state::StateRuntime::init(
...
pub fn sqlite_telemetry_recorder(
    ...
    originator: &str,
) -> atlas_engine_state::DbTelemetryHandle {
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == atlas_engine_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<atlas_engine_state::Anchor> {
    let cursor = cursor?;
    ...
    Some(atlas_engine_state::Anchor {
        ts,
        id: cursor.thread_id(),
    })
}
...
pub async fn list_thread_ids_db(
    context: Option<&atlas_engine_state::StateRuntime>,
    sqlite: &atlas_engine_state::SqliteConfig,
    page_size: usize,
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => atlas_engine_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => atlas_engine_state::SortKey::UpdatedAt,
                ThreadSortKey::RecencyAt => atlas_engine_state::SortKey::RecencyAt,
            },
...
pub async fn list_threads_db(
    ...
    relation_filter: Option<atlas_engine_state::ThreadRelationFilter>,
    archived: bool,
    section: Option<Option<&str>>,
    search_term: Option<&str>,
) -> Option<atlas_engine_state::ThreadsPage> {
    let ctx = context?;
    if ctx.sqlite() != sqlite {
        warn!(
            "state db SQLite home mismatch: expected {}, got {}",
    ...
    if let Some(relation_filter) = relation_filter {
        let filters = atlas_engine_state::ThreadFilterOptions {
    ...
    Some(atlas_engine_state::ThreadsPage {
        items,
