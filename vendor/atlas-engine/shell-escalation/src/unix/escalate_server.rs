...
mod tests {
    ...
    use atlas_engine_protocol::approvals::EscalationPermissions;
    ...
    /// executable paths.
    ///
    /// The `/bin/zsh` and `/tmp/atlas-engine-execve-wrapper` values here are
    ...
    async fn start_session_exposes_wrapper_env_overlay() -> anyhow::Result<()> {
        ...
        let execve_wrapper = PathBuf::from("/tmp/atlas-engine-execve-wrapper");
    ...
    async fn exec_closes_parent_socket_after_shell_spawn() -> anyhow::Result<()> {
        ...
        let server = EscalateServer::new(
            ...
            PathBuf::from("/tmp/atlas-engine-execve-wrapper"),
            DeterministicEscalationPolicy {
                decision: EscalationDecision::run(),
            },
        );
    ...
    async fn handle_escalate_session_respects_run_in_sandbox_decision() -> anyhow::Result<()> {
        ...
        for i in 0..10 {
            ...
            env.insert(format!("ATLAS_AGENT_TEST_VAR{i}"), value);
        }
