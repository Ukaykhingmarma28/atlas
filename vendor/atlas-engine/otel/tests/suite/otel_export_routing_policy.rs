...
use atlas_engine_otel::AuthEnvTelemetryMetadata;
...
fn auth_env_metadata() -> AuthEnvTelemetryMetadata {
    AuthEnvTelemetryMetadata {
        ...
        atlas_engine_api_key_env_present: false,
        atlas_engine_api_key_env_enabled: true,
...
fn otel_export_routing_policy_routes_user_prompt_log_and_trace_events() {
    ...
    let prompt_trace_event = find_span_event_by_name_attr(span_events, "atlas_agent.user_prompt");
...
fn otel_export_routing_policy_routes_tool_result_log_and_trace_events() {
    ...
    let tool_trace_event = find_span_event_by_name_attr(span_events, "atlas_agent.tool_result");
...
fn otel_export_routing_policy_routes_auth_recovery_log_and_trace_events() {
    ...
    let logs = log_exporter.get_emitted_logs().expect("log export");
    let recovery_log = find_log_by_event_name(&logs, "atlas_agent.auth_recovery");
...
fn otel_export_routing_policy_routes_api_request_auth_observability() {
    ...
    let logs = log_exporter.get_emitted_logs().expect("log export");
    let conversation_log = find_log_by_event_name(&logs, "atlas_agent.conversation_starts");
    ...
    let request_log = find_log_by_event_name(&logs, "atlas_agent.api_request");
    ...
    assert_eq!(
        request_log_attrs
            .get("auth.env_atlas_engine_api_key_enabled")
            .map(String::as_str),
        Some("true")
    );
    assert_eq!(
    ...
    let conversation_trace_event =
        find_span_event_by_name_attr(&spans[0].events.events, "atlas_agent.conversation_starts");
    ...
    let request_trace_event =
        find_span_event_by_name_attr(&spans[0].events.events, "atlas_agent.api_request");
...
fn otel_export_routing_policy_routes_websocket_connect_auth_observability() {
    ...
    let logs = log_exporter.get_emitted_logs().expect("log export");
    let connect_log = find_log_by_event_name(&logs, "atlas_agent.websocket_connect");
    ...
    let connect_trace_event =
        find_span_event_by_name_attr(&spans[0].events.events, "atlas_agent.websocket_connect");
...
fn otel_export_routing_policy_routes_websocket_request_transport_observability() {
    ...
    let logs = log_exporter.get_emitted_logs().expect("log export");
    let request_log = find_log_by_event_name(&logs, "atlas_agent.websocket_request");
    let request_log_attrs = log_attributes(&request_log.record);
    assert_eq!(
        request_log_attrs
            .get("auth.connection_reused")
    ...
    let spans = span_exporter.get_finished_spans().expect("span export");
    let request_trace_event =
        find_span_event_by_name_attr(&spans[0].events.events, "atlas_agent.websocket_request");
    let request_trace_attrs = span_event_attributes(request_trace_event);
    assert_eq!(
        request_trace_attrs
            .get("auth.connection_reused")
