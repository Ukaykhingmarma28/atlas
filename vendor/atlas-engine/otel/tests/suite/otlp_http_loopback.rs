...
fn otlp_http_exporter_sends_metrics_to_collector() -> Result<()> {
    ...
    metrics.counter("atlas_agent.conversation.turn.count", /*inc*/ 1, &[])?;
    metrics.record_duration(
    ...
    metrics.counter("atlas_agent.tool.call", /*inc*/ 1, &[("tool", "test")])?;
    metrics.record_duration(
        "atlas_agent.tool.call.duration_ms",
        Duration::from_millis(42),
        &[("tool", "test")],
    )?;
    metrics.histogram(
        "atlas_agent.turn.token_usage",
        /*value*/ 100,
        &[("token_type", "total")],
    )?;
...
-> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::event!(
            ...
            tracing::Level::INFO,
            event.name = "atlas_agent.test.log_exported",
...
-> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let propagated_trace = tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::event!(
            ...
            tracing::Level::INFO,
            event.name = "atlas_agent.test.trace_event",
