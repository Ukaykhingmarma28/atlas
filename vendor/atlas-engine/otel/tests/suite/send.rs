...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "atlas-engine-cli"), ("env", "prod")])?;

    metrics.counter_with_description(
        "atlas_agent.turns",
        "Total number of Atlas Agent turns.",
        /*inc*/ 1,
        &[("model", "gpt-5.1"), ("env", "dev")],
    )?;
    metrics.histogram(
        "atlas_agent.tool_latency",
        /*value*/ 25,
        &[("tool", "shell")],
    )?;
    ...
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&resource_metrics, "atlas_agent.tool_latency");
    ...
    let histogram_attrs = attributes_to_map(
        find_metric(&resource_metrics, "atlas_agent.tool_latency")
            ...
            .expect("atlas-agent.tool_latency histogram attributes should exist"),
    );
...
fn client_sends_enqueued_metric() -> Result<()> {
    ...
    metrics.counter("atlas_agent.turns", /*inc*/ 1, &[("model", "gpt-5.1")])?;
    metrics.shutdown()?;
...
fn shutdown_flushes_in_memory_exporter() -> Result<()> {
    ...
    metrics.counter("atlas_agent.turns", /*inc*/ 1, &[])?;
    metrics.shutdown()?;
