...
use atlas_engine_rmcp_client::with_http_headers_helper;
...
pub struct McpRuntimeInput {
    ...
    pub atlas_apps_tools_cache: ConnectorRuntimeManager<ToolInfo>,
    pub tool_catalog_cache: McpToolCatalogCache,
    pub atlas_apps_tools_cache_key: ConnectorRuntimeContextKey,
    pub client_mcp_extensions: ClientMcpExtensions,
    pub auth: Option<AtlasEngineAuth>,
    pub atlas_apps_auth_manager: Option<Arc<AuthManager>>,
    pub elicitation_reviewer: Option<ElicitationReviewerHandle>,
    pub elicitation_lifecycle: Option<ElicitationLifecycle>,
}

/// Owns all mutable MCP state for one Atlas Agent thread.
///
...
impl McpRuntime {
    ...
    pub async fn replace_fresh(&self, input: McpRuntimeInput) -> anyhow::Result<Vec<ToolInfo>> {
        ...
        self.latest_hard_refresh_atlas_apps_tools_cache().await
    }
    ...
    pub fn current_auth_matches(&self, auth: Option<&AtlasEngineAuth>) -> bool {
    ...
    pub async fn latest_hard_refresh_atlas_apps_tools_cache(
        &self,
...
pub fn apply_http_headers_helper(
    ...
    config: &atlas_engine_config::McpServerConfig,
    local_process_cwd: PathBuf,
) -> Result<Arc<dyn HttpClient>, String> {
    let atlas_engine_config::McpServerTransportConfig::StreamableHttp {
        url,
    ...
    pub(crate) fn resolve_server_environment(
        ...
        if config.is_local_environment() {
            return match config.transport {
                atlas_engine_config::McpServerTransportConfig::Stdio { .. } => Err(format!(
                    "local stdio MCP server `{server_name}` requires a local environment"
                )),
                atlas_engine_config::McpServerTransportConfig::StreamableHttp { .. } => Ok(None),
            };
        }
