...
//! and exposing cached Atlas Agent Apps tools while a client is still connecting.
...
use crate::atlas_apps::normalize_atlas_apps_callable_name;
use crate::atlas_apps::normalize_atlas_apps_callable_namespace;
use crate::atlas_apps::normalize_atlas_apps_tool_title;
use crate::atlas_apps::prepare_openai_file_params_for_model;
...
use atlas_engine_rmcp_client::ToolWithConnectorId;
use atlas_engine_rmcp_client::is_authentication_required_error;
...
/// MCP server capability indicating that Atlas Agent should include [`SandboxState`]
/// in tool-call request `_meta` under this key.
pub const MCP_SANDBOX_STATE_META_CAPABILITY: &str = "atlas-agent/sandbox-state-meta";
...
const MCP_TOOL_CATALOG_CACHE_CAPABILITY: &str = "atlas-agent/tool-catalog-cache";
const MCP_TOOL_CATALOG_CACHEABLE_PROPERTY: &str = "cacheable";
pub(crate) const MCP_TOOLS_LIST_DURATION_METRIC: &str = "atlas_agent.mcp.tools.list.duration_ms";
pub(crate) const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: &str =
    "atlas_agent.mcp.tools.fetch_uncached.duration_ms";
pub(crate) const ATLAS_APPS_REFRESH_DURATION_METRIC: &str = "atlas_agent.apps.refresh.duration_ms";
pub(crate) const DEFAULT_STARTUP_TIMEOUT: Duration = Duration::from_secs(30);
pub(crate) const DEFAULT_TOOL_TIMEOUT: Duration = Duration::from_secs(300);

pub(crate) const ATLAS_APPS_RECONNECT_INITIAL_BACKOFF: Duration = Duration::from_secs(1);
const ATLAS_APPS_RECONNECT_MAX_BACKOFF: Duration = Duration::from_secs(30);
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) atlas_apps_tools_cache_context: Option<ConnectorRuntimeContext<ToolInfo>>,
}
...
impl ManagedClient {
    pub(crate) fn listed_tools(&self) -> Vec<ToolInfo> {
        ...
        if self.atlas_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
#[derive(Default)]
struct AtlasAppsStartupReconnectState {
...
#[derive(Clone)]
struct AtlasAppsStartupStatusContext {
    submit_id: String,
    server_name: String,
    tx_event: Sender<Event>,
}

pub(crate) struct AtlasAppsStartupReconnect {
    factory: Arc<dyn Fn() -> ManagedClientFuture + Send + Sync>,
    state: StdMutex<AtlasAppsStartupReconnectState>,
    startup_status_context: Option<AtlasAppsStartupStatusContext>,
}

impl AtlasAppsStartupReconnect {
    pub(crate) fn new(factory: Arc<dyn Fn() -> ManagedClientFuture + Send + Sync>) -> Self {
        Self {
            factory,
            state: StdMutex::new(AtlasAppsStartupReconnectState::default()),
    ...
    fn with_startup_status_context(
        ...
        self.startup_status_context = tx_event.map(|tx_event| AtlasAppsStartupStatusContext {
            submit_id,
            server_name,
            tx_event,
        });
            ...
            let recovered = {
                ...
                match result {
                    ...
                    Err(error) => {
                        ...
                        let retry_after = atlas_apps_reconnect_backoff(state.consecutive_failures);
...
fn atlas_apps_reconnect_backoff(consecutive_failures: u32) -> Duration {
    let exponent = consecutive_failures.saturating_sub(1).min(5);
    ATLAS_APPS_RECONNECT_INITIAL_BACKOFF
        .saturating_mul(1 << exponent)
        .min(ATLAS_APPS_RECONNECT_MAX_BACKOFF)
}
...
struct ManagedClientStartup {
    ...
    atlas_apps_tools_cache_context: Option<ConnectorRuntimeContext<ToolInfo>>,
...
impl ManagedClientStartup {
    fn start(&self) -> ManagedClientFuture {
        ...
        let is_atlas_apps_mcp_server = server_name == ATLAS_APPS_MCP_SERVER_NAME;
        ...
        async move {
            ...
            let refresh_start = is_atlas_apps_mcp_server.then(Instant::now);
            ...
            {
                emit_duration(
                    ATLAS_APPS_REFRESH_DURATION_METRIC,
...
pub(crate) struct AsyncManagedClient {
    ...
    pub(crate) is_atlas_apps_mcp_server: bool,
    ...
    pub(crate) startup_reconnect: Option<Arc<AtlasAppsStartupReconnect>>,
...
impl AsyncManagedClient {
    ...
    pub(crate) fn new(
        ...
        let cached_server_info = if is_atlas_apps_mcp_server {
            atlas_apps_tools_cache_context
                .as_ref()
        ...
        let startup = Arc::new(ManagedClientStartup {
            ...
            tx_event,
            elicitation_requests,
            atlas_apps_tools_cache_context: atlas_apps_tools_cache_context.clone(),
        ...
        let startup_reconnect = is_atlas_apps_mcp_server.then(|| {
            let startup = Arc::clone(&startup);
            Arc::new(
                AtlasAppsStartupReconnect::new(Arc::new(move || startup.start()))
    ...
    pub(crate) fn has_cached_tools(&self) -> bool {
        self.atlas_apps_tools_cache_context
            .as_ref()
    ...
    pub(crate) async fn listed_tools(&self) -> Option<Vec<ToolInfo>> {
            ...
            && let Some(startup_tools) = self.cached_tools()
        {
            ...
            match self.client().await {
                ...
                Err(_) if self.is_atlas_apps_mcp_server => self.cached_tools(),
                Err(_) => None,
            }
        }
    }
...
pub(crate) async fn list_tools_for_client_uncached(
    server_name: &str,
    is_atlas_apps_mcp_server: bool,
    atlas_apps_refresh_trigger: &'static str,
    ...
    .collect();
    if is_atlas_apps_mcp_server {
        emit_duration(
            MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC,
            fetch_start.elapsed(),
            &[("trigger", atlas_apps_refresh_trigger)],
        );
    } else {
        emit_duration(
            MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC,
...
/// Presents declared Atlas Agent Apps file parameters to the model as local-path inputs and adds plugin
/// names to each tool. Plugin membership is resolved by connector ID, falling back to the MCP
/// server when absent.
pub(crate) fn prepare_atlas_apps_tools_for_model(
...
fn tool_info_from_listed_tool(
    ...
    if is_atlas_apps_mcp_server {
        atlas_apps_tool_info_from_listed_tool(server_name, server_instructions, tool)
    } else {
        regular_mcp_tool_info_from_listed_tool(server_name, server_instructions, tool)
    }
}

/// Converts a Atlas Agent Apps tool by preserving connector fields, removing connector prefixes from
/// model-visible names and titles, and using the connector description for its tool namespace.
fn atlas_apps_tool_info_from_listed_tool(
    server_name: &str,
    server_instructions: Option<&str>,
    tool: ToolWithConnectorId,
) -> ToolInfo {
    ...
    let callable_name = normalize_atlas_apps_callable_name(
        &tool_def.name,
        connector_id.as_deref(),
        connector_name.as_deref(),
    );
    let callable_namespace =
        normalize_atlas_apps_callable_namespace(server_name, connector_name.as_deref());
    if let Some(title) = tool_def.title.as_deref() {
        let normalized_title = normalize_atlas_apps_tool_title(connector_name.as_deref(), title);
...
async fn start_server_task(
    ...
    let fetch_ticket = atlas_apps_tools_cache_context
        .as_ref()
    ...
    let client_tools = list_tools_for_client_uncached(
        &server_name,
        is_atlas_apps_mcp_server,
        /*atlas_apps_refresh_trigger*/ "initial",
        &client,
        startup_timeout,
        catalog_item_limit,
    ...
    let shared_tools = match (atlas_apps_tools_cache_context.as_ref(), fetch_ticket) {
        ...
        _ => unreachable!("Atlas Agent Apps fetch ticket requires cache context"),
    };
    let has_shared_tool_catalog = is_atlas_apps_mcp_server || tool_catalog_cache_context.is_some();
...
fn mcp_initialize_request_params(
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("atlas-agent", env!("CARGO_PKG_VERSION")).with_title("Atlas Agent"),
    )
...
async fn make_rmcp_client(
    ...
    match transport {
        McpServerTransportConfig::Stdio {
            ...
            let cwd = cwd.map(atlas_engine_utils_path_uri::LegacyAppPathString::into_string);
...
mod tests {
    ...
    #[test]
    fn atlas_apps_connector_metadata_is_preserved() {
        ...
        let expected = ToolInfo {
            server_name: ATLAS_APPS_MCP_SERVER_NAME.to_string(),
            supports_parallel_tool_calls: false,
            server_origin: None,
            callable_name: "capture_file_upload".to_string(),
            callable_namespace: "atlas_apps__gmail".to_string(),
