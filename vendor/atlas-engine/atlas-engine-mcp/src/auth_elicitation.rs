...
//! Session orchestration stays in `atlas-engine-core`.

use atlas_engine_protocol::mcp::CallToolResult;
use serde::Serialize;

pub const MCP_TOOL_ATLAS_APPS_META_KEY: &str = "_atlas_apps";
...
pub struct AtlasAppsConnectorAuthFailure {
...
pub struct AtlasAppsAuthElicitation {
...
#[derive(Debug, Clone, PartialEq)]
pub struct AtlasAppsAuthElicitationPlan {
    pub auth_failure: AtlasAppsConnectorAuthFailure,
    pub elicitation: AtlasAppsAuthElicitation,
}

#[derive(Serialize)]
struct AtlasAppsConnectorAuthFailureMeta<'a> {
...
pub fn connector_auth_failure_from_tool_result(
...
) -> Option<AtlasAppsConnectorAuthFailure> {
    ...
    let auth_failure = result
        ...
        .as_ref()?
        .as_object()?
        .get(MCP_TOOL_ATLAS_APPS_META_KEY)?
        .as_object()?
    ...
    Some(AtlasAppsConnectorAuthFailure {
...
pub fn build_auth_elicitation_plan(
    ...
    connector_name: Option<&str>,
    install_url: Option<String>,
) -> Option<AtlasAppsAuthElicitationPlan> {
    let auth_failure =
    ...
    Some(AtlasAppsAuthElicitationPlan {
        auth_failure,
        elicitation,
    })
}
...
pub fn build_auth_elicitation(
    call_id: &str,
    auth_failure: &AtlasAppsConnectorAuthFailure,
) -> AtlasAppsAuthElicitation {
    AtlasAppsAuthElicitation {
        meta: serde_json::json!({
            MCP_TOOL_ATLAS_APPS_META_KEY: {
                CONNECTOR_AUTH_FAILURE_META_KEY: AtlasAppsConnectorAuthFailureMeta {
...
pub fn auth_elicitation_id(call_id: &str) -> String {
    format!("atlas_apps_auth_{call_id}")
}
...
fn auth_elicitation_message(auth_failure: &AtlasAppsConnectorAuthFailure) -> String {
    match auth_failure.auth_reason.as_deref() {
        ...
        Some("missing_link") => format!(
            "Sign in to {} on ChatGPT to use it in Atlas Agent.",
            auth_failure.connector_name
        ),
        _ => format!(
...
mod tests {
    ...
    fn builds_url_elicitation_payload() {
        ...
        assert_eq!(
            ...
            AtlasAppsAuthElicitation {
                ...
                elicitation_id: "atlas_apps_auth_call_123".to_string(),
            }
        );
    }
    ...
    fn builds_auth_elicitation_plan() {
        ...
        assert_eq!(plan.elicitation.elicitation_id, "atlas_apps_auth_call_123");
    }
}
