...
use atlas_engine_rmcp_client::StoredOAuthCredentialSnapshot;
...
pub(crate) struct McpServerConnectionIdentity {
    ...
    runtime_auth: Option<AtlasEngineAuth>,
    runtime_auth_token: Option<String>,
    atlas_apps_cache_identity: Option<(PathBuf, ConnectorRuntimeContextKey)>,
...
impl McpServerConnectionIdentity {
    ...
    pub(crate) fn has_same_connection_config(&self, other: &Self) -> bool {
        let same_runtime_auth = match (&self.runtime_auth, &other.runtime_auth) {
            (Some(AtlasEngineAuth::AgentIdentity(left)), Some(AtlasEngineAuth::AgentIdentity(right))) => {
        ...
        self.transport == other.transport
            ...
            && self.atlas_apps_cache_identity == other.atlas_apps_cache_identity
