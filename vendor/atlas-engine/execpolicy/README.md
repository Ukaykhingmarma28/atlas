...
- From the Atlas Agent CLI, run `atlas_engine execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
atlas-agent execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
atlas-agent execpolicy check \
...
```bash
cargo run -p atlas-engine-execpolicy -- check --rules path/to/policy.rules git status
```
