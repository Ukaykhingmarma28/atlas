...
/// removes legacy allow rules that newer atlas-agent versions no longer offer.
///
/// this migration is intentionally one-shot. once complete, a marker in `atlas_agent_home` prevents
/// policies saved by newer atlas-agent versions from being removed on later startups.
pub async fn prefix_rule_migration(
    atlas_agent_home: &Path,
    policy_path: &Path,
    banned_prefixes: &[&[&str]],
) -> io::Result<()> {
    let marker_path = atlas_agent_home.join(MIGRATION_MARKER_FILENAME);
    if tokio::fs::try_exists(&marker_path).await? {
        return Ok(());
    }
    clean_rules_file(policy_path, banned_prefixes).await?;

    write_migration_marker(atlas_agent_home, &marker_path).await?;
    Ok(())
}

// atomically writes the marker after creating atlas-agent home when needed.
async fn write_migration_marker(atlas_agent_home: &Path, marker_path: &Path) -> io::Result<()> {
    tokio::fs::create_dir_all(atlas_agent_home).await?;
    let atlas_agent_home = atlas_agent_home.to_owned();
    let marker_path = marker_path.to_owned();
    tokio::task::spawn_blocking(move || {
        let mut marker = tempfile::NamedTempFile::new_in(atlas_agent_home)?;
