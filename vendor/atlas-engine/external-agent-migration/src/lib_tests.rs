...
fn migrated_quoted_hook_command(script_name: &str) -> String {
    let hook_path = Path::new("/repo/.atlas-agent")
...
#[test]
fn subagent_requires_minimum_atlas_engine_agent_fields() {
...
fn subagent_target_preserves_dotted_file_stem() {
    let target_agents = Path::new("/repo/.atlas-agent/agents");
...
fn hook_script_copy_keeps_existing_target_scripts() {
    ...
    let target_config_dir = root.path().join(".atlas-agent");
