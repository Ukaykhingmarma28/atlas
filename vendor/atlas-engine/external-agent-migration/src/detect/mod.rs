...
const EXTERNAL_AGENT_CONFIG_DETECT_METRIC: &str = "atlas_agent.external_agent_config.detect";
...
impl ExternalAgentConfigService {
    pub async fn detect(
        ...
        if params.include_home
            ...
            && let Some(item) = memory::detect(&self.atlas_agent_home, &self.external_agent_home)?
        {
            items.push(item);
    ...
    async fn detect_migrations(
        ...
        let target_config = repo_root.map_or_else(
            || self.atlas_agent_home.join("config.toml"),
            |repo_root| repo_root.join(".atlas-agent").join("config.toml"),
        );
        ...
        let target_hooks = repo_root.map_or_else(
            || self.atlas_agent_home.join("hooks.json"),
            |repo_root| repo_root.join(".atlas-agent").join("hooks.json"),
        );
        ...
        let target_subagents = repo_root.map_or_else(
            ...
            |repo_root| repo_root.join(".atlas-agent").join("agents"),
        );
        ...
        let instruction_source_groups = if let Some(repo_root) = repo_root {
            ...
            (!sources.is_empty())
                .then(|| InstructionSourceGroup {
                    scope: self.atlas_agent_home.clone(),
                    sources,
                })
                .into_iter()
                .collect()
