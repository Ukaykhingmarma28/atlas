...
use atlas_engine_core_plugins::marketplace::MarketplacePluginInstallPolicy;
...
const EXTERNAL_AGENT_CONFIG_IMPORT_METRIC: &str = "atlas_agent.external_agent_config.import";

#[derive(Clone)]
pub struct ExternalAgentConfigService {
    pub(super) atlas_agent_home: PathBuf,
...
impl ExternalAgentConfigService {
    ...
    pub fn with_migration_source(&self, migration_source: Option<&str>) -> Self {
        ...
        let connector_metadata_roots = source.connector_metadata_roots(&external_agent_home);
        Self {
            atlas_agent_home: self.atlas_agent_home.clone(),
            connector_metadata_roots,
            external_agent_home,
    ...
    pub fn atlas_agent_home(&self) -> &Path {
        &self.atlas_agent_home
    }

    #[cfg(test)]
    fn new_for_test(atlas_agent_home: PathBuf, external_agent_home: PathBuf) -> Self {
        let source = ExternalAgentSource::default();
        let connector_metadata_roots = source.connector_metadata_roots(&external_agent_home);
        Self {
            atlas_agent_home,
            ...
            let import_result = match migration_item.item_type {
                ...
                ExternalAgentConfigMigrationItemType::Memory if self.source.supports_memory() => {
                    async {
                        ...
                        let target_path = memory_import::resources_root(&self.atlas_agent_home);
    ...
    fn import_config(&self, cwd: Option<&Path>) -> io::Result<Option<(String, String)>> {
        ...
        let target_config = match &scope {
            MigrationScope::Home => self.atlas_agent_home.join("config.toml"),
            MigrationScope::Repository { root } => root.join(".atlas-agent").join("config.toml"),
        };
    ...
    fn import_hooks(&self, cwd: Option<&Path>) -> io::Result<Vec<String>> {
        ...
        let target_hooks = match &scope {
            MigrationScope::Home => self.atlas_agent_home.join("hooks.json"),
            MigrationScope::Repository { root } => root.join(".atlas-agent").join("hooks.json"),
        };
    ...
    fn import_agents_md(&self, cwd: Option<&Path>) -> io::Result<Option<(String, String)>> {
        ...
        let (source_agents_md, target_agents_md) = match scope {
            ...
            MigrationScope::Home => {
                ...
                (source_agents_md, self.atlas_agent_home.join("AGENTS.md"))
            }
        };
