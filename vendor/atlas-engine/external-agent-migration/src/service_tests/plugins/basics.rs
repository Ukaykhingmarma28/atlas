...
async fn authenticated_plugin_migration_uses_chatgpt_curated_marketplace() {
    ...
    .expect("write Atlas Agent config");
    fs::write(
  ...
  "plugins": [{
    ...
    let service = service_for_paths(external_agent_home.clone(), atlas_agent_home)
...
fn marketplace_import_sources_prefers_scoped_source_over_registry_name_collision() {
    let (root, external_agent_home, _atlas_agent_home) = fixture_paths();
...
fn marketplace_import_sources_prefers_supported_declaration_over_materialization() {
    let (_root, external_agent_home, _atlas_agent_home) = fixture_paths();
...
#[tokio::test]
async fn detect_repo_skips_plugins_that_are_already_configured_in_atlas_engine() {
...
"#,
    ...
    let items = service_for_paths(external_agent_home, atlas_agent_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: false,
            include_memory: false,
...
#[tokio::test]
async fn detect_repo_skips_plugins_that_are_disabled_in_atlas_engine() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let atlas_agent_home = root.path().join(".atlas-agent");
    let repo_root = root.path().join("repo");
...
#[tokio::test]
async fn detect_repo_skips_plugins_without_explicit_enabled_in_atlas_engine() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let atlas_agent_home = root.path().join(".atlas-agent");
    let repo_root = root.path().join("repo");
...
async fn import_plugins_requires_details() {
    ...
    let err = service_for_paths(external_agent_home, atlas_agent_home)
...
#[tokio::test]
async fn detect_repo_does_not_skip_plugins_only_configured_in_project_atlas_engine() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let atlas_agent_home = root.path().join(".atlas-agent");
    let repo_root = root.path().join("repo");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create repo external agent dir");
    fs::create_dir_all(repo_root.join(".atlas-agent")).expect("create repo atlas-agent dir");
    fs::create_dir_all(&atlas_agent_home).expect("create atlas-agent home");
    fs::write(
        repo_root.join(EXTERNAL_AGENT_DIR).join("settings.json"),
        r#"{
    ...
    .expect("write repo settings");
    fs::write(
        repo_root.join(".atlas-agent").join("config.toml"),
        r#"
[plugins."formatter@acme-tools"]
enabled = true
"#,
    )
    .expect("write project atlas-agent config");
