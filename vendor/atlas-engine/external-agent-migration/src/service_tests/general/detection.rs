...
async fn detect_home_lists_config_skills_and_agents_md() {
    let (_root, external_agent_home, atlas_agent_home) = fixture_paths();
    let agents_skills = atlas_agent_home
        .parent()
    ...
    let items = service_for_paths(external_agent_home.clone(), atlas_agent_home.clone())
    ...
    let expected = vec![
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                atlas_agent_home.join("config.toml").display()
            ),
            cwd: None,
            details: None,
        },
        ...
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                atlas_agent_home.join("AGENTS.md").display()
            ),
            cwd: None,
            details: None,
        },
...
async fn detect_cursor_home_lists_user_and_managed_skills() {
    ...
    let target_skills = atlas_agent_home
        .parent()
        .map(|parent| parent.join(".agents").join("skills"))
        .unwrap_or_else(|| PathBuf::from(".agents").join("skills"));
    ...
    let mut service = service_for_paths(external_agent_home, atlas_agent_home);
...
async fn detect_cursor_repo_lists_skills_from_skills() {
    ...
    let mut service = service_for_paths(root.path().join(".cursor"), atlas_agent_home);
    service.source = ExternalAgentSource::Cur;
...
async fn detect_home_lists_recent_sessions() {
    let (root, external_agent_home, atlas_agent_home) = fixture_paths();
    ...
    let items = service_for_paths(external_agent_home.clone(), atlas_agent_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: true,
            include_memory: false,
            cwds: None,
...
async fn detect_repo_still_reports_non_plugin_items_when_home_config_is_invalid() {
    ...
    let items = service_for_paths(root.path().join(EXTERNAL_AGENT_DIR), atlas_agent_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: false,
            include_memory: false,
            cwds: Some(vec![repo_root.clone()]),
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root
                        ...
                        .join("settings.json")
                        .display(),
                    repo_root.join(".atlas-agent").join("config.toml").display()
                ),
                cwd: Some(repo_root.clone()),
                details: None,
            },
        ...
        vec![
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root.join(".atlas-agent").join("hooks.json").display()
                ),
                cwd: Some(repo_root.clone()),
                details: Some(MigrationDetails {
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root.join(".atlas-agent").join("agents").display()
                ),
                cwd: Some(repo_root),
                details: Some(MigrationDetails {
...
"#,
    ...
    let _supported_mcp_config: std::collections::HashMap<
        String,
        atlas_engine_config::types::McpServerConfig,
    > = mcp_servers
        .try_into()
        .expect("migrated MCP config should be supported");

    let hooks: JsonValue = serde_json::from_str(
        &fs::read_to_string(repo_root.join(".atlas-agent").join("hooks.json")).expect("read hooks"),
    )
    .expect("parse hooks");
    let _supported_hooks: atlas_engine_config::HooksFile =
...
Research with AtlasEngine carefully."""
"#,
    )
