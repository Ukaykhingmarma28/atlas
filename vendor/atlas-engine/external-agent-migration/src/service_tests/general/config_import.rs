...
async fn import_repo_mcp_preserves_existing_same_named_server() {
    ...
    .expect("write mcp");
    fs::create_dir_all(repo_root.join(".atlas-agent")).expect("create atlas-agent dir");
...
async fn import_home_migrates_supported_config_fields_skills_and_agents_md() {
    ...
    fs::write(
            ...
            format!(r#"{{"model":"{SOURCE_EXTERNAL_AGENT_NAME}","permissions":{{"ask":["git push"]}},"env":{{"FOO":"bar","CI":false,"MAX_RETRIES":3,"MY_TEAM":"atlas_engine","IGNORED":null,"LIST":["a","b"],"MAP":{{"x":1}}}},"sandbox":{{"enabled":true,"network":{{"allowLocalBinding":true}}}}}}"#),
        )
    ...
    service_for_paths(external_agent_home, atlas_agent_home.clone())
        .import(vec![
    ...
    assert_eq!(
        fs::read_to_string(atlas_agent_home.join("AGENTS.md")).expect("read agents"),
...
"#,
    ...
    assert_eq!(
        ...
        "Use Atlas Agent and Atlas Agent utilities."
    );
}
...
async fn import_home_skips_empty_config_migration() {
    ...
    let outcome = service_for_paths(external_agent_home, atlas_agent_home.clone())
        .import(vec![ExternalAgentConfigMigrationItem {
            item_type: ExternalAgentConfigMigrationItemType::Config,
            description: String::new(),
            cwd: None,
    ...
    assert!(!atlas_agent_home.join("config.toml").exists());
}
...
async fn import_local_plugins_returns_completed_status() {
    ...
    fs::write(
        plugin_root.join(".atlas-agent-plugin").join("plugin.json"),
    ...
    let config = fs::read_to_string(atlas_agent_home.join("config.toml")).expect("read config");
