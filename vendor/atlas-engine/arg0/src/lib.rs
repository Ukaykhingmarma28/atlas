...
use atlas_engine_apply_patch::ATLAS_AGENT_CORE_APPLY_PATCH_ARG1;
#[cfg(unix)]
use atlas_engine_exec_server::ATLAS_AGENT_ARG0_EXEC_HELPER_ARG1;
use atlas_engine_exec_server::ATLAS_AGENT_FS_HELPER_ARG1;
...
use atlas_engine_windows_sandbox::ATLAS_AGENT_WINDOWS_SANDBOX_ARG1;
#[cfg(unix)]
...
#[cfg(unix)]
const EXECVE_WRAPPER_ARG0: &str = "atlas-engine-execve-wrapper";
const LOCK_FILENAME: &str = ".lock";
const TOKIO_WORKER_STACK_SIZE_BYTES: usize = 16 * 1024 * 1024;

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct Arg0DispatchPaths {
    /// Stable path to the current Atlas Agent executable for child re-execs.
    ///
    ...
    /// instead of the real Atlas Agent CLI.
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == EXECVE_WRAPPER_ARG0 {
        ...
        let exit_code = runtime.block_on(
            atlas_engine_shell_escalation::run_shell_escalation_execve_wrapper(file, argv),
        );
        match exit_code {
    ...
    if exe_name == ATLAS_AGENT_LINUX_SANDBOX_ARG0 {
        ...
        atlas_engine_apply_patch::main();
    }

    let argv1 = args.next().unwrap_or_default();
    #[cfg(unix)]
    if argv1 == ATLAS_AGENT_ARG0_EXEC_HELPER_ARG1 {
        atlas_engine_exec_server::run_arg0_exec_helper_main();
    }
    if argv1 == ATLAS_AGENT_FS_HELPER_ARG1 {
        atlas_engine_exec_server::run_fs_helper_main();
    }
    #[cfg(target_os = "windows")]
    if argv1 == ATLAS_AGENT_WINDOWS_SANDBOX_ARG1 {
        atlas_engine_windows_sandbox::run_windows_sandbox_wrapper_main();
    }
    if argv1 == ATLAS_AGENT_CORE_APPLY_PATCH_ARG1 {
        let patch_arg = args.next().and_then(|s| s.to_str().map(str::to_owned));
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                let mut stdout = std::io::stdout();
                let mut stderr = std::io::stderr();
                let cwd = match atlas_engine_utils_absolute_path::AbsolutePathBuf::current_dir() {
                    Ok(cwd) => cwd,
                    Err(_) => std::process::exit(1),
                };
                let runtime = match tokio::runtime::Builder::new_current_thread()
                ...
                match runtime.block_on(atlas_engine_apply_patch::apply_patch_with_mode(
                    ...
                    &mut stdout,
                    &mut stderr,
                    atlas_engine_exec_server::LOCAL_FS.as_ref(),
                    /*sandbox*/ None,
                )) {
                    Ok(_) => 0,
                    Err(_) => 1,
    ...
    let (path_entry_guard, updated_path_env_var) = prepare_path_env_var_with_aliases(
        ...
        std::env::var_os("PATH"),
        prepare_path_entry_for_atlas_engine_aliases,
    );
...
fn prepare_path_env_var_with_aliases(
    ...
    match prepare_aliases(path_for_aliases) {
        ...
        Err(err) => {
            // It is possible that Atlas Agent will proceed successfully even if
...
/// While we want to deploy the Atlas Agent CLI as a single executable for simplicity,
...
/// `atlas-engine-linux-sandbox` we *directly* execute
/// [`atlas_engine_linux_sandbox::run_main`] (which never returns). Otherwise we:
///
/// 1.  Load `.env` values from `~/.atlas_engine/.env` before creating any threads.
...
///     `atlas-engine-linux-sandbox` helper path (falling back to the current
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    ...
    let handle = std::thread::Builder::new()
        .name("atlas-engine-main".to_string())
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    let paths = Arg0DispatchPaths {
        atlas_engine_self_exe: current_exe.clone(),
        atlas_engine_linux_sandbox_exe: if cfg!(target_os = "linux") {
...
fn linux_sandbox_exe_path(
    ...
    // Prefer the `atlas-engine-linux-sandbox` alias when available so callers can
    // re-exec through a path whose basename still triggers arg0 dispatch on
    // bubblewrap builds that do not support `--argv0`.
    path_entry_guard
        .and_then(|path_entry| path_entry.paths().atlas_engine_linux_sandbox_exe.clone())
        .or(current_exe)
}
...
const ILLEGAL_ENV_VAR_PREFIX: &str = "ATLAS_AGENT_";

/// Load env vars from ~/.atlas-agent/.env.
///
/// Security: Do not allow `.env` files to create or modify any variables
/// with names starting with `ATLAS_AGENT_`.
fn load_dotenv() {
    if let Ok(atlas_agent_home) = find_atlas_agent_home()
        && let Ok(iter) = dotenvy::from_path_iter(atlas_agent_home.join(".env"))
    {
        set_filtered(iter);
    }
}

/// Helper to set vars from a dotenvy iterator while filtering out `ATLAS_AGENT_` keys.
...
///   with the hidden `--atlas-engine-run-as-apply-patch` flag.
///
...
/// user to install a separate executable, simplifying the deployment of Atlas Agent
/// CLI.
...
fn prepare_path_entry_for_atlas_engine_aliases(
    existing_path: Option<OsString>,
    ...
    #[cfg(not(debug_assertions))]
    {
        ...
        if atlas_agent_home.starts_with(&temp_root) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_root:?} (atlas_agent_home: {atlas_agent_home:?})"
                ),
            ));
        }
    }
    ...
    // Use a ATLAS_AGENT_HOME-scoped temp root to avoid cluttering the top-level directory.
    let temp_root = atlas_agent_home.join("tmp").join("arg0");
    ...
    for filename in &[
        ...
        #[cfg(target_os = "linux")]
        ATLAS_AGENT_LINUX_SANDBOX_ARG0,
        #[cfg(unix)]
        EXECVE_WRAPPER_ARG0,
    ] {
        ...
        #[cfg(windows)]
        {
            ...
            std::fs::write(
                ...
                format!(
                    r#"@echo off
"{exe}" {ATLAS_AGENT_CORE_APPLY_PATCH_ARG1} %*
"#,
                ),
            )?;
        }
    ...
    let paths = Arg0DispatchPaths {
        atlas_engine_self_exe: std::env::current_exe().ok(),
        atlas_engine_linux_sandbox_exe: {
            #[cfg(target_os = "linux")]
            {
                Some(path.join(ATLAS_AGENT_LINUX_SANDBOX_ARG0))
            }
...
mod tests {
    ...
    #[cfg(unix)]
    use anyhow::ensure;
    use atlas_engine_install_context::AtlasEnginePackageLayout;
    use atlas_engine_install_context::InstallContext;
    use atlas_engine_install_context::InstallMethod;
    ...
    fn package_path_test_fixture() -> anyhow::Result<PackagePathTestFixture> {
        ...
        let path_dir = package_dir.join("atlas-engine-path");
        ...
        let install_context = InstallContext {
            ...
            package_layout: Some(AtlasEnginePackageLayout {
    ...
    #[test]
    fn linux_sandbox_exe_path_prefers_atlas_engine_linux_sandbox_alias() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let lock_file = create_lock(temp_dir.path())?;
        let alias_path = temp_dir.path().join("atlas-engine-linux-sandbox");
        let path_entry = Arg0PathEntryGuard::new(
            temp_dir,
            lock_file,
            Arg0DispatchPaths {
                atlas_engine_self_exe: Some(PathBuf::from("/usr/bin/atlas-agent")),
                atlas_engine_linux_sandbox_exe: Some(alias_path.clone()),
    ...
    fn run_main_with_arg0_guard_keeps_aliases_alive_until_main_returns() -> anyhow::Result<()> {
        let temp_dir = TempDir::new()?;
        let alias_path = temp_dir.path().join("atlas-engine-helper-alias");
