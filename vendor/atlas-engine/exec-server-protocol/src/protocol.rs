...
impl EnvironmentInfo {
    ...
    pub fn local() -> Self {
        ...
        Self {
            shell: atlas_engine_shell_command::shell_detect::default_user_shell().into(),
...
pub struct ExecParams {
    ...
    #[serde(default)]
    pub pipe_stdin: bool,
    /// Optional process-visible argv0 override. Values such as `atlas-engine-linux-sandbox` are command
...
mod tests {
    ...
    fn local_environment_info_resolves_relative_temporary_directory() {
        if std::env::var_os("ATLAS_AGENT_TEST_RELATIVE_TMPDIR").is_none() {
            let status = std::process::Command::new(std::env::current_exe().expect("test binary"))
                .arg("--exact")
                .arg(
                    "protocol::tests::local_environment_info_resolves_relative_temporary_directory",
                )
                .env("ATLAS_AGENT_TEST_RELATIVE_TMPDIR", "1")
