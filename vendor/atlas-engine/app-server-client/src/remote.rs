...
use atlas_engine_uds::UnixStream;
...
pub struct RemoteAppServerClient {
    ...
    atlas_agent_home: Option<String>,
...
impl RemoteAppServerClient {
    ...
    pub fn atlas_agent_home(&self) -> Option<&str> {
        self.atlas_agent_home.as_deref()
    }
        ...
        S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
    {
        ...
        let (pending_events, server_version, atlas_agent_home) = initialize_remote_connection(
            &mut stream,
            &endpoint,
            initialize_params,
            INITIALIZE_TIMEOUT,
    ...
    pub async fn shutdown(self) -> IoResult<()> {
        let Self {
            ...
            atlas_agent_home: _atlas_agent_home,
            worker_handle,
        } = self;
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
                ...
                Some(Ok(Message::Text(text))) => {
                    ...
                    match message {
                        JSONRPCMessage::Response(response) if response.id == initialize_request_id => {
                            ...
                            atlas_agent_home = response
                                .result
                                .get("atlasAgentHome")
                                .and_then(serde_json::Value::as_str)
                                .filter(|atlas_agent_home| !atlas_agent_home.is_empty())
                                .map(str::to_string);
                            break Ok(());
                        }
    ...
    Ok((pending_events, server_version, atlas_agent_home))
}
