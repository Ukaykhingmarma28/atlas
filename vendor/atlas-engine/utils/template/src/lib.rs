...
mod tests {
    ...
    fn render_replaces_placeholders_with_and_without_whitespace() {
        let rendered = render(
            ...
            [("name", "Atlas Agent"), ("place", "atlas-engine-rs")],
        )
        .unwrap();

        assert_eq!(
            rendered,
            "Hello, Atlas Agent. You are in atlas-engine-rs. Atlas Agent is repeated."
        );
    }
    ...
    fn parsed_templates_can_be_reused() {
        ...
        assert_eq!(
            template.render([("greeting", "Hello"), ("name", "Atlas Agent")]),
            Ok("Hello, Atlas Agent!".to_string())
        );
        assert_eq!(
    ...
    fn render_supports_literal_delimiter_escapes() {
        ...
        assert_eq!(
            rendered,
            "literal open: {{, literal close: }}, value: Atlas Agent"
        );
    }
    ...
    fn render_errors_when_extra_value_is_provided() {
        ...
        assert_eq!(
            template.render([("name", "Atlas Agent"), ("unused", "extra")]),
    ...
    fn render_errors_when_duplicate_value_is_provided() {
        ...
        assert_eq!(
            template.render([("name", "Atlas Agent"), ("name", "other")]),
    ...
    fn render_function_wraps_parse_errors() {
        let err = render("Hello, }} world.", [("name", "Atlas Agent")]).unwrap_err();
    ...
    fn render_function_wraps_render_errors() {
        let err = render("Hello, {{ name }}.", [("extra", "Atlas Agent")]).unwrap_err();
