...
> **Privacy:** Rollout tracing is not telemetry. Atlas Agent does **not** upload or
> report these traces; it writes local bundles only when
> `ATLAS_AGENT_ROLLOUT_TRACE_ROOT` is set. Those local bundles can contain prompts,
...
during a Atlas Agent session. It records raw runtime evidence into a local bundle on
...
Hot-path Atlas Agent code does not try to build the final graph while the session is
...
model-visible conversation plus the runtime objects that explain how Atlas Agent got
there.
...
```mermaid
flowchart TD
    subgraph Runtime["atlas-engine-core runtime"]
...
one from `ATLAS_AGENT_ROLLOUT_TRACE_ROOT`; fresh spawned child threads derive their
...
Atlas Agent session fail just because diagnostic recording failed. Core emits raw
...
- `state.json`: optional reducer output written by `atlas_engine debug trace-reduce`.

`trace_id` identifies this diagnostic artifact. `rollout_id` identifies the
Atlas Agent rollout/session being observed. Keeping those separate lets us reason about
the stored trace without confusing it with the product-level session identity.

To reduce a bundle:

```bash
atlas-agent debug trace-reduce <trace-bundle>
```

By default this writes `<trace-bundle>/state.json`. Rust callers can also call
`atlas_engine_rollout_trace::replay_bundle` directly.
