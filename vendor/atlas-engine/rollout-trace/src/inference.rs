...
const INFERENCE_CALL_ID_HEADER: &str = "x-atlas-engine-inference-call-id";
...
///
/// A Atlas Agent turn can create multiple attempts when auth recovery retries the
...
impl InferenceTraceContext {
    ...
    /// Builds an enabled context for all upstream attempts made by one Atlas Agent turn.
    pub fn enabled(
        writer: Arc<TraceWriter>,
        thread_id: AgentThreadId,
        atlas_engine_turn_id: AtlasEngineTurnId,
...
impl InferenceTraceAttempt {
    ...
    pub fn record_started(&self, request: &impl Serialize) {
        ...
        append_with_context_best_effort(
            ...
            RawTraceEventPayload::InferenceStarted {
                ...
                atlas_engine_turn_id: attempt.context.atlas_engine_turn_id.clone(),
    ...
    /// trace-specific serialization rules. That keeps atlas-engine-core focused on
    ...
    /// Records a provider stream that Atlas Agent intentionally stopped consuming.
    ///
...
/// Atlas Agent received it, so this helper restores that content in the raw payload.
...
fn append_with_context_best_effort(
    ...
    let event_context = RawTraceEventContext {
        ...
        atlas_engine_turn_id: Some(context.atlas_engine_turn_id.clone()),
    };
...
mod tests {
    ...
    fn enabled_context_records_replayable_inference_attempt() -> anyhow::Result<()> {
        ...
        assert_eq!(inference.atlas_engine_turn_id, "turn-1");
