...
//! Mapping from Atlas Agent protocol events into raw rollout-trace events.
//!
...
//! `atlas-engine-core`: this module translates the protocol surface into the smaller
//! trace vocabulary and keeps the mapping isolated inside `atlas-engine-rollout-trace`.
//!
...
use atlas_engine_protocol::protocol::ExecCommandStatus;
...
use atlas_engine_protocol::protocol::PatchApplyStatus;
use atlas_engine_protocol::protocol::SubAgentActivityEvent;
...
pub(crate) struct AtlasEngineTurnTraceEvent {
    pub context_turn_id: AtlasEngineTurnId,
    pub payload: RawTraceEventPayload,
}

pub(crate) fn atlas_engine_turn_trace_event(
    thread_id: AgentThreadId,
    default_turn_id: &str,
    event: &EventMsg,
) -> Option<AtlasEngineTurnTraceEvent> {
    match event {
        EventMsg::TurnStarted(event) => {
            let atlas_engine_turn_id = event.turn_id.clone();
            Some(AtlasEngineTurnTraceEvent {
                context_turn_id: atlas_engine_turn_id.clone(),
                payload: RawTraceEventPayload::AtlasEngineTurnStarted {
                    atlas_engine_turn_id,
                    thread_id,
                },
            })
        }
        EventMsg::TurnComplete(event) => {
            ...
            Some(AtlasEngineTurnTraceEvent {
                context_turn_id: atlas_engine_turn_id.clone(),
                payload: RawTraceEventPayload::AtlasEngineTurnEnded {
                    atlas_engine_turn_id,
        ...
        EventMsg::TurnAborted(event) => {
            let atlas_engine_turn_id = event
                .turn_id
                .clone()
...
pub(crate) enum ToolRuntimePayload<'a> {
    ...
    CollabCloseBegin(&'a atlas_engine_protocol::protocol::CollabCloseBeginEvent),
    CollabCloseEnd(&'a atlas_engine_protocol::protocol::CollabCloseEndEvent),
...
struct ExecCommandBeginTracePayload<'a> {
    ...
    command: &'a [String],
    cwd: String,
    parsed_cmd: &'a [atlas_engine_protocol::parse_command::ParsedCommand],
