...
use crate::protocol_event::atlas_engine_turn_trace_event;
...
pub const ATLAS_AGENT_ROLLOUT_TRACE_ROOT_ENV: &str = "ATLAS_AGENT_ROLLOUT_TRACE_ROOT";
...
impl ThreadTraceContext {
    ...
    /// Starts a root thread trace from `ATLAS_AGENT_ROLLOUT_TRACE_ROOT`, or disables tracing.
    ///
    /// Trace startup is best-effort. A tracing failure must not make the Atlas Agent
    ...
    pub fn start_root_or_disabled(metadata: ThreadStartedTraceMetadata) -> Self {
        let Some(root) = std::env::var_os(ATLAS_AGENT_ROLLOUT_TRACE_ROOT_ENV) else {
    ...
    /// Emits typed Atlas Agent turn lifecycle events from protocol lifecycle events.
    pub fn record_atlas_engine_turn_event(&self, default_turn_id: &str, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let Some(trace_event) =
            atlas_engine_turn_trace_event(context.thread_id.clone(), default_turn_id, event)
        else {
            return;
        };
    ...
    pub fn record_tool_call_event(&self, atlas_engine_turn_id: impl Into<AtlasEngineTurnId>, event: &EventMsg) {
        ...
        context.append_with_context_best_effort(atlas_engine_turn_id.into(), payload);
    }
    ...
    pub fn record_agent_result_interaction(
        &self,
        child_atlas_engine_turn_id: impl Into<AtlasEngineTurnId>,
    ...
    pub fn record_atlas_engine_turn_started(&self, atlas_engine_turn_id: impl Into<AtlasEngineTurnId>) {
        ...
        let atlas_engine_turn_id = atlas_engine_turn_id.into();
        context.append_with_context_best_effort(
            atlas_engine_turn_id.clone(),
            RawTraceEventPayload::AtlasEngineTurnStarted {
    ...
    pub fn start_code_cell_trace(
        ...
        let context = self.code_cell_trace_context(atlas_engine_turn_id, runtime_cell_id);
    ...
    /// Builds reusable inference trace context for one Atlas Agent turn.
    ///
...
impl EnabledThreadTraceContext {
    ...
    fn append_with_context_best_effort(
        ...
        let event_context = RawTraceEventContext {
            ...
            atlas_engine_turn_id: Some(atlas_engine_turn_id),
        };
