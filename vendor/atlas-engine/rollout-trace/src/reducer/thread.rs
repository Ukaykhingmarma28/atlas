...
use crate::model::AtlasEngineTurn;
...
impl TraceReducer {
    ...
    /// Starts a Atlas Agent turn inside an existing thread.
    pub(super) fn start_atlas_engine_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        self.rollout.atlas_engine_turns.insert(
    ...
    /// Marks a Atlas Agent turn terminal and validates any thread id carried by the raw event.
    pub(super) fn end_atlas_engine_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if let Some(event_thread_id) = thread_id.as_deref()
            && let Some(turn) = self.rollout.atlas_engine_turns.get(&atlas_engine_turn_id)
            && turn.thread_id != event_thread_id
        {
            bail!(
                "atlas-agent turn end for {atlas_engine_turn_id} used thread {event_thread_id}, \
