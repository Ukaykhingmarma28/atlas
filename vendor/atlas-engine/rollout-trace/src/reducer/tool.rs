...
impl TraceReducer {
    ...
    pub(super) fn start_tool_call(
        ...
        atlas_engine_turn_id: Option<String>,
        ...
        let thread_id = self.tool_thread_id(thread_id, atlas_engine_turn_id.as_deref())?;
        self.validate_tool_turn(&thread_id, atlas_engine_turn_id.as_deref())?;
        ...
        self.rollout.tool_calls.insert(
            ...
            ToolCall {
                ...
                thread_id,
                started_by_atlas_engine_turn_id: atlas_engine_turn_id,
    ...
    fn tool_thread_id(
        ...
        let Some(atlas_engine_turn_id) = atlas_engine_turn_id else {
            bail!("tool call start did not include thread or Atlas Agent turn context");
        };
        self.rollout
            .atlas_engine_turns
    ...
    fn validate_tool_turn(&self, thread_id: &str, atlas_engine_turn_id: Option<&str>) -> Result<()> {
        ...
        if let Some(atlas_engine_turn_id) = atlas_engine_turn_id {
            let Some(turn) = self.rollout.atlas_engine_turns.get(atlas_engine_turn_id) else {
                bail!("tool call start referenced unknown Atlas Agent turn {atlas_engine_turn_id}");
            };
            if turn.thread_id != thread_id {
                bail!(
                    "tool call start used thread {thread_id}, but Atlas Agent turn {atlas_engine_turn_id} \
                     belongs to {}",
                    turn.thread_id
                );
            }
