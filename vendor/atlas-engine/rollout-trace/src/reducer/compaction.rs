...
impl TraceReducer {
    ...
    pub(super) fn start_compaction_request(
        ...
        let Some(turn) = self.rollout.atlas_engine_turns.get(&started.atlas_engine_turn_id) else {
            bail!(
                ...
                started.compaction_request_id,
                started.atlas_engine_turn_id
            );
        };
        if turn.thread_id != started.thread_id {
            bail!(
                "compaction request {} used thread {}, but atlas-agent turn {} belongs to {}",
                started.compaction_request_id,
                started.thread_id,
                started.atlas_engine_turn_id,
                turn.thread_id
        ...
        self.rollout.compaction_requests.insert(
            ...
            CompactionRequest {
                ...
                atlas_engine_turn_id: started.atlas_engine_turn_id,
    ...
    pub(super) fn reduce_compaction_installed_event(
        ...
        let Some(turn) = self.rollout.atlas_engine_turns.get(&atlas_engine_turn_id) else {
            bail!(
                "compaction install {compaction_id} referenced unknown atlas-agent turn {atlas_engine_turn_id}"
            );
        };
        if turn.thread_id != thread_id {
            bail!(
                "compaction install {compaction_id} used thread {thread_id}, but atlas-agent turn {atlas_engine_turn_id} belongs to {}",
                turn.thread_id
            );
        }
...
pub(super) struct StartedCompactionRequest {
    ...
    pub(super) atlas_engine_turn_id: String,
