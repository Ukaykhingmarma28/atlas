...
impl TraceReducer {
    ...
    pub(super) fn start_inference_call(
        ...
        let atlas_engine_turn_id = started.atlas_engine_turn_id.clone();
        ...
        let Some(turn) = self.rollout.atlas_engine_turns.get(&atlas_engine_turn_id) else {
            bail!(
                "inference start {inference_call_id} referenced unknown atlas-agent turn {atlas_engine_turn_id}"
            );
        };
        if turn.thread_id != thread_id {
            bail!(
                "inference start {inference_call_id} used thread {thread_id}, \
                 but atlas_engine turn {atlas_engine_turn_id} belongs to {}",
                turn.thread_id
            );
        }
    ...
    /// If a call is still `Running`, Atlas Agent stopped observing that provider stream
    ...
    pub(super) fn close_running_inference_calls_for_turn_end(
        ...
        for inference in self.rollout.inference_calls.values_mut() {
            if inference.atlas_engine_turn_id == atlas_engine_turn_id
