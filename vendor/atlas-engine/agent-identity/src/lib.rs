...
use atlas_engine_protocol::auth::PlanType as AuthPlanType;
...
const AGENT_IDENTITY_JWT_AUDIENCE: &str = "atlas-engine-app-server";
...
const AGENT_IDENTITY_KEY_DERIVATION_CONTEXT: &[u8] = b"atlas-engine-agent-identity-ed25519-v1";
...
/// single Atlas Agent run, while the agent runtime id and private key are the
...
pub fn build_abom(session_source: SessionSource) -> AgentBillOfMaterials {
    AgentBillOfMaterials {
        ...
        agent_harness_id: match &session_source {
            SessionSource::VSCode => "atlas-engine-app".to_string(),
            SessionSource::Cli
            | SessionSource::Exec
            | SessionSource::Mcp
            ...
            | SessionSource::Unknown => "atlas-engine-cli".to_string(),
        },
