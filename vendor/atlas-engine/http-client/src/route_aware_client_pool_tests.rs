...
async fn legacy_custom_ca_fallback_is_limited_to_reqwest_default() {
    const CHILD_POLICY_ENV: &str = "ATLAS_AGENT_HTTP_CLIENT_POOL_INVALID_CA_TEST_POLICY";
    ...
    let Ok(policy_name) = std::env::var(CHILD_POLICY_ENV) else {
        ...
        for ca_env in ["ATLAS_AGENT_CA_CERTIFICATE", "SSL_CERT_FILE"] {
            for policy_name in ["reqwest-default", "respect-system-proxy"] {
                ...
                .arg("--nocapture")
                .env_remove("ATLAS_AGENT_CA_CERTIFICATE")
        ...
        tracing_subscriber::fmt::layer()
            ...
            .with_filter(
                tracing_subscriber::filter::Targets::new()
                    .with_target("atlas_engine_http_client", tracing::Level::TRACE),
            ),
    );
    let _guard = tracing::subscriber::set_default(subscriber);
    tracing::debug!(target: "atlas_engine_http_client", "log capture sentinel");
