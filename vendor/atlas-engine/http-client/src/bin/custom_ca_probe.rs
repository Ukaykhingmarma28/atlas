...
//!
//! The shared reqwest client honors `ATLAS_AGENT_CA_CERTIFICATE` and `SSL_CERT_FILE`, but those
...
//!
//! The detailed explanation of what "hermetic" means here lives in `atlas_engine_http_client::custom_ca`.
//! This binary exists so the tests can exercise
//! [`atlas_engine_http_client::build_reqwest_client_for_subprocess_tests`] in a separate process without
...
fn build_probe_client(
    ...
    if let Some(proxy_url) = proxy_url {
        ...
        return atlas_engine_http_client::build_reqwest_client_with_custom_ca(builder.proxy(proxy))
            .map_err(|error| error.to_string());
    }

    atlas_engine_http_client::build_reqwest_client_for_subprocess_tests(builder)
        .map_err(|error| error.to_string())
}
