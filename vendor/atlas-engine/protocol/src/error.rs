...
use crate::protocol::AtlasEngineErrorInfo;
...
pub type Result<T> = std::result::Result<T, AtlasEngineErr>;
...
pub struct AtlasEngineErr {
    details: AtlasEngineErrorDetails,
    retry_delay: Option<Duration>,
}

/// The semantic category and diagnostic payload for a [`AtlasEngineErr`].
#[derive(Error, Debug, EnumDiscriminants)]
#[strum_discriminants(name(AtlasEngineErrKind))]
...
pub enum AtlasEngineErrorDetails {
    ...
    Stream(String),
    #[error(
        "Atlas Agent ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    /// could not be captured). Analogous to the previous `AtlasEngineError::Spawn` variant.
    ...
    QuotaExceeded,
    #[error(
        "To use Atlas Agent with your ChatGPT plan, upgrade to Plus: https://chatgpt.com/explore/plus."
    )]
    UsageNotIncluded,
    ...
    #[error("atlas-engine-linux-sandbox was required but not provided")]
...
impl From<&AtlasEngineErr> for AtlasEngineErrKind {
    fn from(error: &AtlasEngineErr) -> Self {
        error.details().into()
    }
}

impl fmt::Debug for AtlasEngineErr {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match &self.details {
            AtlasEngineErrorDetails::Stream(message) => formatter
                .debug_tuple("Stream")
                .field(message)
...
impl fmt::Display for AtlasEngineErr {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt::Display::fmt(&self.details, formatter)
    }
}

impl std::error::Error for AtlasEngineErr {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        self.details.source()
    }
}

impl From<AtlasEngineErrorDetails> for AtlasEngineErr {
    fn from(details: AtlasEngineErrorDetails) -> Self {
        Self {
            details,
            retry_delay: None,
        }
...
impl From<CancelErr> for AtlasEngineErr {
    fn from(error: CancelErr) -> Self {
        AtlasEngineErrorDetails::from(error).into()
    }
}

impl From<SandboxErr> for AtlasEngineErr {
    fn from(error: SandboxErr) -> Self {
        AtlasEngineErrorDetails::from(error).into()
    }
}

impl From<io::Error> for AtlasEngineErr {
    fn from(error: io::Error) -> Self {
        AtlasEngineErrorDetails::from(error).into()
    }
}

impl From<serde_json::Error> for AtlasEngineErr {
    fn from(error: serde_json::Error) -> Self {
        AtlasEngineErrorDetails::from(error).into()
    }
}

impl From<JoinError> for AtlasEngineErr {
...
#[cfg(target_os = "linux")]
impl From<landlock::RulesetError> for AtlasEngineErr {
...
#[cfg(target_os = "linux")]
impl From<landlock::PathFdError> for AtlasEngineErr {
    fn from(error: landlock::PathFdError) -> Self {
        AtlasEngineErrorDetails::from(error).into()
    }
}

impl From<CancelErr> for AtlasEngineErrorDetails {
    fn from(_: CancelErr) -> Self {
        AtlasEngineErrorDetails::TurnAborted
    }
}

// TODO(anp): Remove this compatibility macro once callers construct
// `AtlasEngineErrorDetails` directly.
macro_rules! atlas_engine_err_unit_constructors {
    ($($variant:ident),* $(,)?) => {
        $(
            #[doc(hidden)]
            #[allow(non_upper_case_globals)]
            pub const $variant: Self = Self {
                details: AtlasEngineErrorDetails::$variant,
                retry_delay: None,
            };
        )*
    };
...
// TODO(anp): Remove this compatibility macro once callers construct
// `AtlasEngineErrorDetails` directly.
macro_rules! atlas_engine_err_tuple_constructors {
    ($($(#[$attr:meta])* $variant:ident($value:ident: $value_type:ty)),* $(,)?) => {
        $(
            ...
            pub fn $variant($value: $value_type) -> Self {
                AtlasEngineErrorDetails::$variant($value).into()
            }
        )*
    };
}
...
impl AtlasEngineErr {
    atlas_engine_err_unit_constructors!(
        TurnAborted,
        SessionBudgetExceeded,
        ContextWindowExceeded,
        SessionConfiguredNotFirstEvent,
    ...
    atlas_engine_err_tuple_constructors!(
    ...
    pub fn InvalidImageRequest() -> Self {
        AtlasEngineErrorDetails::InvalidImageRequest().into()
    }

    /// Creates an error with no server-provided retry delay.
    pub fn new(details: AtlasEngineErrorDetails) -> Self {
        details.into()
    }

    /// Returns the semantic failure and its diagnostic payload.
    pub fn details(&self) -> &AtlasEngineErrorDetails {
        &self.details
    }
    ...
    pub fn is_retryable(&self) -> bool {
        match self.details() {
            ...
            | AtlasEngineErrorDetails::Interrupted
            | AtlasEngineErrorDetails::EnvVar(_)
            | AtlasEngineErrorDetails::Fatal(_)
            | AtlasEngineErrorDetails::UsageNotIncluded
            | AtlasEngineErrorDetails::QuotaExceeded
            | AtlasEngineErrorDetails::InvalidImageRequest()
            | AtlasEngineErrorDetails::InvalidRequest(_)
            | AtlasEngineErrorDetails::ToolCollision(_)
            | AtlasEngineErrorDetails::RefreshTokenFailed(_)
            | AtlasEngineErrorDetails::UnsupportedOperation(_)
            | AtlasEngineErrorDetails::Sandbox(_)
            | AtlasEngineErrorDetails::LandlockSandboxExecutableNotProvided
            ...
            | AtlasEngineErrorDetails::CyberPolicy { .. } => false,
            AtlasEngineErrorDetails::Stream(..)
            | AtlasEngineErrorDetails::Timeout
            | AtlasEngineErrorDetails::RequestTimeout
    ...
    /// Minimal shim so that existing `e.downcast_ref::<AtlasEngineErr>()` checks continue to compile
    ...
    pub fn to_atlas_engine_protocol_error(&self) -> AtlasEngineErrorInfo {
        match &self.details {
            ...
            AtlasEngineErrorDetails::Sandbox(_) => AtlasEngineErrorInfo::SandboxError,
            _ => AtlasEngineErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            atlas_engine_error_info: Some(self.to_atlas_engine_protocol_error()),
        }
    }

    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match &self.details {
            AtlasEngineErrorDetails::RetryLimit(err) => Some(err.status),
            AtlasEngineErrorDetails::UnexpectedStatus(err) => Some(err.status),
            AtlasEngineErrorDetails::ConnectionFailed(err) => err.source.status(),
            AtlasEngineErrorDetails::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
...
impl std::fmt::Display for UsageLimitReachedError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(limit_name) = self
            ...
            && !limit_name.eq_ignore_ascii_case("atlas-agent")
        {
            return write!(
                f,
        ...
        let message = match self.plan_type.as_ref() {
            ...
            Some(PlanType::Known(KnownPlan::Free)) | Some(PlanType::Known(KnownPlan::Go)) => {
                format!(
                    "You've hit your usage limit. Upgrade to Plus to continue using Atlas Agent (https://chatgpt.com/explore/plus),{}",
                    retry_suffix_after_or(self.resets_at.as_ref())
                )
            }
...
pub fn get_error_message_ui(e: &AtlasEngineErr) -> String {
    let message = match e.details() {
        AtlasEngineErrorDetails::Sandbox(SandboxErr::Denied { output, .. }) => {
        ...
        AtlasEngineErrorDetails::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
