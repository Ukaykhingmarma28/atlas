...
#[test]
fn atlas_engine_err_debug_preserves_legacy_shape() {
    let actual = [
        AtlasEngineErr::Timeout,
        AtlasEngineErr::Stream("disconnected".to_string()),
        AtlasEngineErr::Stream("retry later".to_string()).with_retry_delay(Duration::from_secs(2)),
        AtlasEngineErr::InternalServerError.with_retry_delay(Duration::from_secs(3)),
    ]
    .map(|err| format!("{err:?}"));
...
fn retryability_preserves_error_details_distinctions() {
    let errors = [
        (AtlasEngineErr::ServerOverloaded, false),
        (
        ...
        (
            AtlasEngineErrorDetails::ToolCollision("functions.update_plan".to_string()).into(),
            false,
        ),
        (AtlasEngineErr::InternalServerError, true),
    ];
...
fn server_overloaded_maps_to_protocol() {
    let err = AtlasEngineErr::ServerOverloaded;
    assert_eq!(
        err.to_atlas_engine_protocol_error(),
        AtlasEngineErrorInfo::ServerOverloaded
    );
}
...
fn sandbox_denied_uses_aggregated_output_when_stderr_empty() {
    ...
    let err = AtlasEngineErr::Sandbox(SandboxErr::Denied {
...
fn to_error_event_handles_response_stream_failed() {
    ...
    let err = AtlasEngineErr::ResponseStreamFailed(ResponseStreamFailed {
        source,
    ...
    assert_eq!(
        event.atlas_engine_error_info,
        Some(AtlasEngineErrorInfo::ResponseStreamConnectionFailed {
...
fn usage_limit_reached_error_formats_free_plan() {
    ...
    assert_eq!(
        err.to_string(),
        "You've hit your usage limit. Upgrade to Plus to continue using Atlas Agent (https://chatgpt.com/explore/plus), or try again later."
    );
}
...
#[test]
fn usage_limit_reached_error_hides_upsell_for_non_atlas_engine_limit_name() {
    let base = Utc.with_ymd_and_hms(2024, 1, 1, 0, 0, 0).unwrap();
    let resets_at = base + ChronoDuration::hours(1);
    with_now_override(base, move || {
        let expected_time = format_retry_timestamp(&resets_at);
        ...
        let expected = format!(
            "You've hit your usage limit for atlas_engine_other. Switch to another model now, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
...
fn usage_limit_reached_with_promo_message() {
    ...
    with_now_override(base, move || {
        ...
        let err = UsageLimitReachedError {
            ...
            rate_limits: Some(Box::new(rate_limit_snapshot())),
            promo_message: Some(
                "To continue using Atlas Agent, start a free trial of <PLAN> today".to_string(),
            ),
            rate_limit_reached_type: None,
        };
        let expected = format!(
            "You've hit your usage limit. To continue using Atlas Agent, start a free trial of <PLAN> today, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
