...
use atlas_engine_utils_absolute_path::canonicalize_preserving_symlinks;
...
const PROTECTED_METADATA_ATLAS_AGENT_PATH_NAME: &str = ".atlas-agent";
...
pub const PROTECTED_METADATA_PATH_NAMES: &[&str] = &[
    ...
    PROTECTED_METADATA_ATLAS_AGENT_PATH_NAME,
];
...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer Atlas Agent degrades to warn-and-ignore
    /// instead of failing to load. Atlas Agent 0.112.0 rejected unknown values here,
...
const PROJECT_ROOTS_GLOB_PATTERN_PREFIX: &str = "atlas-engine-project-roots://";
...
impl FileSystemSandboxPolicy {
    ...
    pub fn workspace_write(
        ...
        for writable_root in writable_roots {
            for protected_path in default_read_only_subpaths_for_writable_root(
                writable_root,
                /*protect_missing_dot_atlas_engine*/ false,
            ) {
    ...
    pub fn from_legacy_sandbox_policy_for_cwd(sandbox_policy: &SandboxPolicy, cwd: &Path) -> Self {
        ...
        if let SandboxPolicy::WorkspaceWrite { writable_roots, .. } = sandbox_policy {
            if let Ok(cwd_root) = AbsolutePathBuf::from_absolute_path(cwd) {
                for protected_path in default_read_only_subpaths_for_writable_root(
                    &cwd_root, /*protect_missing_dot_atlas_engine*/ true,
                ) {
    ...
    pub fn with_additional_legacy_workspace_writable_roots(
        ...
        for path in additional_writable_roots {
            ...
            for protected_path in default_read_only_subpaths_for_writable_root(
                path, /*protect_missing_dot_atlas_engine*/ false,
            ) {
                append_default_read_only_path_if_no_explicit_rule(
                    &mut self.entries,
                    protected_path,
    ...
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        ...
        .map(|root| {
            ...
            let protect_missing_dot_atlas_engine = AbsolutePathBuf::from_absolute_path(cwd)
                .ok()
            ...
            // Example: if `<root>/.atlas_engine -> <root>/decoy`, bwrap must still see
            // `<root>/.atlas_engine`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.atlas_engine` so downstream sandboxes can still detect and mask
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    protect_missing_dot_atlas_engine: bool,
) -> Vec<AbsolutePathBuf> {
    ...
    // Keep top-level project metadata under .atlas-agent read-only to the agent by
    ...
    let top_level_atlas_engine = writable_root.join(PROTECTED_METADATA_ATLAS_AGENT_PATH_NAME);
    if protect_missing_dot_atlas_engine || top_level_atlas_engine.as_path().is_dir() {
        subpaths.push(top_level_atlas_engine);
    }
...
mod tests {
    ...
    #[cfg(unix)]
    const SYMLINKED_TMPDIR_TEST_ENV: &str = "ATLAS_AGENT_PROTOCOL_TEST_SYMLINKED_TMPDIR";
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_proactively_protect_missing_dot_atlas_engine() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let expected_dot_atlas_engine = expected_root.join(".atlas-agent");
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .contains(&expected_dot_atlas_engine)
        );
    }
    ...
    fn legacy_workspace_write_projection_preserves_symbolic_project_root() {
        ...
        assert_eq!(
            ...
            FileSystemSandboxPolicy::restricted(vec![
                ...
                FileSystemSandboxEntry::skip_missing_path(
                    FileSystemPath::Special {
                        value: FileSystemSpecialPath::project_roots(Some(".atlas-agent".into())),
                    },
                    FileSystemAccessMode::Read,
                ),
            ])
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_skip_default_dot_atlas_engine_when_explicit_user_rule_exists() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let explicit_dot_atlas_engine = expected_root.join(".atlas-agent");
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Path {
                    path: explicit_dot_atlas_engine.clone(),
                },
                access: FileSystemAccessMode::Write,
                missing_path_behavior: None,
            },
        ...
        assert!(
            !workspace_root
                .protected_metadata_names
                .contains(&".atlas-agent".to_string()),
            "explicit .atlas-agent rule should remove the metadata-name protection"
        );
        assert!(
            !workspace_root
                .read_only_subpaths
    ...
    fn filesystem_policy_blocks_protected_metadata_path_writes_by_default() {
        ...
        let dot_atlas_engine_config = cwd.path().join(".atlas-agent").join("config.toml");
        ...
        assert!(!file_system_policy.can_write_path_with_cwd(&dot_atlas_engine_config, cwd.path()));
        ...
        assert!(!writable_roots[0].is_path_writable(&dot_atlas_engine_config));
    }
    ...
    fn legacy_workspace_write_projection_accepts_relative_cwd() {
        ...
        expected_entries.extend(
            default_read_only_subpaths_for_writable_root(
                &expected_root,
                /*protect_missing_dot_atlas_engine*/ true,
            )
            .into_iter()
            .map(|path| {
                FileSystemSandboxEntry::skip_missing_path(
        ...
        assert!(
            !file_system_policy
                .can_write_path_with_cwd(Path::new(".atlas-agent/config.toml"), relative_cwd,)
        );
        assert!(
    ...
    fn effective_runtime_roots_preserve_symlinked_paths() {
        ...
        let atlas_engine_dir = real_root.join(".atlas-agent");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&atlas_engine_dir).expect("create .atlas-agent");
        ...
        let expected_atlas_engine = link_root.join(".atlas-agent");
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .contains(&expected_atlas_engine)
        );
    }
    ...
    fn project_roots_special_path_preserves_symlinked_root() {
        ...
        let expected_atlas_engine = expected_root.join(".atlas-agent");
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let decoy = root.join("decoy-atlas-agent");
        let dot_atlas_engine = root.join(".atlas-agent");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_atlas_engine).expect("create .atlas-agent symlink");

        let root = AbsolutePathBuf::from_absolute_path(&root).expect("absolute root");
        let expected_dot_atlas_engine = AbsolutePathBuf::from_absolute_path(
            root.as_path()
                .canonicalize()
                .expect("canonicalize root")
                .join(".atlas-agent"),
        )
        .expect("absolute .atlas-agent symlink");
    ...
    fn tmpdir_special_path_preserves_symlinked_tmpdir() {
        ...
        let atlas_engine_dir = real_tmpdir.join(".atlas-agent");
