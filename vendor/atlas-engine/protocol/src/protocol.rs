...
//! Defines the protocol for a Atlas Agent session between a client and an agent.
//!
...
use crate::error::Result as AtlasEngineResult;
...
use atlas_engine_extension_items::image_generation::ImageGenerationFailure;
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for Atlas Agent:";
...
pub struct ConversationStartParams {
    /// Whether Atlas Agent response handoffs are managed through explicit client append calls.
    ...
    /// Whether to route any remaining transcript tail through Atlas Agent when the session ends.
    ...
    pub atlas_engine_responses_as_items: bool,
    /// Optional prefix added to automatic Atlas Agent response items when `atlas_engine_responses_as_items` is set.
    pub atlas_engine_response_item_prefix: Option<String>,
    /// Selects how automatic Atlas Agent handoffs are routed in Frameless Bidi sessions.
    /// Realtime V1 and V2 ignore this setting.
    pub atlas_engine_response_handoff_mode: AtlasEngineResponseHandoffMode,
    ...
    /// Whether to append Atlas Agent's startup context to the realtime backend prompt.
    ...
    /// Developer instructions given to Atlas Agent when this realtime session starts.
    pub realtime_start_instructions: Option<String>,
    /// Developer instructions given to Atlas Agent when this realtime session ends.
...
pub enum Op {
    ...
    TurnInput {
        ...
        mode: TurnInputMode,
        reply: oneshot::Sender<AtlasEngineResult<TurnInputSubmission>>,
    },
    ...
    /// Request Atlas Agent to drop the last N user turns from in-memory context.
    ///
    ...
    /// Request to shut down atlas-agent instance.
    Shutdown,
...
/// running the command proposed by Atlas Agent.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.atlas_engine`, `.git`, notably `.git/hooks`) under
        ...
        match self {
            ...
            SandboxPolicy::WorkspaceWrite {
                ...
                roots
                    .into_iter()
                    .map(|writable_root| {
                        let protect_missing_dot_atlas_engine = cwd_root
                            .as_ref()
...
#[serde(rename_all = "camelCase")]
#[ts(rename_all = "camelCase")]
pub enum AtlasEngineResponseHandoffMode {
    #[default]
    Thinking,
    Commentary,
    BemTags,
...
/// Atlas Agent errors that we expose to clients.
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "snake_case")]
#[ts(rename_all = "snake_case")]
pub enum AtlasEngineErrorInfo {
...
pub struct ErrorEvent {
    pub message: String,
    #[serde(default)]
    pub atlas_engine_error_info: Option<AtlasEngineErrorInfo>,
}

impl ErrorEvent {
    /// Whether this error should mark the current turn as failed when replaying history.
    pub fn affects_turn_status(&self) -> bool {
        self.atlas_engine_error_info
            .as_ref()
            .is_none_or(AtlasEngineErrorInfo::affects_turn_status)
    }
}
...
pub struct TokenUsage {
    ...
    #[schemars(skip)]
    #[ts(skip)]
    pub atlas_engine_rollout_budget_units: Option<serde_json::Number>,
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::AtlasEngine),
...
pub struct TurnContextItem {
    ...
    // Compatibility-only field written with a default value so older Atlas Agent
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                atlas_engine_utils_string::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
impl Product {
    ...
    pub fn from_session_source_name(value: &str) -> Option<Self> {
        ...
        match normalized.as_str() {
            ...
            "atlas-agent" => Some(Self::AtlasEngine),
...
mod tests {
    ...
    #[test]
    fn session_source_restriction_product_defaults_non_subagent_sources_to_atlas_engine() {
        assert_eq!(
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("atlas-agent".to_string()).restriction_product(),
            Some(Product::AtlasEngine)
        );
        assert_eq!(
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("chatgpt".to_string())
                .matches_product_restriction(&[Product::AtlasEngine])
        );
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::AtlasEngine]));
        assert!(
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_atlas_engine.as_path())
        );
    }
        ...
        assert_eq!(
            ...
            vec![
                (
                    canonical_cwd,
                    vec![
                        expected_dot_atlas_engine.to_path_buf(),
    ...
    fn generic_error_affects_turn_status() {
        let event = ErrorEvent {
            ...
            atlas_engine_error_info: Some(AtlasEngineErrorInfo::Other),
        };
        assert!(event.affects_turn_status());
    }
