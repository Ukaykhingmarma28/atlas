...
//! Proxy-aware WebSocket connection setup shared by Atlas Agent API clients.
...
use atlas_engine_http_client::BuildCustomCaTransportError;
use atlas_engine_http_client::HttpClientFactory;
use atlas_engine_http_client::build_rustls_client_config_with_custom_ca;
use futures::Sink;
use futures::Stream;
...
/// Selects whether WebSocket TLS follows Atlas Agent custom-CA policy or Tungstenite defaults.
#[derive(Clone, Copy, PartialEq, Eq)]
pub enum WebSocketTlsMode {
    /// Build an explicit TLS configuration from native roots and configured Atlas Agent custom CAs.
    ExplicitAtlasEngineTls,
...
impl WebSocketConnector {
    /// Creates a connector using native roots and any configured Atlas Agent custom CA bundle.
    pub fn new(
        http_client_factory: &HttpClientFactory,
    ...
    ///
    /// HTTPS proxy connections still build Atlas Agent TLS configuration when they establish their
