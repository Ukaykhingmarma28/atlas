...
pub fn apply_git_patch(req: &ApplyGitRequest) -> io::Result<ApplyGitResult> {
    ...
    if let Ok(cfg) = std::env::var("ATLAS_AGENT_APPLY_GIT_CFG") {
...
mod tests {
    ...
    fn init_repo() -> tempfile::TempDir {
        ...
        let _ = run(root, &["git", "config", "user.email", "atlas-agent@example.com"]);
        let _ = run(root, &["git", "config", "user.name", "Atlas Agent"]);
        dir
    }
