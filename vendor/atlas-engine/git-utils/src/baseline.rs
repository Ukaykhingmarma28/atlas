...
const BASELINE_COMMIT_MESSAGE: &str =
    "Initialize Atlas Agent git baseline\n\nCo-authored-by: Atlas Agent <noreply@openai.com>";
...
fn commit_current_tree(repo: &gix::Repository, message: &str) -> anyhow::Result<()> {
    ...
    let signature = atlas_engine_signature();
...
fn atlas_engine_signature() -> gix::actor::Signature {
    gix::actor::Signature {
        name: "Atlas Agent".into(),
