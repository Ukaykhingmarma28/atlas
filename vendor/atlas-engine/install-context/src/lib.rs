...
const CODE_MODE_HOST_EXECUTABLE_NAME: &str = if cfg!(windows) {
    "atlas-engine-code-mode-host.exe"
} else {
    "atlas-engine-code-mode-host"
};
const PACKAGE_METADATA_FILENAME: &str = "atlas-engine-package.json";
const PATH_DIRNAME: &str = "atlas-engine-path";
const RELEASES_DIRNAME: &str = "releases";
const RESOURCES_DIRNAME: &str = "atlas-engine-resources";
...
/// Version metadata recorded in a bundled Atlas Agent runtime package.
#[derive(Clone, Debug, Deserialize, Eq, PartialEq)]
pub struct AtlasEnginePackageManifest {
    pub version: Version,
}
...
pub struct InstallContext {
    ...
    pub package_layout: Option<AtlasEnginePackageLayout>,
}
...
pub enum InstallMethod {
    Standalone {
        ...
        /// such as
        /// `~/.atlas_engine/packages/standalone/releases/0.111.0-x86_64-unknown-linux-musl`.
        /// Package-layout installs use the package root that contains `bin/`,
        /// `atlas-engine-resources/`, and `atlas-engine-path/`.
    ...
    /// A Atlas Agent binary launched through the npm-managed `atlas_engine.js` shim.
    Npm,
    /// A Atlas Agent binary launched through the bun-managed `atlas_engine.js` shim.
    Bun,
    /// A Atlas Agent binary launched through the pnpm-managed `atlas_engine.js` shim.
    Pnpm,
    /// A Atlas Agent binary that appears to come from a Homebrew install prefix.
    Brew,
    /// Any other execution environment.
    ///
    /// This commonly covers `cargo run`, app-bundled Atlas Agent binaries, custom
    /// internal launchers, and tests that execute Atlas Agent from an arbitrary path.
    Other,
}
...
impl InstallContext {
    pub fn from_exe(
        ...
        let atlas_agent_home = atlas_engine_utils_home_dir::find_atlas_agent_home().ok();
        Self::from_exe_with_atlas_agent_home(
            is_macos,
            current_exe,
            method_override,
            atlas_agent_home.as_deref(),
        )
    }

    fn from_exe_with_atlas_agent_home(
        is_macos: bool,
        current_exe: Option<&Path>,
        method_override: Option<InstallMethod>,
        atlas_agent_home: Option<&Path>,
    ) -> Self {
        let package_layout = current_exe.and_then(AtlasEnginePackageLayout::from_exe);
    ...
    pub fn current() -> &'static Self {
        INSTALL_CONTEXT.get_or_init(|| {
            ...
            let method_override = if std::env::var_os("ATLAS_AGENT_MANAGED_BY_PNPM").is_some() {
                Some(InstallMethod::Pnpm)
            } else if std::env::var_os("ATLAS_AGENT_MANAGED_BY_NPM").is_some() {
                Some(InstallMethod::Npm)
            } else if std::env::var_os("ATLAS_AGENT_MANAGED_BY_BUN").is_some() {
    ...
    pub fn package_manifest(&self) -> Option<AtlasEnginePackageManifest> {
    ...
    pub fn code_mode_host_program(&self) -> PathBuf {
        // prefer the one packed under atlas-engine-resources
...
fn standalone_install_method(
    ...
    let canonical_atlas_agent_home = canonical_absolute_path(atlas_agent_home?)?;
    ...
    let releases_root = canonical_atlas_agent_home
        .join("packages")
...
mod tests {
    ...
    const TEST_RESOURCE_NAME: &str = "atlas-engine-test-helper";
    ...
    fn detects_standalone_install_from_release_layout() -> std::io::Result<()> {
        ...
        let release_dir = atlas_agent_home
            .path()
        ...
        let context = InstallContext::from_exe_with_atlas_agent_home(
            ...
            /*current_exe*/ Some(&exe_path),
            /*method_override*/ None,
            /*atlas_agent_home*/ Some(atlas_agent_home.path()),
        );
        assert_eq!(
            context,
            InstallContext {
  ...
  "variant": "atlas-agent",
  "entrypoint": "bin/atlas-agent",
  "resourcesDir": "atlas-engine-resources",
  "pathDir": "atlas-engine-path"
}
"#,
        )?;
        let exe_path = bin_dir.join(if cfg!(windows) { "atlas_agent.exe" } else { "atlas-agent" });
        ...
        let package_layout = AtlasEnginePackageLayout {
        ...
        assert_eq!(
            ...
            Some(AtlasEnginePackageManifest {
    ...
    fn code_mode_host_program_accepts_symlinks_to_files() -> std::io::Result<()> {
        ...
        fs::create_dir_all(&bin_dir)?;
        fs::write(package_dir.path().join(PACKAGE_METADATA_FILENAME), "{}")?;
        let exe_path = bin_dir.join("atlas-agent");
    ...
    fn standalone_package_layout_keeps_standalone_install_method() -> std::io::Result<()> {
        let atlas_agent_home = tempfile::tempdir()?;
        let package_dir = atlas_agent_home
            .path()
            .join("packages/standalone/releases/1.2.3-x86_64-unknown-linux-musl");
    ...
    #[test]
    fn standalone_package_rg_falls_back_when_atlas_engine_path_is_missing() -> std::io::Result<()> {
        let package_dir = tempfile::tempdir()?;
        let bin_dir = package_dir.path().join(BIN_DIRNAME);
        fs::create_dir_all(&bin_dir)?;
        fs::write(package_dir.path().join(PACKAGE_METADATA_FILENAME), "{}")?;
    ...
    fn package_manager_method_overrides_take_precedence() {
        let pnpm_context = InstallContext::from_exe(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/atlas-agent")),
    ...
    fn brew_is_detected_on_macos_prefixes() {
        let context = InstallContext::from_exe_with_atlas_agent_home(
            /*is_macos*/ true,
            /*current_exe*/ Some(Path::new("/opt/homebrew/bin/atlas-agent")),
            /*method_override*/ None,
            /*atlas_agent_home*/ None,
        );
        assert_eq!(
