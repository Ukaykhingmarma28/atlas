...
use atlas_engine_utils_path_uri::PathUriParseError;
pub use parser::Hunk;
...
/// Special argv[1] flag used when the Atlas Agent executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `atlas-engine-apply-patch` (to avoid forcing
/// `atlas-engine-arg0` to depend on `atlas-engine-core`), it remains part of the "atlas-agent core"
/// process-invocation contract for the standalone `apply_patch` command
/// surface.
pub const ATLAS_AGENT_CORE_APPLY_PATCH_ARG1: &str = "--atlas-engine-run-as-apply-patch";

/// Internal environment variable used to carry the selected update mode
/// through the arg0-dispatched standalone executable.
pub const ATLAS_AGENT_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR: &str =
    "ATLAS_AGENT_APPLY_PATCH_PRESERVE_LINE_ENDINGS";
...
pub fn apply_patch_file_update_mode_from_env() -> ApplyPatchFileUpdateMode {
    match std::env::var(ATLAS_AGENT_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR).as_deref() {
