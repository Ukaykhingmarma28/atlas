...
fn user_layer(atlas_agent_home: &TempDir, config: &str) -> ConfigLayerEntry {
...
fn stack(atlas_agent_home: &TempDir, user: &str, session: &str) -> ConfigLayerStack {
    ConfigLayerStack::new(
        vec![
            user_layer(atlas_agent_home, user),
            ConfigLayerEntry::new(
...
fn session_flags_can_reenable_user_disabled_path() {
    let atlas_agent_home = TempDir::new().expect("temp dir");
    let skill_path = atlas_agent_home.path().join("skills/demo/SKILL.md");
...
fn later_path_rule_reenables_one_skill_disabled_by_name() {
    let atlas_agent_home = TempDir::new().expect("temp dir");
    let root = atlas_agent_home.path().join("reenable-by-path");
