...
use atlas_engine_execpolicy::RuleRef;
use atlas_engine_execpolicy::rule::PatternToken;
use atlas_engine_execpolicy::rule::PrefixPattern;
use atlas_engine_execpolicy::rule::PrefixRule;
...
pub enum RequirementsExecPolicyParseError {
    ...
    #[error(
        "rules prefix_rule at index {rule_index} has decision 'allow', which is not permitted in requirements.toml: Atlas Agent merges these rules with other config and uses the most restrictive result (use 'prompt' or 'forbidden')"
    )]
...
impl RequirementsExecPolicyToml {
    ...
    /// representation used by `atlas-engine-execpolicy`.
