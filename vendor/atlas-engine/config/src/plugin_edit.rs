...
use atlas_engine_utils_path::resolve_symlink_write_paths;
use atlas_engine_utils_path::write_atomically;
use tokio::task;
...
pub async fn apply_user_plugin_config_edits(
    ...
    task::spawn_blocking(move || apply_user_plugin_config_edits_blocking(&atlas_agent_home, edits))
        .await
...
mod tests {
    ...
    async fn set_user_plugin_enabled_writes_plugin_entry() {
        ...
        let config = read_config(atlas_agent_home.path());
...
"#,
        ...
        clear_user_plugin(atlas_agent_home.path(), "demo@market".to_string())
            .await
            .unwrap();
    ...
    async fn set_user_plugin_enabled_follows_config_symlink() {
        ...
        let atlas_agent_home = TempDir::new().unwrap();
        let target_path = atlas_agent_home.path().join("target_config.toml");
        symlink(&target_path, atlas_agent_home.path().join(CONFIG_TOML_FILE)).unwrap();
