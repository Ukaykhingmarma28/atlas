...
/// OAuth client settings used when Atlas Agent launches an MCP OAuth flow.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema)]
#[schemars(deny_unknown_fields)]
pub struct McpServerOAuthConfig {
    ...
    /// Fixed callback port that takes precedence over Atlas Agent's global OAuth callback port.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub callback_port: Option<u16>,
}

/// Authentication flow Atlas Agent attempts after resolving an HTTP MCP server's
...
pub struct McpServerConfig {
    ...
    /// Effective environment id for where Atlas Agent should start this MCP server.
    pub environment_id: String,

    /// When `false`, Atlas Agent skips initializing this MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,

    /// When `true`, `atlas_engine exec` exits with an error if this MCP server fails to initialize.
