...
pub struct LoaderOverrides {
    /// Optional configuration file supplied with the installed Atlas Agent package.
...
impl LoaderOverrides {
    ...
    pub fn without_managed_config_for_tests() -> Self {
        let base = std::env::temp_dir().join("atlas-engine-config-tests");
        Self {
    ...
    pub fn user_config_path(&self, atlas_agent_home: &Path) -> std::io::Result<AbsolutePathBuf> {
...
impl ConfigLayerEntry {
    ...
    // Get the `.atlas_engine/` folder associated with this config layer, if any.
    ...
    /// Returns the `.atlas_engine/` folder that should be used for hook declarations.
    ///
...
impl ConfigLayerStack {
    ...
    /// the base `$ATLAS_AGENT_HOME/config.toml` layer because the active layer is the
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<()> {
    ...
    // user config.
    let mut previous_project_dot_atlas_agent_folder: Option<&AbsolutePathBuf> = None;
    for layer in layers {
        if let ConfigLayerSource::Project {
            dot_atlas_agent_folder: current_project_dot_atlas_agent_folder,
        } = &layer.name
        {
            if let Some(previous) = previous_project_dot_atlas_agent_folder {
                ...
                if previous == current_project_dot_atlas_agent_folder
                    || !current_project_dot_atlas_agent_folder
                        .as_path()
                        .ancestors()
            ...
            previous_project_dot_atlas_agent_folder = Some(current_project_dot_atlas_agent_folder);
        }
    }
