...
//! `~/.atlas_engine/config.toml` and normalizes user-entered key specs into canonical
//! forms consumed by runtime keymap resolution in `atlas-engine-rs/tui/src/keymap.rs`.
//!
//! Responsibilities:
//!
...
See the AtlasEngine keymap documentation for supported actions and examples."
                .to_string(),
        );
    }
