...
//! Schema-heavy configuration TOML types used by Atlas Agent.
...
use atlas_engine_model_provider_info::AMAZON_BEDROCK_RUNTIME_PROVIDER_ID;
...
/// Base config deserialized from ~/.atlas-agent/config.toml.
...
pub struct ConfigToml {
    ...
    /// sanctioned by Atlas Agent will likely degrade model performance.
    ...
    /// file: Use a file in the Atlas Agent home directory.
    ...
    /// Directory where Atlas Agent stores the SQLite state DB.
    /// Defaults to `$ATLAS_AGENT_SQLITE_HOME` when set. Otherwise uses `$ATLAS_AGENT_HOME`.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where Atlas Agent writes log files. Setting this value explicitly
    /// also enables the TUI text log in this directory.
    /// Defaults to `$ATLAS_AGENT_HOME/log`.
    ...
    /// Optional product SKU forwarded on host-owned Atlas Agent Apps MCP requests.
    ...
    /// directories for `.atlas_engine` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
pub use atlas_engine_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
pub use atlas_engine_protocol::protocol::RealtimeVoice;
