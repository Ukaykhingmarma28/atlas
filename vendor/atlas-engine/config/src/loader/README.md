...
# `atlas-engine-config` loader

This module is the canonical place to **load and describe Atlas Agent configuration layers** (user config, CLI/session overrides, cloud-managed config, managed config, and MDM-managed preferences) and to produce:
...
Exported from `atlas_engine_config::loader`:

- `load_config_layers_state(fs, atlas_agent_home, cwd_opt, cli_overrides, options, thread_config_loader) -> ConfigLayerStack`
- `ConfigLayerStack`
...
4. `Project` config (`.atlas_engine/config.toml`)
5. `User` profile config, when present
6. `User` config (`config.toml`)
7. `EnterpriseManaged` cloud-managed config bundle layers
8. `System` config (`/etc/atlas_engine/config.toml` or the Windows system config path)
