...
#[cfg(unix)]
const ATLAS_AGENT_MANAGED_CONFIG_SYSTEM_PATH: &str = "/etc/atlas-agent/managed_config.toml";
...
pub(super) struct LoadedConfigLayers {
    /// If present, data read from a file such as `/etc/atlas_engine/managed_config.toml`.
...
pub(super) async fn load_config_layers_internal(
    ...
    let managed_config_path = AbsolutePathBuf::from_absolute_path(
        managed_config_path.unwrap_or_else(|| managed_config_default_path(atlas_agent_home)),
    )?;
...
pub(super) fn managed_config_default_path(atlas_agent_home: &Path) -> PathBuf {
    #[cfg(unix)]
    {
        let _ = atlas_agent_home;
        PathBuf::from(ATLAS_AGENT_MANAGED_CONFIG_SYSTEM_PATH)
    }
