...
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/atlas-agent/config.toml";
...
///
/// - system    `/etc/atlas_engine/requirements.toml` (Unix) or
///   `%ProgramData%\OpenAI\AtlasEngine\requirements.toml` (Windows)
...
///
/// - package:  optional default configuration supplied with the Atlas Agent package
/// - admin:    managed preferences (*)
/// - system    `/etc/atlas_engine/config.toml` (Unix) or
///   `%ProgramData%\OpenAI\AtlasEngine\config.toml` (Windows)
/// - cloud     enterprise-managed cloud config bundle fragments
/// - user      `${ATLAS_AGENT_HOME}/config.toml`
/// - profile   `${ATLAS_AGENT_HOME}/<name>.config.toml`, when selected
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.atlas_engine/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.atlas_engine/config.toml` (loaded but disabled when untrusted)
...
pub async fn load_config_layers_state(
    ...
    if !ignore_managed_requirements {
        if let Some(bundle) = cloud_config_bundle.get().await.map_err(io::Error::other)? {
            let cloud_config_base_dir = AbsolutePathBuf::from_absolute_path(atlas_agent_home)?;
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        let raw_toml_base_dir = AbsolutePathBuf::from_absolute_path(atlas_agent_home)?;
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/atlas-agent/requirements.toml"))
}
...
#[cfg(windows)]
fn windows_atlas_engine_system_dir() -> PathBuf {
    ...
    program_data.join("Atlas").join("AtlasAgent")
}

#[cfg(windows)]
fn windows_system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    let requirements_toml_file = windows_atlas_engine_system_dir().join("requirements.toml");
    AbsolutePathBuf::try_from(requirements_toml_file)
}

#[cfg(windows)]
fn windows_system_config_toml_file() -> io::Result<AbsolutePathBuf> {
    let config_toml_file = windows_atlas_engine_system_dir().join("config.toml");
...
fn requirements_layers_from_legacy_scheme(
    ...
    let atlas_agent_home = AbsolutePathBuf::from_absolute_path(atlas_agent_home)?;
...
fn legacy_requirements_to_toml_value(legacy: LegacyManagedConfigToml) -> io::Result<TomlValue> {
    ...
    if let Some(sandbox_mode) = sandbox_mode {
        ...
        // Allowing read-only is a requirement for Atlas Agent to function correctly.
...
impl ProjectTrustContext {
    ...
    fn root_checkout_hooks_folder_for_dir(&self, dir: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
        ...
        Some(repo_root.join(relative_dir).join(".atlas-agent"))
    }
}
...
fn project_layer_entry(
    ...
    let source = ConfigLayerSource::Project {
        dot_atlas_agent_folder: dot_atlas_agent_folder.clone(),
    };
...
fn project_ignored_config_keys_warning(
    ...
    let config_path = dot_atlas_agent_folder.join(CONFIG_TOML_FILE);
...
async fn discover_project_layers(
    ...
    for dir in dirs {
        let dot_atlas_engine_abs = dir.join(".atlas-agent");
        let dot_atlas_engine_uri = PathUri::from_abs_path(&dot_atlas_engine_abs);
        if !fs
            .get_metadata(&dot_atlas_engine_uri, /*sandbox*/ None)
            .await
                ...
                let config: TomlValue = match toml::from_str(&contents) {
                    ...
                    Err(e) => {
                        ...
                        layers.push(DiscoveredProjectLayer {
                            dot_atlas_agent_folder: dot_atlas_engine_abs,
...
/// from a file like `/etc/atlas_engine/managed_config.toml` that has the same
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("Atlas Agent").join("config.toml"))
        );
    }
}
