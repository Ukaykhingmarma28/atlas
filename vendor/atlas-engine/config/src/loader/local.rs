    ...
    for layer in layers {
        ...
        output.push(LocalTomlLayer {
            source: ConfigLayerSource::Project {
                dot_atlas_agent_folder: layer.dot_atlas_agent_folder.clone(),
            },
            base_dir: layer.dot_atlas_agent_folder,
            toml: config,
        });
        ...
        output.push(LocalTomlLayer {
            source: ConfigLayerSource::Project {
                dot_atlas_agent_folder: hooks_config_folder.clone(),
            },
...
fn append_legacy_config_layers(
    ...
    if let Some(config) = loaded.managed_config_from_mdm {
        output.push(LocalTomlLayer {
            ...
            base_dir: atlas_agent_home.clone(),
            toml: config.managed_config,
        });
    }
    Ok(())
...
async fn local_requirements_layers(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        entries.extend(
            ...
            .await?
            .map(|layer| layer.with_base_dir(atlas_agent_home)),
        );
    }
