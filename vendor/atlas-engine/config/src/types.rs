...
//! Types used to define loaded and effective Atlas Agent configuration values.
...
pub use atlas_engine_protocol::config_types::AltScreenMode;
pub use atlas_engine_protocol::config_types::ApprovalsReviewer;
pub use atlas_engine_protocol::config_types::ModeKind;
pub use atlas_engine_protocol::config_types::Personality;
pub use atlas_engine_protocol::config_types::ServiceTier;
pub use atlas_engine_protocol::config_types::WebSearchMode;
...
pub enum ResumeCwdMode {
    /// Use the directory where Atlas Agent was launched.
    Current,
...
/// Determine where Atlas Agent should store CLI auth credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in ATLAS_AGENT_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in ATLAS_AGENT_HOME.
    Auto,
    /// Store credentials in memory only for the current process.
    Ephemeral,
}

/// Determine where Atlas Agent should store and read MCP credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum OAuthCredentialsStoreMode {
    ...
    #[default]
    Auto,
    /// ATLAS_AGENT_HOME/.credentials.json
...
pub struct AnalyticsConfigToml {
    /// When `false`, disables analytics across Atlas Agent product surfaces in this profile.
...
pub struct FeedbackConfigToml {
    /// When `false`, disables the feedback flow across Atlas Agent product surfaces.
    pub enabled: Option<bool>,
}
...
pub struct MemoriesToml {
    ...
    /// Minimum remaining percentage required in Atlas Agent rate-limit windows before memory startup runs.
...
pub struct AppConfig {
    /// When `false`, Atlas Agent does not surface this app.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
impl Default for OtelConfig {
    fn default() -> Self {
        OtelConfig {
            ...
            // Upstream defaulted this to `Statsig` — a hardcoded endpoint and
            // client key, on by default. Removed for Atlas (#43, spec D2);
            // metrics now export nowhere unless a user configures an endpoint.
...
/// Fallback resize-reflow row cap when Atlas Agent cannot identify a terminal-specific scrollback size.
...
pub struct Tui {
    ...
    /// Use `/theme` in the TUI or see `$ATLAS_AGENT_HOME/themes` for custom themes.
    #[serde(default)]
    pub theme: Option<String>,

    /// Pet id to preselect in the terminal pet picker.
    ///
    /// Custom pet ids resolve against ATLAS_AGENT_HOME/pets/<pet-id>/pet.json.
    #[serde(default)]
    ...
    /// transcript exceeds this cap. Omit to use Atlas Agent's terminal-specific default. Set to `0` to
...
/// (primarily the Atlas Agent IDE extension). NOTE: these are different from
...
pub struct Notice {
    ...
    /// Tracks whether the user opted out of Atlas Agent-managed fast defaults.
...
pub struct PluginMcpServerConfig {
    /// When `false`, Atlas Agent skips initializing this plugin MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
pub struct MarketplaceConfig {
    /// Last time Atlas Agent successfully added or refreshed this marketplace.
    #[serde(default)]
    pub last_updated: Option<String>,
    /// Git revision Atlas Agent last successfully activated for this marketplace.
    #[serde(default)]
