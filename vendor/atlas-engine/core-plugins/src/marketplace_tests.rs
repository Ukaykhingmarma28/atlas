  ...
  "plugins": [
    {
      ...
      "source": {
        "source": "npm",
        "package": "@acme/atlas-engine-plugin",
        "version": "^1.2.0",
    ...
    assert_eq!(
        resolved,
        ResolvedMarketplacePlugin {
            plugin_id: PluginId::new("npm-plugin".to_string(), "atlas-engine-curated".to_string())
                .unwrap(),
            source: MarketplacePluginSource::Npm {
                package: "@acme/atlas-engine-plugin".to_string(),
  ...
  "plugins": [
    {{
      ...
      "source": {{
        ...
        assert_eq!(
            err.to_string(),
            "plugin `remote-plugin` was not found in marketplace `atlas-engine-curated`"
        );
    }
}
...
fn find_marketplace_plugin_reports_missing_plugin() {
    ...
    fs::write(
        repo_root.join(".agents/plugins/marketplace.json"),
        r#"{"name":"atlas-engine-curated","plugins":[]}"#,
    )
    .unwrap();
    ...
    assert_eq!(
        err.to_string(),
        "plugin `missing` was not found in marketplace `atlas-engine-curated`"
    );
}
...
fn list_marketplaces_supports_repo_root_local_plugin_sources() {
    for path in [".", "./"] {
        ...
        fs::create_dir_all(repo_root.join(".git")).unwrap();
        fs::create_dir_all(repo_root.join(".agents/plugins")).unwrap();
        fs::create_dir_all(repo_root.join(".atlas-agent-plugin")).unwrap();
        fs::write(
            repo_root.join(".agents/plugins/marketplace.json"),
            format!(
                r#"{{
  ...
  "plugins": [
    {{
      ...
      "source": {{
        ...
        .unwrap();
        fs::write(
            repo_root.join(".atlas-agent-plugin/plugin.json"),
            r#"{
  "name":"repo-root-plugin",
  "interface": {
...
fn list_marketplaces_returns_home_and_repo_marketplaces() {
  ...
  "plugins": [
    ...
    assert_eq!(
        ...
        vec![
            Marketplace {
                name: "atlas-engine-curated".to_string(),
                path:
...
fn list_marketplaces_resolves_plugin_interface_paths_to_absolute() {
  ...
  "plugins": [
    {
      ...
      "policy": {
        ...
        "products": ["ATLAS_AGENT", "CHATGPT", "ATLAS"]
      },
      "category": "Design"
    }
  ]
  ...
  "interface": {
    ...
    assert_eq!(
        ...
        Some(vec![Product::AtlasEngine, Product::Chatgpt, Product::Atlas])
    );
    assert_eq!(
  ...
  "plugins": [
    {{
      ...
      "source": {{
        ...
        assert_eq!(
            err.to_string(),
            "plugin `local-plugin` was not found in marketplace `atlas-engine-curated`"
        );
    }
}
...
fn find_installable_marketplace_plugin_rejects_disallowed_product() {
  ...
  "plugins": [
    ...
    assert_eq!(
        err.to_string(),
        "plugin `chatgpt-plugin` is not available for install in marketplace `atlas-engine-curated`"
    );
}
...
fn find_installable_marketplace_plugin_rejects_explicit_empty_products() {
  ...
  "plugins": [
    ...
    assert_eq!(
        err.to_string(),
        "plugin `disabled-plugin` is not available for install in marketplace `atlas-engine-curated`"
    );
}
