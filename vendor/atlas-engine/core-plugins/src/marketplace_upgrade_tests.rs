...
"#,
    ...
    assert_eq!(
        read_configured_git_marketplace(atlas_agent_home.path(), "good")
...
"#
    ...
    std::fs::write(atlas_agent_home.path().join(CONFIG_TOML_FILE), &config).expect("write config");
    let stack = config_layer_stack(atlas_agent_home.path(), &config);
...
fn upgrade_uses_validated_source_for_git_operations() {
    ...
    let normalized_url = url::Url::from_directory_path(remote_repo.path())
        .expect("remote repository URL")
        .to_string();
    let raw_source = atlas_agent_home.path().join("missing-raw-source");
...
"#
    ...
    let install_root = marketplace_install_root(atlas_agent_home.path());
...
fn up_to_date_fast_path_validates_marketplace_name() {
    ...
    let missing_source = atlas_agent_home.path().join("missing-source");
