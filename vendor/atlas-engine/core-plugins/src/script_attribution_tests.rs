...
use atlas_engine_plugin::PluginLoadOutcome;
...
fn synced_plugin_root(atlas_agent_home: &Path, marketplace: &str, plugin_name: &str) -> AbsolutePathBuf {
    ...
    let root = PluginStore::new(atlas_agent_home.to_path_buf()).plugin_root(
        &plugin_id,
...
fn cached_remote_plugin_root(atlas_agent_home: &Path, plugin_name: &str) -> AbsolutePathBuf {
    let plugin_id = PluginId::new(
        plugin_name.to_string(),
        REMOTE_GLOBAL_MARKETPLACE_NAME.to_string(),
    )
    .expect("plugin id");
    let root = PluginStore::new(atlas_agent_home.to_path_buf()).plugin_root(&plugin_id, "1.2.3");
    fs::create_dir_all(root.as_path()).expect("create cached remote plugin root");
    root
}
fn installed_remote_plugin_root(atlas_agent_home: &Path, plugin_name: &str) -> AbsolutePathBuf {
    let root = cached_remote_plugin_root(atlas_agent_home, plugin_name);
    let plugin_id = PluginId::new(
        plugin_name.to_string(),
        REMOTE_GLOBAL_MARKETPLACE_NAME.to_string(),
    )
    .expect("plugin id");
    PluginStore::new(atlas_agent_home.to_path_buf())
...
fn resolves_primary_runtime_scripts_from_the_installed_plugin_cache() {
    ...
    fs::create_dir_all(source_root.join(".atlas-agent-plugin")).expect("create manifest directory");
    fs::write(
...
fn roots_for(atlas_agent_home: &Path, plugins: Vec<LoadedPlugin>) -> TrustedPluginRoots {
...
async fn resolves_relocated_script_through_executor_filesystem() {
    ...
    let environment =
        atlas_engine_exec_server::Environment::create_for_tests(/*exec_server_url*/ None)
...
fn recognizes_windows_executor_plugin_cache_root() {
    ...
    let script = PathUri::parse(
        "file:///C:/Users/user/.atlas-agent/plugins/cache/openai-primary-runtime/presentations/0.1.29/skills/presentations/container_tools/mark_artifact_operation_started.mjs",
    )
...
fn only_emits_safe_normalized_relative_script_paths() {
    ...
    assert_eq!(
        normalized_relative_script_path(Path::new(
            "/home/user/.atlas-agent/plugins/cache/openai-curated/sample/scripts/run.py"
        )),
        None
    );
}
