...
const ATLAS_AGENT_PRODUCT_SKU: &str = "atlas-agent";
...
pub fn invalidate_cached_remote_plugin_catalog_scopes(
    ...
    for scope in scopes {
        catalog_cache::remove_cached_directory_plugins(atlas_agent_home, config, auth, *scope);
    }
}
...
pub(crate) fn has_fresh_cached_remote_plugin_catalog(
    ...
    catalog_cache::load_cached_directory_plugins(atlas_agent_home, config, auth, scope).is_some_and(
        |cached| {
            matches!(
                cached.freshness,
...
fn remove_remote_plugin_cache(
    ...
    let store = PluginStore::try_new(atlas_agent_home.clone())
    ...
    let legacy_remote_plugin_cache_root = atlas_agent_home
...
async fn fetch_directory_plugins_for_scope_with_cache(
    ...
    if cache_mode == RemotePluginCatalogCacheMode::PreferCache
        && let Some(atlas_agent_home) = atlas_agent_home
        && let Some(cached) =
...
fn authenticated_request(
    ...
    request
        ...
        .header(OAI_PRODUCT_SKU_HEADER, ATLAS_AGENT_PRODUCT_SKU)
}
