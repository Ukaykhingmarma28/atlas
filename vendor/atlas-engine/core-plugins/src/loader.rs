...
use atlas_engine_config::HooksFile;
use atlas_engine_config::SkillConfigRules;
use atlas_engine_config::skill_config_rules_from_stack;
use atlas_engine_config::types::McpServerConfig;
use atlas_engine_config::types::McpServerTransportConfig;
use atlas_engine_config::types::PluginConfig;
use atlas_engine_config::types::PluginMcpServerConfig;
use atlas_engine_connectors::parse_plugin_app_config;
use atlas_engine_connectors::parse_plugin_app_config_value;
use atlas_engine_mcp::parse_agent_plugin_mcp_config;
use atlas_engine_mcp::parse_plugin_mcp_config;
use atlas_engine_plugin::AppDeclaration;
use atlas_engine_plugin::LoadedPlugin;
use atlas_engine_plugin::PluginCapabilitySummary;
use atlas_engine_plugin::PluginHookSource;
use atlas_engine_plugin::PluginId;
use atlas_engine_plugin::PluginIdError;
...
use atlas_engine_skills::SkillRootLoadRequest;
use atlas_engine_skills::SkillRootLoader;
use atlas_engine_skills::SkillRootSnapshots;
use atlas_engine_utils_absolute_path::AbsolutePathBuf;
use atlas_engine_utils_plugins::PluginIdentity;
use atlas_engine_utils_plugins::PluginSkillRoot;
use atlas_engine_utils_plugins::SkillDiscoveryMode;
use atlas_engine_utils_plugins::find_plugin_manifest_path;
use atlas_engine_utils_plugins::migrated_command_skills_root;
...
async fn load_plugins_from_layer_stack_with_scope(
    ...
    let configured_plugins = merge_configured_plugins_with_remote_installed(
        configured_plugins_from_stack(config_layer_stack, store.atlas_agent_home().as_path()),
        extra_plugins,
        store,
        remote_global_catalog_active,
    );
...
pub fn refresh_curated_plugin_cache(
    ...
    let store = PluginStore::try_new(atlas_agent_home.to_path_buf()).map_err(|err| err.to_string())?;
    let curated_marketplace_paths = curated_marketplace_paths_for_cache_refresh(atlas_agent_home)?;
...
fn curated_marketplace_paths_for_cache_refresh(
    ...
    let api_marketplace_path = atlas_agent_home
        .join(".tmp/plugins")
...
fn refresh_non_curated_plugin_cache_with_mode(
    ...
    for plugin_id in configured_non_curated_plugin_ids {
        ...
        let refresh_result = (|| -> Result<bool, String> {
            let materialized =
                materialize_marketplace_plugin_source(atlas_agent_home, &source).map_err(|err| {
...
fn configured_plugins_from_atlas_agent_home(
    atlas_agent_home: &Path,
...
pub fn configured_curated_plugin_ids_from_atlas_agent_home(atlas_agent_home: &Path) -> Vec<PluginId> {
    curated_plugin_ids_from_config_keys(configured_plugins_from_atlas_agent_home(
        atlas_agent_home,
    ...
    match &manifest_paths.hooks {
        ...
        Some(PluginManifestHooks::Inline(hooks_files)) => {
            let manifest_path = find_plugin_manifest_path(plugin_root.as_path())
                ...
                .unwrap_or_else(|| plugin_root.join(".atlas-agent-plugin/plugin.json"));
...
pub async fn load_configured_plugin_mcp_servers(
    ...
    let configured_plugins = configured_plugins_from_stack(config_layer_stack, atlas_agent_home);
...
pub fn materialize_marketplace_plugin_source(
    ...
    match source {
        ...
        MarketplacePluginSource::Git {
            ...
            let staging_root = atlas_agent_home.join("plugins/.marketplace-plugin-source-staging");
...
fn run_git_output(args: &[&str], cwd: Option<&Path>) -> Result<String, String> {
    ...
    command
        .args(["-c", atlas_engine_git_utils::SAFE_BARE_REPOSITORY_CONFIG])
        .args(args);
    ...
    atlas_engine_git_utils::no_console_window(&mut command);
