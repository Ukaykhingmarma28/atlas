...
pub enum RemotePluginBundleInstallError {
    ...
    DownloadBody {
        url: String,
        #[source]
        source: atlas_engine_http_client::HttpError,
    },
...
pub async fn download_and_install_remote_plugin_bundle(
    ...
    tokio::task::spawn_blocking(move || {
        install_remote_plugin_bundle(atlas_agent_home, bundle, bundle_bytes)
    })
    .await
    .map_err(|err| {
...
fn install_remote_plugin_bundle(
    ...
    let staging_root = atlas_agent_home.join(REMOTE_PLUGIN_INSTALL_STAGING_DIR);
    ...
    let store = PluginStore::try_new(atlas_agent_home)?;
...
mod tests {
    ...
    fn install_persists_remote_plugin_install_metadata() {
        ...
        let store = PluginStore::new(atlas_agent_home.path().to_path_buf());
