...
pub async fn fetch_remote_featured_plugin_ids(
    ...
    url.query_pairs_mut().append_pair(
        "platform",
        product.unwrap_or(Product::AtlasEngine).to_app_platform(),
    );
...
fn ensure_atlas_engine_backend_auth(
    auth: Option<&AtlasEngineAuth>,
) -> Result<&AtlasEngineAuth, RemotePluginMutationError> {
...
async fn post_remote_plugin_mutation(
    ...
    let auth = ensure_atlas_engine_backend_auth(auth)?;
