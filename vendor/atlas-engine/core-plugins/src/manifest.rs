...
use atlas_engine_utils_plugins::AGENT_PLUGIN_MANIFEST_RELATIVE_PATH;
...
pub type UriPluginManifest = atlas_engine_plugin::manifest::PluginManifest<PathUri>;
...
pub(crate) fn load_plugin_manifest_with_format(plugin_root: &Path) -> Option<LoadedPluginManifest> {
    ...
    let overlay = if is_agent_plugin {
        let overlay_path = plugin_root.join(".atlas-agent-plugin/plugin.json");
...
fn resolve_raw_plugin_manifest(
    ...
    let interface = interface.and_then(|interface| {
        ...
        let interface = atlas_engine_plugin::manifest::PluginManifestInterface {
            display_name,
            short_description,
            long_description,
            developer_name,
    ...
    Ok(atlas_engine_plugin::manifest::PluginManifest {
        name,
        version,
        description,
        keywords,
        paths: atlas_engine_plugin::manifest::PluginManifestPaths {
...
fn resolve_manifest_hooks(
    plugin_root: &PathUri,
    hooks: Option<RawPluginManifestHooks>,
) -> Option<atlas_engine_plugin::manifest::PluginManifestHooks<PathUri>> {
    match hooks? {
        RawPluginManifestHooks::Path(path) => {
            resolve_manifest_path(plugin_root, "hooks", Some(&path))
                .map(|path| atlas_engine_plugin::manifest::PluginManifestHooks::Paths(vec![path]))
        }
...
fn resolve_manifest_mcp_servers(
    plugin_root: &PathUri,
    mcp_servers: Option<RawPluginManifestMcpServers>,
) -> Option<atlas_engine_plugin::manifest::PluginManifestMcpServers<PathUri>> {
    match mcp_servers? {
...
mod tests {
    ...
    use atlas_engine_plugin::PluginProvider;
    use atlas_engine_plugin::ResolvedPlugin;
    use atlas_engine_plugin::manifest::PluginManifest as GenericPluginManifest;
    use atlas_engine_plugin::manifest::PluginManifestHooks;
    use atlas_engine_plugin::manifest::PluginManifestInterface;
    use atlas_engine_plugin::manifest::PluginManifestMcpServers;
    use atlas_engine_plugin::manifest::PluginManifestPaths;
    ...
    fn write_manifest(plugin_root: &Path, version: Option<&str>, interface: &str) {
        fs::create_dir_all(plugin_root.join(".atlas-agent-plugin")).expect("create manifest dir");
        let version = version
