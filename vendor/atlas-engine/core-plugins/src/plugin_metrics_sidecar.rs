...
use atlas_engine_analytics::PluginMeasurementRow;
...
pub const PLUGIN_METRICS_OUTPUT_ENV_VAR: &str = "ATLAS_AGENT_PLUGIN_METRICS_OUTPUT";
...
impl PluginMetricsSidecar {
    pub fn create(resolved: ResolvedPluginMetricsOperation) -> Option<Self> {
        let sidecar_dir = tempfile::Builder::new()
            .prefix("atlas-engine-plugin-metrics-")
            .tempdir()
            .ok()?;
    ...
    pub async fn create_remote(
        ...
        let directory_path = temp_dir
            .join(&format!("atlas-engine-plugin-metrics-{execution_id}"))
            .ok()?;
