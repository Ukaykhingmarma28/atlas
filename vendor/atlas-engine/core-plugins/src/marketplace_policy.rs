...
use atlas_engine_config::MarketplaceAllowedSourceKind;
use atlas_engine_config::MarketplaceAllowedSourceToml;
use atlas_engine_config::RequirementSource;
use atlas_engine_config::types::MarketplaceConfig;
use atlas_engine_config::types::MarketplaceSourceType;
...
use atlas_engine_utils_path::paths_match_after_normalization;
use regex::Regex;
...
impl MarketplacePolicy {
    ...
    pub(crate) fn validate_install(
        ...
        let configured_root = resolve_configured_marketplace_root(
            marketplace_name,
            marketplace,
            &marketplace_install_root(atlas_agent_home),
        )
        .ok_or_else(|| {
...
pub(crate) fn project_effective_user_config(
    ...
    let allowed_marketplace_names =
        allowed_configured_marketplace_names_with_policy(&user_config, &policy, atlas_agent_home);
...
fn allowed_configured_marketplace_names_with_policy(
    ...
    marketplaces
        ...
        .filter_map(|(marketplace_name, marketplace)| {
            let allowed = match managed_marketplace_config_name(atlas_agent_home, marketplace) {
...
pub(crate) fn validate_marketplace_source_for_add(
    ...
    if let MarketplaceSource::Local { path } = source
        && let Some(expected_name) = managed_local_marketplace_name(atlas_agent_home, path)
    {
...
fn managed_marketplace_name(
    ...
    if paths_match_after_normalization(
        ...
        curated_plugins_api_marketplace_path(atlas_agent_home),
    ) {
...
fn managed_marketplace_config_name(
    ...
    managed_local_marketplace_name(atlas_agent_home, path)
}

fn managed_local_marketplace_name(atlas_agent_home: &Path, root: &Path) -> Option<&'static str> {
    for marketplace_name in [
        OPENAI_BUNDLED_MARKETPLACE_NAME,
        OPENAI_BUNDLED_ALPHA_MARKETPLACE_NAME,
    ] {
        let expected_root = atlas_agent_home
...
pub(crate) fn primary_runtime_marketplace_root() -> Option<PathBuf> {
    Some(
        primary_runtime_cache_dir()?
            .join("atlas-engine-runtimes/atlas-engine-primary-runtime/plugins")
