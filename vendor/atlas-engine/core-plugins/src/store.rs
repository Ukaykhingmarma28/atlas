...
use atlas_engine_utils_plugins::AgentPluginSchemaStatus;
use atlas_engine_utils_plugins::agent_plugin_schema_status;
...
const REMOTE_PLUGIN_INSTALL_METADATA_FILE: &str = ".atlas-agent-remote-plugin-install.json";
...
impl PluginStore {
    pub fn new(atlas_agent_home: PathBuf) -> Self {
        Self::try_new(atlas_agent_home)
            .unwrap_or_else(|err| panic!("plugin cache root should be absolute: {err}"))
    }

    pub fn try_new(atlas_agent_home: PathBuf) -> Result<Self, PluginStoreError> {
        let root = AbsolutePathBuf::from_absolute_path_checked(atlas_agent_home.join(PLUGINS_CACHE_DIR))
            .map_err(|err| PluginStoreError::io("failed to resolve plugin cache root", err))?;
        let data_root =
            AbsolutePathBuf::from_absolute_path_checked(atlas_agent_home.join(PLUGINS_DATA_DIR))
        ...
        let atlas_agent_home = AbsolutePathBuf::from_absolute_path_checked(atlas_agent_home)
            .map_err(|err| PluginStoreError::io("failed to resolve Atlas Agent home", err))?;
...
fn plugin_manifest_for_source(
    ...
    match manifest {
        ...
        InstallManifest::Fallback(contents) => parse_plugin_manifest(
            source_path,
            &source_path.join(".atlas-agent-plugin/plugin.json"),
            contents,
        )
...
fn replace_plugin_root_atomically(
    ...
    if let InstallManifest::Fallback(contents) = manifest {
        ...
        let manifest_path = staged_version_root.join(".atlas-agent-plugin/plugin.json");
