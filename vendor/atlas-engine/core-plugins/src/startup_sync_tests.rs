...
fn git_command_sanitizes_ambient_repository_environment() {
    ...
    assert_eq!(
        ...
        [
            OsStr::new("-c"),
            OsStr::new(atlas_engine_git_utils::SAFE_BARE_REPOSITORY_CONFIG),
        ]
    );
...
async fn ordinary_clone_rejects_tracked_embedded_bare_repository() {
    ...
    run_setup_git(
        ...
        &[
            "-c",
            "user.name=Atlas Agent Tests",
            "-c",
            "user.email=atlas-engine-tests@example.com",
            "commit",
            "--quiet",
            "-m",
...
fn write_curated_plugin_sha(atlas_agent_home: &Path) {
    write_file(
...
fn has_plugins_clone_dirs(atlas_agent_home: &Path) -> bool {
    let Ok(entries) = std::fs::read_dir(atlas_agent_home.join(".tmp")) else {
        return false;
    };
...
fn assert_curated_gmail_repo(repo_path: &Path) {
    ...
    assert!(
        repo_path
            .join("plugins/gmail/.atlas-agent-plugin/plugin.json")
            .is_file()
    );
}

#[test]
fn curated_plugins_repo_path_uses_atlas_agent_home_tmp_dir() {
...
fn sync_openai_plugins_repo_via_git_succeeds_with_local_rewritten_remote() {
    ...
    std::fs::create_dir_all(work_repo.join("plugins/gmail/.atlas-agent-plugin"))
    ...
    std::fs::write(
        work_repo.join("plugins/gmail/.atlas-agent-plugin/plugin.json"),
        r#"{"name":"gmail"}"#,
    )
    ...
    run_git(
        ...
        &[
            "-c",
            "user.name=Atlas Agent Test",
            "-c",
            "user.email=atlas-agent@example.com",
            "commit",
            "-m",
            "init",
        ],
    ...
    assert!(
        curated_plugins_repo_path(tmp.path())
            .join("plugins/linear/.atlas-agent-plugin/plugin.json")
            .is_file()
    );
    assert_eq!(
        read_curated_plugins_sha(tmp.path()).as_deref(),
...
if [ "$1" = "-C" ] && [ "$3" = "reset" ]; then
  mkdir -p "$2/plugins/linear/.atlas-agent-plugin"
  printf '%s\n' '{{"name":"linear"}}' > "$2/plugins/linear/.atlas-agent-plugin/plugin.json"
  exit 0
fi
if [ "$1" = "-C" ] && [ "$3" = "clean" ]; then
  exit 0
...
async fn sync_openai_plugins_repo_skips_export_archive_when_snapshot_exists() {
    ...
    let plugin_manifest_path = curated_root.join("plugins/linear/.atlas-agent-plugin/plugin.json");
...
fn curated_repo_zipball_bytes(sha: &str) -> Vec<u8> {
  ...
  "plugins": [
    ...
    writer
        .start_file(
            format!("{root}/plugins/gmail/.atlas-agent-plugin/plugin.json"),
            options,
        )
