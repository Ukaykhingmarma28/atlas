...
#[tokio::test]
async fn returns_fallback_plugins_when_remote_disabled_for_atlas_engine_auth() {
    let atlas_agent_home = tempdir().expect("tempdir should succeed");
    write_file(
...
async fn returns_api_curated_fallback_plugins_for_direct_provider_auth() {
    ...
    let auth = AtlasEngineAuth::from_api_key("test-api-key");
    let discoverable_plugins = list_discoverable_plugins(
        &plugins_manager,
        discovery_input(plugins, &[], &[], &[]),
        Some(&auth),
...
async fn returns_microsoft_fallback_plugins() {
    ...
    install_marketplace_plugin(atlas_agent_home.path(), curated_root.as_path(), "teams").await;
...
#[tokio::test]
async fn omits_openai_curated_but_keeps_configured_marketplaces_for_remote_atlas_engine_auth() {
    let atlas_agent_home = tempdir().expect("tempdir should succeed");
    let curated_root = curated_plugins_repo_path(atlas_agent_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);

    let bundled_marketplace_name = OPENAI_BUNDLED_MARKETPLACE_NAME;
    let bundled_marketplace_root = atlas_agent_home
        .path()
...
async fn clear_cache_invalidates_cached_tool_suggest_metadata() {
    ...
    let curated_root = curated_plugins_repo_path(atlas_agent_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);
    let plugin_manifest = curated_root.join("plugins/slack/.atlas-agent-plugin/plugin.json");
    write_file(
        &plugin_manifest,
        r#"{
  "name": "slack",
...
"#
    ...
    install_marketplace_plugin(atlas_agent_home.path(), curated_root.as_path(), "installed").await;
...
async fn normalizes_description() {
    ...
    write_openai_curated_marketplace(&curated_root, &["installed", "slack"]);
    write_file(
        &curated_root.join("plugins/slack/.atlas-agent-plugin/plugin.json"),
        r#"{
  "name": "slack",
  "description": "  Plugin\n   with   extra   spacing  "
}"#,
...
async fn omits_installed_curated_plugins() {
    ...
    let curated_root = curated_plugins_repo_path(atlas_agent_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);
    install_marketplace_plugin(atlas_agent_home.path(), curated_root.as_path(), "slack").await;
...
async fn does_not_expand_local_sales_apps() {
    ...
    let sales_marketplace_root = atlas_agent_home
        .path()
...
"#,
    ...
    let auth = AtlasEngineAuth::create_dummy_chatgpt_auth_for_testing();
    let mut plugins = load_plugins_config(atlas_agent_home.path(), atlas_agent_home.path()).await;
