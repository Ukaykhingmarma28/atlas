...
pub(crate) fn maybe_start_remote_installed_plugin_bundle_sync(
    ...
    let key = RemoteInstalledPluginBundleSyncKey {
        plugin_cache_root: remote_plugin_cache_root(&atlas_agent_home),
    };
    ...
    tokio::spawn(async move {
        let result =
            sync_remote_installed_plugin_bundles_once(atlas_agent_home, &config, Some(&auth)).await;
        match result {
            Ok(outcome) => {
                info!(
...
fn remote_plugin_cache_root(atlas_agent_home: &Path) -> PathBuf {
    atlas_agent_home.join(PLUGINS_CACHE_DIR)
}
...
mod tests {
    ...
    fn remote_installed_plugin_sync_in_flight_dedupes_by_cache_root() {
        ...
        let key = RemoteInstalledPluginBundleSyncKey {
            plugin_cache_root: remote_plugin_cache_root(atlas_agent_home.path()),
        };
    ...
    async fn sync_same_version_backfills_metadata_without_materialization() {
        ...
        let atlas_agent_home = tempfile::tempdir().expect("create atlas-agent home");
        let cached_manifest = atlas_agent_home
            .path()
        ...
        let metadata_path = PluginStore::new(atlas_agent_home.path().to_path_buf())
            .plugin_base_root(&plugin_id)
            .join(".atlas-agent-remote-plugin-install.json");
        assert_eq!(
    ...
    async fn sync_all_scopes_paginates_and_reconciles_each_marketplace() {
        ...
        for (marketplace_name, plugin_name, _, _) in cached_plugins {
            ...
            assert!(
                plugin_root
                    .join("1.2.3/.atlas-agent-plugin/plugin.json")
                    .is_file()
            );
            assert_eq!(
                serde_json::from_str::<serde_json::Value>(
    ...
    fn stale_remote_plugin_cleanup_removes_stale_marketplace_caches_and_keeps_canonical_cache() {
        let atlas_agent_home = tempfile::tempdir().expect("create atlas-agent home");
        let created_by_me_cached_manifest = atlas_agent_home
            .path()
            .join(PLUGINS_CACHE_DIR)
        ...
        std::fs::write(&cached_manifest, r#"{"name":"private-plugin"}"#)
            .expect("write cached plugin manifest");
        let canonical_cached_manifest = atlas_agent_home
            .path()
            .join(PLUGINS_CACHE_DIR)
