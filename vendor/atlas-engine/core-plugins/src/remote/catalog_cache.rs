...
pub(crate) fn load_cached_directory_plugins(
    ...
    let cache_path = cache_path(atlas_agent_home, &cache_key);
...
pub(crate) fn write_cached_directory_plugins(
    ...
    let _ = atlas_engine_utils_path::write_atomically(&cache_path, &contents);
}
...
fn cache_path(atlas_agent_home: &Path, cache_key: &RemotePluginCatalogCacheKey) -> PathBuf {
