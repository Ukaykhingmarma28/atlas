...
use atlas_engine_utils_plugins::AGENT_PLUGIN_SCHEMA_PREFIX;
use atlas_engine_utils_plugins::AGENT_PLUGIN_SCHEMA_URI;
use atlas_engine_utils_plugins::SUPPORTED_AGENT_PLUGIN_SCHEMA_URIS;
use serde::Deserialize;
use serde_json::Value as JsonValue;

const ATLAS_AGENT_AGENT_PLUGIN_EXTENSION_NAMESPACE: &str = "com.openai";
...
pub(super) fn parse_agent_plugin_manifest_uri(
    ...
    let atlas_engine_extension = object
        .get("extensions")
        .and_then(JsonValue::as_object)
        .and_then(|extensions| extensions.get(ATLAS_AGENT_AGENT_PLUGIN_EXTENSION_NAMESPACE))
        .and_then(|extension| {
            if extension.is_object() {
                ...
                tracing::warn!(
                    path = %manifest_path,
                    namespace = ATLAS_AGENT_AGENT_PLUGIN_EXTENSION_NAMESPACE,
    ...
    if let Some(extension_contents) = atlas_engine_extension.as_deref() {
        apply_atlas_engine_agent_plugin_extension(
            &mut resolved,
            plugin_root,
            manifest_path,
            extension_contents,
...
fn apply_atlas_engine_agent_plugin_extension(
