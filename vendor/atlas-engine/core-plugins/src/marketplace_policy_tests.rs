...
fn strict_install_validates_configured_name_source_and_root() {
    ...
    let config_file = AbsolutePathBuf::try_from(atlas_agent_home.path().join("config.toml"))
...
"#
    ...
    assert_eq!(
        policy.validate_install(&stack, atlas_agent_home.path(), &configured_path, "company"),
        Ok(())
    );
    assert!(
        policy
            .validate_install(&stack, atlas_agent_home.path(), &configured_path, "other")
            .expect_err("unconfigured name should fail")
            .contains("must be added to config")
    );
    assert!(
        policy
            .validate_install(&stack, atlas_agent_home.path(), &other_path, "company")
...
"#,
    ...
    let err = MarketplacePolicy::from_requirements(stack.requirements())
        .validate_install(&stack, atlas_agent_home.path(), &marketplace_path, "debug")
...
"#,
    ...
    let marketplace_path = AbsolutePathBuf::try_from(
        curated_plugins_repo_path(atlas_agent_home.path()).join(".agents/plugins/marketplace.json"),
    )
    .expect("absolute marketplace path");
    let policy = MarketplacePolicy::from_requirements(stack.requirements());
...
fn managed_bundled_source_is_bound_to_its_expected_name() {
    let atlas_agent_home = TempDir::new().expect("create Atlas Agent home");
    let bundled_root = atlas_agent_home
        .path()
...
"#,
    ...
    let expected_name =
        validate_marketplace_source_for_add(atlas_agent_home.path(), stack.requirements(), &source)
...
"#,
    ...
    assert_eq!(
        configured_plugins_from_stack(&stack, atlas_agent_home.path())
            .into_keys()
            .collect::<Vec<_>>(),
...
"#,
    ...
    assert!(!marketplace_install_root(atlas_agent_home.path()).exists());
}
