...
pub async fn add_marketplace(
    ...
    tokio::task::spawn_blocking(move || {
        add_marketplace_sync(atlas_agent_home.as_path(), &requirements, request)
    })
    .await
...
fn add_marketplace_sync(
    ...
    add_marketplace_sync_with_cloner(atlas_agent_home, requirements, request, clone_git_source)
}
    ...
    F: Fn(&str, Option<&str>, &[String], &Path) -> Result<(), MarketplaceAddError>,
{
    ...
    let managed_marketplace_name =
        validate_marketplace_source_for_add(atlas_agent_home, requirements, &source)
    ...
    let install_root = marketplace_install_root(atlas_agent_home);
    ...
    if let Some(existing_root) =
        installed_marketplace_root_for_source(atlas_agent_home, &install_root, &install_metadata)?
    {
        ...
        validate_marketplace_name_for_add(managed_marketplace_name, &marketplace_name)
            .map_err(MarketplaceAddError::InvalidRequest)?;
        record_added_marketplace_entry(atlas_agent_home, &marketplace_name, &install_metadata)?;
...
"#,
    ...
    fn write_marketplace_source(source: &Path, marker: &str) -> std::io::Result<()> {
        ...
        fs::create_dir_all(source.join("plugins/sample/.atlas-agent-plugin"))?;
        fs::write(
  ...
  "plugins": [
    {
      ...
      "source": {
        ...
        fs::write(
            source.join("plugins/sample/.atlas-agent-plugin/plugin.json"),
            r#"{"name":"sample"}"#,
        )?;
