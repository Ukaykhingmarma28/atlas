...
use crate::loader::configured_curated_plugin_ids_from_atlas_agent_home;
...
use atlas_engine_config::clear_user_plugin;
use atlas_engine_config::set_user_plugin_enabled;
...
pub struct RecommendedPluginCandidatesInput<'a> {
    ...
    pub auth: Option<&'a AtlasEngineAuth>,
...
fn featured_plugin_ids_cache_key(
    ...
    FeaturedPluginIdsCacheKey {
        ...
        account_id: auth.and_then(AtlasEngineAuth::get_account_id),
        chatgpt_user_id: auth.and_then(AtlasEngineAuth::get_chatgpt_user_id),
        is_workspace_account: auth.is_some_and(AtlasEngineAuth::is_workspace_account),
    }
}
...
fn target_curated_marketplace(auth_mode: Option<AuthMode>) -> TargetCuratedMarketplace {
    if auth_mode.is_some_and(AuthMode::uses_atlas_engine_backend) {
...
impl PluginsManager {
    ...
    pub fn new_with_options(
        ...
        // Product restrictions are enforced at marketplace admission time for a given ATLAS_AGENT_HOME:
        ...
        // contents of that ATLAS_AGENT_HOME and does not re-filter configured plugins by product, so
        // already-admitted plugins may continue exposing MCP servers/tools from shared local state.
        //
        // This assumes a single ATLAS_AGENT_HOME is only used by one product.
        Self {
            atlas_agent_home: atlas_agent_home.clone(),
            store: PluginStore::new(atlas_agent_home),
    ...
    async fn install_resolved_plugin(
        ...
        let result: StorePluginInstallResult = tokio::task::spawn_blocking(move || {
            let materialized =
                materialize_marketplace_plugin_source(atlas_agent_home.as_path(), &resolved.source)
    ...
    async fn uninstall_plugin_id(&self, plugin_id: PluginId) -> Result<(), PluginUninstallError> {
        ...
        clear_user_plugin(&self.atlas_agent_home, plugin_id.as_key())
            .await
            .map_err(anyhow::Error::from)?;
    ...
    pub async fn read_plugin_detail_for_marketplace_plugin(
        ...
        let source_path = if plugin.source.is_install_materialized() && plugin.installed {
            ...
            let materialized = tokio::task::spawn_blocking(move || {
                materialize_marketplace_plugin_source(atlas_agent_home.as_path(), &source)
            })
            .await
            .map_err(|err| {
                MarketplaceError::InvalidPlugin(format!(
        ...
        if config.plugins_enabled {
            ...
            tokio::spawn(async move {
                ...
                let mut scopes = crate::remote::cached_remote_plugin_catalog_scopes(
                    manager.atlas_agent_home.as_path(),
            ...
            match target_curated_marketplace(self.auth_mode()) {
                ...
                TargetCuratedMarketplace::OpenAiApi => {
                    let api_marketplace_path =
                        curated_plugins_api_marketplace_path(self.atlas_agent_home.as_path());
