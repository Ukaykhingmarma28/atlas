...
//! Internal `atlas_engine.exe --run-as-windows-sandbox` wrapper.
//!
...
pub const ATLAS_AGENT_WINDOWS_SANDBOX_ARG1: &str = "--run-as-windows-sandbox";

const COMMAND_CWD_FLAG: &str = "--command-cwd";
const ATLAS_AGENT_HOME_FLAG: &str = "--atlas-engine-home";
...
pub fn create_windows_sandbox_command_args_for_permission_profile(
    ...
    let mut args = vec![
        ATLAS_AGENT_WINDOWS_SANDBOX_ARG1.to_string(),
        ATLAS_AGENT_HOME_FLAG.to_string(),
        atlas_agent_home.to_string_lossy().into_owned(),
...
async fn run_windows_sandbox_wrapper_request(request: WindowsSandboxWrapperRequest) -> Result<i32> {
    ...
    let spawned =
        crate::spawn_windows_sandbox_session_for_level(crate::WindowsSandboxSessionRequest {
            ...
            atlas_agent_home: request.atlas_agent_home.as_path(),
