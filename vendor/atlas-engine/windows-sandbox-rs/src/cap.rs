...
pub struct CapSids {
    ...
    /// apply per-workspace denies (e.g. protect `CWD/.atlas_engine`)
...
pub fn cap_sid_file(atlas_agent_home: &Path) -> PathBuf {
    atlas_agent_home.join("cap_sid")
}
...
pub fn load_or_create_cap_sids(atlas_agent_home: &Path) -> Result<CapSids> {
    let path = cap_sid_file(atlas_agent_home);
    if path.exists() {
...
pub fn workspace_cap_sid_for_cwd(atlas_agent_home: &Path, cwd: &Path) -> Result<String> {
    let path = cap_sid_file(atlas_agent_home);
    let mut caps = load_or_create_cap_sids(atlas_agent_home)?;
...
pub fn writable_root_cap_sid_for_path(atlas_agent_home: &Path, root: &Path) -> Result<String> {
    let path = cap_sid_file(atlas_agent_home);
    let mut caps = load_or_create_cap_sids(atlas_agent_home)?;
...
pub fn workspace_write_cap_sid_for_root(
    ...
    if canonical_path_key(root) == canonical_path_key(cwd) {
        workspace_cap_sid_for_cwd(atlas_agent_home, cwd)
    } else {
        writable_root_cap_sid_for_path(atlas_agent_home, root)
    }
}
