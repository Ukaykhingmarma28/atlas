...
pub(crate) const RESOURCES_DIRNAME: &str = "atlas-engine-resources";
...
impl HelperExecutable {
    fn file_name(self) -> &'static str {
        match self {
            Self::CommandRunner => "atlas-engine-command-runner.exe",
        }
    }
...
pub(crate) fn helper_bin_dir(atlas_agent_home: &Path) -> PathBuf {
    sandbox_bin_dir(atlas_agent_home)
}
...
pub(crate) fn resolve_helper_for_launch(
    ...
    match copy_helper_if_needed(kind, atlas_agent_home, log_dir) {
        Ok(path) => {
            log_note(
                &format!(
...
pub fn resolve_current_exe_for_launch(atlas_agent_home: &Path, fallback_executable: &str) -> PathBuf {
    ...
    resolve_exe_for_launch(&source, atlas_agent_home)
}

pub fn resolve_exe_for_launch(source: &Path, atlas_agent_home: &Path) -> PathBuf {
    let Some(file_name) = source.file_name() else {
        return source.to_path_buf();
    };
    let destination = helper_bin_dir(atlas_agent_home).join(file_name);
    match copy_from_source_if_needed(source, &destination) {
        Ok(_) => destination,
        Err(err) => {
            let sandbox_log_dir = crate::sandbox_dir(atlas_agent_home);
            log_note(
                &format!(
...
pub(crate) fn copy_helper_if_needed(
    ...
    let cache_key = format!("{}|{}", kind.file_name(), atlas_agent_home.display());
    ...
    let destination = helper_destination_for_source(kind, atlas_agent_home, &source)?;
    log_note(
        &format!(
...
fn helper_destination_for_source(
    ...
    Ok(helper_bin_dir(atlas_agent_home).join(file_name))
}
...
mod tests {
    ...
    fn copy_runner_into_shared_bin_dir() {
        ...
        let runner_source = source_dir.join("atlas-engine-command-runner.exe");
        fs::write(&runner_source, b"runner").expect("runner");
        let runner_suffix = helper_version_suffix(&runner_source).expect("runner suffix");
        let runner_destination = helper_bin_dir(&atlas_agent_home).join(materialized_file_name(
    ...
    fn helper_source_lookup_checks_resource_dir() {
        ...
        let exe = release_dir.join("atlas_agent.exe");
        let helper = resources_dir.join("atlas-engine-command-runner.exe");
    ...
    fn helper_source_lookup_prefers_package_resource_dir_over_bin_resource_dir() {
        ...
        let exe = bin_dir.join("atlas_agent.exe");
        let package_helper = package_resources_dir.join("atlas-engine-command-runner.exe");
        let bin_helper = bin_resources_dir.join("atlas-engine-command-runner.exe");
        fs::write(&exe, b"atlas-agent").expect("write exe");
    ...
    fn helper_source_lookup_prefers_direct_sibling_over_resource_dir() {
        ...
        fs::create_dir_all(&resources_dir).expect("create resources dir");
        let exe = release_dir.join("atlas_agent.exe");
        let sibling_helper = release_dir.join("atlas-engine-command-runner.exe");
        let resource_helper = resources_dir.join("atlas-engine-command-runner.exe");
        fs::write(&exe, b"atlas-agent").expect("write exe");
    ...
    fn materialized_file_name_adds_suffix_before_extension() {
        ...
        assert_eq!(file_name, "atlas-engine-command-runner-test-suffix.exe");
    }
}
