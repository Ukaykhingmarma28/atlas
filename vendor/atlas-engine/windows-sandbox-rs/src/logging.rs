...
pub fn current_log_file_path_for_atlas_agent_home(atlas_agent_home: &Path) -> PathBuf {
    current_log_file_path(&crate::sandbox_dir(atlas_agent_home))
}
...
mod tests {
    ...
    #[test]
    fn current_log_file_path_for_atlas_agent_home_uses_sandbox_dir() {
        let atlas_agent_home = Path::new("atlas-engine-home");

        assert_eq!(
            current_log_file_path_for_atlas_agent_home(atlas_agent_home),
            current_log_file_path(&atlas_agent_home.join(".sandbox"))
        );
    }
}
