...
use crate::sandbox_utils::ensure_atlas_agent_home_exists;
...
use crate::workspace_acl::protect_workspace_atlas_engine_dir;
use anyhow::Context;
use anyhow::Result;
...
fn prepare_spawn_context_common(
    ...
    ensure_atlas_agent_home_exists(atlas_agent_home)?;
    let sandbox_base = atlas_agent_home.join(".sandbox");
...
pub(crate) fn prepare_legacy_session_security(
    ...
    let caps = load_or_create_cap_sids(atlas_agent_home)?;
    let (h_token, readonly_sid, readonly_sid_str, write_root_sids) = unsafe {
        if uses_write_capabilities {
            let write_root_sids = root_capability_sids(atlas_agent_home, cwd, capability_roots)?;
...
pub(crate) fn root_capability_sids(
    ...
    for root in roots {
        let sid_str = workspace_write_cap_sid_for_root(atlas_agent_home, cwd, &root)?;
    ...
    unsafe {
                ...
                matching_root_capability(current_dir, acl_sids.write_root_sids)
        {
            ...
            if is_command_cwd_root(&workspace_sid.root, &canonical_cwd) {
                let _ = protect_workspace_atlas_engine_dir(current_dir, workspace_sid.sid.as_ptr());
...
pub(crate) fn prepare_elevated_spawn_context_for_permissions(
    ...
    let sandbox_base = atlas_agent_home.join(".sandbox");
    ensure_atlas_agent_home_exists(&sandbox_base)?;
    ...
    let (psid_to_use, cap_sids) = if uses_write_capabilities {
        let cap_sids = root_capability_sids(atlas_agent_home, cwd, effective_write_roots)?
            .into_iter()
...
mod tests {
    ...
    fn legacy_capability_roots_use_effective_write_roots() {
        ...
        let workspace = temp.path().join("workspace");
        let active_root = temp.path().join("active-root");
        let sandbox_root = atlas_agent_home.join(".sandbox");
        std::fs::create_dir_all(&atlas_agent_home).expect("create atlas-agent home");
        std::fs::create_dir_all(&workspace).expect("create workspace");
        std::fs::create_dir_all(&active_root).expect("create active root");
        ...
        let writable_roots = vec![
            ...
            AbsolutePathBuf::try_from(atlas_agent_home.as_path()).expect("atlas-agent home"),
