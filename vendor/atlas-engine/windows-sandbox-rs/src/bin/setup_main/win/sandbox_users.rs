...
use atlas_engine_windows_sandbox::dpapi_protect;
...
pub const SANDBOX_USERS_GROUP: &str = "AtlasEngineSandboxUsers";
const SANDBOX_USERS_GROUP_COMMENT: &str = "Atlas Agent sandbox internal group (managed)";
...
fn write_secrets(
    ...
    let secrets_dir = sandbox_secrets_dir(atlas_agent_home);
...
pub(super) fn prepare_setup_marker(atlas_agent_home: &Path, real_user: &str) -> Result<()> {
    let marker_path = sandbox_dir(atlas_agent_home).join("setup_marker.json");
