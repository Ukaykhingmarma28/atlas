...
use atlas_engine_windows_sandbox::SetupFailure;
use atlas_engine_windows_sandbox::add_deny_write_ace;
use atlas_engine_windows_sandbox::convert_string_sid_to_sid;
use atlas_engine_windows_sandbox::ensure_allow_mask_aces_with_inheritance;
use atlas_engine_windows_sandbox::ensure_allow_write_aces;
use atlas_engine_windows_sandbox::extract_setup_failure;
use atlas_engine_windows_sandbox::hide_newly_created_users;
use atlas_engine_windows_sandbox::install_wfp_filters;
use atlas_engine_windows_sandbox::log_note;
use atlas_engine_windows_sandbox::log_writer;
use atlas_engine_windows_sandbox::path_mask_allows;
use atlas_engine_windows_sandbox::path_write_aces_need_refresh;
use atlas_engine_windows_sandbox::sandbox_bin_dir;
use atlas_engine_windows_sandbox::sandbox_dir;
use atlas_engine_windows_sandbox::sandbox_secrets_dir;
use atlas_engine_windows_sandbox::string_from_sid_bytes;
use atlas_engine_windows_sandbox::sync_persistent_deny_read_acls;
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(atlas_agent_home) = std::env::var("ATLAS_AGENT_HOME") {
            let sbx_dir = sandbox_dir(Path::new(&atlas_agent_home));
...
fn real_main() -> Result<()> {
    ...
    let sbx_dir = sandbox_dir(&payload.atlas_agent_home);
    ...
    if let Err(err) = &result {
        ...
        if let Err(write_err) = write_setup_error_report(&payload.atlas_agent_home, &report) {
            let _ = log_line(
                &mut log,
...
fn run_setup(payload: &Payload, log: &mut dyn Write, sbx_dir: &Path) -> Result<()> {
    ...
    if writes_setup_marker {
        prepare_setup_marker(&payload.atlas_agent_home, &payload.real_user)?;
    }
    match payload.mode {
...
fn lock_persistent_sandbox_dirs(
    ...
    lock_sandbox_dir(
        &sandbox_dir(&payload.atlas_agent_home),
        &payload.real_user,
        sandbox_group_sid,
        GRANT_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_dir(&payload.atlas_agent_home).display()
            ),
        ))
    })?;
    lock_sandbox_dir(
        &sandbox_secrets_dir(&payload.atlas_agent_home),
        &payload.real_user,
        sandbox_group_sid,
        DENY_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_secrets_dir(&payload.atlas_agent_home).display()
            ),
        ))
    })?;
    let legacy_users = sandbox_dir(&payload.atlas_agent_home).join("sandbox_users.json");
...
fn lock_sandbox_bin_dir(
    ...
    lock_sandbox_dir(
        &sandbox_bin_dir(&payload.atlas_agent_home),
        &payload.real_user,
        sandbox_group_sid,
        GRANT_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_EXECUTE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_bin_dir(&payload.atlas_agent_home).display()
            ),
        ))
    })
}
...
fn run_setup_full(payload: &Payload, log: &mut dyn Write, sbx_dir: &Path) -> Result<()> {
    ...
    if refresh_only {
        setup_runtime_bin::ensure_atlas_engine_app_runtime_paths_readable(
            sandbox_group_psid,
            &mut refresh_errors,
            log,
        )?;
    ...
    for path in &payload.deny_write_paths {
        ...
        // legacy protected children such as `.git`, `.atlas_engine`, and `.agents`.
        //
...
mod tests {
    ...
    fn write_root_refresh_replaces_stale_delete_child_grant() {
        ...
        let atlas_agent_home = temp.path().join("atlas-engine-home");
        let workspace = temp.path().join("workspace");
        fs::create_dir_all(&atlas_agent_home).expect("create atlas-agent home");
        fs::create_dir_all(&workspace).expect("create workspace");

        let sid = workspace_write_cap_sid_for_root(&atlas_agent_home, &workspace, &workspace)
    ...
    fn deny_path_includes_nested_active_root_sid() {
        ...
        let atlas_agent_home = temp.path().join("atlas-engine-home");
        let workspace = temp.path().join("workspace");
        let protected_dir = workspace.join(".atlas-agent");
