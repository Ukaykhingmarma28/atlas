...
use atlas_engine_windows_sandbox::ConsoleMode;
use atlas_engine_windows_sandbox::ErrorPayload;
use atlas_engine_windows_sandbox::ErrorStage;
use atlas_engine_windows_sandbox::ExitPayload;
use atlas_engine_windows_sandbox::FramedMessage;
use atlas_engine_windows_sandbox::IPC_PROTOCOL_VERSION;
use atlas_engine_windows_sandbox::LocalSid;
use atlas_engine_windows_sandbox::Message;
use atlas_engine_windows_sandbox::OutputPayload;
use atlas_engine_windows_sandbox::OutputStream;
use atlas_engine_windows_sandbox::PipeSpawnHandles;
use atlas_engine_windows_sandbox::ResizePayload;
use atlas_engine_windows_sandbox::SpawnReady;
use atlas_engine_windows_sandbox::SpawnRequest;
use atlas_engine_windows_sandbox::StderrMode;
use atlas_engine_windows_sandbox::StdinMode;
use atlas_engine_windows_sandbox::WindowsSandboxTokenMode;
use atlas_engine_windows_sandbox::allow_null_device;
use atlas_engine_windows_sandbox::create_readonly_token_with_caps_and_user_from;
use atlas_engine_windows_sandbox::create_workspace_write_token_with_caps_and_user_from;
use atlas_engine_windows_sandbox::decode_bytes;
use atlas_engine_windows_sandbox::encode_bytes;
use atlas_engine_windows_sandbox::get_current_token_for_restriction;
use atlas_engine_windows_sandbox::hide_current_user_profile_dir;
use atlas_engine_windows_sandbox::log_note;
use atlas_engine_windows_sandbox::read_frame;
use atlas_engine_windows_sandbox::read_handle_loop;
use atlas_engine_windows_sandbox::spawn_process_with_pipes;
use atlas_engine_windows_sandbox::to_wide;
use atlas_engine_windows_sandbox::token_mode_for_permission_profile;
use atlas_engine_windows_sandbox::write_frame;
use std::ffi::OsStr;
use std::fs::File;
...
// Kept in sync with atlas_engine_exec_server::ATLAS_AGENT_FS_HELPER_ARG1 without introducing
// a dependency cycle.
const FS_HELPER_ARG: &str = "--atlas-engine-run-as-fs-helper";
const READ_ACL_MUTEX_NAME: &str = "Local\\AtlasEngineSandboxReadAcl";
...
struct IpcSpawnedProcess {
    ...
    conpty_owner: Option<atlas_engine_windows_sandbox::ConptyInstance>,
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    let log_dir = req.atlas_agent_home.clone();
    hide_current_user_profile_dir(req.atlas_agent_home.as_path());
    ...
    let (pi, job, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, mut conpty) = atlas_engine_windows_sandbox::spawn_conpty_process_as_user(
            h_token.raw(),
            &req.command,
            &effective_cwd,
            &req.env,
