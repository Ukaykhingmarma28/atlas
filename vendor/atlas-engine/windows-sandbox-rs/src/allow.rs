...
mod tests {
    ...
    #[test]
    fn denies_atlas_engine_and_agents_inside_writable_root() {
        let tmp = TempDir::new().expect("tempdir");
        let command_cwd = tmp.path().join("workspace");
        let atlas_engine_dir = command_cwd.join(".atlas-agent");
        let agents_dir = command_cwd.join(".agents");
        let _ = fs::create_dir_all(&atlas_engine_dir);
        ...
        let expected_deny: HashSet<PathBuf> = [
            dunce::canonicalize(&atlas_engine_dir).unwrap(),
