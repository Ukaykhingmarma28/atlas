...
pub const OFFLINE_USERNAME: &str = "AtlasEngineSandboxOffline";
pub const ONLINE_USERNAME: &str = "AtlasEngineSandboxOnline";
const ERROR_CANCELLED: u32 = 1223;
const SECURITY_BUILTIN_DOMAIN_RID: u32 = 0x0000_0020;
const DOMAIN_ALIAS_RID_ADMINS: u32 = 0x0000_0220;
const SETUP_EXE_FILENAME: &str = "atlas-engine-windows-sandbox-setup.exe";
...
pub fn sandbox_dir(atlas_agent_home: &Path) -> PathBuf {
    atlas_agent_home.join(".sandbox")
}

pub fn sandbox_bin_dir(atlas_agent_home: &Path) -> PathBuf {
    atlas_agent_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(atlas_agent_home: &Path) -> PathBuf {
    atlas_agent_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(atlas_agent_home: &Path) -> PathBuf {
    sandbox_dir(atlas_agent_home).join("setup_marker.json")
}

pub fn sandbox_users_path(atlas_agent_home: &Path) -> PathBuf {
    sandbox_secrets_dir(atlas_agent_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let mut read_roots = gather_read_roots(command_cwd, &permissions, env_map, atlas_agent_home);
...
fn run_setup_refresh_inner(
    ...
    let payload = ElevationPayload {
        ...
        atlas_agent_home: request.atlas_agent_home.to_path_buf(),
    ...
    run_setup_singleflight(b64.clone(), || {
        run_setup_refresh_payload(&b64, request.atlas_agent_home)
    })
}

fn run_setup_refresh_payload(b64: &str, atlas_agent_home: &Path) -> Result<()> {
    let exe = find_setup_exe();
    let sbx_dir = sandbox_dir(atlas_agent_home);
    let log_path = current_log_file_path(&sbx_dir);
    let cleared_report = match clear_setup_error_report(atlas_agent_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| atlas_agent_home.to_path_buf());
    log_note(
        &format!(
    ...
    if let Err(err) = clear_setup_error_report(atlas_agent_home) {
        log_note(
...
fn gather_helper_read_roots(atlas_agent_home: &Path) -> Vec<PathBuf> {
    let helper_dir = helper_bin_dir(atlas_agent_home);
...
fn gather_full_read_roots_for_permissions(
    ...
    let mut roots = gather_helper_read_roots(atlas_agent_home);
    roots.extend(
...
pub(crate) fn effective_write_roots_for_permissions(
    ...
    filter_sensitive_write_roots(write_roots, atlas_agent_home)
}
...
const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "ATLAS_AGENT_NETWORK_ALLOW_LOCAL_BINDING";
...
const WINDOWS_SANDBOX_PROXY_PORTS_ENV_KEY: &str = "ATLAS_AGENT_WINDOWS_SANDBOX_PROXY_PORTS";
...
fn report_helper_failure(
    ...
    match read_setup_error_report(atlas_agent_home) {
...
fn verify_setup_completed(atlas_agent_home: &Path) -> Result<()> {
    if sandbox_setup_is_complete(atlas_agent_home) {
        Ok(())
    } else {
        Err(failure(
...
fn run_setup_exe(
    ...
    run_setup_singleflight(payload_b64.clone(), || {
        run_setup_exe_payload(&payload_b64, needs_elevation, atlas_agent_home)
    })
}
...
fn run_setup_exe_payload(
    ...
    let cleared_report = match clear_setup_error_report(atlas_agent_home) {
        ...
        Err(err) => {
            log_note(
                ...
                Some(&sandbox_dir(atlas_agent_home)),
            );
            false
        }
    };
    ...
    if !needs_elevation {
        ...
        verify_setup_completed(atlas_agent_home)?;
        if let Err(err) = clear_setup_error_report(atlas_agent_home) {
            log_note(
                &format!(
...
fn run_elevated_setup_inner(
    ...
    let sbx_dir = sandbox_dir(request.atlas_agent_home);
    ...
    run_setup_exe(&payload, needs_elevation, request.atlas_agent_home)
}
...
pub fn run_elevated_provisioning_setup(
    ...
    let payload = ElevationPayload {
        ...
        command_cwd: atlas_agent_home.to_path_buf(),
        read_roots: Vec::new(),
        write_roots: Vec::new(),
    ...
    run_setup_exe(&payload, /*needs_elevation*/ false, atlas_agent_home)
}
...
fn build_payload_roots(
    ...
    let mut read_roots = if let Some(roots) = overrides.read_roots.as_deref() {
        ...
        let mut read_roots = gather_helper_read_roots(request.atlas_agent_home);
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, atlas_agent_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to ATLAS_AGENT_HOME or anything under ATLAS_AGENT_HOME/.sandbox,
    // ATLAS_AGENT_HOME/.sandbox-bin, or ATLAS_AGENT_HOME/.sandbox-secrets. These locations contain sandbox
    // control/state and helper binaries and must remain tamper-resistant.
    let atlas_agent_home_key = canonical_path_key(atlas_agent_home);
    let sbx_dir_key = canonical_path_key(&sandbox_dir(atlas_agent_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(atlas_agent_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(atlas_agent_home));
    let secrets_dir_prefix = format!("{}/", secrets_dir_key.trim_end_matches('/'));

    roots.retain(|root| {
        let key = canonical_path_key(root);
        key != atlas_agent_home_key
            && key != sbx_dir_key
...
mod tests {
    ...
    fn setup_completion_requires_ready_artifacts() {
        let atlas_agent_home = TempDir::new().expect("tempdir");
        let err = verify_setup_completed(atlas_agent_home.path())
    ...
    fn setup_exe_lookup_checks_package_resource_dir_for_bin_exe() {
        ...
        let exe = bin_dir.join("atlas_agent.exe");
        let setup_exe = resources_dir.join("atlas-engine-windows-sandbox-setup.exe");
    ...
    fn offline_proxy_settings_ignore_proxy_env_when_online_identity_selected() {
        ...
        env.insert(
            "ATLAS_AGENT_NETWORK_ALLOW_LOCAL_BINDING".to_string(),
            "1".to_string(),
        );
    ...
    #[test]
    fn expanded_write_roots_still_drop_protected_atlas_agent_home() {
        let tmp = TempDir::new().expect("tempdir");
        let user_profile = tmp.path().join("user-profile");
        let atlas_agent_home = user_profile.join("AtlasAgentHome");
        let documents = user_profile.join("Documents");
        ...
        let roots = super::filter_sensitive_write_roots(roots, &atlas_agent_home);
    ...
    fn effective_write_roots_match_payload_filtering_for_overrides() {
        ...
        let sandbox_root = super::sandbox_dir(&atlas_agent_home);
        fs::create_dir_all(&atlas_agent_home).expect("create atlas-agent home");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        ...
        assert!(!effective_write_roots.contains(&forbidden_atlas_agent_home));
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        let extra_atlas_engine = extra_write_root.join(".atlas-agent");
        let explicit_deny = tmp.path().join("explicit-deny");
        fs::create_dir_all(&command_git).expect("create command .git");
        fs::create_dir_all(&extra_atlas_engine).expect("create extra .atlas-agent");
        let writable_roots = vec![
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_atlas_engine).expect("canonical extra .atlas-agent"),
                explicit_deny,
            ]
            .into_iter()
