...
# Run a suite of smoke tests against the Windows sandbox via the Atlas Agent CLI
...
def _resolve_atlas_engine_cmd() -> List[str]:
    """Resolve the Atlas Agent CLI to invoke `atlas_engine sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run AtlasEngine.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "atlas_agent.exe",
        ws_root / "target" / "release" / "atlas_agent.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "atlas_agent.exe",
            cargo_base / "release" / "atlas_agent.exe",
        ])
    ...
    if shutil.which("atlas-agent"):
        return ["atlas-agent"]

    raise FileNotFoundError(
        "Atlas Agent CLI not found. Build it first, e.g.\n"
        "  cargo build -p atlas-engine-cli --release\n"
        "or for debug:\n"
        "  cargo build -p atlas-engine-cli\n"
    )

ATLAS_AGENT_CMD = _resolve_atlas_engine_cmd()
print(ATLAS_AGENT_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    # Map policy to atlas-agent CLI overrides.
    ...
    argv = [*ATLAS_AGENT_CMD, "sandbox", "windows", *policy_flags, *overrides, "--", *cmd_argv]
    print(cmd_argv)
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"ATLAS_AGENT_HOME": str(proxy_home)},
            )
    ...
    rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo hi > \\\\.\\pipe\\atlas_engine_testpipe"], WS_ROOT)
    ...
    # 34. WS: policy tamper (.atlas-agent artifacts) denied
    atlas_agent_home = Path(os.environ["USERPROFILE"]) / ".atlas-agent"
    cap_sid_target = atlas_agent_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", f"echo tamper > \"{cap_sid_target}\""],
        WS_ROOT,
    )
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .atlas-agent\\policy.json"], WS_ROOT)
    add("WS: .atlas-agent cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .atlas-agent policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo owned > atlas_engine_escape.txt"], fake_root)
