...
/// Filename used for the redacted `atlas_engine doctor --json` feedback attachment.
pub const DOCTOR_REPORT_ATTACHMENT_FILENAME: &str = "atlas-engine-doctor-report.json";
...
pub fn emit_feedback_request_tags_with_auth_env(
    ...
    tracing::info!(
        ...
        auth_env_atlas_engine_api_key_present = tracing::field::debug(auth_env.atlas_engine_api_key_env_present),
        auth_env_atlas_engine_api_key_enabled = tracing::field::debug(auth_env.atlas_engine_api_key_env_enabled),
...
#[derive(Clone)]
pub struct AtlasEngineFeedback {
    inner: Arc<FeedbackInner>,
}

impl Default for AtlasEngineFeedback {
    fn default() -> Self {
        Self::new()
    }
}

impl AtlasEngineFeedback {
    pub fn new() -> Self {
...
pub struct FeedbackUploadOptions<'a> {
    ...
    ///
    /// These are included after `atlas-engine-logs.log` and before path-backed rollout
    ...
    fn feedback_attachments(
        ...
        if include_logs {
            attachments.push(Attachment {
                ...
                filename: String::from("atlas-engine-logs.log"),
...
mod tests {
    ...
    fn ring_buffer_drops_front_when_full() {
        let fb = AtlasEngineFeedback::with_capacity(/*max_bytes*/ 8);
        {
    ...
    fn logger_layer_filters_noisy_trace_payloads() {
        let fb = AtlasEngineFeedback::new();
        ...
        tracing::trace!(target: "atlas_engine_api::responses_websocket_timing", payload = "secret");
        tracing::trace!(target: "atlas_engine_http_client::transport", "transport-trace");
        tracing::trace!(target: "atlas_engine_api::sse", "sse-trace");
        tracing::trace!(target: "atlas_engine_api::sse::responses", "nested-sse-trace");
        tracing::debug!(target: "atlas_engine_http_client::transport", "transport-debug");
        tracing::debug!(target: "atlas_engine_api::sse::responses", "sse-debug");
        tracing::trace!(target: "atlas_engine_feedback_test", "unrelated-trace");
        log::trace!(target: "atlas_engine_feedback_test", "unrelated-log-trace");
        log::trace!(
    ...
    fn feedback_attachments_gate_connectivity_diagnostics() {
        let extra_filename = format!("atlas-engine-feedback-extra-{}.jsonl", ThreadId::new());
        ...
        let snapshot_with_diagnostics = AtlasEngineFeedback::new()
        ...
        let attachments_without_diagnostics = AtlasEngineFeedback::new()
            .snapshot(/*session_id*/ None)
    ...
    fn path_backed_attachments_use_binary_content_types() {
        ...
        let gzip_filename = format!("atlas-engine-desktop-app-logs-{suffix}.tar.gz");
        let unknown_filename = format!("atlas-engine-feedback-extra-{suffix}.binunknown");
        ...
        let attachments = AtlasEngineFeedback::new()
            .snapshot(/*session_id*/ None)
