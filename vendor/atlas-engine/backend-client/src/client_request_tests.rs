...
async fn migrated_requests_preserve_query_auth_and_json_body() {
    ...
    assert!(requests[0].starts_with(
        "GET /api/atlas-agent/tasks/list?limit=10&task_filter=mine+%2F+shared&cursor=next%3Dpage&environment_id=env%26one HTTP/1.1\r\n"
    ));
    assert!(
        requests[0]
            .to_ascii_lowercase()
    ...
    assert!(requests[1].starts_with("POST /api/atlas-agent/tasks HTTP/1.1\r\n"));
    assert!(
        requests[1]
            .to_ascii_lowercase()
            .contains("authorization: bearer request-token\r\n")
