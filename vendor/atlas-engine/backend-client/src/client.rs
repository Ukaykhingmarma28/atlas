...
use atlas_engine_protocol::protocol::SpendControlLimitSnapshot;
use http::Method;
use http::StatusCode;
...
impl Client {
    ...
    pub fn from_auth(
        ...
        Self::new(base_url, http_client_factory)
            .with_user_agent(get_atlas_engine_user_agent())
            .with_auth_provider(atlas_engine_model_provider::auth_provider_from_auth(auth))
    }
    ...
    fn headers(&self) -> HeaderMap {
        ...
        if let Some(ua) = &self.user_agent {
            ...
            h.insert(USER_AGENT, HeaderValue::from_static("atlas-engine-cli"));
        }
    ...
    pub async fn get_rate_limits(&self) -> Result<RateLimitSnapshot> {
        ...
        let preferred = snapshots
            .iter()
            .find(|snapshot| snapshot.limit_id.as_deref() == Some("atlas-agent"))
            .cloned();
    ...
    fn token_usage_profile_url(&self) -> String {
        match self.path_style {
            PathStyle::AtlasEngineApi => format!("{}/api/atlas-agent/profiles/me", self.base_url),
    ...
    fn list_tasks_url(
        ...
        let url = match self.path_style {
            PathStyle::AtlasEngineApi => format!("{}/api/atlas-agent/tasks/list", self.base_url),
    ...
    pub async fn list_sibling_turns(
        ...
        let url = match self.path_style {
            PathStyle::AtlasEngineApi => format!(
                "{}/api/atlas-agent/tasks/{}/turns/{}/sibling_turns",
    ...
    /// Fetch the selected cloud-managed config bundle from atlas-engine-backend.
    ///
    /// `GET /api/atlas_engine/config/bundle` (Atlas Agent API style) or
    ...
    pub async fn get_config_bundle(
        ...
        let url = match self.path_style {
            PathStyle::AtlasEngineApi => format!("{}/api/atlas-agent/config/bundle", self.base_url),
    ...
    /// Fetch authenticated Atlas Agent user settings from the active backend route.
    ///
    /// Uses `GET /api/atlas_engine/settings/user` for Atlas Agent API hosts and
    /// `GET /wham/settings/user` for ChatGPT `backend-api` hosts.
    pub async fn get_user_settings(
        &self,
    ) -> std::result::Result<AtlasEngineUserSettingsResponse, RequestError> {
        ...
        let (body, ct) = self.exec_request_detailed(req, "GET", &url).await?;
        self.decode_json::<AtlasEngineUserSettingsResponse>(&url, &ct, &body)
            .map_err(RequestError::from)
    }

    pub async fn list_workspace_messages(
        &self,
    ) -> std::result::Result<AtlasEngineWorkspaceMessagesResponse, RequestError> {
        ...
        let (body, ct) = self.exec_request_detailed(req, "GET", &url).await?;
        self.decode_json::<AtlasEngineWorkspaceMessagesResponse>(&url, &ct, &body)
            .map_err(RequestError::from)
    }
    ...
    pub async fn create_task(&self, request_body: serde_json::Value) -> Result<String> {
        let url = match self.path_style {
            PathStyle::AtlasEngineApi => format!("{}/api/atlas-agent/tasks", self.base_url),
    ...
    fn user_settings_url(&self) -> String {
        match self.path_style {
            PathStyle::AtlasEngineApi => format!("{}/api/atlas-agent/settings/user", self.base_url),
...
mod tests {
    use super::*;
    use atlas_engine_backend_openapi_models::models::AdditionalRateLimitDetails;
    use atlas_engine_backend_openapi_models::models::RateLimitReachedKind;
    use atlas_engine_backend_openapi_models::models::RateLimitReachedType as BackendRateLimitReachedType;
    ...
    fn usage_payload_maps_primary_and_additional_rate_limits() {
        let payload = RateLimitStatusPayload {
            ...
            additional_rate_limits: Some(Some(vec![AdditionalRateLimitDetails {
                limit_name: "atlas_engine_other".to_string(),
                metered_feature: "atlas_engine_other".to_string(),
                rate_limit: Some(Some(Box::new(crate::types::RateLimitStatusDetails {
                    primary_window: Some(Some(Box::new(crate::types::RateLimitWindowSnapshot {
                        used_percent: 70,
        ...
        assert_eq!(snapshots[0].limit_id.as_deref(), Some("atlas-agent"));
    ...
    fn preferred_snapshot_selection_matches_get_rate_limits_behavior() {
        let snapshots = [
            ...
            RateLimitSnapshot {
                ...
                limit_name: Some("atlas-agent".to_string()),
                primary: Some(RateLimitWindow {
                    used_percent: 10.0,
                    window_minutes: Some(60),
                    resets_at: Some(2),
        ...
        assert_eq!(preferred.limit_id.as_deref(), Some("atlas-agent"));
    }
    ...
    fn add_credits_nudge_email_uses_expected_paths_and_bodies() {
        let atlas_engine_client = test_client("https://example.test", PathStyle::AtlasEngineApi);
        assert_eq!(
            atlas_engine_client.send_add_credits_nudge_email_url(),
    ...
    fn token_usage_profile_uses_expected_paths() {
        let atlas_engine_client = test_client("https://example.test", PathStyle::AtlasEngineApi);
        assert_eq!(
            atlas_engine_client.token_usage_profile_url(),
    ...
    fn workspace_messages_uses_expected_paths() {
        let atlas_engine_client = test_client("https://example.test", PathStyle::AtlasEngineApi);
        assert_eq!(
            atlas_engine_client.workspace_messages_url(),
    ...
    async fn user_settings_request_uses_expected_paths_and_revalidates_cached_responses() {
        ...
        for (request_path, commit_attribution_enabled) in [
            ("/api/atlas-agent/settings/user", true),
        ...
        let atlas_engine_response = Client::new(
            server.uri(),
        ...
        assert_eq!(
            [atlas_engine_response, chatgpt_response],
            [
