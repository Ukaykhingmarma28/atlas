...
pub async fn spawn_process(request: SpawnRequest<'_>) -> Result<SpawnedProcess> {
    if request.sandbox == SandboxType::WindowsRestrictedToken {
        #[cfg(target_os = "windows")]
        {
            ...
            let atlas_agent_home = atlas_engine_utils_home_dir::find_atlas_agent_home()
                .context("windows sandbox: failed to resolve atlas_agent_home")?;
            let empty_paths = &[];
            let overrides = windows.filesystem_overrides;

            return atlas_engine_windows_sandbox::spawn_windows_sandbox_session_for_level(
                atlas_engine_windows_sandbox::WindowsSandboxSessionRequest {
                    permission_profile: windows.permission_profile,
                    workspace_roots: windows.workspace_roots,
                    atlas_agent_home: atlas_agent_home.as_path(),
    ...
    if request.tty {
        atlas_engine_utils_pty::pty::spawn_process(
            program,
            args,
            request.cwd,
            request.env,
        ...
        atlas_engine_utils_pty::pipe::spawn_process(
            program,
            args,
            request.cwd,
            request.env,
        ...
        atlas_engine_utils_pty::pipe::spawn_process_no_stdin(
            program,
            args,
            request.cwd,
            request.env,
