...
use crate::landlock::ATLAS_AGENT_LINUX_SANDBOX_ARG0;
...
pub struct SandboxTransformRequest<'a> {
    ...
    pub atlas_engine_linux_sandbox_exe: Option<&'a Path>,
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MissingLinuxSandboxExecutable => {
                write!(f, "missing atlas-engine-linux-sandbox executable path")
            }
...
impl SandboxManager {
    ...
    pub fn transform(
        ...
        let (argv, arg0_override, pending_sandboxed_request) = match sandbox {
            ...
            SandboxType::LinuxSeccomp => {
                let pending = pending_sandboxed_request?;
                let exe = atlas_engine_linux_sandbox_exe
    ...
    #[cfg(target_os = "windows")]
    fn transform_for_direct_spawn_with_atlas_agent_home(
        &self,
        request: SandboxDirectSpawnTransformRequest<'_>,
...
fn wrap_windows_sandbox_exec_request_for_direct_spawn(
    ...
    atlas_agent_home: &Path,
    proxy_settings_mode: atlas_engine_windows_sandbox::WindowsSandboxProxySettingsMode,
    ...
    *program = helper.to_string_lossy().into_owned();
    ...
    let mut wrapper_args =
        atlas_engine_windows_sandbox::create_windows_sandbox_command_args_for_permission_profile(
            inner_command,
            &native_cwd,
            workspace_roots,
            &request.env,
...
fn linux_sandbox_arg0_override(exe: &Path) -> String {
    if exe.file_name().and_then(|name| name.to_str()) == Some(ATLAS_AGENT_LINUX_SANDBOX_ARG0) {
        os_string_to_command_component(exe.as_os_str().to_owned())
    } else {
        ATLAS_AGENT_LINUX_SANDBOX_ARG0.to_string()
    }
}
