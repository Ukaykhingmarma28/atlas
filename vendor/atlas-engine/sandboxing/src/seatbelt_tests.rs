...
use atlas_engine_network_proxy::build_config_state;
...
use atlas_engine_protocol::permissions::PROTECTED_METADATA_PATH_NAMES;
...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    let unreadable = absolute_path("/tmp/atlas-engine-unreadable");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.atlas-agent".to_string(),
...
fn explicit_unreadable_paths_are_excluded_from_readable_roots() {
    let root = absolute_path("/tmp/atlas-engine-readable");
    let unreadable = absolute_path("/tmp/atlas-engine-readable/private");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        FileSystemSandboxEntry {
...
fn create_seatbelt_args_allowlists_explicit_unix_socket_paths_without_proxy() {
    ...
    let extra_allow_unix_sockets = vec![absolute_path("/tmp/atlas-engine-browser-use")];
    let args = create_seatbelt_command_args(CreateSeatbeltCommandArgsParams {
...
async fn create_seatbelt_args_merges_proxy_and_explicit_unix_socket_paths() -> anyhow::Result<()> {
    ...
    let network_socket = "/tmp/atlas-engine-proxy-use";
    let explicit_socket = "/tmp/atlas-engine-browser-use";
...
#[test]
fn create_seatbelt_args_with_read_only_git_and_atlas_engine_subpaths() {
    ...
    // write to .atlas-agent/config.toml in the vulnerable root.
    ...
    assert!(
        ...
        "expected explicit writable root .git/.atlas-agent carveouts in policy:\n{policy_text}",
    );
    assert!(
    ...
    let expected_definitions = [
        ...
        format!(
            ...
            dot_atlas_engine_canonical.to_string_lossy()
        ),
        format!(
            "-DWRITABLE_ROOT_2={}",
    ...
    // Verify that .atlas-agent/config.toml cannot be modified under the generated
    // Seatbelt policy.
    let config_toml = dot_atlas_engine_canonical.join("config.toml");
    ...
    // Verify that writing a file to the folder containing .git and .atlas-agent is allowed.
...
#[test]
fn create_seatbelt_args_block_first_time_dot_atlas_engine_creation_with_metadata_name_regex() {
    let tmp = TempDir::new().expect("tempdir");
    ...
    let dot_atlas_engine = repo_root.join(".atlas-agent");
    let config_toml = dot_atlas_engine.join("config.toml");
    let policy = SandboxPolicy::WorkspaceWrite {
    ...
    let shell_command: Vec<String> = [
        ...
        "bash",
        dot_atlas_engine.to_string_lossy().as_ref(),
...
fn create_seatbelt_args_for_cwd_as_git_repo() {
    ...
    let expected_dot_atlas_engine = format!(
        "-DWRITABLE_ROOT_0_EXCLUDED_1={}",
        dot_atlas_engine_canonical.to_string_lossy()
    );
    assert!(
        args.contains(&expected_dot_atlas_engine),
        "missing {expected_dot_atlas_engine}: {args:#?}"
    );
...
struct PopulatedTmp {
    ...
    /// ran `git commit` themselves, or modified .atlas-agent/config.toml to
    ...
    dot_atlas_engine_canonical: PathBuf,
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".atlas-agent")).expect("create .atlas-agent");
    fs::write(
        vulnerable_root.join(".atlas-agent").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .atlas-agent/config.toml");
    ...
    let dot_atlas_engine_canonical = vulnerable_root_canonical.join(".atlas-agent");
