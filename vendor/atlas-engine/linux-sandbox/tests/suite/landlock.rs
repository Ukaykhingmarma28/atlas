...
use atlas_engine_core::exec::process_exec_tool_call;
...
fn atlas_engine_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_atlas-engine-linux-sandbox"));
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> atlas_engine_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<atlas_engine_protocol::exec_output::ExecToolCallOutput> {
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let atlas_engine_linux_sandbox_exe = Some(atlas_engine_linux_sandbox_exe());
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        ...
        Err(err) => match err.details() {
            ...
            AtlasEngineErrorDetails::Sandbox(SandboxErr::Timeout { .. }) => true,
...
fn expect_denied(
    result: Result<atlas_engine_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> atlas_engine_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let atlas_engine_linux_sandbox_exe: Option<PathBuf> = Some(atlas_engine_linux_sandbox_exe());
...
#[tokio::test]
async fn sandbox_blocks_git_and_atlas_engine_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let dot_atlas_engine = tmpdir.path().join(".atlas-agent");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_atlas_engine).expect("create .atlas-agent");

    let git_target = dot_git.join("config");
    let atlas_engine_target = dot_atlas_engine.join("config.toml");
    ...
    let atlas_engine_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", atlas_engine_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
        ...
        .await,
        ".atlas-agent write should be denied under bubblewrap",
    );
    assert_ne!(git_output.exit_code, 0);
    assert_ne!(atlas_engine_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_atlas_engine_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let tmpdir = tempfile::tempdir().expect("tempdir");
    let decoy = tmpdir.path().join("decoy-atlas-agent");
    std::fs::create_dir_all(&decoy).expect("create decoy dir");

    let dot_atlas_engine = tmpdir.path().join(".atlas-agent");
    symlink(&decoy, &dot_atlas_engine).expect("create .atlas-agent symlink");
    ...
    let atlas_engine_output = expect_denied(
        ...
        .await,
        ".atlas-agent symlink replacement should be denied",
    );
    assert_ne!(atlas_engine_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_reports_atlas_engine_symlink_build_failure_without_panicking() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    .await
    {
        Err(err) => match err.details() {
            ...
            details => panic!(".atlas-agent symlink build failure should deny: {details:?}"),
        },
        Ok(output) => panic!(".atlas-agent symlink build failure should deny: {output:?}"),
    };
...
if grep -E '(^|[[:space:]])\.(git|atlas_engine|agents)(/|$)' status.before; then
  cat status.before
  exit 21
fi
"#,
    ...
    let mkdir_atlas_engine_output = expect_denied(
        run_cmd_result_with_cwd_and_writable_roots(
            &["mkdir", ".atlas-agent"],
            &subdir,
            std::slice::from_ref(&subdir),
        ...
        .await,
        "child .atlas-agent directory creation should be denied",
    );
    assert_ne!(mkdir_atlas_engine_output.exit_code, 0);
    assert!(!subdir.join(".atlas-agent").exists());
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = atlas_engine_linux_sandbox_exe()
        .parent()
