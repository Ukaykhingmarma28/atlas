...
fn linux_sandbox_command(
    ...
    let mut cmd = Command::new(env!("CARGO_BIN_EXE_atlas-engine-linux-sandbox"));
    cmd.args(args)
        .current_dir(cwd)
        .env_clear()
        .envs(env)
...
async fn unsupported_system_bwrap_falls_back_to_bundled_bwrap() {
    if option_env!("ATLAS_AGENT_BWRAP_SHA256")
    ...
    let Some(system_bwrap) = atlas_engine_sandboxing::find_system_bwrap_in_path() else {
    ...
    let sandbox_executable = tempdir.path().join("atlas-engine-linux-sandbox");
    let original_executable = env!("CARGO_BIN_EXE_atlas-engine-linux-sandbox");
    ...
    let resources_dir = tempdir.path().join("atlas-engine-resources");
...
async fn managed_proxy_full_filesystem_uses_minimal_dev_nodes() {
    ...
    if let Some(file) = &shared_memory_file {
        env.insert(
            "ATLAS_AGENT_TEST_SHM_PATH".to_string(),
    ...
    let output = run_linux_sandbox_direct(
        &[
            ...
            concat!(
                ...
                "fi; ",
                "if [ -n \"${ATLAS_AGENT_TEST_SHM_PATH:-}\" ]; then ",
                "test \"$(cat \"$ATLAS_AGENT_TEST_SHM_PATH\")\" = host-before; ",
                "printf sandbox-after >\"$ATLAS_AGENT_TEST_SHM_PATH\"; ",
                "fi; ",
                "stat -c '%d:%i' /dev",
            ),
        ],
...
async fn managed_proxy_mode_routes_through_bridge_and_blocks_direct_egress() {
    ...
    let sandbox_helper_dir = std::path::Path::new(env!("CARGO_BIN_EXE_atlas-engine-linux-sandbox"))
        .parent()
