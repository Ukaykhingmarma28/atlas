...
use atlas_engine_network_proxy::write_attribution_frame;
...
fn proxy_socket_parent_dir() -> PathBuf {
    if let Some(atlas_agent_home) = std::env::var_os("ATLAS_AGENT_HOME") {
        let candidate = PathBuf::from(atlas_agent_home).join("tmp");
