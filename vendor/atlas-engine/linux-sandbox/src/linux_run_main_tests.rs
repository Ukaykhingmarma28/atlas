...
#[cfg(test)]
use atlas_engine_protocol::protocol::NetworkSandboxPolicy;
#[cfg(test)]
...
fn inserts_bwrap_argv0_before_command_separator() {
    ...
    apply_inner_command_argv0_for_launcher(
        &mut argv,
        /*supports_argv0*/ true,
        "/tmp/atlas-engine-arg0-session/atlas-engine-linux-sandbox".to_string(),
    );
    assert_eq!(
        argv,
        vec![
            ...
            "/proc".to_string(),
            "--argv0".to_string(),
            "atlas-engine-linux-sandbox".to_string(),
            "--".to_string(),
            "/bin/true".to_string(),
        ]
    );
...
fn rewrites_bwrap_helper_command_not_nested_user_command_when_current_exe_appears_later() {
    ...
    let mut argv = vec![
        ...
        "--atlas-engine-run-as-apply-patch".to_string(),
        "patch".to_string(),
    ];
...
fn split_only_filesystem_policy_requires_direct_runtime_enforcement() {
    ...
    let policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        atlas_engine_protocol::permissions::FileSystemSandboxEntry {
            path: atlas_engine_protocol::permissions::FileSystemPath::Path { path: docs },
...
fn synthetic_mount_registry_root_is_unique_to_effective_user() {
    ...
    assert_eq!(
        ...
        std::env::temp_dir().join(format!(
            "atlas-engine-bwrap-synthetic-mount-targets-{effective_uid}"
        ))
    );
}
