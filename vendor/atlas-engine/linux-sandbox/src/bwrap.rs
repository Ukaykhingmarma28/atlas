...
//! - sensitive subpaths such as `.git`, `.agents`, and `.atlas_engine` remain
...
use atlas_engine_protocol::permissions::is_protected_metadata_name;
use atlas_engine_protocol::protocol::FileSystemAccessMode;
use atlas_engine_protocol::protocol::FileSystemPath;
use atlas_engine_protocol::protocol::FileSystemSandboxPolicy;
use atlas_engine_protocol::protocol::FileSystemSpecialPath;
use atlas_engine_protocol::protocol::WritableRoot;
    ...
    let missing_auto_metadata_read_only_project_root_subpaths: HashSet<PathBuf> =
        file_system_sandbox_policy
            ...
            .filter_map(|entry| {
                ...
                // protection consistently for `.git`, `.agents`, and `.atlas_engine`.
                ...
                if project_subpath != Path::new(".git")
                    ...
                    && project_subpath != Path::new(".atlas-agent")
                {
                    return None;
                }
...
mod tests {
    ...
    use atlas_engine_protocol::protocol::FileSystemAccessMode;
    use atlas_engine_protocol::protocol::FileSystemPath;
    use atlas_engine_protocol::protocol::FileSystemSandboxEntry;
    use atlas_engine_protocol::protocol::FileSystemSandboxPolicy;
    use atlas_engine_protocol::protocol::FileSystemSpecialPath;
    use atlas_engine_utils_absolute_path::AbsolutePathBuf;
    ...
    fn writable_roots_under_symlinked_ancestors_bind_real_target() {
        ...
        let real_atlas_engine = temp_dir.path().join("real-atlas-agent");
        let logical_atlas_engine = logical_home.join(".atlas-agent");
        let real_memories = real_atlas_engine.join("memories");
        let logical_memories = logical_atlas_engine.join("memories");
        ...
        std::os::unix::fs::symlink(&real_atlas_engine, &logical_atlas_engine)
            .expect("create symlinked atlas-agent home");
    ...
    fn missing_read_only_subpath_uses_empty_file_bind_data() {
        ...
        assert_empty_directory_mounted_read_only(&args.args, &workspace.join(".atlas-agent"));
    ...
    fn missing_project_root_metadata_carveouts_use_metadata_path_masks() {
        ...
        let dot_atlas_engine = path_to_string(&temp_dir.path().join(".atlas-agent"));
        ...
        assert_empty_directory_mounted_read_only(&args.args, Path::new(&dot_atlas_engine));
        ...
        assert!(synthetic_targets.contains(&PathBuf::from(&dot_atlas_engine)));
        assert_eq!(
            protected_create_target_paths(&args),
            Vec::<PathBuf>::new(),
    ...
    fn mounts_dev_before_writable_dev_binds() {
        ...
        assert_eq!(
            ...
            vec![
                PathBuf::from("/.git"),
                PathBuf::from("/.agents"),
                PathBuf::from("/.atlas-agent"),
                PathBuf::from("/dev/.git"),
                PathBuf::from("/dev/.agents"),
                PathBuf::from("/dev/.atlas-agent"),
            ]
        );
        assert_eq!(
            args.args,
