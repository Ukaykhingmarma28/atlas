...
# atlas-engine-linux-sandbox

This crate is responsible for producing:

- a `atlas-engine-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the Atlas Agent CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `atlas-engine-exec` CLI can check if its arg0 is `atlas-engine-linux-sandbox` and, if so, execute as if it were `atlas-engine-linux-sandbox`
  - this should also be true of the `atlas_engine` multitool CLI

On Linux, Atlas Agent prefers the first `bwrap` found on `PATH`
...
the helper falls back to the bundled `atlas-engine-resources/bwrap` binary shipped
with Atlas Agent.
Atlas Agent also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the bundled helper. Atlas Agent surfaces the same startup warning
...
it cannot create the required user namespaces, so Atlas Agent rejects sandboxed shell
...
- If `bwrap` is missing, the helper falls back to the bundled
  `atlas-engine-resources/bwrap` path.
- If `bwrap` is missing, Atlas Agent also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, Atlas Agent surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; Atlas Agent rejects sandboxed
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.atlas_engine`) are re-applied as read-only via `--ro-bind`.
...
**Notes**
- The CLI surface is `atlas_engine sandbox`; the host OS selects the sandbox backend.
