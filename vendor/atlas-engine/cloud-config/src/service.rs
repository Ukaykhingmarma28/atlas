...
fn auth_identity(auth: &AtlasEngineAuth) -> (Option<String>, Option<String>) {
    (auth.get_chatgpt_user_id(), auth.get_account_id())
}

fn cloud_config_eligible_auth(auth: &AtlasEngineAuth) -> bool {
    ...
    C: BundleClient + 'static,
{
    pub(crate) fn new(
        ...
        let atlas_agent_home = AbsolutePathBuf::resolve_path_against_base(atlas_agent_home, "/");
        Self {
            auth_manager,
            client,
            cache: CloudConfigBundleCache::new(atlas_agent_home.clone()),
            atlas_agent_home,
            timeout,
    ...
    async fn load_valid_cached_bundle(
        ...
        match self.cache.load(chatgpt_user_id, account_id).await {
            Ok(signed_payload) => {
                if let Err(err) = validate_bundle(&signed_payload.bundle, &self.atlas_agent_home) {
                    tracing::warn!(
    ...
    async fn validate_and_cache_remote_bundle(
        ...
        if let Err(err) = validate_bundle(&bundle, &self.atlas_agent_home) {
            emit_fetch_final_metric(
                trigger,
                "error",
                "invalid_bundle",
