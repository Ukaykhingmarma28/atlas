...
struct TestExternalChatgptAuth {
    current: RwLock<AtlasEngineAuth>,
...
async fn get_bundle_ignores_invalid_cache_and_refetches() {
    let atlas_agent_home = tempdir().expect("tempdir");
    let cache = create_test_cache(atlas_agent_home.path());
    cache
        .save(
...
async fn get_bundle_refreshes_external_auth_after_unauthorized() {
    ...
    let initial_auth = AtlasEngineAuth::from_external_chatgpt_tokens(
    ...
    let refreshed_auth = AtlasEngineAuth::from_external_chatgpt_tokens(
        &refreshed_token,
        "account-12345",
        Some("enterprise"),
    )
...
async fn dropping_loader_cancels_in_flight_startup_and_refresh() {
    for starts_from_cache in [false, true] {
        ...
        if starts_from_cache {
            create_test_cache(atlas_agent_home.path())
                .save(
                    Some("user-12345".to_string()),
                    Some("account-12345".to_string()),
                    test_bundle(),
