...
| `atlas-memory` | On-device RAG/memory engine: MiniLM → usearch HNSW plus grafeo graph memory, behind a `MemorySearchFn` seam; and the shared-memory record store (`record`: SQLite per repository scope, redact-on-write, one-time legacy migration). Read its `README.md` and `MIGRATION.md` before changing on-disk index formats. |
