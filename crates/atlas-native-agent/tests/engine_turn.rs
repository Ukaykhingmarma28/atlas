...
/// A stand-in memory tool server: one `memory_search` tool over streamable
/// HTTP on loopback, recording each call's arguments and bearer header.
mod memory_server {
...
struct OfferingMemory {
    url: String,
    asked: std::sync::Mutex<Vec<atlas_agent_servers::SessionMcpRequest>>,
    settled: Arc<std::sync::Mutex<Vec<Option<String>>>>,
}
    ...
    fn offer(
        ...
        atlas_agent_servers::SessionMcpOffer::new(vec![server], move |session| {
            settled
                .lock()
                .unwrap_or_else(std::sync::PoisonError::into_inner)
                .push(session.map(|id| id.to_string()));
        })
    }
}
