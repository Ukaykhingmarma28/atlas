...
//! port: the native code in `session.rs` / `embedding.rs` has to satisfy exactly
//! these assertions, so a green run after the swap is evidence that observable
//! behaviour did not move. (The graph memory and the dream gates it also pinned
//! were removed in #89, with their sections here.)
//!
//! It pins *behaviour as it actually was*, not behaviour as it ought to be. Where
//! Cersei did something surprising (`MEMORY:` lines with a negative confidence
//! surviving) the test
