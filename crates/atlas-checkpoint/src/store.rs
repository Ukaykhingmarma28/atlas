...
impl Store {
    ...
    /// Every ledger row in a Workspace, ordered by Session then turn.
    pub fn usage_deltas_for_workspace(&self, workspace_id: &str) -> Result<Vec<UsageDeltaRow>> {
    ...
    /// Message counts per (Session, turn) across a Workspace, with the stamp
