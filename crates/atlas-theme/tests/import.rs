...
fn a_zed_family_becomes_one_atlas_theme_per_member() {
    ...
    assert_eq!(variant.keys["syntax.keyword"], ThemeKeyValue::Color("#31748fff".to_string()));
    assert!(
        imported[0]
            .report
            .ignored
...
fn text_mate_scopes_resolve_by_specificity_with_the_later_rule_winning() {
    ...
    assert_eq!(
        ...
        report.ignored.iter().filter(|entry| entry.category == "font styles").count()
    );
}
...
fn semantic_token_colours_take_precedence_and_modifiers_are_declined() {
    ...
    assert_eq!(theme.dark.as_ref().unwrap().keys["syntax.type"], ThemeKeyValue::Color("#e5c07b".to_string()));
    assert!(
