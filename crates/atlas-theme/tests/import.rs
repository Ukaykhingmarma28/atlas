...
fn options(origin: &str) -> ImportOptions {
    ImportOptions { origin: origin.to_string(), ..ImportOptions::default() }
}
...
/// which is exactly what the TypeScript resolver needs to produce all 134 keys.
fn assert_complete(theme: &Theme) {
    let reparsed = parse_theme(&atlas_theme::toml_writer::theme_to_toml(theme), "assert").unwrap();
    assert_eq!(&reparsed, theme, "{}: the written TOML does not re-read", theme.id);
    assert!(theme.dark.is_some() || theme.light.is_some(), "{}: no variant", theme.id);
    for (appearance, variant) in [("dark", theme.dark.as_ref()), ("light", theme.light.as_ref())] {
        let Some(variant) = variant else { continue };
        let missing: Vec<_> =
            BASE_TOKENS.iter().filter(|token| !variant.base.contains_key(**token)).collect();
        assert!(missing.is_empty(), "{}/{appearance}: missing {missing:?}", theme.id);
...
fn color(theme: &Theme, appearance: &str, key: &str) -> String {
    ...
    variant.keys.get(key).map(ThemeKeyValue::color).unwrap_or_else(|| panic!("{key} is unset")).to_string()
}
...
fn a_tweakcn_registry_item_becomes_a_complete_two_variant_theme() {
    ...
    assert!(
        report.derived.iter().any(|entry| entry.target == "dark.base.destructive-foreground"),
    ...
    assert!(
        report.warnings.iter().any(|warning| warning.contains("spacing")),
        "the spacing warning is required by decision 20: {:?}",
        report.warnings
    );

    // tweakcn's shadow ingredients have nowhere to go and are categorised.
    assert_eq!(report.counts.ignored_by_category.get("shadow recipe"), Some(&6));
...
fn a_pasted_globals_css_yields_light_and_dark() {
    ...
    assert_eq!(theme.light.as_ref().unwrap().base["background"], "oklch(1 0 0)");
    assert_eq!(theme.dark.as_ref().unwrap().base["background"], "oklch(0.145 0 0)");
    // `@theme` reaches both variants, and `--letter-spacing` is tweakcn's name
    // for the tracking token.
    assert_eq!(theme.dark.as_ref().unwrap().base["font-sans"], "Inter, sans-serif");
    assert_eq!(theme.dark.as_ref().unwrap().base["tracking-normal"], "0.01em");
    // `--color-background: var(--background)` is an alias, not a value.
    assert!(!report.ignored.iter().any(|entry| entry.source.contains("color-background")));
}

#[test]
fn a_dark_only_paste_is_not_filed_as_a_light_variant() {
    let source = ":root { --background: #0b0b0f; --foreground: #f4f4f5; }";
    let imported = import_themes(source, None, None, &ImportOptions {
    ...
    assert!(imported[0].report.summary.iter().any(|line| line.contains("dark variant")));
}
...
fn a_zed_family_becomes_one_atlas_theme_per_member() {
    ...
    let ids: Vec<&str> = imported.iter().map(|entry| entry.theme.id.as_str()).collect();
    ...
    let ansi = variant.keys.keys().filter(|key| key.starts_with("terminal.ansi.")).count();
    assert_eq!(ansi, 16);
    ...
    // font_style survives, which is most of what "near-lossless" buys.
    assert_eq!(
        variant.keys["syntax.keyword"],
        ThemeKeyValue::Styled(atlas_theme::ThemeKeyStyle {
            color: "#31748fff".to_string(),
...
fn a_vs_code_theme_resolves_its_include_and_the_child_wins() {
    let imported = import_themes(VSCODE, None, Some(fixtures()), &options("vscode-nocturne.json")).unwrap();
    let ImportedTheme { theme, report, .. } = &imported[0];
    assert_complete(theme);
    assert_eq!(report.fidelity, Fidelity::Lossy);
    assert_eq!(theme.name, "Nocturne Bright");
    assert!(report.warnings.is_empty(), "the include resolved: {:?}", report.warnings);
    assert!(report.summary.iter().any(|line| line.contains("vscode-base.json")));
...
fn text_mate_scopes_resolve_by_specificity_with_the_later_rule_winning() {
    ...
    // fontStyle: italic survives as a styled key.
    let keyword = &theme.dark.as_ref().unwrap().keys["syntax.keyword"];
    assert!(matches!(keyword, ThemeKeyValue::Styled(style) if style.font_style.as_deref() == Some("italic")));
}
...
fn semantic_token_colours_take_precedence_and_modifiers_are_declined() {
    ...
    // `class` carries a fontStyle.
    let kind = &theme.dark.as_ref().unwrap().keys["syntax.type"];
    assert!(matches!(kind, ThemeKeyValue::Styled(style) if style.color == "#e5c07b"));
    // A selector with modifiers is conditional and is reported, not applied.
    assert!(
        report.ignored.iter().any(|entry| entry.category == "semantic tokens"),
        "{:?}",
        report.ignored
    );
}
...
fn a_pasted_theme_reports_the_include_it_could_not_follow() {
    ...
    assert!(
        report.warnings.iter().any(|warning| warning.contains("vscode-base.json")),
        "{:?}",
        report.warnings
    );
...
fn a_vs_code_import_is_honest_about_how_much_it_left_behind() {
    ...
    let categories: BTreeSet<&str> =
        report.counts.ignored_by_category.keys().map(String::as_str).collect();
    for expected in ["editor furniture", "widget", "app chrome", "textmate scopes"] {
        assert!(categories.contains(expected), "missing {expected}: {categories:?}");
    }
    assert!(report.summary.iter().any(|line| line.contains("starting point")));
}
...
fn an_atlas_theme_survives_a_trip_through_shadcn_and_back() {
    ...
    let back = import_themes(&export.json, Some(ImportFormat::Shadcn), None, &options("round trip"))
        .unwrap();
    ...
    for (appearance, source) in [("dark", original.dark.as_ref()), ("light", original.light.as_ref())] {
    ...
    assert!(export.report.notes.iter().any(|note| note.contains("no shadcn equivalent")));
}
...
fn bad_input_fails_with_something_a_user_can_act_on() {
    ...
    for (source, expected) in cases {
        let error = import_themes(source, None, None, &options("bad")).unwrap_err().to_string();
        assert!(error.contains(expected), "{source:?} gave {error:?}, wanted {expected:?}");
    }
}
...
fn browser_mock_snapshot_is_current() {
    ...
    themes.extend(import(ZED, "zed-rose-pine.json").into_iter().map(|entry| entry.theme));
    themes.push(
        import_themes(VSCODE, None, Some(fixtures()), &options("vscode-nocturne.json"))
            .unwrap()
            .remove(0)
            .theme,
    );
