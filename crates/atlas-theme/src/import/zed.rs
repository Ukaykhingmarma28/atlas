...
/// Zed's `syntax` map is `{scope: {color, font_style, font_weight}}`.
/// `font_style` survives the trip — Atlas's `ThemeKeyStyle` has the same field —
/// which is most of what "near-lossless" means for a code theme.
fn map_syntax(style: &Map<String, Value>, draft: &mut VariantDraft, report: &mut ImportReport) {
    ...
    for (target, scopes) in SYNTAX_MAP {
        for scope in *scopes {
            ...
            let font_style = entry
                .get("font_style")
                .and_then(Value::as_str)
                .filter(|style| !style.eq_ignore_ascii_case("normal"))
                .map(str::to_string);
            let value = match font_style {
                Some(font_style) => ThemeKeyValue::Styled(ThemeKeyStyle { color: color.to_string(), font_style: Some(font_style) }),
                None => ThemeKeyValue::Color(color.to_string()),
            };
            if draft.map_key(target, &format!("syntax.{scope}"), value) {
