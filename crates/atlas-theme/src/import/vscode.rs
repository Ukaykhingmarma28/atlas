...
use crate::{ThemeError, ThemeKeyStyle, ThemeKeyValue};

/// Workbench colour key → Atlas theme key(s).
const COLOR_MAP: &[(&str, &[&str])] = &[
    ("editor.background", &["editor.background", "diff.context.background", "scrollbar.track.background"]),
    ("editor.foreground", &["editor.foreground"]),
    ("editorCursor.foreground", &["editor.caret", "terminal.cursor"]),
    ("editorGutter.background", &["editor.gutter.background"]),
    ("editorLineNumber.foreground", &["editor.gutter.foreground", "editor.fold.foreground"]),
    ...
    ("editor.selectionBackground", &["editor.selection.background", "selection.background", "terminal.selection"]),
    ("editorBracketMatch.background", &["editor.match_bracket.background"]),
    ("editorBracketMatch.border", &["editor.match_bracket.border"]),
    ("editor.foldBackground", &["editor.fold.background"]),
    ("scrollbarSlider.background", &["scrollbar.thumb.background"]),
    ("scrollbarSlider.hoverBackground", &["scrollbar.thumb.hover"]),
    ("editorWidget.background", &["panel.overlay.background", "panel.elevated.background"]),
    ("sideBar.background", &["panel.background", "comms.outer.background"]),
    ("activityBar.background", &["panel.rail.background"]),
    ...
    ("tab.activeBorderTop", &["tab.active.border"]),
    ("focusBorder", &["border.focus", "border.strong"]),
    ...
    ("list.inactiveSelectionBackground", &["ghost_element.selected", "ghost_element.active"]),
    ("descriptionForeground", &["text.muted", "comms.other_mention.foreground"]),
    ("disabledForeground", &["text.disabled"]),
    ("input.placeholderForeground", &["text.placeholder"]),
    ("textLink.foreground", &["text.accent", "comms.mention.foreground"]),
    ...
    ("gitDecoration.addedResourceForeground", &["stat.added", "diff.added.text", "comms.unread.foreground", "comms.unread_strong.foreground", "capture.live"]),
    ("gitDecoration.deletedResourceForeground", &["stat.removed", "diff.removed.text"]),
    ("diffEditor.insertedTextBackground", &["diff.added.background", "diff.emphasis_added.background"]),
    ("diffEditor.removedTextBackground", &["diff.removed.background", "diff.emphasis_removed.background"]),
    ("diffEditor.insertedLineBackground", &["diff.add_line.background", "diff.add_side.background"]),
    ("diffEditor.removedLineBackground", &["diff.remove_line.background", "diff.remove_side.background"]),
...
fn map_semantic(
    ...
    for (selector, value) in semantic {
        ...
        let (color, font_style) = match value {
            Value::String(color) => (Some(color.clone()), None),
            Value::Object(entry) => (
                entry.get("foreground").and_then(Value::as_str).map(str::to_string),
                entry
                    .get("fontStyle")
                    .and_then(Value::as_str)
                    .filter(|style| style.contains("italic"))
                    .map(|_| "italic".to_string()),
            ),
            _ => (None, None),
        };
        let Some(color) = color else { continue };
        let entry = match font_style {
            Some(font_style) => ThemeKeyValue::Styled(ThemeKeyStyle { color, font_style: Some(font_style) }),
            None => ThemeKeyValue::Color(color),
        };
        if draft.map_key(target, &format!("semanticTokenColors.{selector}"), entry) {
            used += 1;
        }
    }
    used
...
fn map_token_colors(token_colors: &[Value], draft: &mut VariantDraft, report: &mut ImportReport) {
    ...
    for (target, candidates) in SCOPE_TABLE {
        for candidate in *candidates {
            ...
            let entry = if rule.italic {
                ThemeKeyValue::Styled(ThemeKeyStyle { color, font_style: Some("italic".to_string()) })
            } else {
                ThemeKeyValue::Color(color)
            };
            if draft.map_key(target, &format!("tokenColors[{candidate}]"), entry) {
