...
there: no editor, no terminal, no syntax, no diff. All 134 theme keys resolve
from the base tokens or from Atlas's defaults.
...
collapses onto Atlas's 20 roles **with `font_style` intact**, which is most of
what "near-lossless" means for a code theme.
...
`settings.fontStyle: italic` survives as a styled key. `markup.*` and
grammar-specific scopes have no Atlas role and are counted as dropped.
...
mirror image of the shadcn importer — the palette and up to 134 theme keys
describe surfaces shadcn has no vocabulary for and are dropped, with the counts
by family in the result. To move a theme between two Atlas installs, copy the
TOML instead.
