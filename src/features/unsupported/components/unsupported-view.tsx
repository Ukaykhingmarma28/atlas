...
export function UnsupportedView({ filePath }: UnsupportedViewProps) {
  ...
  return (
    <div className="h-full w-full flex flex-col bg-[var(--background)]">
      <div className="flex items-center px-3 h-[32px] border-b border-[var(--border)] shrink-0 text-[11px] font-mono text-[var(--muted-foreground)] truncate">
        {filePath}
      </div>
      <div className="flex-1 min-h-0 flex items-center justify-center px-6">
        <div className="flex flex-col items-center gap-4 text-center max-w-md">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 h-7 rounded border border-[var(--border)] bg-[var(--card)] hover:bg-[var(--atlas-element-hover)] text-[11px] text-[var(--secondary-foreground)] hover:text-[var(--foreground)] cursor-pointer transition-colors"
          >
            <FolderOpen size={12} />
            Open in Finder
          </button>
