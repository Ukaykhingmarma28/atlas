    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-text-tertiary mt-0.5">
          ...
          <kbd className="px-1 py-0.5 rounded bg-bg-elevated border border-border font-mono text-3xs">
            {switcherHint}
          </kbd>{" "}
      ...
      <div className="grid grid-cols-2 gap-3">
        {LAYOUT_TEMPLATES.map((t) => (
          ...
          >
            ...
            <div className="mt-2 text-sm font-medium text-text-primary">{t.name}</div>
            <div className="text-2xs text-text-tertiary leading-snug">{t.description}</div>
          </button>
        ))}
      </div>
    </div>
