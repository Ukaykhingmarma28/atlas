...
export function ConflictsView({ onOpenFile }: { onOpenFile: (path: string) => void }) {
  ...
  return (
    <div className="shrink-0 border-b border-border">
      <div className="flex items-center justify-between px-2 h-6 bg-[var(--bg-sidebar)] border-b border-border-subtle">
        <span className="text-2xs font-semibold text-text-tertiary uppercase tracking-wider">
          Conflicts ({state.files.length})
        </span>
        <span className="text-3xs text-text-tertiary">resolve each file, then Continue</span>
      </div>
      {state.files.map((f) => (
        <div
          key={f.path}
          className="group flex items-center gap-1.5 h-control-md px-2 text-xs hover:bg-bg-hover"
        >
          ...
          {f.markerCount > 0 ? (
            <span className="shrink-0 text-3xs font-mono text-[var(--status-warning)]">
              {f.markerCount} marker{f.markerCount === 1 ? "" : "s"}
            </span>
          ) : (
            <span className="shrink-0 text-3xs font-mono text-text-tertiary">no markers</span>
          )}
          <HintGroup>
