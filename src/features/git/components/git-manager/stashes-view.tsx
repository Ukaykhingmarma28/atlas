    ...
    <div className="h-full flex flex-col">
      <div className="shrink-0 border-b border-border p-2">
        <button
          ...
          className="w-full flex items-center justify-center gap-1.5 h-7 rounded-md text-xs font-medium bg-bg-elevated text-text-secondary hover:text-text-primary hover:bg-bg-hover disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
        >
          <Archive size={12} />
          Stash all changes
        </button>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto hide-scrollbar">
        {stashes.length === 0 ? (
          <div className="px-3 py-8 text-center text-xs text-text-tertiary">No stashes</div>
        ) : (
          stashes.map((s) => (
            <div
              key={s.index}
            ...
            >
              <span className="text-xs text-text-secondary truncate">{s.message}</span>
              <div className="flex items-center gap-2">
                <span className="text-3xs font-mono text-text-tertiary flex-1">
