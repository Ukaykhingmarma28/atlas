  ...
  return (
    <div className="aspect-[16/10] w-full rounded-md border border-[var(--border)] bg-[var(--bg-base)] p-1 flex gap-1">
      {template.panels.left && (
        <div className="w-1.5 rounded-sm bg-[var(--bg-elevated)] shrink-0" />
      )}
      <div className="flex-1 flex gap-0.5 min-w-0">
        {template.columns.map((col, i) => (
          <div
            key={i}
            className="flex-1 min-w-0 rounded-sm bg-[var(--bg-elevated)] border border-[var(--border-subtle)] flex items-center justify-center"
          >
